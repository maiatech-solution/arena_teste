...
Route::get('/teste-vida', function() {
    return "O Laravel está lendo este arquivo!";
});

Route::get('/teste-arquivo', function() {
    if (class_exists(\App\Http\Controllers\Bar\BarCompanyController::class)) {
        return "Arquivo encontrado e reconhecido com sucesso!";
    }
    return "Erro: O Laravel NÃO está vendo o arquivo BarCompanyController.php. Verifique o nome da pasta 'Bar' e o nome do arquivo.";
});
...
Route::middleware(['auth', 'gestor'])->prefix('bar')->name('bar.')->group(function () {
    ...
    Route::get('bar/configuracoes/empresa', [\App\Http\Controllers\Bar\BarCompanyController::class, 'edit'])->name('bar.company.edit');
    Route::put('bar/configuracoes/empresa', [\App\Http\Controllers\Bar\BarCompanyController::class, 'update'])->name('bar.company.update');
