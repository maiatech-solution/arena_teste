...
Route::middleware(['auth', 'gestor'])->prefix('bar')->name('bar.')->group(function () {
    ...
    // 👥 6. GESTÃO DE USUÁRIOS (LAYOUT DARK)
    Route::prefix('usuarios')->name('users.')->group(function () {
        Route::get('/', [App\Http\Controllers\Bar\BarUserController::class, 'index'])->name('index');
        Route::get('/create', [App\Http\Controllers\Bar\BarUserController::class, 'create'])->name('create');
        Route::post('/', [App\Http\Controllers\Bar\BarUserController::class, 'store'])->name('store');
        Route::get('/{user}/edit', [App\Http\Controllers\Bar\BarUserController::class, 'edit'])->name('edit');
        Route::put('/{user}', [App\Http\Controllers\Bar\BarUserController::class, 'update'])->name('update');
        Route::delete('/{user}', [App\Http\Controllers\Bar\BarUserController::class, 'destroy'])->name('destroy');
    });

    // 🏢 7. DADOS DA EMPRESA (LAYOUT DARK)
    Route::prefix('configuracoes')->name('company.')->group(function () {
        Route::get('/empresa', [App\Http\Controllers\Bar\BarCompanyController::class, 'edit'])->name('edit');
        Route::put('/empresa', [App\Http\Controllers\Bar\BarCompanyController::class, 'update'])->name('update');
    });
});
