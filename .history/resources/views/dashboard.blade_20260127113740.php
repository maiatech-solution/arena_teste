<x-app-layout>
    ...
    <style>
        ...
        #client-autocomplete-results {
            position: absolute;
            /* Obrigatório para o cálculo JS funcionar */
            max-height: 200px;
            overflow-y: auto;
            border-radius: 8px;
            /* Arredondado em todos os cantos fica mais moderno */
...
<div id="quick-booking-modal" class="modal-overlay hidden" onclick="closeQuickBookingModal()">
    <div class="bg-white rounded-xl shadow-2xl max-w-lg w-full max-h-[95vh] flex flex-col transition-all duration-300 transform scale-100 mx-4 sm:mx-0"
        ...
        <div class="p-4 overflow-y-auto custom-scrollbar">
            <form id="quick-booking-form">
                ...
                <div id="slot-info-display" class="mb-4 p-3 bg-indigo-50 border border-indigo-100 rounded-lg text-xs sm:text-sm text-gray-700"></div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative">
                        <label for="client_name" class="block text-xs font-bold text-gray-500 uppercase">Nome do Cliente *</label>
                    ...
                    {{-- Campo WhatsApp --}}
                    <div>
                        <label for="client_contact" class="block text-xs font-bold text-gray-500 uppercase">WhatsApp (11 dígitos) *</label>
                        <input type="tel" name="client_contact" id="client_contact" required maxlength="11" placeholder="Ex: 91999999999"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm h-10">
                        <p id="whatsapp-error-message" class="text-[10px] text-red-600 mt-1 hidden font-semibold">⚠️ Insira 11 dígitos.</p>
                    ...
                    <div class="p-3 border border-indigo-100 rounded-lg bg-indigo-50/50 flex items-center">
                        <input type="checkbox" name="is_recurrent" id="is-recurrent" value="1" class="h-4 w-4 text-indigo-600 border-gray-300 rounded">
                        <label for="is-recurrent" class="ml-3 text-sm font-bold text-indigo-700 uppercase tracking-tighter">Reserva Recorrente (6 meses)</label>
                    </div>

                    <div>
                        <label for="notes" class="block text-xs font-bold text-gray-500 uppercase">Observações</label>
                        <textarea name="notes" id="notes" rows="2" class="mt-1 block w-full border-gray-300 rounded-md text-sm shadow-sm"></textarea>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="p-4 border-t bg-gray-50 rounded-b-xl grid grid-cols-2 gap-3">
            <button type="button" onclick="closeQuickBookingModal()" class="px-4 py-2 bg-gray-200 text-gray-700 font-bold rounded-lg text-sm">Cancelar</button>
            <button type="submit" form="quick-booking-form" id="submit-quick-booking" class="px-4 py-2 bg-green-600 text-white font-bold rounded-lg shadow-md text-sm">Confirmar</button>
        </div>
    </div>
</div>
    ...
    `;
            ...
            // =========================================================
            // LÓGICA DE AUTOCOMPLETE DUPLO (NOME E WHATSAPP) - COMPLETO
            // =========================================================
            const autocompleteResults = document.getElementById('client-autocomplete-results');
            let debounceTimer;

            /**
             * Função unificada para busca de clientes com posicionamento dinâmico
             * @param {HTMLElement} inputElement - O campo (Nome ou WhatsApp) que disparou a busca
             */
            const performClientSearch = (inputElement) => {
                ...
                // Regra: Se o campo tiver menos de 2 caracteres, esconde a lista imediatamente
                if (query.length < 2) {
                ...
                debounceTimer = setTimeout(() => {
                    // Busca na API (suporta nome ou telefone no parâmetro query)
                    fetch(`/api/clientes/search?query=${encodeURIComponent(query)}&arena_id=${arenaId}`)
                        .then(response => response.json())
                        .then(data => {
                            if (!autocompleteResults) return;

                            // Limpa resultados anteriores antes de processar
                            autocompleteResults.innerHTML = '';

                            // CORREÇÃO: Se não houver resultados (ex: Adriano G não existe), esconde a lista
                            ...
                            // Calcula a posição para a lista aparecer exatamente abaixo do input usado
                            const topPos = inputElement.offsetTop + inputElement.offsetHeight;
                            autocompleteResults.style.top = `${topPos}px`;
                    ...
                    `;
                                ...
                                // Ao clicar em um cliente da lista
                                div.onclick = () => {
                                    // Preenche Nome
                                    if (clientNameInput()) {
                                    ...
                                    // Preenche WhatsApp e valida Status/VIP
                                    if (phone && clientContactInput()) {
                                        const cleanPhone = phone.replace(/\D/g, '');
                                        clientContactInput().value = cleanPhone;

                                        // Dispara busca de reputação e ajuste de sinal para VIP
                            ...
                            // Mostra a lista apenas se tiver resultados
                ...
                }, 350); // Delay de 350ms para evitar excesso de requisições
            };

            // --- Registro dos Eventos ---

            // 1. Escuta tanto o campo de Nome quanto o de Contato
            [clientNameInput(), clientContactInput()].forEach(input => {
                if (input) {
            ...
            // 2. Fecha a lista ao clicar fora (no modal ou no documento)
            document.addEventListener('click', function(e) {
                if (autocompleteResults &&
                    !autocompleteResults.contains(e.target) &&
                    e.target !== clientNameInput() &&
                    e.target !== clientContactInput()) {
