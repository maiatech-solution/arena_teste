            ...
            async function sendCancellationRequest(reservaId, method, urlBase, reason, supervisorToken = null) {
                ...
                try {
                    ...
                    if (response.ok && result.success) {
                        ...
                        // 🚀 ATUALIZAÇÃO DO CALENDÁRIO (BLINDADA)
                        if (calendar) {
                            // Pequeno delay de 300ms para garantir que o banco processou a liberação
                            setTimeout(() => {
                                calendar.refetchEvents(); // Busca dados novos
                                calendar.render(); // Força o redesenho visual
                                console.log("Calendário sincronizado após cancelamento.");
                            }, 300);
                        }
                    } else {
                        // ERRO (Ex: Caixa Fechado ou Autorização Inválida)
