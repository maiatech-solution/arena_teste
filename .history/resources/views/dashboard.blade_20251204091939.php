<x-app-layout>
    ...
    <script>
        ...
        async function debugApiData() {
            console.log("--- INÍCIO DEBUG API (Reservas Confirmadas) ---");
            const url = RESERVED_API_URL + '?start=' + moment().startOf('month').format('YYYY-MM-DD') + '&end=' + moment().endOf('month').format('YYYY-MM-DD');
            ...
            try {
                ...
                // Filtra e loga apenas eventos de cliente que estão como 'confirmed' ou 'concluida'
                const clientEvents = data.filter(event => event.extendedProps.status !== 'free' && event.extendedProps.status !== 'maintenance');

                if (clientEvents.length > 0) {
                    console.log(`\n=== ANÁLISE DE EVENTOS DE CLIENTE (${clientEvents.length} encontrados) ===`);
                    clientEvents.forEach(event => {
                        console.log(`ID: ${event.id}, Status: ${event.extendedProps.status}, PaymentStatus: ${event.extendedProps.payment_status}, is_paid: ${event.extendedProps.is_paid}`);
                    });
                    console.log("Se algum evento deveria estar pago, mas is_paid é 'undefined' ou 'false', o problema está no Controller Laravel.");
                } else {
                    console.log("Nenhum evento de cliente (confirmed/pending/concluida) encontrado no período.");
                }
        ...
        async function sendCancellationRequest(reservaId, method, urlBase, reason) {
            ...
            try {
                ...
                submitBtn.disabled = false;
                submitBtm.textContent = 'Confirmar Ação';
            }
        }
        ...
        window.onload = function() {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventDidMount: function(info) {
                    ...
                    const isPaidFlag = extendedProps.is_paid === true || extendedProps.is_paid === 1;

                    // LOG DE DEBUG PARA VOCÊ VERIFICAR
                    console.log(`[DEBUG API] Processando evento ID: ${event.id}, Status: ${extendedProps.status}, Paid: ${extendedProps.is_paid}`);
