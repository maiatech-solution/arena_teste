<x-app-layout>
        ...
        <div class="bg-white p-6 rounded-xl shadow-2xl max-w-lg w-full transition-all duration-300 transform scale-100" onclick="event.stopPropagation()">
            ...
            <form id="quick-booking-form">
                ...
                <div id="manual_client_fields">
                    {{-- O campo de busca (acima) já é o primeiro "Nome". Estes são os campos manuais que se tornam obrigatórios caso o campo de busca seja ignorado. --}}
                    <div class="mb-4">
    ...
    <script>
        ...
        function selectClient(client) {
            ...
            // 4. Remove a obrigatoriedade dos campos manuais e limpa
            // (Para garantir que apenas user_id seja enviado para o back-end, mas os inputs tenham valor)
            clientNameInput().removeAttribute('required');
            clientContactInput().removeAttribute('required');

            // 5. Preenche os campos manuais em background
            clientNameInput().value = client.name;
            // CRÍTICO: Usamos o contato preenchido pelo registro para atender à validação
        ...
        // =========================================================
        // 🚨 NOVO: FUNÇÃO DE VALIDAÇÃO WHATSAPP
        // =========================================================
        ...
        function validateClientContact(contact) {
            ...
            // Padrão esperado: DD + 9 + 8 dígitos (Ex: 91985320997)
            ...
            if (isValid) {
                ...
                clientContactInput().classList.remove('border-red-500');
                clientContactInput().classList.add('border-gray-300');
            } else {
                errorElement.classList.remove('hidden');
                clientContactInput().classList.add('border-red-500');
                clientContactInput().classList.remove('border-gray-300');
            }
        ...
        async function handleQuickBookingSubmit(event) {
            ...
            // --- NOVO: VALIDAÇÃO WHATSAPP ---
            if (!userId && clientContact && !validateClientContact(clientContact)) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function(info) {
                    ...
                    if (isAvailable) {
                        ...
                        // 3. Oculta a mensagem de erro do WhatsApp no modal
                        document.getElementById('whatsapp-error-message').classList.add('hidden');
