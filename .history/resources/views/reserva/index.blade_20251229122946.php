...
<body class="font-sans antialiased arena-bg">
    ...
    <div id="booking-modal" class="modal-overlay hidden items-center justify-center z-50 p-4 backdrop-blur-sm">
        <div id="modal-content"
                ...
                <div
                    ...
                    <p class="mt-1 text-sm font-semibold">
                        @if ($errors->has('reserva_conflito_id'))
                            Este horário acabou de ser ocupado. Por favor, escolha outro slot verde no calendário.
                        @else
            ...
            {{-- Alerta Front-End (JavaScript) --}}
            <div class="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500 text-amber-800 dark:text-amber-300 rounded-r-xl hidden"
                id="frontend-alert-box">
            ...
            @if (!Auth::check() || (Auth::check() && Auth::user()->isClient()))
                ...
                <div class="mb-8 text-center sm:text-left">
                    <h4 class="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight leading-none">Dados
                        da Reserva</h4>
                    <p class="text-gray-500 dark:text-gray-400 mt-2 font-medium italic">Elite Soccer - Onde o jogo
                        acontece</p>
                </div>
                ...
                <form id="booking-form" method="POST" action="{{ route('reserva.store') }}">
                    ...
                    {{-- 🎯 CAMPOS HIDDEN CRÍTICOS (Incluindo email_cliente para evitar Erro 500) --}}
                    ...
                    {{-- Envia o e-mail do usuário logado ou vazio para o sistema gerar o aleatório --}}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                        <div
                            ...
                            <span
                                class="text-[10px] font-black uppercase tracking-widest text-indigo-500 block mb-1">Data
                                Agendada</span>
                            <span id="modal-date"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    <label
                                        ...
                                        Completo</label>
                                    <input type="text" name="nome_cliente" placeholder="Como quer ser chamado?"
                                        required value="{{ old('nome_cliente') }}"
                    ...
                    {{-- Alerta de Sinal (Obrigatório) --}}
                    <div
                        class="mb-8 p-6 bg-red-50 dark:bg-red-900/20 border-l-8 border-red-600 rounded-r-2xl shadow-sm">
                        <div
                            class="flex items-center text-red-700 dark:text-red-400 mb-2 font-black uppercase tracking-wider text-sm">
                        ...
                        <p class="text-xs sm:text-sm text-red-800 dark:text-red-300 font-bold leading-relaxed">
                            Sua vaga só é garantida após o envio imediato do comprovante de sinal via WhatsApp. Caso o
                            envio não ocorra, o sistema poderá liberar o horário para outros clientes.
                        </p>
                    </div>

                    {{-- Rodapé: Valor e Botões --}}
                    <div
