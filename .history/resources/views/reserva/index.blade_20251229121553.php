...
<div id="booking-modal" class="modal-overlay hidden items-center justify-center z-50 p-4">
    ...
    <div id="modal-content" class="bg-white dark:bg-gray-800 p-8 sm:p-10 rounded-[2rem] shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-100 border-t-[10px]
        ...
        @if (!Auth::check() || (Auth::check() && Auth::user()->isClient()))
            ...
            <div class="text-center mb-8">
                <h4 class="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight">Confirme Sua Reserva</h4>
                <p class="text-gray-500 dark:text-gray-400 mt-2 font-medium">Verifique os detalhes abaixo antes de prosseguir</p>
            </div>
            ...
            <form id="booking-form" method="POST" action="{{ route('reserva.store') }}">
                ...
                <div class="mb-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="bg-indigo-50 dark:bg-indigo-900/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800 flex flex-col items-center justify-center text-center">
                        <span class="text-xs font-bold uppercase tracking-widest text-indigo-500 mb-1">Data Agendada</span>
                        <span id="modal-date" class="font-extrabold text-gray-900 dark:text-gray-100 whitespace-nowrap"></span>
                    </div>
                    <div class="bg-indigo-50 dark:bg-indigo-900/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800 flex flex-col items-center justify-center text-center">
                        <span class="text-xs font-bold uppercase tracking-widest text-indigo-500 mb-1">Horário</span>
                        <span id="modal-time" class="font-extrabold text-gray-900 dark:text-gray-100 text-xl"></span>
                    </div>
                </div>

                {{-- DADOS DO CLIENTE --}}
                @if (Auth::check() && Auth::user()->isClient())
                    <div class="mb-8 p-6 bg-green-50 dark:bg-green-900/30 rounded-2xl border border-green-200 dark:border-green-800 shadow-sm text-center">
                        <p class="text-green-800 dark:text-green-300 font-bold text-lg mb-1">{{ Auth::user()->name }}</p>
                        <p class="text-green-600 dark:text-green-400 text-sm font-medium">{{ Auth::user()->contato_cliente }}</p>
                ...
                @else
                    <div class="space-y-5 mb-8">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1 ml-1">Nome Completo</label>
                                <input type="text" name="nome_cliente" required value="{{ old('nome_cliente') }}"
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1 ml-1">WhatsApp (DDD+nº)</label>
                                <input type="tel" name="contato_cliente" id="guest-contact" required value="{{ old('contato_cliente') }}"
                                    class="w-full border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 transition" maxlength="11">
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="flex items-center justify-between mb-8 px-2">
                    <span class="text-2xl font-black text-gray-800 dark:text-gray-200 uppercase">Valor do Jogo:</span>
                    <span class="text-4xl font-black text-green-600 dark:text-green-400">R$ <span id="modal-price"></span></span>
                </div>

                {{-- AVISO CRÍTICO --}}
                <div class="mb-8 p-5 bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500 rounded-xl flex items-start">
                    <svg class="w-6 h-6 text-amber-600 dark:text-amber-500 mr-3 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    <p class="text-sm text-amber-800 dark:text-amber-200 font-semibold leading-snug">
                        Sua reserva ficará pendente. Você deve enviar o comprovante do sinal via WhatsApp em até 15 minutos para garantir o horário.
                    </p>
                </div>

                {{-- BOTÕES --}}
                <div class="flex flex-col sm:flex-row gap-4">
                    <button type="button" id="close-modal" class="flex-1 p-4 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-bold rounded-2xl hover:bg-gray-200 dark:hover:bg-gray-600 transition duration-200">
                        Cancelar
                    </button>
                    <button type="submit" class="flex-1 p-4 bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-700 shadow-xl shadow-indigo-500/30 transition duration-300 transform hover:scale-[1.02] active:scale-95 uppercase tracking-wider">
                        Confirmar Agora
                    </button>
                </div>
            </form>
