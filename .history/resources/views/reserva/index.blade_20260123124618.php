...
<head>
    ...
    <title>{{ $site_info->nome_fantasia ?? 'Arena' }} | Agendamento Online</title>
...
<body class="font-sans antialiased arena-bg">
    ...
    <div class="min-h-screen flex flex-col items-center justify-start p-4 md:p-8 py-16">
        <div class="w-full max-w-5xl mx-auto p-6 sm:p-10 bg-white/95 dark:bg-gray-800/90 backdrop-blur-sm shadow-2xl shadow-gray-900/70 dark:shadow-indigo-900/50 rounded-3xl transform transition-all duration-300 ease-in-out">

            {{-- Nome da Empresa Dinâmico --}}
            <h1 class="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-gray-100 mb-6 border-b-4 border-indigo-600 dark:border-indigo-400 pb-3 text-center tracking-tight uppercase">
                ⚽ {{ $site_info->nome_fantasia ?? 'Agendamento Online' }}
            </h1>

            {{-- Endereço Dinâmico no Subtítulo --}}
            <p class="text-gray-600 dark:text-gray-400 mb-8 text-center text-base sm:text-lg font-medium">
                @if($site_info)
                {{ $site_info->logradouro }}, {{ $site_info->numero }} - {{ $site_info->bairro }}<br>
                <span class="text-sm font-normal opacity-75">Selecione uma data para ver os horários detalhados.</span>
                @else
                Selecione uma data para ver os horários detalhados e a quantidade de vagas disponíveis.
                @endif
            </p>

            {{-- Seção de Erros (Reserva Duplicada) --}}
            ...
            {{-- Alerta de WhatsApp para Pré-Reserva --}}
            ...
            <div class="bg-green-50 dark:bg-green-900/30 border border-green-400 dark:border-green-700 p-8 rounded-3xl relative mb-12 text-center shadow-2xl shadow-green-400/40 dark:shadow-green-900/70" role="alert">
                ...
                <p class="mb-6 text-lg text-gray-700 dark:text-gray-300">
                    Sua solicitação na <strong>{{ $site_info->nome_fantasia ?? 'Arena' }}</strong> foi enviada! <br>
                    <strong>Clique abaixo imediatamente</strong> para confirmar via WhatsApp.
                </p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <button onclick="selectArena('{{ $arena->id }}', '{{ $arena->name }}')"
                        ...
                        <div class="p-6">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-sm mt-1 font-medium italic">Clique para ver os horários</p>
                        </div>
                    </button>
                    @endforeach
                </div>
            ...
            <div id="calendar-wrapper" class="hidden opacity-0 transform translate-y-4 transition-all duration-500">
                <div class="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4 px-2">
                    ...
                    <div class="flex flex-col items-end gap-1">
                        <span class="text-gray-400 text-[10px] font-black uppercase tracking-widest">{{ $site_info->nome_fantasia ?? 'Arena' }}</span>
        ...
        @if ($errors->any()) border-red-600 dark:border-red-500 @else border-indigo-600 dark:border-indigo-500 @endif"
            ...
            <div class="mb-8 text-center sm:text-left">
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    {{-- Badge da Empresa (Novo) --}}
                    <div class="inline-flex items-center gap-2 bg-gray-100 dark:bg-gray-700 px-4 py-2 rounded-full border border-gray-200 dark:border-gray-600">
                        <span class="text-[10px] font-black uppercase text-gray-600 dark:text-gray-400 tracking-widest">
                            {{ $site_info->nome_fantasia ?? 'Arena' }}
                        </span>
                    </div>
                    {{-- Badge da Quadra --}}
                    <div class="inline-flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/30 px-4 py-2 rounded-full border border-indigo-100 dark:border-indigo-800">
                        <svg class="w-4 h-4 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                        </svg>
                        <span class="text-xs font-black uppercase text-indigo-700 dark:text-indigo-300 tracking-widest" id="modal-arena-name-display">---</span>
                    </div>
                </div>
            </div>
            ...
            <form id="booking-form" method="POST" action="{{ route('reserva.store') }}">
                ...
                {{-- Dados do Cliente --}}
                <div class="mb-8">
                    @if (Auth::check() && Auth::user()->is_gestor === false)
                ...
                <div class="mb-8 p-6 bg-red-50 dark:bg-red-900/20 border-l-8 border-red-600 rounded-r-3xl shadow-sm">
                    <p class="text-xs sm:text-sm text-red-800 dark:text-red-300 font-bold leading-relaxed">
                        Importante: Sua vaga na <strong>{{ $site_info->nome_fantasia ?? 'Arena' }}</strong> só é garantida após o envio imediato do comprovante de sinal via WhatsApp.
                    </p>
                </div>
