...
<body class="font-sans antialiased arena-bg">
   ...
   {{-- --- Modal de Confirmação de Dados --- --}}
    <div id="booking-modal" class="modal-overlay hidden items-center justify-center z-50 p-4 backdrop-blur-sm">
        <div id="modal-content"
            ...
            @if ($errors->any() && old('data_reserva'))
                <div class="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 text-red-700 dark:text-red-300 rounded-r-xl shadow-sm">
                    ...
                    <p class="mt-1 text-sm font-semibold">@if ($errors->has('reserva_conflito_id')) Este horário acabou de ser ocupado. Escolha outro slot verde. @else Verifique os campos em vermelho. @endif</p>
                </div>
            @endif

            {{-- Alerta Front-End --}}
            <div class="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500 text-amber-800 dark:text-amber-300 rounded-r-xl hidden" id="frontend-alert-box">
                <p id="frontend-alert-message" class="font-bold flex items-center"><span class="text-sm"></span></p>
            </div>

            @auth
                @if (Auth::user()->isGestor())
                    <div class="p-6 bg-red-100 dark:bg-red-900/40 border-l-8 border-red-600 text-red-700 dark:text-red-200 rounded-xl mb-6 shadow-md">
                        <p class="font-black text-xl uppercase italic">⚠️ Gestor Logado</p>
                        <p class="text-sm mt-1">Use o Dashboard para agendar ou faça logout para reservar como cliente.</p>
                        <form method="POST" action="{{ route('logout') }}" class="mt-4"><@csrf><button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-lg font-bold text-xs uppercase hover:bg-red-700 transition">Sair Agora</button></form>
                    </div>
                @endif
            @endauth
            ...
            @if (!Auth::check() || (Auth::check() && Auth::user()->isClient()))
                ...
                <div class="mb-8 text-center sm:text-left">
                    <h4 class="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight leading-none">Dados do Agendamento</h4>
                    <p class="text-gray-500 dark:text-gray-400 mt-2 font-medium">Confirme os detalhes da sua partida na Elite Soccer</p>
                </div>
                ...
                <form id="booking-form" method="POST" action="{{ route('reserva.store') }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                        <div class="bg-indigo-50 dark:bg-indigo-900/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800">
                            <span class="text-[10px] font-black uppercase tracking-widest text-indigo-500 block mb-1">Data da Partida</span>
                            <span id="modal-date" class="font-bold text-gray-800 dark:text-gray-100 block leading-tight"></span>
                        </div>
                        <div class="bg-indigo-50 dark:bg-indigo-900/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800">
                            <span class="text-[10px] font-black uppercase tracking-widest text-indigo-500 block mb-1">Horário Selecionado</span>
                            <span id="modal-time" class="font-black text-2xl text-indigo-600 dark:text-indigo-400 block leading-none"></span>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        @if (Auth::check() && Auth::user()->isClient())
                            <div class="p-6 bg-green-50 dark:bg-green-900/20 rounded-2xl border border-green-200 dark:border-green-800 flex items-center justify-between">
                                <div>
                                    <p class="text-xs font-bold text-green-600 uppercase tracking-widest mb-1">Reserva para:</p>
                                    <p class="text-xl font-black text-gray-900 dark:text-gray-100 leading-none">{{ Auth::user()->name }}</p>
                                    <p class="text-sm text-green-700 dark:text-green-400 font-medium mt-1">{{ Auth::user()->contato_cliente }}</p>
                                </div>
                                <div class="bg-green-600 p-2 rounded-full text-white shadow-lg"><svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg></div>
                        ...
                        @else
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">Seu Nome Completo</label>
                                    <input type="text" name="nome_cliente" placeholder="Ex: João Silva" required value="{{ old('nome_cliente') }}"
                                        class="w-full bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 dark:text-white rounded-xl p-4 focus:ring-4 focus:ring-indigo-500/20 outline-none transition shadow-sm border-2">
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">WhatsApp (DDD + Número)</label>
                                    <input type="tel" name="contato_cliente" placeholder="91988887777" required value="{{ old('contato_cliente') }}"
                                        class="w-full bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 dark:text-white rounded-xl p-4 focus:ring-4 focus:ring-indigo-500/20 outline-none transition shadow-sm border-2" maxlength="11">
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="mb-8 p-6 bg-red-50 dark:bg-red-900/20 border-l-8 border-red-600 rounded-r-2xl shadow-sm">
                        <div class="flex items-center text-red-700 dark:text-red-400 mb-2">
                            ...
                            <span class="font-black uppercase tracking-wider">Atenção ao Pagamento</span>
                        </div>
                        <p class="text-sm text-red-800 dark:text-red-300 font-bold leading-relaxed">
                            Sua vaga será garantida apenas após o envio imediato do comprovante de sinal via WhatsApp. Caso não envie, o sistema liberará o horário automaticamente.
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-6 pt-6 border-t border-gray-100 dark:border-gray-700">
                        <div class="text-center sm:text-left">
                            <span class="text-xs font-black text-gray-400 uppercase tracking-widest block">Total a Pagar</span>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                            <button type="button" id="close-modal" class="px-8 py-4 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300 font-black rounded-2xl hover:bg-gray-200 transition uppercase text-xs tracking-widest">Sair</button>
                            <button type="submit" class="px-8 py-4 bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-700 shadow-xl shadow-indigo-500/40 transition transform hover:scale-105 active:scale-95 uppercase text-xs tracking-widest">Confirmar Reserva</button>
                        </div>
                    </div>
                </form>
            @endif
