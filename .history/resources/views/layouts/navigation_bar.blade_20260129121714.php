    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20">
                ...
                @if(!request()->routeIs('bar.dashboard'))
                    <div class="hidden space-x-2 sm:-my-px sm:ms-10 sm:flex items-center">
                        ...
                        <x-nav-link :href="route('bar.pdv')" :active="request()->routeIs('bar.pdv')"
                            class="px-4 py-2 text-xs font-black uppercase tracking-widest transition-all duration-200 {{ request()->routeIs('bar.pdv') ? 'text-orange-500 bg-orange-500/10 rounded-xl border-b-2 border-orange-500 shadow-[0_4px_10px_rgba(234,88,12,0.2)]' : 'text-gray-400 hover:text-white' }}">
                            {{ __('PDV') }}
                        </x-nav-link>

                        <x-nav-link :href="route('bar.products.index')" :active="request()->routeIs('bar.products.*')"
                            class="px-4 py-2 text-xs font-black uppercase tracking-widest transition-all duration-200 {{ request()->routeIs('bar.products.*') ? 'text-orange-500 bg-orange-500/10 rounded-xl border-b-2 border-orange-500 shadow-[0_4px_10px_rgba(234,88,12,0.2)]' : 'text-gray-400 hover:text-white' }}">
                            {{ __('Estoque') }}
                        </x-nav-link>

                        <x-nav-link :href="route('bar.tables.index')" :active="request()->routeIs('bar.tables.*')"
                            class="px-4 py-2 text-xs font-black uppercase tracking-widest transition-all duration-200 {{ request()->routeIs('bar.tables.*') ? 'text-orange-500 bg-orange-500/10 rounded-xl border-b-2 border-orange-500 shadow-[0_4px_10px_rgba(234,88,12,0.2)]' : 'text-gray-400 hover:text-white' }}">
                            {{ __('Mesas') }}
                        </x-nav-link>

                        <x-nav-link :href="route('bar.cash.index')" :active="request()->routeIs('bar.cash.*')"
                            class="px-4 py-2 text-xs font-black uppercase tracking-widest transition-all duration-200 {{ request()->routeIs('bar.cash.*') ? 'text-orange-500 bg-orange-500/10 rounded-xl border-b-2 border-orange-500 shadow-[0_4px_10px_rgba(234,88,12,0.2)]' : 'text-gray-400 hover:text-white' }}">
                            {{ __('Caixa') }}
                        </x-nav-link>

                        <x-nav-link :href="route('bar.relatorios.index')" :active="request()->routeIs('bar.relatorios.*')"
                            class="px-4 py-2 text-xs font-black uppercase tracking-widest transition-all duration-200 {{ request()->routeIs('bar.relatorios.*') ? 'text-orange-500 bg-orange-500/10 rounded-xl border-b-2 border-orange-500 shadow-[0_4px_10px_rgba(234,88,12,0.2)]' : 'text-gray-400 hover:text-white' }}">
                            {{ __('Relatórios') }}
                        </x-nav-link>
                    </div>
                @endif
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-800 focus:outline-none transition duration-150">
