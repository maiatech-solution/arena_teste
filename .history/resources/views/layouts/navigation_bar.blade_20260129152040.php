<nav x-data="{ open: false }" class="bg-gray-900 border-b border-orange-600/30 shadow-2xl">
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden border-t border-gray-800 bg-gray-900">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->has_admin_access)
                <x-responsive-nav-link :href="route('admin.company.edit')" class="text-gray-300">Dados do
                    estabelecimento</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('admin.users.index')" class="text-gray-300">Gerenciar
                    Usuários</x-responsive-nav-link> {{-- ✅ Adicionado no Mobile --}}
            @endif
