    ...
    <style>
       ...
       /* ✅ PADRÃO: Estilo para Eventos RESOLVIDOS (PAGO, ATRASADO, CANCELADO, REJEITADO) */
        .fc-event-paid {
            ...
            color: #ffffff !important;
            opacity: 0.6 !important; /* Efeito de "passado" */
            filter: grayscale(80%);
            font-weight: normal !important;
        }

        /* ✅ GARANTIR QUE O TEXTO FIQUE BRANCO EM EVENTOS ESCUROS */
        .fc-event-recurrent, .fc-event-quick, .fc-event-no-show {
            color: #ffffff !important;
        }
...
function showDashboardMessage(message, type = 'success') {
    ...
    const isResolvedStatus = ['concluida', 'lancada_caixa', 'cancelada', 'rejeitada', 'no_show'].includes(status);
    ...
    if (status === 'pending') {
        ...
        prefix = status === 'cancelada' ? '(CANCEL)' : (status === 'rejeitada' ? '(REJEIT)' : '(PAGO)');
    }
