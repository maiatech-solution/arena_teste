...
{{-- MODAIS RESTANTES (CANCELAMENTO, FALTA, RENOVAÇÃO) --}}
    ...
    <script>
        ...
        const formatMoneyQuick = (input) => {
            ...
            let intPart = value.substring(0, value.length - 2).replace(/^0+/, '') || '0';
        ...
        // === FUNÇÃO CORRIGIDA DE VISIBILIDADE DE MENSAGENS ===
                    ...
                    container.innerHTML = `
                        <div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 mb-6 rounded-lg shadow-md flex justify-between items-center animate-pulse">
                            <div class="flex items-center">
                                ...
                                <div><p class="font-bold">Pendências!</p><p class="text-sm">Existem ${count} pré-reservas.</p></div>
                            </div>
                            <a href="{{ route('admin.reservas.pendentes') }}" class="bg-orange-600 text-white font-bold py-2 px-6 rounded-lg text-sm shadow">Ver</a>
                        </div>`;
                } else {
                    container.innerHTML = '';
                }
            } catch (e) { console.error("Erro check pendencias", e); }
        };

        // === SUBMISSÃO DE AGENDAMENTO RÁPIDO ===
        async function handleQuickBookingSubmit(e) {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    const msg = result.errors ? Object.values(result.errors).flat().join(' ') : (result.message || 'Erro');
                    showDashboardMessage(msg, 'warning');
                }
            } catch (error) {
        ...
        // === LÓGICA DE CANCELAMENTO & NO-SHOW ===
        ...
        window.cancelarPontual = (id, isRecurrent, paidVal, isPaid) => {
            ...
            const msg = isRecurrent ? "Cancelar APENAS este horário?" : "Cancelar esta reserva?";
            openCancellationModal(id, 'PATCH', urlBase, msg, paidVal);
        };

        window.cancelarSerie = (id, paidVal, isPaid) => {
            openCancellationModal(id, 'DELETE', CANCEL_SERIE_URL, "⚠️ Cancelar a SÉRIE INTEIRA? Todos os futuros serão apagados.", paidVal);
        };

        function openCancellationModal(id, method, urlBase, msg, paidVal) {
            ...
            if (val > 0) refundArea.classList.remove('hidden');
            else refundArea.classList.add('hidden');
        ...
        document.getElementById('confirm-cancellation-btn').addEventListener('click', async () => {
            ...
            if (reason.length < 5) return showDashboardMessage("Motivo obrigatório (mín. 5 letras).", 'warning');

            const refundChoice = document.querySelector('input[name="refund_choice"]:checked')?.value;
            const shouldRefund = refundChoice === 'refund';

            try {
                const response = await fetch(currentUrlBase.replace(':id', currentReservaId), {
                    method: 'POST',
                ...
                if(res.success) {
                    ...
                    showDashboardMessage(res.message || "Erro ao cancelar", 'error');
                }
        ...
        window.openNoShowModal = (id, name, paidVal, isPaid) => {
            ...
            modal.classList.remove('hidden');
            setTimeout(() => modal.querySelector('#no-show-modal-content').classList.remove('opacity-0', 'scale-95'), 10);
        };
        ...
        document.getElementById('confirm-no-show-btn').addEventListener('click', async () => {
            ...
            if(reason.length < 3) return showDashboardMessage("Motivo obrigatório.", 'warning');

            const refundChoice = document.querySelector('input[name="no_show_refund_choice"]:checked')?.value;
            const id = document.getElementById('no-show-reserva-id').value;

            try {
                const res = await fetch(NO_SHOW_URL.replace(':id', id), {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken},
                    body: JSON.stringify({
                        _method: 'PATCH',
                        no_show_reason: reason,
                        should_refund: refundChoice === 'refund_all',
                        paid_amount: document.getElementById('paid-amount-ref').value
                    })
                });
                ...
                if(data.success) {
                    ...
                    closeNoShowModal();
                    calendar.refetchEvents();
                } else showDashboardMessage(data.message, 'error');
            } catch(e) { showDashboardMessage("Erro de rede", 'error'); }
        });

        // === CALENDÁRIO ===
        document.addEventListener('DOMContentLoaded', () => {
            // Auto-hide blade messages
            setTimeout(() => {
                document.querySelectorAll('.session-msg').forEach(el => el.remove());
            }, 5000);

            // Init global variables for renewal
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventDidMount: function(info) {
                    ...
                    else if (status === 'pending') info.el.classList.add('fc-event-pending');
                    else if (status === 'paid' || status === 'concluida') info.el.classList.add('fc-event-paid');
                ...
                eventClick: function(info) {
                    ...
                    if (e.classNames.includes('fc-event-available')) {
                        ...
                        document.getElementById('slot-info-display').innerHTML = `Data: ${moment(e.start).format('DD/MM')} - ${moment(e.start).format('HH:mm')}`;
                    } else if (props.status === 'pending') {
                        // Abrir Modal de Pendência
                        document.getElementById('pending-reserva-id').value = e.id;
                        document.getElementById('pending-modal-content').innerHTML = `Cliente <b>${e.title}</b> quer reservar.<br>Valor: R$ ${props.price}`;
                        document.getElementById('pending-action-modal').classList.remove('hidden');
                    } else {
                        // Abrir Detalhes
                        let content = `<p><b>Cliente:</b> ${e.title}</p>
                                       <p><b>Status:</b> ${props.status.toUpperCase()}</p>
                                       <p><b>Valor:</b> R$ ${props.price}</p>`;

                        let buttons = `<a href="${PAYMENT_INDEX_URL}?reserva_id=${e.id}" class="block w-full text-center bg-green-600 text-white py-2 rounded mb-2">Ir para Caixa</a>`;

                        // Lógica de botões condicionais
                        if (moment(e.end).isAfter(moment()) && props.status === 'confirmed') {
                            buttons += `<button onclick="cancelarPontual(${e.id}, ${props.is_recurrent}, ${props.total_paid}, ${props.is_paid})" class="w-full bg-red-600 text-white py-2 rounded mb-2">Cancelar</button>`;
                            if(props.is_paid) {
                                buttons += `<button onclick="openNoShowModal(${e.id}, '${e.title}', ${props.total_paid}, true)" class="w-full bg-red-800 text-white py-2 rounded">Marcar Falta (Pago)</button>`;
                            }
                        } else if (moment(e.end).isBefore(moment()) && props.status === 'confirmed') {
                            // Evento passado pendente de ação
                             buttons += `<button onclick="openNoShowModal(${e.id}, '${e.title}', ${props.total_paid}, ${props.is_paid})" class="w-full bg-red-800 text-white py-2 rounded">Registrar Falta</button>`;
                        }

                        buttons += `<button onclick="closeEventModal()" class="w-full bg-gray-300 py-2 rounded mt-2">Fechar</button>`;
            ...
            document.querySelectorAll('.input-money-quick').forEach(i => i.addEventListener('input', e => e.target.value = formatMoneyQuick(e.target)));
        ...
        window.openRenewalModal = () => {
             ...
             globalExpiringSeries.forEach(s => {
                 list.innerHTML += `<div class="border p-2 rounded flex justify-between"><span>${s.client_name} (${s.slot_time})</span> <button onclick="handleRenewal(${s.master_id})" class="bg-yellow-500 text-white px-2 rounded">Renovar</button></div>`;
             });
        ...
        document.getElementById('reject-pending-btn').addEventListener('click', async () => {
            ...
            if(area.classList.contains('hidden')) {
                ...
                const id = document.getElementById('pending-reserva-id').value;

                try {
                    await fetch(REJECT_PENDING_URL.replace(':id', id), {
        ...
        // Handler de Confirmação de Pendência
        document.getElementById('confirm-pending-btn').addEventListener('click', async () => {
            ...
            try {
                await fetch(CONFIRM_PENDING_URL.replace(':id', id), {
                    method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':csrfToken},
