<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <div class="space-y-10">
                    ...
                    <div class="p-6 bg-green-50 dark:bg-green-900/10 rounded-xl shadow-lg border border-green-200 dark:border-green-800" id="series-container-pontual">
                        <div class="flex justify-between items-center border-b border-green-300 dark:border-green-800 pb-3 mb-4">
                            <h3 class="text-xl font-bold text-green-700 dark:text-green-400 uppercase tracking-tighter">
                                Reservas Pontuais
                                <span class="text-sm font-normal text-gray-500 dark:text-gray-400 ml-2">({{ $groupedReservas['pontual']->count() }} registros na página)</span>
                            </h3>
                        </div>
                    ...
                    @foreach ($groupedReservas as $seriesId => $seriesReservas)
                        ...
                        <div class="p-6 bg-fuchsia-50 dark:bg-fuchsia-900/10 rounded-xl shadow-lg border border-fuchsia-200 dark:border-fuchsia-800" id="series-container-{{ $seriesId }}">
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-fuchsia-300 dark:border-fuchsia-800 pb-3 mb-4">
                                <h3 class="text-xl font-bold text-fuchsia-700 dark:text-fuchsia-400 uppercase tracking-tighter">
                                    Série Recorrente #{{ $seriesId }}
                                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400 block md:inline md:ml-3">
                                        ({{ $seriesReservas->count() }} slots | Expira: {{ \Carbon\Carbon::parse($maxDate)->format('d/m/Y') }})
                                    </span>
                                </h3>
                                ...
                                @if ($totalFutureSlots > 0)
                                    ...
                                    @else
                                        <span class="text-[10px] font-bold text-gray-400 border border-gray-300 px-2 py-1 rounded uppercase italic">Ação Restrita a Gestores</span>
                                    @endif
                                @else
                                    <span class="mt-3 md:mt-0 text-xs font-bold text-gray-400 bg-gray-100 px-3 py-1 rounded-full uppercase tracking-widest italic">Série Finalizada</span>
                                @endif
                            </div>
                    ...
                    {{-- Empty State --}}
                    @if ($reservas->isEmpty())
                    <div class="text-center py-20 bg-gray-50 dark:bg-gray-700/50 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-600">
                        ...
                        <p class="mt-4 text-gray-500 dark:text-gray-400 font-medium italic">Nenhuma reserva encontrada para este cliente.</p>
                    </div>
                    @endif
                </div>
    ...
    <div id="series-cancellation-modal" class="fixed inset-0 bg-gray-900 bg-opacity-80 hidden items-center justify-center z-[100] backdrop-blur-sm transition-opacity duration-300">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg p-8 m-4 transform transition-all duration-300 scale-95 opacity-0" id="series-cancellation-modal-content" onclick="event.stopPropagation()">

            <div class="flex items-center mb-6 text-red-700 dark:text-red-500">
                <svg class="w-8 h-8 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.398 16c-.77 1.333.192 3 1.732 3z"></path></svg>
            ...
            <div class="space-y-4 mb-8">
                <p id="series-cancellation-message" class="text-base text-gray-700 dark:text-gray-300 leading-relaxed">
                    Você removerá permanentemente <span id="slots-count-placeholder" class="font-black text-red-600 text-xl px-1"></span> reservas futuras da série <span class="font-black text-indigo-600">#<span id="series-id-placeholder"></span></span> pertencente ao cliente <span class="font-black text-gray-900 dark:text-white uppercase"><span id="client-name-placeholder"></span></span>.
                </p>
                <p class="text-sm font-bold text-gray-500 border-l-4 border-red-500 pl-4 bg-red-50 dark:bg-red-900/10 py-2">
            ...
            <form id="series-cancellation-form" onsubmit="return false;">
                ...
                <div class="mb-8">
                    <label for="justificativa-gestor" class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-3">
    ...
    <script>
        ...
        function displayAjaxMessage(message, type = 'success') {
            ...
            const colorClasses = type === 'success' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200';
        ...
        document.getElementById('series-cancellation-form').addEventListener('submit', async function(event) {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    // Efeito visual de remoção do container
                    ...
                    displayAjaxMessage(result.message || 'Erro ao processar requisição.', 'error');
                }
            } catch (error) {
