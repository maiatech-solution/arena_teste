                ...
                <form method="POST" action="{{ route('admin.users.update', $user) }}">
                    ...
                    <div class="mb-6">
                        <label for="whatsapp_contact"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Contato WhatsApp
                            (Opcional)</label>
                        <input type="text" name="whatsapp_contact" id="whatsapp_contact"
                            value="{{ old('whatsapp_contact', $user->whatsapp_contact) }}" placeholder="Ex: 91988887777"
                    ...
                    <div class="mb-6">
                        ...
                        <select name="role" id="role" required
                            ...
                            @elseif ($user->role === 'admin')
                                <option value="admin" selected disabled>Admin (Somente Super Admin pode alterar)
                                </option>
                            @endif
                        </select>
                    </div>
                    ...
                    <div id="vip-container"
                        class="mb-4 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-100 dark:border-indigo-800 {{ old('role', $user->role) !== 'cliente' ? 'hidden' : '' }}">
                    ...
                    {{-- 🚫 Container Blacklist (Versão corrigida para detectar MAIÚSCULAS/minúsculas) --}}
                    <div id="blacklist-container"
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="is_blacklisted" id="is_blacklisted" value="1"
                                {{--
                Lógica Reforçada:
                1. Verifica se o 'old' existe (caso de erro de validação)
                2. Se não houver 'old', ele limpa o texto do banco (trim) e converte para minúsculo (lower)
                para comparar sem erro com 'blacklist'.
            --}}
                                @if (old('is_blacklisted')) checked
            @elseif(!old('_token') && strtolower(trim($user->customer_qualification)) === 'blacklist')
                checked @endif
                    ...
                    <div
                        ...
                        <div class="mb-4">
                            <label for="password"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nova Senha
                                (Opcional)</label>
                        ...
                        <div class="mb-4">
                            <label for="password_confirmation"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300">Confirmar Nova
                                Senha</label>
