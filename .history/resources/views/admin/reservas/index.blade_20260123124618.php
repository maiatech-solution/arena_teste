<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <form action="{{ route('admin.reservas.pendentes') }}" method="GET" class="flex items-center space-x-2">
                <select name="arena_id" onchange="this.form.submit()" class="rounded-md border-gray-300 dark:bg-gray-700 dark:text-white text-sm shadow-sm focus:border-indigo-500">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-6">
                        <a href="{{ route('admin.reservas.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-800 uppercase tracking-widest hover:bg-gray-300 transition">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap text-sm font-bold">
                                        ...
                                        <div class="mt-1 flex items-center text-[10px] text-red-600 dark:text-red-400 font-extrabold uppercase tracking-wider">
                                        ...
                                        $q->where('start_time', '<', $reserva->end_time)
                                            ...
                                            <div class="mt-1 text-xs text-red-600 font-normal flex items-center italic">
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="flex flex-col space-y-2 items-center">
                                            @php
                                            // Definimos a variável aqui para não repetir a chamada da função
                                            $cashIsClosed = \App\Http\Controllers\FinanceiroController::isCashClosed($reserva->date->toDateString());
                                            @endphp

                                            @if($cashIsClosed)
                                            {{-- BOTÃO BLOQUEADO: Aparece quando o caixa está fechado --}}
                                            <button type="button"
                                                disabled
        ...
        '{{ $reserva->price }}',
        '{{ $reserva->arena->name }}' {{-- 🏟️ QUINTO PARÂMETRO ADICIONADO --}}
    )"
                                            ...
                                            {{-- Botão Rejeitar: Sempre ativo para limpeza de pauta --}}
                                            <form action="{{ route('admin.reservas.rejeitar', $reserva->id) }}" method="POST" class="w-full">
                                                @csrf @method('PATCH')
                                                ...
                                                <button type="submit" onclick="return confirm('Rejeitar esta solicitação?')"
