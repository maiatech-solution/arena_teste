<x-app-layout>
                ...
                <div class="space-y-6">
                    ...
                    <div
                        ...
                        <div
                            ...
                            {{-- 📱 NOTIFICAÇÃO DE WHATSAPP PÓS-AÇÃO --}}
                            @if (session('whatsapp_link'))
                                <div
                                    class="p-4 bg-emerald-50 dark:bg-emerald-900/20 border-2 border-emerald-100 dark:border-emerald-800 rounded-2xl mb-2 flex items-center justify-between group animate-pulse">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-[9px] font-black text-emerald-600 uppercase tracking-widest">
                                                Notificação Pronta</p>
                                            <p class="text-[11px] text-emerald-800 dark:text-emerald-200 font-bold">
                                                Avisar cliente via WhatsApp?</p>
                                        </div>
                                    </div>
                                    <a href="{{ session('whatsapp_link') }}" target="_blank"
                                        class="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-md">Enviar</a>
                            ...
                            {{-- 🛠️ STATUS: MANUTENÇÃO (REATIVAR) --}}
                            @if ($reserva->status === 'maintenance')
                                <div class="space-y-3">
                                    <div
                                        class="p-4 bg-pink-50 dark:bg-pink-900/20 border border-pink-100 dark:border-pink-800 rounded-2xl text-center">
                                        <p class="text-xs font-bold text-pink-700 dark:text-pink-300 uppercase">Horário
                                            em Manutenção</p>
                                    </div>
                                    <button type="button" onclick="openReactivateModal()"
                                        class="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-xs uppercase hover:bg-indigo-700 transition shadow-lg">
                            ...
                            {{-- 2️⃣ STATUS: CONFIRMADO OU CONCLUÍDO (PAGAMENTO / NO-SHOW / MANUTENÇÃO) --}}
                            @if (in_array($reserva->status, ['confirmed', 'completed']))
                                <div class="space-y-3">
                                    ...
                                    @else
                                        <div
                                            class="w-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 py-3 rounded-2xl font-black text-[10px] uppercase text-center border border-emerald-100">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 mt-4">
                                        ...
                                        @else
                                            <form action="{{ route('admin.reservas.no_show', $reserva->id) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="w-full bg-red-50 text-red-600 py-3 rounded-2xl font-black text-[10px] uppercase hover:bg-red-600 hover:text-white transition">No-Show
                                                    🚨</button>
                                            </form>
                                        @endif
                                        ...
                                        <button type="button"
                                            @if ($isClosed) onclick="alert('🚫 Caixa Fechado')" class="w-full bg-gray-100 text-gray-400 py-3 rounded-2xl font-black text-[10px] uppercase cursor-not-allowed border border-gray-200"
                    @else onclick="openCancellationModal('{{ $reserva->client_name }}', '{{ $reserva->id }}', '{{ route($cancellationRoute, $reserva->id) }}', 'Cancelar Agendamento')"
                    class="w-full bg-gray-50 text-gray-500 py-3 rounded-2xl font-black text-[10px] uppercase hover:bg-gray-100 transition" @endif>
                            ...
                            {{-- 3️⃣ MOTIVO (SE CANCELADA/REJEITADA) --}}
    ...
    <script>
        ...
        /**
         * Atualiza a página ou volta para a listagem mantendo o contexto
         */
        ...
        function openMaintenanceModal(reservaId, paidAmount) {
            ...
            // Oculta opções financeiras se não houver saldo pago
            if (amount <= 0) {
