<x-app-layout>
                                ...
                                <tr class="odd:bg-white even:bg-gray-50 hover:bg-indigo-50 transition duration-150">
                                    ...
                                    <td class="px-4 py-3 text-sm font-medium min-w-[100px]">
                                        <div class="flex flex-col space-y-1">
                                            ...
                                            @if ($reserva->is_fixed)
                                                {{-- BOTÃO DE MANUTENÇÃO: Passa ID, Destino, Se está ocupado, e Nome do Cliente --}}
                                                ...
                                                @else
                                                    @php
                                                        // Verifica se existe um cliente "atropelando" esse slot fixo
                                                        $isOccupied = $reserva->client_name || $reserva->user_id;
                                                        $clientName =
                                                            $reserva->client_name ??
                                                            ($reserva->user->name ?? 'Cliente Externo');
                                                    @endphp
                                                    <button
                                                        onclick="handleFixedSlotToggle({{ $reserva->id }}, 'maintenance', {{ $isOccupied ? 'true' : 'false' }}, '{{ $clientName }}')"
                                                        class="bg-pink-600 hover:bg-pink-700 text-white px-3 py-1 text-[10px] font-bold rounded shadow uppercase text-center">
                                                        Manutenção
                                                    </button>
                                                @endif
                                            @else
                                                {{-- BOTÕES DE RESERVA ATIVA --}}
                                                @if (in_array($reserva->status, ['confirmed', 'pending']))
                                                    ...
                                                    {{-- ... demais botões de cancelamento que você já tem ... --}}
                                                @endif
                                            @endif
                                        </div>
                                    </td>
    ...
    <script>
         ...
         * Com verificação de ocupação.
         */
        async function handleFixedSlotToggle(id, targetAction, isOccupied = false, clientName = '') {
            const actionText = targetAction === 'confirmed' ? 'disponibilizar (Livre)' :
                'marcar como indisponível (Manutenção)';

            // ✅ NOVA LÓGICA: Se o gestor tentar colocar em manutenção um slot que JÁ tem cliente
            if (targetAction === 'maintenance' && isOccupied) {
                const proceed = confirm(
                    `🚨 ATENÇÃO: Este horário possui uma reserva ativa de: ${clientName}.\n\n` +
                    `Se você confirmar a Manutenção, o cliente precisará ser avisado ou a reserva cancelada manualmente.\n\n` +
                    `Deseja prosseguir com a Manutenção mesmo assim?`
                );
                if (!proceed) return;
            } else {
                // Alerta padrão para slots vazios
            ...
            // A URL UPDATE_SLOT_STATUS_URL deve apontar para o método que altera para 'maintenance' no DB
