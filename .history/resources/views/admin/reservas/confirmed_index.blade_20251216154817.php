<x-app-layout>
                        ...
                        <form method="GET" action="{{ route('admin.reservas.confirmadas') }}" class="flex flex-col md:flex-row items-end md:items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
                            ...
                            <div class="flex space-x-2 w-full md:w-auto items-end flex-grow">
                                ...
                                <div class="flex items-end space-x-1 h-[42px]">
                                    <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white h-full px-4 rounded-lg shadow-md transition duration-150 flex items-center justify-center">
                                    ...
                                    @if (isset($search) && $search || $startDate || $endDate)
                                        <a href="{{ route('admin.reservas.confirmadas', ['only_mine' => $isOnlyMine ? 'true' : 'false']) }}"
                                            class="text-red-500 hover:text-red-700 h-full p-2 transition duration-150 border border-red-200 rounded-lg flex items-center justify-center">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse ($reservas as $reserva)
                                <tr class="odd:bg-white even:bg-gray-50 hover:bg-indigo-50 transition duration-150">
                                    ...
                                    {{-- CÉLULA DE STATUS COM CORREÇÃO CARBON --}}
                                    <td class="px-4 py-3 text-center whitespace-nowrap">
                                        @php
                                            ...
                                            if (in_array($status, ['pending', 'unpaid', 'partial'])) {
                                                ...
                                                // Extraímos apenas a hora limpa do end_time
                                                $horaFinalLimpa = \Carbon\Carbon::parse($reserva->end_time)->format('H:i:s');
                                                // Concatenamos a data da reserva com essa hora limpa
                                    ...
                                    <td class="px-4 py-3 text-left min-w-[120px]">
                                        ...
                                        @else
                                            <span class="text-gray-600 bg-gray-100 px-2 py-0.5 text-xs rounded-full whitespace-nowrap shadow-sm">Cliente Web</span>
                                        @endif
                                    </td>

                                    <td class="px-4 py-3 text-sm font-medium min-w-[100px]">
                                        <div class="flex flex-col space-y-1">
                                            <a href="{{ route('admin.reservas.show', $reserva) }}" class="inline-block w-full text-center bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 text-xs font-semibold rounded-md shadow transition duration-150">Detalhes</a>
                                            <a href="{{ route('admin.payment.index', ['reserva_id' => $reserva->id, 'date' => \Carbon\Carbon::parse($reserva->date)->format('Y-m-d')]) }}" class="inline-block w-full text-center bg-green-600 hover:bg-green-700 text-white px-3 py-1 text-xs font-semibold rounded-md shadow transition duration-150">Lançar Caixa</a>
                                            ...
                                            @if ($reserva->is_recurrent)
                                                <button onclick="openCancellationModal({{ $reserva->id }}, 'PATCH', '{{ route('admin.reservas.cancelar_pontual', ':id') }}', 'Cancelar SOMENTE ESTA reserva recorrente?', 'Cancelar ESTE DIA')"
                                                    class="inline-block w-full text-center bg-yellow-600 hover:bg-yellow-700 text-white px-3 py-1 text-xs font-semibold rounded-md shadow transition duration-150">Cancelar ESTE DIA</button>
                                                <button onclick="openCancellationModal({{ $reserva->id }}, 'DELETE', '{{ route('admin.reservas.cancelar_serie', ':id') }}', 'Cancelar TODA A SÉRIE futura?', 'Cancelar SÉRIE')"
                                                    class="inline-block w-full text-center bg-red-800 hover:bg-red-900 text-white px-3 py-1 text-xs font-semibold rounded-md shadow transition duration-150">Cancelar SÉRIE</button>
                                            @else
                                                <button onclick="openCancellationModal({{ $reserva->id }}, 'PATCH', '{{ route('admin.reservas.cancelar', ':id') }}', 'Deseja cancelar esta reserva pontual?', 'Confirmar Cancelamento')"
                                                    class="inline-block w-full text-center bg-red-600 hover:bg-red-700 text-white px-3 py-1 text-xs font-semibold rounded-md shadow transition duration-150">Cancelar</button>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
    ...
    <div id="cancellation-modal" class="fixed inset-0 bg-gray-600 bg-opacity-75 hidden items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 m-4 transform transition-all" id="cancellation-modal-content">
            ...
            <div class="flex justify-end space-x-3">
                <button onclick="closeCancellationModal()" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition duration-150">Voltar</button>
    ...
    <script>
        ...
        document.getElementById('confirm-cancellation-btn').addEventListener('click', async function() {
            ...
            if (reason.length < 5) return alert("Forneça um motivo com pelo menos 5 caracteres.");
