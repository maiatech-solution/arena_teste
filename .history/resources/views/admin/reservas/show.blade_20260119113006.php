<x-app-layout>
    ...
    <script>
        ...
        safeAddEventListener('maintenanceForm', 'submit', function(e) {
            ...
            fetch(url, {
                ...
                .then(async response => {
                    ...
                    // 📱 Mensagem WhatsApp: Manutenção
                    let msg =
                        `Olá *${clienteNome}*, informamos que o seu horário do dia ${reservaData} às ${reservaHora}h precisou ser interrompido para *MANUTENÇÃO DE EMERGÊNCIA* (${reason}).`;

                    if (financeAction === 'refund') {
                        msg +=
                            `\n\nComo houve devolução do valor no nosso sistema, por favor, *envie sua CHAVE PIX* para realizarmos o estorno do seu pagamento.`;
                    }

                    window.open(`https://wa.me/55${clienteContato}?text=${encodeURIComponent(msg)}`,
                        '_blank');
                    window.location.reload();
                })
        ...
        document.querySelectorAll('button[value="restore_client"]').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                if (confirmar) {
                    ...
                    // Armazena no localStorage para abrir após o reload da página pelo Controller
                    localStorage.setItem('pending_wa_reativacao',
                        `https://wa.me/55${clienteContato}?text=${encodeURIComponent(msgRetorno)}`);
                } else {
                    e.preventDefault();
                }
            });
        ...
        // Verifica se há mensagem de reativação pendente para abrir
        window.addEventListener('load', () => {
            const pendingWA = localStorage.getItem('pending_wa_reativacao');
            if (pendingWA) {
                localStorage.removeItem('pending_wa_reativacao');
                window.open(pendingWA, '_blank');
            }
        });
    </script>
</x-app-layout>
