                ...
                <div class="space-y-6">
                    ...
                    <div
                        ...
                        @php
                            ...
                            // Preparamos o valor pago antes para evitar o erro de sintaxe no onclick
                        ...
                        @if (in_array($reserva->status, ['confirmed', 'completed']))
                            <div class="space-y-3">
                                @php $saldoRestante = $reserva->price - $totalPaid; @endphp
                                ...
                                @if (!$isClosed)
                                    <button type="button"
                                        onclick="{{ $isColab ? "window.requisitarAutorizacao(token => { if(token) openMaintenanceModal('{$reserva->id}', '{$totalPaid}') })" : "openMaintenanceModal('{$reserva->id}', '{$totalPaid}')" }}"
                            ...
                            {{-- 🚩 ZONA DE RISCO: Cancelar e No-Show --}}
                            <div class="pt-4 mt-4 border-t border-gray-100 dark:border-gray-700 space-y-2">
                                <p
                                    class="text-[9px] font-black text-gray-400 uppercase tracking-widest text-center mb-2">
                                    Ações Sensíveis</p>

                                <button type="button"
                                    onclick="{{ $isColab ? 'window.requisitarAutorizacao(token => { if(token) openNoShowModal() })' : 'openNoShowModal()' }}"
                                    class="w-full py-2 text-[10px] font-black uppercase text-gray-400 hover:text-black transition border border-dashed border-gray-200 rounded-xl">
