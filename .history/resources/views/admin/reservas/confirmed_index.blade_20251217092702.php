<x-app-layout>
    ...
    // 1. Verificar se o status é de não pago ou parcial para checar atraso
    if (in_array($status, ['pending', 'unpaid', 'partial'])) {
        // Extraímos apenas o horário para evitar erro de data duplicada
    ...
    // 🎯 CÁLCULO DO SALDO PARA O STATUS PARCIAL
    $saldoParaExibir = (float)$reserva->price - (float)($reserva->total_paid ?? 0);

    // 2. Lógica de exibição final (Badges)
    if ($status === 'paid' || $status === 'completed' || $reserva->status === 'completed') {
        ...
        // ✅ AQUI A MUDANÇA: Usamos a variável $saldoParaExibir que calculamos acima
