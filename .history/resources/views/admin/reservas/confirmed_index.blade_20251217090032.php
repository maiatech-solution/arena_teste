<x-app-layout>
    ...
    // 1. Verificar se o status é de não pago (pending/unpaid) ou parcial
    if (in_array($status, ['pending', 'unpaid', 'partial'])) {

        // 🎯 A CORREÇÃO ESTÁ AQUI:
        // Usamos o format('H:i:s') para extrair apenas o horário, mesmo que end_time tenha data.
        $onlyTime = \Carbon\Carbon::parse($reserva->end_time)->format('H:i:s');

        // Agora montamos a string limpa: "YYYY-MM-DD HH:MM:SS"
        $dateTimeString = \Carbon\Carbon::parse($reserva->date)->format('Y-m-d') . ' ' . $onlyTime;

        try {
            $reservaEndTime = \Carbon\Carbon::parse($dateTimeString);

            // 2. Checar se a hora de término da reserva já passou do momento atual
    ...
    // Lógica de exibição final (Badges)
