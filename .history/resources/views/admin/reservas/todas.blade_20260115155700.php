<x-app-layout>
    ...
    <script>
        ...
        async function handleFixedSlotToggle(id, targetAction, isOccupied = false, clientName = '') {
            // 1. Caso seja Manutenção em um horário que JÁ tem cliente
            if (targetAction === 'maintenance' && isOccupied) {
                const proceed = confirm(
                    `🚨 CONFLITO COM RESERVA ATIVA!\n\n` +
                    `O cliente "${clientName}" já reservou este horário.\n\n` +
                    `Como a manutenção é um imprevisto da Arena, o ideal é tratar isso diretamente na reserva para não prejudicar o histórico do cliente.\n\n` +
                    `Deseja ir para a tela de DETALHES desta reserva agora?`
                );

                if (proceed) {
                    // Redireciona para a rota 'show' da reserva
                    // Montamos a URL dinamicamente injetando o ID
                    window.location.href = `/admin/reservas/${id}/show`;
                    return;
                }
                return; // Cancela a operação se não quiser redirecionar
            }

            // 2. Lógica padrão para Slots Vazios (Inventário)
            const actionText = targetAction === 'confirmed' ? 'disponibilizar (Livre)' : 'marcar como Manutenção';

            if (!confirm(`Confirma a ação de ${actionText} o horário no sistema?`)) {
                return;
            }

            // Envio AJAX para slots sem conflito
            sendAjaxRequest(id, 'POST', UPDATE_SLOT_STATUS_URL,
                "Ajuste de disponibilidade de grade por manutenção técnica", {
                    status: targetAction
                });
        }
