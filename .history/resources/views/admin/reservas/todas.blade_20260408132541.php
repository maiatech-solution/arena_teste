<x-app-layout>
    ...
    <script>
        ...
        document.getElementById('confirm-price-update-btn')?.addEventListener('click', function() {
            ...
            const userRole = "{{ auth()->user()->role }}"; // Identifica se é colaborador
            ...
            // 2. Define o Escopo (Hoje ou Série)
            let scope = 'single';
            ...
            // 3. Função que realmente envia os dados
            const dispararEnvio = (token = null) => {
                sendAjaxRequest(currentReservaId, 'PATCH', UPDATE_PRICE_URL, justification, {
                    new_price: newPrice,
                    scope: scope,
                    supervisor_token: token // Aqui o token entra na mala do AJAX
                });
            };

            // 4. LÓGICA DE AUTORIZAÇÃO (A CORREÇÃO)
            // Se for colaborador, obriga a pedir senha. Se for admin, envia direto.
            if (userRole === 'colaborador') {
                window.requisitarAutorizacao(token => {
                    if (token) {
                        dispararEnvio(token);
                    } else {
                        // Se cancelar a senha, não faz nada e não trava o botão
                        console.log("Ação cancelada pelo usuário ou falha no token.");
                    }
                });
            } else {
                dispararEnvio();
