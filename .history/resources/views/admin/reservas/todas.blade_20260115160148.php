        ...
        async function handleFixedSlotToggle(id, targetAction, isOccupied = false, clientName = '') {
            ...
            if (targetAction === 'maintenance' && isOccupied) {
                const proceed = confirm(
                    ...
                    `Deseja ser redirecionado para os DETALHES desta reserva para aplicar a manutenção e tratar o pagamento do cliente?`
                );

                if (proceed) {
                    // Rota exata conforme seu arquivo de rotas: /admin/reservas/{reserva}/show
                    window.location.href = `/admin/reservas/${id}/show`;
                    return;
                }
                return; // Cancela se o usuário clicar em "Cancelar" no confirm
            }
