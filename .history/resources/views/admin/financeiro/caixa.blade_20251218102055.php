    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 shadow-sm rounded-xl border border-gray-100 dark:border-gray-700 print:hidden">
                <form method="GET" action="{{ route('admin.financeiro.relatorio_caixa') }}" class="flex flex-wrap items-end gap-4">
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Selecionar Data Específica</label>
                        <input type="date" name="data" value="{{ $data }}"
                               class="w-full rounded-lg border-gray-300 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:ring-indigo-500">
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('admin.financeiro.relatorio_caixa', ['data' => now()->format('Y-m-d')]) }}"
                           class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg font-bold hover:bg-gray-200 transition text-sm flex items-center">
                            Hoje
                        </a>
                    </div>

                    <div class="ml-auto border-l pl-4">
                        <button type="button" onclick="window.print()" class="bg-emerald-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-emerald-700 transition flex items-center gap-2 text-sm">
            ...
            <div class="bg-white dark:bg-gray-800 p-8 shadow-lg rounded-xl print:shadow-none print:p-0 border border-gray-100 dark:border-gray-700">
                    ...
                    <div class="text-right">
                        @if($caixaStatus && $caixaStatus->status == 'closed')
                            <span class="bg-green-100 text-green-700 px-4 py-1 rounded-full text-xs font-black uppercase border border-green-200">
                                ✅ Caixa Conferido & Fechado
                            </span>
                        @else
                            <span class="bg-amber-100 text-amber-700 px-4 py-1 rounded-full text-xs font-black uppercase border border-amber-200">
                                🔓 Caixa Aberto / Em Movimento
                            </span>
                        @endif
                        <p class="text-[10px] text-gray-400 mt-2 italic">Gerado em: {{ now()->format('d/m/Y H:i') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    <div class="space-y-3">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900/40 p-4 rounded-xl space-y-2 border border-gray-100 dark:border-gray-700">
                            ...
                            @empty
                                <p class="text-gray-400 text-xs italic">Nenhuma movimentação registrada.</p>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="bg-indigo-600 dark:bg-indigo-900 p-8 rounded-2xl flex flex-col justify-center items-center shadow-inner text-white relative overflow-hidden">
                        ...
                        <span class="text-xs font-bold uppercase opacity-80 tracking-widest">Saldo Total em Sistema</span>
                        <span class="text-4xl font-black mt-2">R$ {{ number_format($movimentacoes->sum('amount'), 2, ',', '.') }}</span>
                        <p class="text-[10px] mt-2 opacity-60">Soma de todas as entradas líquidas do dia</p>
                    </div>
                </div>
                            ...
                            @foreach($movimentacoes as $m)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition">
                                    ...
                                    <td class="py-3 px-2">
                                        ...
                                        <div class="text-[11px] text-gray-500">{{ $m->reserva->client_name ?? 'Lançamento Manual / Avulso' }}</div>
                                    </td>
                                    <td class="py-3 px-2 text-center uppercase">
                                        <span class="text-[10px] px-2 py-0.5 rounded border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 font-bold">
                                            {{ $m->payment_method }}
                                        </span>
                                    </td>
                    ...
                    <div class="space-y-2">
                        ...
                        @if($caixaStatus && $caixaStatus->closed_by_user_id)
                             <p class="text-[9px] text-indigo-500 font-bold italic uppercase tracking-widest">
                                Fechado por: {{ \App\Models\User::find($caixaStatus->closed_by_user_id)->name }}
                             </p>
                        @endif
                    </div>
                    <div class="space-y-2">
                        <div class="border-b border-gray-400 dark:border-gray-600 w-full h-8"></div>
                        <p class="text-[10px] uppercase font-black text-gray-400 tracking-tighter">Conferência (Diretoria / Gerência)</p>
                    </div>
                </div>
            </div>
        </div>
