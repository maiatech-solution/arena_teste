                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm divide-y dark:divide-gray-700 font-bold">
                            ...
                            @foreach ($cancelamentos as $c)
                                @php
                                    // 1. Filtra todas as movimentações dessa reserva (#ID)
                                    $movimentacaoVinculada = $multasAvulsas->filter(function ($m) use ($c) {
                                        return str_contains($m->description, "#{$c->id}");
                                    });

                                    // 2. Calcula o Saldo Real que ficou no bolso (Sinais - Estornos)
                                    $saldoNoBolso = $movimentacaoVinculada->sum('amount');

                                    // 3. O prejuízo só diminui se o saldo for positivo (Multa Retida)
                                    // Se o saldo for 0 ou negativo (Estornou tudo), o prejuízo é o preço cheio da reserva.
                                    $prejuizoLinha = $c->price - ($saldoNoBolso > 0 ? $saldoNoBolso : 0);
                                @endphp

                                <tr class="hover:bg-red-50/30 dark:hover:bg-red-900/10 transition duration-150">
                                    {{-- ... colunas de data e arena ... --}}
                                    ...
                                    <td class="p-4">
                                        ...
                                        <div
                                            ...
                                            {{-- Labels Inteligentes baseado no Saldo --}}
                                            @if ($saldoNoBolso < 0 || ($movimentacaoVinculada->where('amount', '<', 0)->count() > 0 && $saldoNoBolso == 0))
                                                <span
                                                    class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-gray-100 dark:bg-gray-700 text-gray-500 border border-gray-200 dark:border-gray-600 uppercase tracking-tighter">
                                                    Sinal Estornado
                                                </span>
                                            @elseif($saldoNoBolso > 0)
                                                <span
                                                    ...
                                                    Multa Retida (R$ {{ number_format($saldoNoBolso, 2, ',', '.') }})
                                                </span>
                                            @endif
                                        </div>
                                    </td>
                            ...
                            {{-- 2. LISTAGEM DE MULTAS AVULSAS (Só as sem vínculo #ID) --}}
                            @foreach ($multasAvulsas as $m)
                                @php
                                    $possuiVinculo = $cancelamentos->contains(
                                ...
                                @if (!$possuiVinculo)
                                    <tr
                                        class="bg-gray-50/50 dark:bg-gray-800/50 transition duration-150 border-l-4 border-gray-300">
                                        <td class="p-4 dark:text-gray-300 text-center">
                                            {{ \Carbon\Carbon::parse($m->paid_at)->format('d/m/Y') }}
                                            <div
                                                class="text-[10px] text-gray-400 font-black uppercase italic text-center">
                                                Financeiro</div>
                                        </td>
                                        ...
                                        <td class="p-4">
                                            ...
                                            <div class="text-[10px] text-gray-400 font-mono italic uppercase">Lançamento
                                                Avulso Caixa</div>
                                        </td>
                                        <td class="p-4 text-center">
                                            <span
                                                class="bg-gray-500 text-white px-3 py-1 rounded text-[9px] font-black uppercase shadow-sm">💰
                                                Ajuste</span>
                                        </td>
                                        <td class="p-4 text-right font-mono font-black text-gray-500 text-lg italic">
