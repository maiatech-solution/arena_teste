            ...
            @foreach($movimentacoes as $m)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition duration-150">
                    ...
                    <td class="py-3 px-2 text-center border-r border-gray-50 dark:border-gray-700">
                        @if($m->reserva)
                            <span class="text-indigo-600 dark:text-indigo-400 font-bold tracking-tighter">
                    ...
                    <td class="py-3 px-2 pl-4">
                        ...
                        <div class="text-[11px] text-gray-500 truncate max-w-[150px]">{{ $m->reserva->client_name ?? 'Lançamento Manual' }}</div>
                    </td>

                    {{-- DIFERENCIAÇÃO DE SINAL OU PAGAMENTO --}}
                    <td class="py-3 px-2 text-center">
                        @if($m->type == 'signal')
                            <span class="px-2 py-1 rounded text-[9px] font-black uppercase bg-blue-100 text-blue-700 border border-blue-200">
                                🎫 Sinal
                            </span>
                        @elseif($m->type == 'full_payment' || $m->type == 'payment_settlement')
                            <span class="px-2 py-1 rounded text-[9px] font-black uppercase bg-emerald-100 text-emerald-700 border border-emerald-200">
                                💰 Saldo/Total
                            </span>
                        @else
