    ...
    <tbody class="text-sm divide-y dark:divide-gray-700 font-bold">
        ...
        @foreach ($cancelamentos as $c)
            @php
                ...
                $vinc = $multasAvulsas->filter(fn($m) => str_contains($m->description, "#{$c->id}"));
            ...
            <tr class="hover:bg-red-50/30 dark:hover:bg-red-900/10 transition duration-150">
                ...
                <td class="p-4">
                    ...
                    <div class="text-[10px] text-indigo-500 font-mono italic flex items-center gap-2">
                        {{ $c->client_contact ?? 'S/ Contato' }}

                        {{-- Labels de Auditoria --}}
                        @if ($vinc->contains(fn($m) => $m->amount < 0))
                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-gray-100 text-gray-500 border border-gray-200 uppercase tracking-tighter">
                                Sinal Estornado
                            </span>
                        @elseif($saldoFin > 0)
                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-green-100 text-green-600 border border-green-200 uppercase tracking-tighter">
        ...
        @foreach ($multasAvulsas as $m)
            @php
                $vinculado = $cancelamentos->contains(fn($can) => str_contains($m->description, "#{$can->id}"));
            @endphp
            @if (!$vinculado)
                <tr class="bg-gray-50/50 dark:bg-gray-800/50 border-l-4 border-gray-300">
                    <td class="p-4 text-center">
                        {{ \Carbon\Carbon::parse($m->paid_at)->format('d/m/Y') }}
                        <div class="text-[10px] text-gray-400 font-black uppercase italic">Financeiro</div>
                    </td>
                    <td class="p-4 text-xs text-gray-500 uppercase italic">{{ $m->arena->name ?? 'Geral' }}</td>
                    <td class="p-4">
                        <div class="font-black dark:text-white uppercase">{{ str_replace(['Multa', '#', 'No-Show'], '', $m->description) }}</div>
                        <div class="text-[10px] text-gray-400 font-mono italic">Lançamento Avulso</div>
                    </td>
                    <td class="p-4 text-center">
                        <span class="bg-gray-500 text-white px-3 py-1 rounded text-[9px] font-black uppercase">💰 Ajuste</span>
                    </td>
