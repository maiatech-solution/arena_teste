                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm divide-y dark:divide-gray-700 font-bold">
                            ...
                            @foreach ($cancelamentos as $c)
                                @php
                                    // 🎯 BUSCA ROBUSTA: Tenta achar o ID com ou sem a cerquilha #
                                    $vinc = $multasAvulsas->filter(function ($m) use ($c) {
                                        return str_contains($m->description, "#{$c->id}") ||
                                            str_contains($m->description, "Reserva {$c->id}") ||
                                            str_contains($m->description, "ID: {$c->id}");
                                    });

                                    $saldoFin = $vinc->sum('amount');

                                    // Cálculo: Preço menos o que ficou (se saldo > 0).
                                    // Ex Adalberton: 120 - 40 = 80.
                                ...
                                <tr class="hover:bg-red-50/30 dark:hover:bg-red-900/10 transition duration-150">
                                    ...
                                    <td class="p-4">
                                        ...
                                        <div
                                            ...
                                            @if ($vinc->where('amount', '<', 0)->count() > 0)
                                                <span
