<x-app-layout>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div :class="!editando ? 'opacity-70' : ''">
                                    ...
                                    <x-text-input id="nome_fantasia" name="nome_fantasia" type="text"
                                        class="mt-1 block w-full bg-gray-50" :value="old('nome_fantasia', $info->nome_fantasia)" required />
                                </div>

                                <div :class="!editando ? 'opacity-70' : ''">
                                    <x-input-label for="cnpj" :value="__('CNPJ (Opcional)')" />
                                    <x-text-input id="cnpj" name="cnpj" type="text"
                                        class="mt-1 block w-full bg-gray-50" :value="old('cnpj', $info->cnpj)" />
                                </div>

                                <div :class="!editando ? 'opacity-70' : ''">
                                    <x-input-label for="email_contato" :value="__('E-mail de Contato')" />
                                    <x-text-input id="email_contato" name="email_contato" type="email"
                                        class="mt-1 block w-full bg-gray-50" :value="old('email_contato', $info->email_contato)" />
                                </div>

                                <div :class="!editando ? 'opacity-70' : ''">
                                    <x-input-label for="whatsapp_suporte" :value="__('WhatsApp da arena')" />
                                    <x-text-input id="whatsapp_suporte" name="whatsapp_suporte" type="text"
                                        class="mt-1 block w-full bg-gray-50" :value="old('whatsapp_suporte', $info->whatsapp_suporte)" maxlength="11" />
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div :class="!editando ? 'opacity-70' : ''">
                                        ...
                                        <x-text-input id="cep" name="cep" type="text"
                                            class="mt-1 block w-full bg-gray-50" :value="old('cep', $info->cep)"
                                    ...
                                    <div class="md:col-span-2" :class="!editando ? 'opacity-70' : ''">
                                        ...
                                        <x-text-input id="logradouro" name="logradouro" type="text"
                                            class="mt-1 block w-full bg-gray-50" :value="old('logradouro', $info->logradouro)" />
                                    </div>

                                    <div :class="!editando ? 'opacity-70' : ''">
                                        <x-input-label for="numero" :value="__('Número')" />
                                        <x-text-input id="numero" name="numero" type="text"
                                            class="mt-1 block w-full bg-gray-50" :value="old('numero', $info->numero)" />
                                    </div>

                                    <div :class="!editando ? 'opacity-70' : ''">
                                        <x-input-label for="bairro" :value="__('Bairro')" />
                                        <x-text-input id="bairro" name="bairro" type="text"
                                            class="mt-1 block w-full bg-gray-50" :value="old('bairro', $info->bairro)" />
                                    </div>

                                    <div :class="!editando ? 'opacity-70' : ''">
                                        <x-input-label for="cidade" :value="__('Cidade')" />
                                        <x-text-input id="cidade" name="cidade" type="text"
                                            class="mt-1 block w-full bg-gray-50" :value="old('cidade', $info->cidade)" />
                                    </div>

                                    <div :class="!editando ? 'opacity-70' : ''">
                                        <x-input-label for="estado" :value="__('UF')" />
                                        <x-text-input id="estado" name="estado" type="text"
                                            class="mt-1 block w-full bg-gray-50" :value="old('estado', $info->estado)" maxlength="2" />
                                    </div>
                                </div>
                            </div>
                        </fieldset>
    ...
    <script>
        async function consultarCep(cep) {
            ...
            if (valor.length === 8) {
                // Feedback visual: avisa o usuário que está buscando
                ...
                try {
                    // Usando BrasilAPI (Agregador mais resiliente que o ViaCEP)
                    ...
                    // Foca no número para facilitar para o gestor
                    document.getElementById('numero').focus();

                } catch (error) {
                    console.error('Erro na busca do CEP:', error);

                    // Se der erro (como o 502 que você viu antes), avisa e libera para manual
                    alert('O serviço de busca de CEP está instável. Por favor, preencha os dados manualmente.');

                    // Limpa os textos de "Buscando..." para permitir a digitação manual
                    campos.forEach(id => {
                        const el = document.getElementById(id);
                        if (el) el.value = '';
                    });
