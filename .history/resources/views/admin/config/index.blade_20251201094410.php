<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg mb-8">
                <div class="p-6 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    {{-- ✅ MENSAGEM DE PROCESSO AUTOMÁTICO --}}
                    <div class="mt-4 p-4 bg-blue-100 border border-blue-400 rounded-lg dark:bg-blue-900 dark:border-blue-700 mb-6">
                        <p class="text-sm font-semibold text-blue-800 dark:text-blue-200">
                            ✅ Processo Automático: As reservas fixas (slots disponíveis) são agora **geradas automaticamente** após você clicar em "Salvar Configuração Semanal", respeitando a duração em meses definida para cada dia.
                        </p>
                    </div>
                    ...
                    <form id="config-form" action="{{ route('admin.config.store') }}" method="POST">
                        ...
                        {{-- 🛑 CAMPO DE DURAÇÃO GLOBAL REMOVIDO --}}
                        ...
                        <div class="space-y-6">
                            ...
                            @foreach (\App\Models\ArenaConfiguration::DAY_NAMES as $dayOfWeek => $dayName)
                                @php
                                    ...
                                    // 🆕 NOVO: Obtém o valor de meses salvo, se existir, senão usa 12 (1 ano)
                                    $savedRecurrentMonths = $configModel ? ($configModel->recurrent_months ?? 12) : 12;
                                ...
                                <div class="p-4 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-inner">
                                    ...
                                    {{-- 🆕 NOVO CAMPO: DURAÇÃO DA RECORRÊNCIA POR DIA --}}
                                    <div id="months-input-{{ $dayOfWeek }}" class="mt-4 p-3 bg-indigo-50 border border-indigo-200 rounded-lg dark:bg-indigo-900 dark:border-indigo-700"
                                        style="{{ !$isDayActive ? 'display: none;' : '' }}">
                                        <label for="recurrent_months_{{ $dayOfWeek }}" class="block text-sm font-bold text-indigo-700 dark:text-indigo-200 mb-2">
                                            DURAÇÃO da Recorrência para {{ $dayName }} (em meses)
                                        </label>
                                        <input type="number" name="recurrent_months[{{ $dayOfWeek }}]" id="recurrent_months_{{ $dayOfWeek }}"
                                               value="{{ old('recurrent_months.' . $dayOfWeek, $savedRecurrentMonths) }}"
                                               min="1" max="52"
                                               required
                                               class="mt-1 block w-full sm:w-1/4 border-indigo-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-indigo-800 dark:border-indigo-600 dark:text-white text-lg font-mono text-center"
                                               {{ !$isDayActive ? 'disabled' : '' }}>
                                        <p class="text-xs text-indigo-600 dark:text-indigo-400 mt-1">
                                            Defina por quantos meses (máx: 52) os horários de {{ $dayName }} serão gerados.
                                        </p>
                                        @error('recurrent_months.' . $dayOfWeek)
    ...
    <script>
        ...
        // 🎯 CORREÇÃO 1: Inicialização do nextIndex mais robusta para garantir que é um número.
        // Usa o length como próximo índice disponível (já que os índices começam em 0)
        @foreach (\App\Models\ArenaConfiguration::DAY_NAMES as $dayOfWeek => $dayName)
        ...
        // 🎯 CORREÇÃO 2: Garantir que o botão Adicionar e os inputs de meses estão sendo habilitados.
        function updateDayInputsState(dayOfWeek, isDisabled) {
            const slotContainer = document.getElementById(`slots-container-${dayOfWeek}`);
            const monthInputContainer = document.getElementById(`months-input-${dayOfWeek}`);

            const monthInput = document.getElementById(`recurrent_months_${dayOfWeek}`);
            const addBtn = document.querySelector(`.add-slot-btn[data-day="${dayOfWeek}"]`);

            // Inputs dentro do slotContainer
            // ⚠️ CUIDADO: Se esta linha falhar, o JS para aqui. Garanta que slotContainer não é null.
            if (!slotContainer) return;

            const inputs = slotContainer.querySelectorAll('input[type="time"], input[type="number"], .slot-active-checkbox');
            const deleteBtns = slotContainer.querySelectorAll('.delete-slot-config-btn');


            // 1. Alterna a visibilidade do container principal de slots
            slotContainer.style.display = isDisabled ? 'none' : 'block';

            // 2. Input de Meses
            if (monthInput) {
                // 🎯 CORREÇÃO 3: Habilita/Desabilita o input e o container de meses
                monthInput.disabled = isDisabled;
            }
            if (monthInputContainer) {
                monthInputContainer.style.display = isDisabled ? 'none' : 'block';
            }
            ...
            // 🎯 CORREÇÃO 4: Habilita/Desabilita o botão Add Slot
        ...
        document.querySelectorAll('.day-toggle-master').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                const slotContainer = document.getElementById(`slots-container-${day}`);

                if (!isDisabled) {
                    // Se estiver ativando o dia, garanta que o primeiro slot visível seja ativado
                    const firstSlotCheckbox = slotContainer.querySelector('.slot-active-checkbox');
                    if (firstSlotCheckbox) {
                        firstSlotCheckbox.checked = true;
                    }
                } else {
                    // Se estiver desativando o dia, desativa todos os slots internos
                    slotContainer.querySelectorAll('.slot-active-checkbox').forEach(cb => cb.checked = false);
                }

                // CHAMA A FUNÇÃO CORRIGIDA
                updateDayInputsState(day, isDisabled);
        ...
        document.querySelectorAll('.add-slot-btn').forEach(button => {
            button.addEventListener('click', function() {
                // 🎯 CORREÇÃO 5: Verifica se o botão está desabilitado ANTES de tentar processar
                if (this.disabled) return;

                const dayOfWeek = this.dataset.day;
                const container = document.getElementById(`slots-container-${dayOfWeek}`);

                // 🎯 CORREÇÃO 6: Garante que o índice é um número (forçando o cast)
                let index = Number(nextIndex[dayOfWeek]);

                // Fallback de segurança caso o index seja NaN ou null
                if (isNaN(index) || index === null || typeof index === 'undefined') {
                    index = container.querySelectorAll('.slot-item').length;
                }
                ...
                nextIndex[dayOfWeek] = index + 1; // Incrementa o índice para a próxima faixa
        ...
        document.addEventListener('DOMContentLoaded', function() {
            @foreach (\App\Models\ArenaConfiguration::DAY_NAMES as $dayOfWeek => $dayName)
                updateRemoveButtonState({{ $dayOfWeek }});
                // 🆕 Inicializa o estado dos inputs de cada dia (incluindo o campo de meses)
                const isDayActive = document.getElementById(`day-active-{{ $dayOfWeek }}`).checked;
                updateDayInputsState({{ $dayOfWeek }}, !isDayActive);
            @endforeach
        });

        // --- RESTANTE DAS FUNÇÕES (Exclusão, Preço, Status) PERMANECEM INALTERADAS ---
