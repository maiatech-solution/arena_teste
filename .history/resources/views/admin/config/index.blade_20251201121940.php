<x-app-layout>
                                      ...
                                      <tr id="row-{{ $reserva->id }}">
                                          ...
                                          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                              <button id="status-btn-{{ $reserva->id }}"
                                                      class="fixed-reserva-status-btn {{ $reserva->status === 'free' ? 'status-confirmed' : 'status-cancelled' }}"
                                                      data-id="{{ $reserva->id }}"
                                                      data-current-status="{{ $reserva->status }}"
                                                      onclick="toggleStatus({{ $reserva->id }})">
                                                  {{-- ✅ CORREÇÃO APLICADA AQUI: O status 'free' significa 'Disponível' --}}
                                                  {{ $reserva->status === 'free' ? 'Disponível' : 'Indisponível (Manutenção)' }}
                                              </button>
                                          </td>
                                      </tr>
                                  @empty
    ...
    <script>
        ...
        async function toggleStatus(id) {
            ...
            const newStatus = currentStatus === 'free' ? 'cancelled' : 'confirmed'; // ✅ CORRIGIDO: Agora o JS usa 'free' como status atual para decidir a ação
            ...
            try {
                ...
                if (response.ok && result.success) {
                    // ✅ AÇÃO CRÍTICA: Atualiza o status na UI para o status REAL do DB ('free' ou 'cancelled')
                    const finalDbStatus = newStatus === 'confirmed' ? 'free' : 'cancelled';
                    button.setAttribute('data-current-status', finalDbStatus);

                    if (finalDbStatus === 'free') {
