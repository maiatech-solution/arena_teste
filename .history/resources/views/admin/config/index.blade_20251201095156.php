<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg mb-8">
                <div class="p-6 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    {{-- ✅ MENSAGEM DE PROCESSO AUTOMÁTICO ATUALIZADA --}}
                    ...
                    <form id="config-form" action="{{ route('admin.config.store') }}" method="POST">
                        ...
                        {{-- 🆕 CAMPO DE DURAÇÃO FIXA (ESCONDIDO) --}}
                        <div class="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg dark:bg-indigo-900 dark:border-indigo-700">
                            <label for="recurrent_months" class="block text-sm font-bold text-indigo-700 dark:text-indigo-200 mb-2">
                                DURAÇÃO da Recorrência (em meses)
                            </label>
                            <input type="number" name="recurrent_months" id="recurrent_months"
                                   value="{{ old('recurrent_months', 6) }}"
                                   min="1" max="52"
                                   required
                                   class="mt-1 block w-full sm:w-1/3 border-indigo-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-indigo-800 dark:border-indigo-600 dark:text-white text-lg font-mono text-center">
                            <p class="text-xs text-indigo-600 dark:text-indigo-400 mt-1">
                                O agendamento recorrente será criado automaticamente por **6 meses**. Altere este campo se precisar de outra duração.
                            </p>
                            @error('recurrent_months')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        {{-- FIM CAMPO DE DURAÇÃO --}}
                            ...
                            @foreach (\App\Models\ArenaConfiguration::DAY_NAMES as $dayOfWeek => $dayName)
                                ...
                                <div class="p-4 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-inner">
                                    <div class="flex items-center space-x-4 mb-4 border-b pb-2 justify-between">
                                        ...
                                        {{-- Botão de Exclusão de Dia Inteiro --}}
                                        @if ($isDayActive)
                                        <button type="button"
    ...
    <script>
        ...
        // 🔄 FUNÇÃO ORIGINAL RESTAURADA (para lidar apenas com slots)
        function updateSlotInputsState(dayOfWeek, isDisabled) {
            const container = document.getElementById(`slots-container-${dayOfWeek}`);

            // ⚠️ Verificação de segurança: O container DEVE existir quando esta função é chamada.
            if (!container) return;
        ...
        document.querySelectorAll('.add-slot-btn').forEach(button => {
            button.addEventListener('click', function() {
                // ⚠️ Verificação de segurança extra para evitar cliques se desabilitado
                if (this.disabled) return;

                const dayOfWeek = this.dataset.day;
                const container = document.getElementById(`slots-container-${dayOfWeek}`);

                // 🎯 CORREÇÃO DE ROBUSTEZ: Garantir que index é lido como número
        ...
        document.addEventListener('DOMContentLoaded', function() {
            @foreach (\App\Models\ArenaConfiguration::DAY_NAMES as $dayOfWeek => $dayName)
                updateRemoveButtonState({{ $dayOfWeek }});
                // Garante que o estado inicial dos inputs está correto (disabled se o dia estiver inativo)
                const isDayActive = document.getElementById(`day-active-{{ $dayOfWeek }}`).checked;
                updateSlotInputsState({{ $dayOfWeek }}, !isDayActive);
            @endforeach
        });
