<x-app-layout>
    ...
    <style>
        ...
        /* Cor para Disponível (STATUS_FREE) */
        .status-free {
            background-color: #d1fae5; /* Green 100 */
            color: #065f46; /* Green 900 */
        }
        /* Cor para Manutenção (STATUS_MANUTENCAO) */
        .status-maintenance {
            background-color: #fee2e2; /* Red 100 */
            color: #991b1b; /* Red 900 */
        }
        /* Cor para Reservas de Cliente (PENDING/CONFIRMED) */
        .status-client-occupied {
             background-color: #ffedd5; /* Orange 100 */
             color: #9a3412; /* Orange 900 */
        }
        /* Cor para Cancelada (STATUS_CANCELADA) */
        .status-cancelled {
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- ... Tabela de Gerenciamento de Reservas Fixas Geradas ... --}}
             <div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg">
                 <div class="p-6 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                      <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Gerenciar Horários Recorrentes Gerados (Próximas Reservas)</h3>
                      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Esta lista exibe o calendário completo (slots fixos e reservas de clientes). Use os botões para cancelar reservas de clientes ou marcar slots fixos para manutenção.</p>
                          ...
                          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                              ...
                              <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                  @forelse ($fixedReservas as $reserva)
                                      @php
                                        // 1. Mapeamento de Status para Classes CSS
                                        $statusClass = '';
                                        $isClientReservationActive = !$reserva->is_fixed && in_array($reserva->status, ['confirmed', 'pending']);

                                        if ($reserva->is_fixed) {
                                            $statusClass = $reserva->status === 'free' ? 'status-free' : ($reserva->status === 'maintenance' ? 'status-maintenance' : 'status-cancelled');
                                        } elseif ($isClientReservationActive) {
                                            $statusClass = 'status-client-occupied';
                                        } else {
                                            $statusClass = 'status-cancelled'; // Cancelada, Rejeitada ou Expirada
                                        }

                                        // 2. Determinação da Ação do Botão
                                        $buttonAction = '';
                                        $buttonText = '';
                                        $isToggleable = true;

                                        if ($reserva->is_fixed) {
                                            if ($reserva->status === 'free') {
                                                // Slot Livre: Ação é marcar como INDISPONÍVEL
                                                $buttonAction = 'cancelled'; // Envia 'cancelled' para o Controller (que salvará 'maintenance')
                                                $buttonText = 'Indisponível (Manut.)';
                                            } elseif ($reserva->status === 'maintenance') {
                                                // Slot em Manutenção: Ação é DISPONIBILIZAR
                                                $buttonAction = 'confirmed'; // Envia 'confirmed' para o Controller (que salvará 'free')
                                                $buttonText = 'Disponibilizar';
                                            } else {
                                                 // Slot Fixo em status inesperado (cancelled)
                                                 $isToggleable = false;
                                                 $buttonText = 'Inválido';
                                            }
                                        } elseif ($isClientReservationActive) {
                                            // Reserva de Cliente Ativa: Ação é CANCELAR
                                            $buttonAction = 'cancelled'; // Envia 'cancelled' para o Controller (que salvará 'cancelled' no cliente)
                                            $buttonText = 'Cancelar Reserva';
                                        } else {
                                            // Reserva de Cliente Cancelada/Expirada/Slot Fixo Inválido: Não é toggleable
                                            $isToggleable = false;
                                            $buttonText = 'Inativo';
                                        }
                                      @endphp
                                      <tr id="row-{{ $reserva->id }}">
                                          ...
                                          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                              @if ($reserva->is_fixed)
                                                  <span class="font-semibold">{{ $reserva->client_name }}</span>
                                              @else
                                                  <span class="font-bold text-indigo-700 dark:text-indigo-400">{{ $reserva->client_name }}</span>
                                              @endif
                                          </td>
                                          ...
                                          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 flex items-center">
                                              ...
                                              @if ($reserva->is_fixed && $reserva->status !== 'maintenance')
                                          ...
                                          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                              {{-- Usamos o Acessor status_text do Model para obter o nome amigável --}}
                                              <span class="fixed-reserva-status-btn {{ $statusClass }}">
                                                  {{ $reserva->status_text }}
                                              </span>
                                          </td>


                                          {{-- Botão de Ações (Toggle) --}}
                                          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                              @if ($isToggleable)
                                              <button id="status-btn-{{ $reserva->id }}"
                                                      class="fixed-reserva-status-btn bg-indigo-500 hover:bg-indigo-600 text-white font-bold"
                                                      data-id="{{ $reserva->id }}"
                                                      data-current-status="{{ $reserva->status }}"
                                                      data-next-action="{{ $buttonAction }}"
                                                      onclick="toggleStatus({{ $reserva->id }})">
                                                  {{ $buttonText }}
                                              </button>
                                              @else
                                                  <span class="text-gray-400 text-xs">{{ $buttonText }}</span>
                                              @endif
                                          </td>
                                      </tr>
                                  @empty
                                      <tr>
                                          <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">Nenhuma reserva futura encontrada. Configure os horários acima.</td>
                                      </tr>
                                  @endforelse
                              </tbody>
                          </table>
    ...
    <script>
        ...
        function toggleEdit(id, isEditing) {
            ...
            // O botão de status é desabilitado/habilitado se estivermos editando o preço
        ...
        async function updatePrice(id) {
            ...
            try {
                ...
                // Tratamento robusto para erros de JSON/Sessão
                let result = {};
                try {
                    result = await response.json();
                } catch (e) {
        ...
        /**
         * Alterna o status do slot fixo ou cancela a reserva do cliente.
         */
        async function toggleStatus(id) {
            const button = document.getElementById(`status-btn-${id}`);
            const currentDbStatus = button.getAttribute('data-current-status');
            const nextAction = button.getAttribute('data-next-action'); // 'confirmed' ou 'cancelled'

            // Usamos a ação que será enviada para o Controller para determinar o texto
            const actionText = nextAction === 'confirmed' ? 'disponibilizar' : (nextAction === 'cancelled' ? 'marcar como indisponível/cancelar' : 'alterar');
            ...
            const editIcon = document.getElementById(`edit-icon-${id}`);
            if (editIcon) editIcon.classList.add('opacity-50', 'pointer-events-none');
            ...
            try {
                ...
                const response = await fetch(url, {
                    ...
                    credentials: 'include', // 🛑 CRÍTICO: Inclui cookies de sessão/autenticação
                    body: JSON.stringify({ status: nextAction }) // Envia o próximo status de Ação (confirmed/cancelled)
                });
                ...
                if (response.ok && result.success) {
                    ...
                    // --- ATUALIZAÇÃO DA LINHA NA UI ---
                    const row = document.getElementById(`row-${id}`);
                    if (row) {
                        const statusSpan = row.querySelector('td:nth-child(6) .fixed-reserva-status-btn');
                        const priceCell = row.querySelector('td:nth-child(5)'); // Célula do preço
                        ...
                        // Lógica para determinar o NOVO estado da UI
                        // Se a ação era DISPONIBILIZAR (confirmed), o novo status REAL no DB é 'free'.
                        if (nextAction === 'confirmed') {
                             newDbStatus = 'free';
                             newStatusText = 'Livre (Slot)';
                             newClass = 'status-free';
                             newButtonText = 'Indisponível (Manut.)';
                             newButtonAction = 'cancelled';

                             // Reabilita edição de preço, se for um slot fixo
                             if (priceCell) priceCell.querySelectorAll('.icon-edit, .price-input').forEach(el => el.classList.remove('hidden'));

                        // Se a ação era INDISPONIBILIZAR/CANCELAR (cancelled)...
                        } else if (nextAction === 'cancelled') {
                             // Se o status atual indica que é um SLOT FIXO ('free' ou 'maintenance'), o Controller salva 'maintenance'.
                             if (currentDbStatus === 'free' || currentDbStatus === 'maintenance') {
                                 newDbStatus = 'maintenance';
                                 newStatusText = 'Manutenção';
                                 newClass = 'status-maintenance';
                                 newButtonText = 'Disponibilizar';
                                 newButtonAction = 'confirmed';

                                 // Desabilita edição de preço
                                 if (priceCell) priceCell.querySelectorAll('.icon-edit, .icon-save, .price-input').forEach(el => el.classList.add('hidden'));

                             } else {
                                 // Era uma RESERVA DE CLIENTE ATIVA -> Agora é CANCELADA
                                 newDbStatus = 'cancelled';
                                 newStatusText = 'Cancelada';
                                 newClass = 'status-cancelled';
                                 newButtonText = 'Inativo';
                                 newButtonAction = ''; // Inativo

                                 // Desabilita o botão de vez e remove ícones de edição
                                 button.disabled = true;
                                 if (editIcon) editIcon.remove(); // Remove o ícone de edição (não se edita preço de reserva cancelada)
                                 button.parentNode.innerHTML = `<span class="text-gray-400 text-xs">${newButtonText}</span>`;
                                 return; // Sai após atualizar status e desabilitar

                             }
                        }

                        // Aplica as mudanças no botão e no status span
                        button.setAttribute('data-current-status', newDbStatus);
                        button.setAttribute('data-next-action', newButtonAction);
                        button.textContent = newButtonText;

                        // Atualiza a célula de Status (texto e classe)
                        statusSpan.textContent = newStatusText;
                        // Remove todas as classes de status antigas e adiciona a nova
                        statusSpan.className = 'fixed-reserva-status-btn ' + newClass;
                    }
                ...
                if (editIcon) editIcon.classList.remove('opacity-50', 'pointer-events-none');
            }
        }
    </script>
</x-app-layout>
