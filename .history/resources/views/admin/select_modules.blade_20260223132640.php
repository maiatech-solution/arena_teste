    ...
    <div
        ...
        <div class="max-w-5xl w-full text-center space-y-12">
            <div class="space-y-4">
                ...
                <p class="text-gray-400 text-lg">
                    @if ($company->modules_active > 0)
                        Gerenciamento de Plano: @if (Auth::user()->is_admin)
                            Maia e Marcos, vocês podem alterar os módulos aqui!
                        @else
                            Faça o upgrade para liberar novos recursos!
                        @endif
                    @else
            ...
            <form action="{{ route('modules.store') }}" method="POST" class="flex flex-wrap justify-center gap-8">
                ...
                {{-- MÓDULO 1: ARENA --}}
                {{-- Removido 'modules_active == 0' para que o cliente comum não veja no setup inicial --}}
                @if (Auth::user()->is_admin || $company->modules_active == 1 || $company->modules_active == 3)
                    <button type="submit" name="module" value="1"
                        ...
                        <div class="pt-4">
                            @if ($company->modules_active == 1)
                                <span
                                    class="text-[10px] bg-blue-600 text-white px-5 py-2 rounded-full uppercase font-black">Plano
                                    Ativo</span>
                            @elseif($company->modules_active == 3)
                                <span
                                    class="text-[10px] bg-red-500/10 text-red-500 border border-red-500/20 px-5 py-2 rounded-full uppercase font-black group-hover:bg-red-600 group-hover:text-white transition-all">Desativar
                                    Bar (Downgrade)</span>
                            @else
                                <span
                                    class="text-[10px] bg-blue-500/10 text-blue-500 border border-blue-500/20 px-5 py-2 rounded-full uppercase font-black group-hover:bg-blue-600 group-hover:text-white transition-all">Ativar
                                    Arena</span>
                            @endif
                        </div>
                    </button>
                ...
                {{-- MÓDULO 2: PDV --}}
                {{-- Visível para Admin, no Setup Inicial (0) ou se já estiver ativo (2) --}}
                @if (Auth::user()->is_admin ||
                        $company->modules_active == 0 ||
                        $company->modules_active == 2 ||
                        $company->modules_active == 3)
                    <button type="submit" name="module" value="2"
                        ...
                        <div class="pt-4">
                            @if ($company->modules_active == 2)
                                <span
                                    class="text-[10px] bg-orange-600 text-white px-5 py-2 rounded-full uppercase font-black">Plano
                                    Ativo</span>
                            @elseif($company->modules_active == 3)
                                <span
                                    class="text-[10px] bg-red-500/10 text-red-500 border border-red-500/20 px-5 py-2 rounded-full uppercase font-black group-hover:bg-red-600 group-hover:text-white transition-all">Desativar
                                    Arena (Downgrade)</span>
                            @else
                                <span
                                    class="text-[10px] bg-orange-500/10 text-orange-500 border border-orange-500/20 px-5 py-2 rounded-full uppercase font-black group-hover:bg-orange-600 group-hover:text-white transition-all">Ativar
                                    PDV</span>
                            @endif
                        </div>
                    </button>
                ...
                {{-- MÓDULO 3: COMBO FULL --}}
                {{-- Sempre visível para oferecer o upgrade máximo --}}
                <button type="submit" name="module" value="3"
                    ...
                    <div
                        class="absolute top-0 right-0 bg-green-500 text-black text-[9px] font-black px-6 py-1 uppercase tracking-tighter -rotate-45 translate-x-4 translate-y-3">
                        Completo</div>
                    <div class="text-7xl group-hover:scale-110 transition-transform">🚀</div>
                    ...
                    <div class="pt-4">
                        @if ($company->modules_active == 3)
                            <span
                                class="text-[10px] bg-green-600 text-white px-5 py-2 rounded-full uppercase font-black">Plano
                                Ativo</span>
                        @else
                            <span
                                class="text-[10px] bg-green-500/10 text-green-500 border border-green-500/20 px-5 py-2 rounded-full uppercase font-black group-hover:bg-green-600 group-hover:text-white transition-all">Ativar
                                Combo</span>
                        @endif
                    </div>
                </button>
