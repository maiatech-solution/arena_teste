<x-app-layout>
                                    ...
                                    <tr class="{{ $rowHighlight }} transition duration-150">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-center text-sm space-x-1">
                                            @if ($canPay)
                                                ...
                                                <button type="button"
                                                    onclick="markAsDebt({{ $reserva->id }}, '{{ addslashes($reserva->client_name) }}')"
    ...
    <div id="debtModal" class="fixed inset-0 z-50 hidden overflow-y-auto flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl transform transition-all sm:max-w-lg sm:w-full border dark:border-gray-700">
            <form id="debtForm">
                @csrf
                ...
                <div class="p-6">
                    <div class="flex items-center mb-4">
                        ...
                        <h3 class="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight">Autorizar Pagamento Posterior</h3>
                    </div>

                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                        O cliente <span id="debtClientName" class="font-bold text-indigo-600"></span> usufruiu da quadra mas não pagou agora? Descreva o motivo abaixo:
                    </p>

                    <div>
                        <label class="block text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1">Motivo da Pendência</label>
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 px-6 py-3 flex flex-row-reverse gap-2">
                    <button type="submit" id="submitDebtBtn" class="bg-amber-600 text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-amber-700 transition flex items-center">
                        <span id="submitDebtText">CONFIRMAR DÍVIDA</span>
                    </button>
                    <button type="button" onclick="closeDebtModal()" class="text-gray-700 dark:text-gray-300 font-bold text-sm">CANCELAR</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function checkCashierStatus() {
            ...
            // ADICIONADO: 'completed' e 'pago' para garantir que pendências e pagos liberem o caixa
            const finalS = ['pago', 'falta', 'cancelada', 'rejeitada', 'no_show', 'paid', 'completed'];
        ...
        function markAsDebt(id, clientName) {
            if (document.getElementById('js_isActionDisabled')?.value === '1') return alert('🚫 Caixa Fechado.');

            if (!confirm(
                    `O cliente ${clientName} vai pagar em outro dia?\n\nIsso marcará o jogo como concluído hoje, permitindo fechar o caixa, mas o pagamento continuará em aberto para o futuro.`
                    )) return;
