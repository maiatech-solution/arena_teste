...
{{-- 1. BARRA DE FILTRO E KPIS (Grid ajustado para 6 colunas em telas grandes) --}}
<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
    ...
    {{-- ✅ KPI 1: Saldo Global Acumulado (CAIXA TOTAL - Tudo) --}}
    <div class="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
        ...
        <div class="mt-1 text-2xl font-bold text-indigo-700 dark:text-indigo-300">
            {{-- Usa $totalGlobalBalance, que deve ser o saldo do DB --}}
            R$ {{ number_format($totalGlobalBalance ?? 0, 2, ',', '.') }}
        </div>
    </div>

    {{-- ✅ KPI 2: Total de ENTRADAS RECEBIDAS HOJE --}}
    <div class="bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
        <div class="text-sm font-medium text-cyan-600 dark:text-cyan-400">ENTRADAS RECEBIDAS HOJE</div>
        <div class="mt-1 text-2xl font-bold text-cyan-700 dark:text-cyan-300">
            {{-- Usa $totalReceived, o saldo LÍQUIDO do dia. Se só houver entrada, será R$ 250,00 --}}
            R$ {{ number_format($totalReceived ?? 0, 2, ',', '.') }}
        </div>
        <div class="text-xs text-gray-500">
            {{-- 🛑 CORREÇÃO CRÍTICA AQUI: O KPI é a soma de TUDO que é positivo, R$ 250,00 --}}
            {{-- Se você queria R$ 200,00 (só os "signals"), a variável correta é $totalSignalsToday --}}
            Sinais iniciais (R$ {{ number_format($totalSignalsToday ?? 0, 2, ',', '.') }})
        </div>
    </div>

    {{-- 🛑 KPI 3: Pendente (Renomeado para VALORES A RECEBER) --}}
    <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
        <div class="text-sm font-medium text-yellow-600 dark:text-yellow-400">VALORES A RECEBER</div>
    ...
    {{-- 🟢 NOVO KPI 4: Reservas Pagas (RESERVAS) --}}
    <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
        <div class="text-sm font-medium text-green-600 dark:text-green-400">RESERVAS PAGAS</div>
        <div class="mt-1 text-2xl font-bold text-green-700 dark:text-green-300">
            {{-- Variável a ser calculada no Controller: o número de reservas com status 'paid' ou 'completed' --}}
            {{ $paidReservaCount ?? '0' }}
        </div>
        <div class="text-xs text-gray-500">Total de agendamentos concluídos/pagos hoje.</div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        {{-- 🛑 CORREÇÃO: Coluna Sinal para Total Pago --}}
                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Total Pago (R$)</th>
    ...
    <div class="p-6 text-gray-900 dark:text-gray-100">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {{-- 🛑 CORREÇÃO CRÍTICA: Iteração simplificada. O Controller garante que $financialTransactions é um Collection --}}
                    @forelse ($financialTransactions as $transaction)
                        ...
                        @endphp
                        <tr>
                            {{-- CORREÇÃO: Usamos o Carbon do objeto $transaction->paid_at (se o model tiver $dates) ou parseamos a string --}}
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                {{ $transaction->paid_at ? \Carbon\Carbon::parse($transaction->paid_at)->format('H:i:s') : 'N/A' }}
                            </td>
