    ...
    <script>
        ...
        if (!window.__CAIXA_SCRIPT_LOADED) {
            ...
            function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
                ...
                // 🛡️ Prevenção Real de Duplicidade: Se já foi vinculado, sai fora.
                ...
                form.onsubmit = function(e) {
                    ...
                    // Trava global para não clicar duas vezes
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                ...
                                if (json.success) {
                                    // 🖨️ IMPRESSÃO: Só entra se o Controller enviar 'resumo'
                                    if (json.resumo) {
                                        const area = document.getElementById('area-impressao-reserva');
                                        if (area) {
                                            const r = json.resumo;
                                            // Template que aceita tanto Reserva quanto Fechamento
                                            area.innerHTML = `
                                <div style="text-align:center; font-family:monospace; font-size:12px; text-transform:uppercase;">
                                    ...
                                    --------------------------------<br>
                                    ${r.cliente ? `CLIENTE: ${r.cliente}<br>` : ''}
                                    ${r.data ? `DATA: ${r.data}<br>` : ''}
                                    ${r.periodo ? `PERIODO: ${r.periodo}<br>` : ''}
                                    --------------------------------<br>
                                    ${r.metodo ? `METODO: ${r.metodo}<br>` : ''}
                                    ${r.detalhes ? `${r.detalhes}<br>` : ''}
                                    <strong>VALOR: R$ ${r.pago || r.total || '0,00'}</strong><br>
                                    --------------------------------<br>
                                    MAIATECH SYSTEM
                                </div>
                            `;
                                    ...
                                    // Se não tiver resumo, dá o alert (UMA VEZ) e recarrega
                                    alert(json.message);
                                    window.location.reload();
                                } else {
