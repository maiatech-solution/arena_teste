                ...
                form.onsubmit = function(e) {
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                if (json.success) {
                                    // 1. LIMPEZA TOTAL DE MODAIS E FUNDOS
                                    document.querySelectorAll('.modal-backdrop, .modal, #modalSenha').forEach(
                                        el => {
                                    ...
                                    // 2. SE FOR FECHAMENTO (TIVER RESUMO), ABRE O MODAL
                                    if (json.resumo) {
                                        const r = json.resumo;

                                        // Preenchimento dos IDs (Sincronizado com o HTML acima)
                                        ...
                                        // Aqui tratamos se vier 'credito'/'debito' ou 'outros' do PHP
                                        document.getElementById('resumoCredito').innerText = "R$ " + (r
                                            .credito ? r.credito.toFixed(2) : "0,00").replace('.', ',');
                                        document.getElementById('resumoDebito').innerText = "R$ " + (r.debito ?
                                            r.debito.toFixed(2) : "0,00").replace('.', ',');
                    ...
                    // Ações que NÃO precisam de modal de credenciais extra (O fechamento já é validado no PHP)
