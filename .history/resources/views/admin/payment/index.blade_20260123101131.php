    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // 1. Captura o ID diretamente de dentro deste formulário específico
                const idInput = form.querySelector('[name="reserva_id"]') ||
                    form.querySelector('[name="arena_id"]');

                // Se não achar o valor ou se for o teimoso "1", tenta pegar pelo ID do elemento
                let id = idInput ? idInput.value : '';

                // Log de segurança para você ver no console
                console.log(`📡 Tentando enviar ${formId} | ID identificado: ${id}`);

                // 2. Validação: impede o envio se o ID for inválido
                if (!id || id === '1') {
                    alert('Erro: O ID da operação é inválido. Feche o modal e tente novamente.');
                    return;
                }

                // 3. Interface: Bloqueia botão e mostra carregamento
                btn.disabled = true;
                ...
                fetch(urlTemplate.replace('{id}', id), {
                    ...
                    .then(data => {
                        ...
                        window.location.reload(); // Recarrega para atualizar o caixa
                    })
                    .catch(err => {
                        console.error('❌ Erro no envio:', err);
                        alert('Não foi possível processar: ' + err.message);

                        // Destrava o botão para nova tentativa em caso de erro
                        btn.disabled = false;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            try {
                ...
                console.error('❌ Erro ao registrar formulários:', e);
            }
        });
    </script>
</x-app-layout>
