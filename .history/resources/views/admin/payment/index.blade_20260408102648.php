            ...
            function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
                ...
                form.onsubmit = function(e) {
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        // Limpa modais de autorização antes do envio
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                if (json.success) {
                                    // 🛡️ LIMPEZA: Remove fundos de modais anteriores
                                    document.querySelectorAll('.modal-backdrop, .modal').forEach(m => m
                                    .remove());
                                    document.body.classList.remove('modal-open');

                                    if (json.resumo) {
                                        const r = json.resumo;
                                        // Preenche os campos do modal de resumo
                                        document.getElementById('resumoArena').innerText = r.arena;
                                        document.getElementById('resumoData').innerText = "DATA: " + r.data;
                                        document.getElementById('resumoDinheiro').innerText = "R$ " +
                                            parseFloat(r.gaveta).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoPix').innerText = "R$ " + parseFloat(r
                                            .banco).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoDebito').innerText = "R$ " + parseFloat(r
                                            .debito).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoCredito').innerText = "R$ " + parseFloat(
                                            r.credito).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoSistema').innerText = "R$ " + parseFloat(
                                            r.sistema).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoInformado').innerText = "R$ " +
                                            parseFloat(r.informado).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoDiferenca').innerText = "R$ " +
                                            parseFloat(r.diferenca).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoOperador').innerText = r.operador;

                                        // Cor da diferença
                                        const difDiv = document.getElementById('resumoDiferencaCor');
                                        difDiv.style.color = r.diferenca < 0 ? "#dc2626" :
                                        "#16a34a"; // Vermelho ou Verde

                                        // Exibe o modal de resumo
                                        document.getElementById('modalResumoFechamento').classList.remove(
                                            'hidden');
                                    } else {
                                        alert(json.message);
