<x-app-layout>
    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            // 🛑 Inicializa trava global se não existir
            if (typeof window.caixaProcessando === 'undefined') {
            ...
            form.onsubmit = function(e) {
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    // 🛡️ TRAVA DE SEGURANÇA: Se já estiver processando ou o botão estiver desativado, aborta.
                    if (window.caixaProcessando || (btn && btn.disabled)) {
                        console.warn("🚫 Clique duplicado ignorado pelo sistema.");
                        return false;
                    }
                    ...
                    // 🛑 ESTADO DE BLOQUEIO TOTAL
                    window.caixaProcessando = true; // Ativa trava global
                    if (btn) {
                        btn.disabled = true;
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(json => {
                            ...
                            if (json.success || isAlreadyPaid) {
                                // ✅ SUCESSO OU DUPLICIDADE:
                                // Em ambos os casos, a missão foi cumprida.
                                // Matamos qualquer nova interação e recarregamos.
                                ...
                                // Forçamos o recarregamento. A trava global 'caixaProcessando'
                                // garante que se o Tiro 2 chegar agora, ele não entrará no 'else' abaixo.
                                window.location.reload();
                            } else {
                                // ❌ ERRO REAL: (Só entra aqui se não for sucesso E não for duplicidade)
                                alert(json.message || 'Erro ao processar.');

                                // Somente em erro REAL liberamos a trava para o usuário tentar corrigir
                                window.caixaProcessando = false;
