<x-app-layout>
    ...
    <script>
        ...
        function checkCashierStatus() {
            ...
            // 2. NOVA LÓGICA: Contagem de Pendências Reais
            // Em vez de procurar o que está pago, vamos procurar o que impede o fechamento.
            let pendenciasCount = 0;

            // Vasculhamos a tabela em busca de palavras-chave de pendência
            document.querySelectorAll('table:first-of-type tbody tr').forEach(row => {
                const text = row.innerText.toUpperCase();

                // Se a linha contém botões de ação ou status de dívida, é uma pendência
                if (text.includes('$ BAIXAR') ||
                    text.includes('🕒 P. DEPOIS') ||
            ...
            if (pendenciasCount > 0) {
                btn.disabled = true;
                statusEl.innerHTML = `🚨 Pendentes nesta Arena: ${pendenciasCount}`;
