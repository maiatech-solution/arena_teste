...
<div class="space-y-4 mb-6">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div>
            <h3 class="text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest flex items-center mb-1">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <span class="text-[10px] text-indigo-500 font-bold uppercase italic">Exibindo consolidado de todas as unidades</span>
            @endif
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        @empty
            <div class="col-span-full bg-gray-50 dark:bg-gray-800/40 border border-dashed border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-center">
                <p class="text-xs text-gray-400 italic font-medium">Nenhuma movimentação financeira registrada nesta data.</p>
            </div>
        @endforelse
    </div>
</div>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        ...
                                        // 1. PRIORIDADE TOTAL: Se está pago, está pago (independente se o jogo é futuro ou passado)
                                        elseif ($currentStatus === 'paid') {
                                            $statusClass =
                                                'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300';
                                            $statusLabel = 'PAGO';
                                        }
                                        // 2. DÍVIDA: O jogo foi finalizado (completed) mas o financeiro NÃO está pago
                                        elseif ($reserva->status === 'completed' && $currentStatus !== 'paid') {
                                            $statusClass =
                                                'bg-amber-100 text-amber-800 border border-amber-300 animate-pulse';
                                            $statusLabel = 'DÍVIDA ATIVA';
                                        }
                                        // 3. ATRASO: Jogo confirmado que já passou do horário e não foi pago/finalizado
                                        elseif ($isOverdue) {
                                            $statusClass = 'bg-red-700 text-white animate-pulse';
                                            $statusLabel = 'ATRASADO';
                                        }
                                        // 4. PAGAMENTO PARCIAL (Sinal)
                                        elseif ($currentStatus === 'partial') {
                                            $statusClass = 'bg-yellow-100 text-yellow-800';
                                            $statusLabel = 'PARCIAL';
                                        }
                                        // 5. STATUS PADRÃO (PENDENTE OU SINAL)
                                        else {
