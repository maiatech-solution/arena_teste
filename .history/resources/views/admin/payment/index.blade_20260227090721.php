<x-app-layout>
    ...
    <script>
        ...
        function checkCashierStatus() {
            ...
            /**
             * 🛡️ LISTA DE STATUS REAIS:
             * Removi 'parcial' e 'pendente' daqui.
             * Agora, um jogo só conta como "OK" se estiver realmente finalizado
             * ou se for uma dívida já assumida (debt/dívida).
             */
            const finalStatuses = [
                'pago', 'falta', 'cancelada', 'rejeitada', 'no_show',
                'paid', 'completed', 'atrasado', 'dívida ativa', 'dívida', 'debt'
            ];
            ...
            document.querySelectorAll('table:first-of-type tbody tr').forEach(row => {
                ...
                if (statusCell) {
                    ...
                    const isFinalStatus = finalStatuses.some(s => text.includes(s));

                    /**
                     * 🔍 A PROVA REAL:
                     * Se a linha ainda contém botões como "$ BAIXAR" ou "P. DEPOIS",
                     * ela NÃO pode ser considerada completada, mesmo que o status seja "parcial".
                     */
                    const hasActionButtons = row.innerHTML.includes('$ BAIXAR') || row.innerHTML.includes(
                        'P. DEPOIS');

                    if (isFinalStatus && !hasActionButtons) {
                        completedCount++;
                    }
                }
            });

            // 3. Habilitação do Botão (Lógica de bloqueio)
