<x-app-layout>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        ...
                                        // 2. Quanto entrou no caixa especificamente NESTA DATA selecionada:
                                        // Filtramos a coleção de transações pela reserva e pela data do filtro da página
                                        $pagoNoDia = $financialTransactions
                                            ->where('reserva_id', $reserva->id)
                                            ->where('payment_date', $selectedDate)
                                            ->sum('amount');

                                        // 3. Total acumulado que a reserva já tem (Sinais + Pagamentos de hoje)
                                        ...
                                        // 5. Lógica de Atraso (Verifica se o horário já passou e não foi pago)
                                    ...
                                    <tr class="{{ $rowHighlight }} transition duration-150">
                                        ...
                                        {{-- Coluna: Total Pago (DINÂMICO PELO DIA SELECIONADO) --}}
                                        <td class="px-4 py-4 text-right whitespace-nowrap">
                                            ...
                                            @if ($pagoGeral > $pagoNoDia)
                                                <div class="text-[9px] text-gray-400 font-medium italic">
                                                    Acumulado: R$ {{ number_format($pagoGeral, 2, ',', '.') }}
                                                </div>
                                            @endif
                                        </td>
    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            // Captura o cargo do usuário uma única vez
            const userRole = "{{ Auth::user()->role ?? 'guest' }}";

            // 🛑 .onsubmit garante que apenas UM evento de envio exista por formulário
            form.onsubmit = function(e) {
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    // 🛑 TRAVA DE SEGURANÇA: Se o botão já estiver desativado, aborta a execução.
                    // Isso impede que o segundo clique (durante o processamento) dispare a Mensagem 2.
                    ...
                    // Captura de IDs dinâmicos para a URL
                    ...
                    // 🛑 ESTADO DE PROCESSAMENTO: Desativa o botão e mostra o spinner imediatamente
                    if (btn) {
                        btn.disabled = true;
                        btn.dataset.originalText = btn.innerText; // Guarda o texto original (ex: CONCLUIR)
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(json => {
                            if (json.success) {
                                // ✅ SUCESSO: O alerta aparecerá apenas uma vez.
                                // Bloqueamos interações com o form para garantir que o usuário espere o reload.
                                ...
                                // ❌ ERRO DE VALIDAÇÃO: (Onde ocorria a Mensagem 2)
                                // Como travamos o botão no início, esta mensagem só aparecerá se o
                                // primeiro envio realmente falhar.
                                alert(json.message || 'Erro ao processar.');

                                // Reativa o botão para correção de valores se necessário
                                if (btn) {
                                    btn.disabled = false;
                                    btn.innerText = btn.dataset.originalText || "CONCLUIR PAGAMENTO";
                                }
                ...
                if (acoesCriticas.includes(formId) && userRole === 'colaborador') {
                    // Se for dívida, fecha o modal antes de pedir a senha para limpar o visual
