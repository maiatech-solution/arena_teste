            ...
            form.onsubmit = function(e) {
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(res => res.json())
                        .then(json => {
                            // 🛡️ FILTRO ANTI-VERGONHA:
                            // Se a mensagem for exatamente essa, nós matamos o processo aqui mesmo.
                            if (json.message === "Esta reserva já foi baixada anteriormente.") {
                                console.warn("🚫 Tentativa duplicada ignorada pelo sistema.");
                                // Não damos alert. Apenas forçamos o reload para garantir que a tela atualize.
                                window.location.reload();
                                return;
                            }

                            if (json.success) {
                                // ✅ SUCESSO REAL: Aparece apenas no primeiro processamento.
                                alert(json.message);
                                document.body.style.pointerEvents = 'none';
                                window.location.reload();
                            } else {
                                // ❌ ERRO REAL: (Ex: você tentou baixar um valor maior que o total)
                                alert(json.message || 'Erro ao processar.');

                                // Só liberamos a trava se for um erro que o usuário precise corrigir
