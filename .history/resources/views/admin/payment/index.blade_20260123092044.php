<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="space-y-4 mb-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-4">
                    ...
                    {{-- CARD 4: VALORES PENDENTES (As Dívidas que NÃO entram no saldo acima) --}}
                    <div
                        ...
                        <div class="text-[9px] text-amber-600 dark:text-amber-500 mt-1 leading-tight font-bold">
                            * Valores autorizados para "Pagar Depois".
                        </div>
                    </div>
            ...
            <div
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between">
                    ...
                    <div
                        ...
                        @else
                            {{-- STATUS DINÂMICO QUE SERÁ ATUALIZADO PELO JS TAMBÉM --}}
                            <div id="cashStatusContainer">
                                @if (!request('arena_id'))
                                    <span class="font-bold text-amber-500 italic">Selecione uma unidade para
                                        operar...</span>
                                @else
                                    {{-- O JS vai assumir o controle aqui, mas o PHP renderiza o estado inicial --}}
                                    <span id="cashStatus" class="font-bold text-gray-600">Verificando integridade da
                                        arena...</span>
                                @endif
                            </div>
                        @endif
                    ...
                    <div class="w-full sm:w-auto">
                        ...
                        @else
                            @if (request('arena_id'))
                                {{-- O BOTÃO COMEÇA DESABILITADO E O JS LIBERA SE NÃO HOUVER 'PENDENTES' (SEM DÍVIDA ATIVA) --}}
                                <button id="openCloseCashModalBtn" onclick="openCloseCashModal()" disabled
                                    class="w-full sm:w-auto px-6 py-2 bg-gray-400 text-white font-bold rounded-lg shadow-md cursor-not-allowed opacity-70 uppercase tracking-wider text-xs transition-all duration-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div
                    ...
                    <form method="GET" action="{{ route('admin.payment.index') }}" id="dateFilterForm">
                        ...
                        <div class="flex items-center justify-between mb-1">
                            ...
                            @if (request()->has('arena_id') || request()->has('search'))
                                <a href="{{ route('admin.payment.index', ['date' => $selectedDate]) }}"
                                    class="text-[10px] uppercase tracking-wider bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition font-bold"
                                    title="Ver visão geral de todas as arenas">
                                    Limpar Filtros
                                </a>
                            @endif
                        </div>
                        ...
                        <input type="date" name="date" id="date" value="{{ $selectedDate }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white text-base transition-all focus:scale-[1.01]"
                            onchange="this.form.style.opacity='0.5'; this.form.submit()">
                    </form>
                </div>

                {{-- BLOCO 2: BARRA DE PESQUISA --}}
                <div
                    class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg p-5 border border-gray-200 dark:border-gray-700">
                            ...
                            <div class="flex items-end gap-2 mt-auto">
                                <div class="relative flex-1">
                                    <input type="text" name="search" id="search"
                                        value="{{ request('search') }}" placeholder="Ex: Nome do cliente..."
                                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white pr-10">
                                ...
                                <button type="submit"
                                    class="h-[42px] px-4 bg-indigo-600 text-white rounded-md shadow-md hover:bg-indigo-700 transition flex items-center justify-center group">
            ...
            {{-- 4. GESTÃO DE ARENAS E LANÇAMENTOS (UNIFICADO) --}}
            <div class="space-y-4 mb-6">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    {{-- Título e Filtro Ativo --}}
                    <div>
                        <h3
                            class="text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest flex items-center mb-1">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @if (request('arena_id'))
                            <a href="{{ route('admin.payment.index', ['date' => $selectedDate, 'search' => request('search')]) }}"
                                class="inline-flex items-center text-[10px] bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-md font-bold text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white transition uppercase tracking-tighter border border-red-200 dark:border-red-800">
                                <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                                        clip-rule="evenodd"></path>
                                </svg>
                                Limpar Filtro e Ver Geral
                            </a>
                        @else
                            <span class="text-[10px] text-indigo-500 font-bold uppercase italic">Exibindo consolidado
                                de todas as unidades</span>
                        @endif
                    </div>
                    ...
                    <div class="flex-shrink-0">
                        <button onclick="openTransactionModal()"
                            class="w-full md:w-auto inline-flex items-center justify-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-xl font-bold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 transition duration-150 shadow-md hover:scale-105 {{ $isActionDisabled ? 'opacity-50 cursor-not-allowed' : '' }}"
                            {{ $isActionDisabled ? 'disabled' : '' }}>
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                    d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                            </svg>
                            Nova Movimentação (Avulso)
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    @forelse($faturamentoPorArena as $arena)
                        @php $isActive = request('arena_id') == $arena->id; @endphp

                        <a href="{{ route('admin.payment.index', ['date' => $selectedDate, 'arena_id' => $arena->id, 'search' => request('search')]) }}"
                            class="p-4 rounded-2xl shadow-sm border transition-all flex justify-between items-center
                {{ $isActive
                    ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-500 ring-2 ring-indigo-500 ring-opacity-50 border-l-8 border-l-indigo-600 scale-[1.02]'
                    : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 hover:shadow-md hover:border-indigo-300 border-l-4 border-l-indigo-400' }}">
                            ...
                            <div class="{{ $isActive ? 'text-indigo-600' : 'text-indigo-200' }}">
                                @if ($isActive)
                    ...
                    @empty
                        <div
                            class="col-span-full bg-gray-50 dark:bg-gray-800/40 border border-dashed border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-center">
                            <p class="text-xs text-gray-400 italic font-medium">Nenhuma movimentação financeira
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                        <h3 class="text-lg font-bold flex items-center">
                            <span class="bg-indigo-600 text-white p-1.5 rounded-lg mr-3">
                            ...
                            @if (request()->has('reserva_id'))
                                <span class="text-indigo-600 dark:text-indigo-400">Reserva Selecionada <small
                                        class="opacity-50">#{{ request('reserva_id') }}</small></span>
                            @elseif(request()->has('arena_id'))
                                @php $arenaNome = $faturamentoPorArena->firstWhere('id', request('arena_id'))->name ?? 'Arena Selecionada'; @endphp
                                <span>Agendamentos: <span
                                        class="text-indigo-600 dark:text-indigo-400">{{ $arenaNome }}</span></span>
                            @else
                                <span>Agendamentos do Dia <small
                                        class="text-gray-400 ml-2">{{ \Carbon\Carbon::parse($selectedDate)->format('d/m/Y') }}</small></span>
                            @endif
                        </h3>

                        @if (request()->has('reserva_id') || request()->has('arena_id'))
                            <a href="{{ route('admin.payment.index', ['date' => $selectedDate, 'search' => request('search')]) }}"
                                class="inline-flex items-center px-3 py-1.5 border border-gray-300 dark:border-gray-600 text-xs font-bold rounded-lg text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition shadow-sm">
                            ...
                            <thead class="bg-gray-50 dark:bg-gray-700/50">
                                <tr>
                                    ...
                                    <th
                                        ...
                                        Status</th>
                                    <th
                                        class="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase tracking-widest">
                                        Total</th>
                                    <th
                                        class="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase tracking-widest">
                                        Pago</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        ...
                                        // Lógica de Atraso e Status (Sua lógica original mantida e otimizada)
                                        $isOverdue = false;
                                        if (
                                        ...
                                        // Definição de Cores das Linhas e Status
                                        ...
                                        if ($reserva->status === 'no_show') {
                                            ...
                                            $statusClass = 'bg-green-600 text-white shadow-sm';
                                            $statusLabel = 'PAGO';
                                        } elseif ($reserva->status === 'completed' && $currentStatus !== 'paid') {
                                            $statusClass = 'bg-amber-500 text-white animate-pulse';
                                            $statusLabel = 'DÍVIDA';
                                        } elseif ($isOverdue) {
                                    ...
                                    <tr
                                        class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition border-l-4 {{ $restante > 0 ? 'border-l-amber-400' : 'border-l-green-500' }}">
                                        <td
                                            class="px-4 py-4 whitespace-nowrap text-sm font-black text-gray-600 dark:text-gray-400">
                                            {{ \Carbon\Carbon::parse($reserva->start_time)->format('H:i') }}
                                        </td>
                                        <td class="px-4 py-4">
                                            <div
                                                class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-tighter">
                                                {{ $reserva->client_name }}
                                            </div>
                                            <div
                                                class="text-[10px] text-indigo-500 font-bold flex items-center mt-0.5">
                                                🏟️ {{ $reserva->arena->name ?? 'N/A' }}
                                                <span class="mx-1 text-gray-300">•</span>
                                                <span
                                                    class="{{ $reserva->is_recurrent ? 'text-fuchsia-500' : 'text-blue-500' }}">
                                                    {{ $reserva->is_recurrent ? 'RECORRENTE' : 'AVULSO' }}
                                                </span>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <span
                                                class="px-2.5 py-1 inline-flex text-[10px] font-black rounded-md {{ $statusClass }}">
                                                {{ $statusLabel }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-4 text-right text-sm font-medium text-gray-400">R$
                                            {{ number_format($total, 2, ',', '.') }}</td>
                                        <td class="px-4 py-4 text-right text-sm font-bold text-green-600">R$
                                            {{ number_format($pago, 2, ',', '.') }}</td>
                                        <td
                                            class="px-4 py-4 text-right text-sm font-black {{ $restante > 0 ? 'text-red-600' : 'text-gray-300' }}">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-center space-x-1">
                                            @if ($canPay)
                                                <button type="button"
                                                    ...
                                                    class="bg-green-600 hover:bg-green-700 text-white text-[10px] font-black px-3 py-1.5 rounded-lg transition-all shadow-sm hover:scale-105 {{ $isActionDisabled ? 'opacity-30 cursor-not-allowed' : '' }}"
                                                    {{ $isActionDisabled ? 'disabled' : '' }}>
                                                    $ BAIXAR
                                                </button>
                                                <button type="button"
                                                    onclick="openDebtModal({{ $reserva->id }}, '{{ addslashes($reserva->client_name) }}')"
                                                    class="bg-amber-100 text-amber-700 hover:bg-amber-500 hover:text-white text-[10px] font-black px-3 py-1.5 rounded-lg transition-all border border-amber-200 {{ $isActionDisabled ? 'opacity-30 cursor-not-allowed' : '' }}"
                                                    {{ $isActionDisabled ? 'disabled' : '' }}>
                                                    🕒 DEPOIS
                                                </button>
                                            @endif
                                            @if ($canBeNoShow)
                                                <button type="button"
                                                    ...
                                                    class="bg-white dark:bg-gray-700 text-red-600 hover:bg-red-600 hover:text-white text-[10px] font-black px-3 py-1.5 rounded-lg transition-all border border-red-100 dark:border-red-900/50 {{ $isActionDisabled ? 'opacity-30 cursor-not-allowed' : '' }}"
                                                    {{ $isActionDisabled ? 'disabled' : '' }}>
                                                    X FALTA
                                                </button>
                                            @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-4 py-12 text-center">
                                            ...
                                            <p class="text-gray-400 font-medium italic">Nenhum agendamento encontrado
                                                para esta busca.</p>
                                        </td>
                                    </tr>
                                @endforelse
        ...
        <div
            ...
            <form id="transactionForm">
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        {{-- Seleção de Arena: Agora com ID para automação via JS --}}
                        <div>
                            ...
                            <select name="arena_id" id="modal_transaction_arena_id" required
                                ...
                                @foreach ($faturamentoPorArena as $arena)
                                    {{-- Pré-seleciona se o filtro de arena estiver ativo na URL --}}
    ...
    <script>
        ...
        function openCash(date) {
            // 1. Busca os elementos necessários
            ...
            const modalDateDisplay = document.getElementById('openCashDateDisplay');

            // 2. Validação: Impede a abertura se não houver arena identificada
            ...
            modalArenaInput.value = arenaIdInput.value; // Garante que o ID da arena vá para o form
            modalDateDisplay.innerText = date.split('-').reverse().join('/');

            // 4. Limpa justificativas anteriores para evitar duplicidade de texto
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // Captura o ID dinamicamente (Reserva, No-Show ou Arena para reabertura)
                const id = document.getElementById('modalReservaId')?.value ||
                    document.getElementById('noShowReservaId')?.value ||
                    document.getElementById('reopenCashArenaId')?.value || '';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 5. Reabertura de Caixa (Usa o reopenCashArenaId capturado acima)
