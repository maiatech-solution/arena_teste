                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                ...
                                if (json.success) {
                                    ...
                                    // 🖨️ Impressão na bobina se houver
                                    if (json.print_url) {
                                        imprimirCupomArena(json.print_url);
                                    }

                                    // ✨ LÓGICA DO NOVO MODAL DE RESUMO (EXCLUSIVO FECHAMENTO)
                                    if (formId === 'closeCashForm') {
                                        // Fecha o modal de entrada de dados
                                        closeCloseCashModal();

                                        // Alimenta o Modal de Resumo com os valores que calculamos no passo anterior
                                        ...
                                        // Exibe o modal de sucesso final
                                        document.getElementById('modalResumoFinal').classList.replace('hidden',
                                            'flex');

                                        // Não faz o reload aqui, deixa o usuário clicar no botão do novo modal
                                        return;
                                    }

                                    // Delay padrão para os outros formulários (Pagamentos/No-Show)
                                    setTimeout(() => {
                                        alert(json.message);
