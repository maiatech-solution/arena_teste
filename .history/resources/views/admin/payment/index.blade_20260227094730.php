<x-app-layout>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        ...
                                        // Filtramos comparando apenas a DATA (Y-m-d) para evitar erros de hora/minuto
                                        $pagoNoDia = $financialTransactions
                                            ->where('reserva_id', $reserva->id)
                                            ->filter(function ($t) use ($selectedDate) {
                                                return \Carbon\Carbon::parse($t->payment_date)->toDateString() ===
                                                    \Carbon\Carbon::parse($selectedDate)->toDateString();
                                            })
                                            ->sum('amount');

                                        // 3. Total acumulado (Sinais de outros dias + Pagamentos de hoje)
    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            // 🛑 Limpa qualquer onsubmit anterior para evitar execução múltipla
            form.onsubmit = null;
            ...
            form.onsubmit = function(e) {
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    // 🛡️ TRAVA DE SEGURANÇA: Botão desativado ou form já enviando
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(res => res.json())
                        .then(json => {
                            // 💡 AJUSTE: Se for sucesso OU se for o erro de duplicidade do servidor
                            const isAlreadyPaid = json.message === "Esta reserva já foi baixada anteriormente.";

                            if (json.success || isAlreadyPaid) {
                                // Silencia o alerta se for apenas duplicidade, mas recarrega a página
                                if (json.success) alert(json.message);

                                window.location.reload();
                            } else {
                                // Erros reais (ex: valor excedido) continuam aparecendo
                ...
                // --- LÓGICA DE DECISÃO (FECHAMENTO / CRÍTICAS) ---
