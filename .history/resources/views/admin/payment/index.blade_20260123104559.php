<x-app-layout>
    ...
    <script>
        /**
         * --- 1. FUNÇÕES DE SUPORTE E CÁLCULO ---
         */
        ...
        /**
         * --- 2. GESTÃO DE MODAIS (ABRIR/FECHAR) ---
         */
        function openDebtModal(id, clientName) {
            ...
            console.log("🛠️ Modal Dívida aberto para ID:", id);
        }
        ...
        function openPaymentModal(id, totalPrice, remaining, signalAmount, clientName, isRecurrent = false) {
            ...
            const opt = document.getElementById('recurrentOption');
            if (opt) isRecurrent ? opt.classList.remove('hidden') : opt.classList.add('hidden');
        ...
        // Funções de apoio para No-Show e Transações seguem a mesma lógica de toggle de classe...
        function toggleCustomRefundInput() {
            ...
            document.getElementById('custom_refund_amount').value = shouldRefund ? paidAmount.toFixed(2) : 0;
            shouldRefund ? customDiv.classList.remove('hidden') : customDiv.classList.add('hidden');
        }

        /**
         * --- 3. LÓGICA DE CÁLCULO EM TEMPO REAL ---
         */
        function calculateAmountDue() {
            ...
            if (balanceCents < 0) {
                ...
                trocoMessageEl.innerHTML =
                    `🚨 <strong>ATENÇÃO:</strong> Devolver Troco: R$ ${fromCents(Math.abs(balanceCents)).replace('.', ',')}`;
        ...
        /**
         * --- 4. MOTOR AJAX (O CORAÇÃO DO SISTEMA) ---
         */
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            const form = document.getElementById(formId);
            if (!form) return console.warn(`⚠️ Form ${formId} não encontrado.`);

            form.onsubmit = null; // Limpa eventos residuais
            form.addEventListener('submit', function(e) {
                e.preventDefault();
                console.log(`%c 🔥 SUBMIT: #${formId} `, 'background: #22c55e; color: #fff; font-weight: bold;');

                const btn = document.getElementById(btnId);
                const spinner = document.getElementById(spinnerId);

                // Busca o ID dinamicamente
                ...
                if (!idInput || !idInput.value) return alert("❌ Erro: ID não encontrado.");

                const targetUrl = urlTemplate.replace('{id}', idInput.value).replace('{reserva}', idInput.value);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            try {
                ...
                console.log('✅ Todos os formulários registrados!');
            } catch (e) {
                console.error('❌ Erro crítico no registro:', e);
            }

            // Listeners de interface
            document.getElementById('actualCashAmount')?.addEventListener('input', () => {
