<x-app-layout>
    ...
    @if (session('success') && session('total_esperado_print'))
        <div id="thermal-receipt"
            class="mb-10 bg-white p-4 rounded-lg border-2 border-dashed border-gray-300 max-w-[80mm] mx-auto text-black shadow-2xl no-print animate-in zoom-in duration-500 font-mono text-sm">

            <div class="text-center mb-4">
                <h2 class="font-black text-lg">FECHAMENTO DE CAIXA</h2>
                <p>{{ session('arena_name_print') ?? 'GERAL' }}</p>
                <p>Data: {{ \Carbon\Carbon::parse(session('date_print'))->format('d/m/Y') }}</p>
                <p>Responsável: {{ auth()->user()->name }}</p>
            ...
            <div class="space-y-1">
                <div class="flex justify-between">
                    ...
                    <span class="font-bold text-right">R$
                        {{ number_format(session('total_esperado_print'), 2, ',', '.') }}</span>
                </div>
                <div class="flex justify-between italic text-xs">
                    <span>-- Dinheiro (Gaveta):</span>
                    <span>R$ {{ number_format(session('gaveta_print'), 2, ',', '.') }}</span>
                </div>
                <div class="flex justify-between italic text-xs">
                    <span>-- Pix/Banco:</span>
                    <span>R$ {{ number_format(session('banco_print'), 2, ',', '.') }}</span>
                </div>
                <div class="flex justify-between italic text-xs">
                    <span>-- Cartão/Outros:</span>
                    <span>R$ {{ number_format(session('outros_print'), 2, ',', '.') }}</span>
                </div>

                <div class="border-b border-black my-2 text-center">--- CONFERÊNCIA ---</div>

                <div class="flex justify-between">
                    <span>VALOR INFORMADO:</span>
                    <span class="font-bold underline">R$
                        {{ number_format(session('total_informado_print'), 2, ',', '.') }}</span>
                </div>

                @php $dif = session('total_informado_print') - session('total_esperado_print'); @endphp
                <div class="flex justify-between {{ $dif < 0 ? 'text-red-600' : 'text-green-600' }}">
                    <span>DIFERENÇA:</span>
                    <span class="font-black">R$ {{ number_format($dif, 2, ',', '.') }}</span>
                </div>
            </div>

            <div class="mt-6 text-center text-[10px] uppercase">
                <p>Relatório emitido em:</p>
                <p>{{ now()->format('d/m/Y H:i:s') }}</p>
            </div>

            <button onclick="window.print()"
                class="w-full mt-8 py-4 bg-black text-white font-black rounded-xl uppercase text-[12px] tracking-widest hover:bg-gray-800 transition-all no-print">
                🖨️ Imprimir Cupom
            </button>
        </div>

        {{-- O CSS que você já tem cuida para que só este ID apareça na impressão --}}
    @endif
    ...
    {{-- MODAL 6: MOVIMENTAÇÃO AVULSA (SANGRIA/REFORÇO) --}}
    <div id="modalResumoFechamento" class="fixed inset-0 z-[9999] hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="fixed inset-0 bg-black opacity-50"></div>
            <div class="bg-white rounded-lg shadow-xl overflow-hidden max-w-md w-full z-10 p-6">
                <div id="conteudoImpressao">
                    <div class="text-center font-mono border-b-2 border-dashed border-gray-300 pb-4">
                        <h2 class="text-xl font-bold">RESUMO DE FECHAMENTO</h2>
                        <p id="resumoArena" class="text-sm"></p>
                        <p id="resumoData" class="text-sm"></p>
                    </div>

                    <div class="py-4 font-mono text-sm space-y-2">
                        <div class="flex justify-between"><span>DINHEIRO:</span> <span id="resumoDinheiro"></span>
                        </div>
                        <div class="flex justify-between"><span>PIX:</span> <span id="resumoPix"></span></div>
                        <div class="flex justify-between"><span>CARTÃO CRÉDITO:</span> <span
                                id="resumoCredito"></span></div>
                        <div class="flex justify-between"><span>CARTÃO DÉBITO:</span> <span id="resumoDebito"></span>
                        </div>
                        ...
                        <div id="resumoDiferencaCor" class="flex justify-between font-bold border-t pt-2 uppercase">
                            <span>DIFERENÇA:</span> <span id="resumoDiferenca"></span>
                        </div>
                    </div>

                    <div class="text-center text-[10px] font-mono uppercase pb-4">
                ...
                <div class="flex gap-2 mt-4 no-print">
                    ...
                    <button onclick="window.location.reload()"
                        class="flex-1 bg-gray-200 text-gray-800 font-bold py-3 rounded-lg hover:bg-gray-300">
                        FECHAR
                    </button>
                </div>
            </div>
            ...
            function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
                ...
                form.onsubmit = function(e) {
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        // 🛡️ LIMPEZA AGRESSIVA: Fecha o modal
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                if (json.success) {
                                    ...
                                    // 🖨️ IMPRESSÃO DO RESUMO DE FECHAMENTO (Caso exista no retorno)
                                    if (json.resumo) {
                                        imprimirResumoFechamento(json.resumo);
                                    }

                                    // 🖨️ IMPRESSÃO NA BOBINA (Bar/Reservas)
                                    if (json.print_url) {
                                        if (typeof imprimirCupomArena === 'function') {
                                            imprimirCupomArena(json.print_url);
                                        }
                                    }

                                    // Delay para o navegador processar a impressão antes do reload
                                    setTimeout(() => {
                                        alert(json.message);
