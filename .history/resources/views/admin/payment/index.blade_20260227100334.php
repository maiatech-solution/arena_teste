            ...
            form.onsubmit = function(e) {
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(res => {
                            // Se a resposta não for ok e for o erro de "já baixada", a gente mata o processo AQUI
                            return res.json().then(json => {
                                if (json.message === "Esta reserva já foi baixada anteriormente.") {
                                    console.warn(
                                        "🚫 BLOQUEIO: Mensagem duplicada detectada e abortada.");
                                    // Forçamos o reload sem dar o alert escroto
                                    window.location.reload();
                                    throw new Error(
                                    "Duplicidade detectada"); // Interrompe o fluxo para não chegar no alert
                                }
                                return json;
                            });
                        })
                        .then(json => {
                            // Se chegou aqui, é porque é uma resposta REAL e ÚNICA
                            if (json.success) {
                                alert(json.message);
                                window.location.reload();
                            } else {
                        ...
                        .catch(err => {
                            // Se for o erro que a gente jogou ali em cima, não faz nada
                            if (err.message === "Duplicidade detectada") return;
