<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
           {{-- 1. ESTRUTURA DE KPIS (FORÇANDO 5 COLUNAS NA MESMA LINHA) --}}
<div class="space-y-4">
    {{-- A classe 'grid-cols-5' sem prefixo força 5 colunas em QUALQUER tela --}}
    <div class="grid grid-cols-5 gap-2 lg:gap-4">

        {{-- CARD 1: SALDO EM CAIXA (LÍQUIDO REAL) --}}
        <div class="bg-green-600 dark:bg-green-700 overflow-hidden shadow-md rounded-lg p-2 sm:p-4 flex flex-col justify-center border-b-4 border-green-900">
            <div class="text-[8px] sm:text-[10px] font-bold text-green-50 uppercase tracking-tighter truncate">
                💰 Saldo Caixa
            </div>
            <div class="mt-1 text-xs sm:text-lg lg:text-2xl font-extrabold text-white truncate">
                R$ {{ number_format($totalRecebidoDiaLiquido, 2, ',', '.') }}
            </div>
        </div>

        {{-- CARD 2: RECEITA JOGOS HOJE --}}
        <div class="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-300 dark:border-indigo-800 overflow-hidden shadow-md rounded-lg p-2 sm:p-4 flex flex-col justify-center">
            <div class="text-[8px] sm:text-[10px] font-medium text-gray-700 dark:text-gray-300 uppercase tracking-tighter truncate">
                🎾 Receita Agenda
            </div>
            <div class="mt-1 text-xs sm:text-lg lg:text-2xl font-extrabold text-indigo-700 dark:text-indigo-300 truncate">
                R$ {{ number_format($totalAntecipadoReservasDia, 2, ',', '.') }}
            </div>
        </div>

        {{-- CARD 3: PENDENTE A RECEBER --}}
        <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-800 overflow-hidden shadow-md rounded-lg p-2 sm:p-4 flex flex-col justify-center">
            <div class="text-[8px] sm:text-[10px] font-medium text-gray-700 dark:text-gray-300 uppercase tracking-tighter truncate">
                ⏳ Pendente
            </div>
            <div class="mt-1 text-xs sm:text-lg lg:text-2xl font-extrabold text-yellow-700 dark:text-yellow-300 truncate">
                R$ {{ number_format($totalPending, 2, ',', '.') }}
            </div>
        </div>

        {{-- CARD 4: RESERVAS ATIVAS --}}
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md rounded-lg p-2 sm:p-4 flex flex-col justify-center text-center">
            <div class="text-[8px] sm:text-[10px] font-medium text-gray-700 dark:text-gray-300 uppercase tracking-tighter truncate">
                📅 Ativas
            </div>
            <div class="mt-1 text-xs sm:text-lg lg:text-2xl font-extrabold text-gray-900 dark:text-white truncate">
                {{ $totalReservasDia }}
            </div>
        </div>

        {{-- CARD 5: FALTAS (NO-SHOW) --}}
        <div class="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 overflow-hidden shadow-md rounded-lg p-2 sm:p-4 flex flex-col justify-center text-center">
            <div class="text-[8px] sm:text-[10px] font-medium text-gray-700 dark:text-gray-300 uppercase tracking-tighter truncate">
                ❌ Faltas
            </div>
            <div class="mt-1 text-xs sm:text-lg lg:text-2xl font-extrabold text-red-700 dark:text-red-300 truncate">
                {{ $noShowCount }}
            </div>
        </div>
