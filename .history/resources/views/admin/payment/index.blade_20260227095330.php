<x-app-layout>
    ...
    <script>
        ...
        // Garante que a variável exista no escopo global
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.onsubmit = function(e) {
                ...
                if (window.caixaProcessando) {
                    ...
                    console.warn("🚫 Bloqueio de segurança: Requisição duplicada abortada no ato.");
                    return false;
                }

                // Ativa a trava antes de qualquer processamento
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(res => res.json())
                        .then(json => {
                            // Se o servidor responder que já foi pago, tratamos como sucesso silencioso
                            const jaPago = json.message === "Esta reserva já foi baixada anteriormente.";

                            if (json.success || jaPago) {
                                if (json.success) alert(json.message);

                                // Bloqueio físico final antes do reload
                ...
                if (acoesCriticas.includes(formId) && userRole === 'colaborador') {
                    window.requisitarAutorizacao(token => {
                        if (token) {
                            ...
                            window.caixaProcessando = false; // Libera se cancelar a senha
                        }
                    });
                } else {
                    enviarParaOServidor();
