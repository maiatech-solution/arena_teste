<x-app-layout>
    ...
    <script>
        ...
        function openPaymentModal(id, totalPrice, remaining, signalAmount, clientName, isRecurrent = false) {
            ...
            // 🔥🔥🔥 CORREÇÃO CRÍTICA DO BUG DO SUBMIT 🔥🔥🔥
            // Registra o handler AJAX somente quando o form já existe no DOM
            if (!window.paymentFormInitialized) {
                console.log("🔧 Registrando handler AJAX do pagamento...");

                setupAjaxForm(
                    'paymentForm',
                    'submitPaymentBtn',
                    'submitPaymentSpinner',
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.onsubmit = function(e) {
                ...
                // 🛑 TRAVA 1: Verifica se este formulário específico já está rodando
                if (window.caixaProcessandoGlobal[formId]) {
                    console.warn("🚫 BLOQUEIO: Requisição duplicada detectada para " + formId);
                    return false;
                }
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(json => {
                            ...
                            if (mensagemServidor.includes("já foi baixada anteriormente")) {
                            ...
                            if (json.success) {
                                ...
                                // Erro real (ex: falta de saldo)
                                alert(json.message || 'Erro ao processar.');

                                // Libera a trava para o usuário corrigir e tentar de novo
                        ...
                        .catch(err => {
                            ...
                            alert("Erro de conexão. Verifique se o registro foi salvo.");
                        });
                };
                ...
                if (acoesCriticas.includes(formId) && userRole === 'colaborador') {
                    window.requisitarAutorizacao(token => {
                        if (token) {
                            ...
                            // Se cancelou a senha, libera a trava do formulário
                            window.caixaProcessandoGlobal[formId] = false;
                        }
                    });
                } else {
