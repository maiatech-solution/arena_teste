            ...
            form.onsubmit = function(e) {
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(res => res.json())
                        .then(json => {
                            // 💡 Definição exata da mensagem que o Controller envia no "Tiro 2"
                            const isAlreadyPaid = json.message === "Esta reserva já foi baixada anteriormente.";

                            if (json.success || isAlreadyPaid) {
                                // Bloqueio total da tela para evitar qualquer novo clique
                                document.body.style.pointerEvents = 'none';
                                form.style.opacity = '0.5';

                                // 📣 SÓ mostra o alerta se for a PRIMEIRA vez (sucesso real)
                                // Se for o erro de duplicidade (isAlreadyPaid), ele entra aqui mas NÃO dá o alert.
                                if (json.success) {
                                    alert(json.message);
                                }

                                // Recarrega a página para atualizar o faturamento e os checks de QUITADO
                                window.location.reload();
                            } else {
                                // Erros REAIS (como "Valor excede o preço final") caem aqui
