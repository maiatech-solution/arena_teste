    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.onsubmit = function(e) {
                // 🛑 TRAVA DE MEMÓRIA REAL: O SessionStorage sobrevive até a página atualizar de fato
                if (sessionStorage.getItem('travado_' + formId)) {
                    console.warn("🚫 BLOQUEIO DE SESSÃO: Você já clicou nessa merda!");
                    e.preventDefault();
                    return false;
                }
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    // Grava na memória do navegador que este formulário está em processo
                    sessionStorage.setItem('travado_' + formId, 'true');
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(res => res.json())
                        .then(json => {
                            // Se a mensagem for a de duplicidade, limpamos o lixo e recarregamos
                            if (json.message === "Esta reserva já foi baixada anteriormente.") {
                                sessionStorage.removeItem('travado_' + formId);
                            ...
                            if (json.success) {
                                alert(json.message);
                                sessionStorage.removeItem('travado_' + formId); // Limpa antes de recarregar
                                window.location.reload();
                            } else {
                                alert(json.message || 'Erro ao processar.');
                                sessionStorage.removeItem('travado_' + formId); // Libera em caso de erro real
