<x-app-layout>
    ...
    <script>
        ...
        function checkCashierStatus() {
            ...
            // 🛡️ LISTA AMPLIADA: Incluindo os novos status que estavam travando a lógica
            const finalStatuses = [
                'pago', 'falta', 'cancelada', 'rejeitada', 'no_show',
                'paid', 'completed', 'atrasado', 'dívida ativa', 'dívida',
                'parcial', 'pendente', 'debt'
            ];

            document.querySelectorAll('table:first-of-type tbody tr').forEach(row => {
                const statusCell = row.querySelector('td:nth-child(3)');
                if (statusCell) {
                    // Convertemos para minúsculo e removemos espaços extras para evitar erros
                    const text = statusCell.innerText.trim().toLowerCase();

                    // Verifica se o texto da célula contém qualquer um dos status da lista
                    const isCompleted = finalStatuses.some(s => text.includes(s));
