<x-app-layout>
    ...
    if (!form) {
        console.warn(`⚠️ Formulário ${formId} não encontrado no DOM.`);
        return;
    }

    form.addEventListener('submit', function(e) {
        e.preventDefault();
        console.log(`🚀 Tentando enviar: ${formId}`);

        const btn = document.getElementById(btnId);
        const id = document.getElementById('modalReservaId')?.value || 
                   document.getElementById('closeCashArenaId')?.value || 
                   '';

        console.log(`ID capturado: ${id}`);
        
        if (!id && urlTemplate.includes('{id}')) {
            console.error('❌ Abortando: ID não encontrado para uma rota que exige ID.');
            alert('Erro técnico: ID da reserva não identificado.');
            return;
        }
        ...
        .then(async r => {
            ...
            if (!r.ok) throw new Error(data.message || `Erro HTTP ${r.status}`);
            return data;
        })
        .then(data => {
            console.log('✅ Sucesso do Servidor:', data);
