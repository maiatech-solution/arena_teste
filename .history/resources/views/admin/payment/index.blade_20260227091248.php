<x-app-layout>
    ...
    <script>
        ...
        function checkCashierStatus() {
            ...
            // DEBUG: Vamos ver no console o que ele está achando
            let pendencias = 0;

            // Procura por QUALQUER link ou botão que tenha as rotas de baixar, pendenciar ou falta
            // Ajustei para pegar pelos fragmentos das URLs que você usa nas rotas
            const links = document.querySelectorAll('a[href*="finalizar"], a[href*="pendenciar"], a[href*="no-show"]');

            pendencias = links.length;

            console.log("Debug Trava: Encontradas " + pendencias + " ações pendentes.");

            if (pendencias > 0) {
                btn.disabled = true;
                btn.style.opacity = '0.5'; // Visualmente desativado
                btn.style.pointerEvents = 'none'; // Impede clique forçado
                statusEl.innerHTML = `🚨 Pendentes nesta Arena: ${pendencias}`;
