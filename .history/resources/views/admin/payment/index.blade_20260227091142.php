    ...
    <script>
        ...
        function checkCashierStatus() {
            ...
            if (!isFiltered) {
                ...
                statusEl.className = 'text-amber-500 font-bold text-xs uppercase';
                return;
            }

            // 2. NOVA LÓGICA DE SEGURANÇA:
            // Varre a tabela em busca de botões de ação pendente.
            // Se encontrar qualquer botão de cobrança, o caixa não pode fechar.
            let pendenciasEncontradas = 0;

            // Pegamos todas as linhas da tabela de agendamentos
            const rows = document.querySelectorAll('table:first-of-type tbody tr');

            rows.forEach(row => {
                const acoesCell = row.querySelector('td:last-child'); // Última coluna (Ações)
                if (acoesCell) {
                    const html = acoesCell.innerHTML.toUpperCase();
                    // Verifica se existem os botões críticos que exigem ação do operador
                    if (html.includes('$ BAIXAR') || html.includes('P. DEPOIS') || html.includes('X FALTA')) {
                        pendenciasEncontradas++;
                    }
                }
            });

            // 3. Habilitação do Botão baseada na realidade da tela
            if (pendenciasEncontradas > 0) {
                // TRAVA O BOTÃO
                btn.disabled = true;
                statusEl.innerHTML = `🚨 Pendentes nesta Arena: ${pendenciasEncontradas}`;
                statusEl.className = 'text-red-500 font-black text-xs uppercase italic';
            } else {
                // LIBERA O BOTÃO
                btn.disabled = false;
                statusEl.innerHTML = "✅ Arena pronta para fechar!";
                statusEl.className = 'text-green-600 font-black text-xs uppercase italic';
            }
        }
