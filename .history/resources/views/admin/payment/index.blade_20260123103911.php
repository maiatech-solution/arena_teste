    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // Captura o ID de qualquer um dos campos possíveis nos seus modais
                const idInput = form.querySelector('#debtReservaId') ||
                    form.querySelector('#noShowReservaId') ||
                    form.querySelector('#modalReservaId') ||
                ...
                // Faz a substituição na URL se necessário
                if (targetUrl.includes('{id}') || targetUrl.includes('{reserva}')) {
                    if (!idValue || idValue === '0' || idValue === '') {
                        alert('Erro: ID da operação não identificado. Feche o modal e tente novamente.');
                        return;
                    }
                    // Substitui ambos os padrões para garantir compatibilidade com o web.php
                    targetUrl = targetUrl.replace('{id}', idValue).replace('{reserva}', idValue);
                }

                console.log(`📡 Enviando ${formId} para: ${targetUrl}`);
