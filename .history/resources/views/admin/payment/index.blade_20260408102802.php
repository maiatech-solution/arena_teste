                ...
                form.onsubmit = function(e) {
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                if (json.success) {
                                    // 🛡️ LIMPEZA RADICAL: Remove tudo que possa estar na frente
                                    ...
                                    // 🖨️ SE TIVER RESUMO, MOSTRA O MODAL (Sem alert para não travar)
                                    if (json.resumo) {
                                        const r = json.resumo;
                                        document.getElementById('resumoArena').innerText = r.arena;
                                        document.getElementById('resumoData').innerText = "DATA: " + r.data;
                                        document.getElementById('resumoGaveta').innerText = "R$ " + r.gaveta
                                            .toFixed(2).replace('.', ',');
                                        document.getElementById('resumoBanco').innerText = "R$ " + r.banco
                                            .toFixed(2).replace('.', ',');
                                        document.getElementById('resumoOutros').innerText = "R$ " + r.outros
                                            .toFixed(2).replace('.', ',');
                                        ...
                                        const difElem = document.getElementById('resumoDiferenca');
                                        difElem.style.color = r.diferenca < 0 ? '#ef4444' : '#22c55e';
                                        ...
                                        modalResumo.style.display =
                                            'flex'; // Garante o display flex para centralizar
                                    } else {
                                        // Se não tiver resumo (outros formulários), aí sim dá o alert
                                        alert(json.message);
                    ...
                    // Ignora modal de credenciais para o fechamento (o PHP já valida supervisor)
