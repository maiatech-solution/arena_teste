                ...
                form.onsubmit = function(e) {
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                if (json.success) {
                                    // 1. LIMPEZA RADICAL: Esconde modais de senha e fundos escuros
                                    ...
                                    if (json.resumo) {
                                        ...
                                        // Garante que crédito/débito apareçam se existirem
                                        ...
                                        if (difWrapper) difWrapper.style.color = r.diferenca < 0 ? '#dc2626' :
                                            '#16a34a';

                                        // EXIBE O MODAL SEM ALERT
                                        const modal = document.getElementById('modalResumoFechamento');
                                        modal.classList.remove('hidden');
                                        modal.style.display = 'flex';

                                        // NÃO FAZ RECARGA AQUI. A recarga só acontece no botão "FECHAR" do modal.
                                    } else {
                                        // Para outros forms que não são fechamento, mantém o alert e reload
                                        alert(json.message);
                                        window.location.reload();
                                    }
                                } else {
                                    // Se der erro no fechamento (ex: falta supervisor)
