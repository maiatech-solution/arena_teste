<x-app-layout>
    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            // 🛑 TRAVA DE SEGURANÇA: Verifica se este formulário já foi configurado antes
            if (form.dataset.ajaxConfigured === 'true') {
                return;
            }
            form.dataset.ajaxConfigured = 'true'; // Marca como configurado

            const userRole = "{{ Auth::user()->role ?? 'guest' }}";

            form.addEventListener('submit', function(e) {
                e.preventDefault();
                e.stopImmediatePropagation(); // 🛑 Impede que o evento borbulhe para outros listeners duplicados
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(json => {
                            if (json.success) {
                                // 🔔 O alerta só aparecerá uma vez agora
                                alert(json.message);
                    ...
                    if (diffCents !== 0 && userRole === 'colaborador') {
                        window.requisitarAutorizacao(function(token) {
                            if (token) {
                                ...
                                // Removido alert genérico para não confundir se o usuário apenas fechou o modal
