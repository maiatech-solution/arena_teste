<x-app-layout>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-3 lg:gap-4">
                    ...
                    <div
                        ...
                        <div class="mt-1 text-2xl font-extrabold text-white truncate"
                            title="Valor exato do banco: {{ $totalRecebidoDiaLiquido }}">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        ...
                                        // 2. Identificação de "Hoje" para Destaque
                                        ...
                                        // 4. Status Visual (Cores e Labels)
                                        $statusClass = '';
                                        $statusLabel = '';
                                    ...
                                    <tr class="{{ $rowHighlight }} transition">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white">
                                                {{ $reserva->client_name }} (ID: {{ $reserva->id }})</div>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-center text-sm font-medium">
                                            ...
                                            @elseif($reserva->status === 'no_show')
                                                <span class="text-xs text-red-500 italic font-medium">
                                                    {{ $restante <= 0 ? 'Falta Registrada' : 'Falta (Aguardando Pagamento)' }}
                                                </span>
                                            @elseif(in_array($reserva->status, ['canceled', 'rejected']))
                                                <span
                                                    class="text-xs text-gray-500 italic font-medium">{{ ucfirst($reserva->status) }}</span>
                                            @elseif($pago >= $total)
                                                <span
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($groupedTransactions as $reservaId => $transactions)
                                    ...
                                    @if ($reservaId)
                                        ...
                                        {{-- Linha de Agrupamento por Reserva --}}
                                        <tr
                                            class="bg-gray-100 dark:bg-gray-700/60 border-t-2 border-indigo-500 dark:border-indigo-400">
                                            {{-- Colspan cobrindo: Hora, ID, Pagador, Tipo e Descrição --}}
                                            <td colspan="5"
                                                class="px-4 py-2.5 text-sm font-bold text-gray-800 dark:text-gray-100">
                                                <div class="flex items-center">
                                                    {{-- Badge Identificador --}}
                                                    <span
                                                        class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-extrabold bg-indigo-600 text-white dark:bg-indigo-500 mr-3 tracking-wider uppercase shadow-sm">
                                                        Reserva
                                                    </span>

                                                    {{-- Info do Cliente e ID --}}
                                                    <span class="flex items-center tracking-tight">
                                                        ✅ Movimentações:
                                                        <span
                                                            class="ml-2 text-indigo-700 dark:text-indigo-300 font-extrabold italic">
                                                            {{ $reservaInfo }}
                                                        </span>
                                                    </span>
                                                </div>
                                            ...
                                            {{-- 6ª Coluna: Valor Total Acumulado desta Reserva --}}
                                            <td
                                                ...
                                                <div class="flex flex-col items-end leading-none">
                                                    <span
                                                        class="text-[9px] text-indigo-600 dark:text-indigo-400 uppercase font-bold mb-1">
                                                        Total Grupo
                                                    </span>
                                    ...
                                    @foreach ($transactions as $transaction)
                                        @php
                                            ...
                                            // Lógica de Identificação de Pagamento Retroativo
                                            $reservaData = $transaction->reserva
                                                ? \Carbon\Carbon::parse($transaction->reserva->date)->format('Y-m-d')
                                                : null;
                                            $isRetroactive = $reservaData && $reservaData !== $selectedDate;
                                        @endphp

                                        <tr
                                            class="{{ $rowClass }} transition duration-150 ease-in-out border-b border-gray-100 dark:border-gray-700/50">
                                            {{-- 1. Hora do Pagamento --}}
                                            <td
                                            ...
                                            {{-- 2. ID da Reserva + Alerta Retroativo --}}
                                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                                                <div class="flex flex-col gap-1">
                                                    <span
                                                        class="{{ $isRefund ? 'text-red-700 dark:text-red-400' : 'text-indigo-600 dark:text-indigo-400' }}">
                                                        #{{ $transaction->reserva_id ?? '--' }}
                                                    </span>

                                                    @if ($isRetroactive)
                                                        <span
                                                            class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-black bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 border border-amber-200/50 dark:border-amber-800/50 w-fit uppercase tracking-tighter"
                                                            title="Dinheiro recebido hoje, mas o jogo foi em {{ \Carbon\Carbon::parse($transaction->reserva->date)->format('d/m/Y') }}">
                                                            📅 Origem:
                                                            {{ \Carbon\Carbon::parse($transaction->reserva->date)->format('d/m') }}
                                                        </span>
                                                    @endif
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm">
                                                ...
                                                <div
                                                    class="text-[10px] text-gray-500 dark:text-gray-400 flex items-center mt-0.5">
                                                    <svg class="w-3 h-3 mr-1 opacity-60" fill="none"
                                            ...
                                            {{-- 4. Tipo e Forma de Pagamento --}}
                                            <td
                                                class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                                                <div
                                                    class="font-extrabold text-gray-800 dark:text-gray-200 text-[10px] uppercase tracking-widest mb-1">
                                                    {{ $displayType }}
                                                </div>
                                                <div
                                                    class="inline-flex items-center px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-[9px] font-bold text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-600">
                                                    {{ $displayMethod }}
                                                </div>
                                            </td>
                                            ...
                                            <td
                                                class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 leading-tight">
                                                <div class="flex flex-col">
                                                    <span class="font-medium">{{ $transaction->description }}</span>

                                                    @if ($isRetroactive)
                                                        <div
                                                            class="flex items-center mt-1.5 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-2 py-1 rounded w-fit border border-indigo-100 dark:border-indigo-800/40">
                                                            ...
                                                            Referente ao dia
                                                            {{ \Carbon\Carbon::parse($transaction->reserva->date)->format('d/m/Y') }}
                                                        </div>
                                                    @endif
                                                </div>
                                            </td>
                                            ...
                                            {{-- 6. Valor Final --}}
                                            <td
                                                class="px-4 py-3 whitespace-nowrap text-sm text-right font-mono {{ $amountClass }}">
                                                <span class="text-xs mr-0.5 font-normal opacity-70">R$</span>
                                ...
                                <tr class="bg-gray-100 dark:bg-gray-700 font-bold">
                                    ...
                                    <td id="valor-liquido-total-real" {{-- ID adicionado aqui para o JS --}}
    ...
    <script>
        ...
        function openPaymentModal(id, totalPrice, remaining, signalAmount, clientName, isRecurrent = false) {
            // 1. Verificação de Bloqueio de Caixa
            // Convertemos a variável do Blade para um booleano real no JS
            const isActionDisabled = {{ $isActionDisabled ? 'true' : 'false' }};

            if (isActionDisabled) {
                // Usa o seu sistema de mensagens para avisar o operador
            ...
            // 3. Formatação e Exibição do Sinal (Currency)
            ...
            // 4. Definição do Preço Final Inicial (Editável no Modal)
            document.getElementById('modalFinalPrice').value = totalPrice.toFixed(2);

            // 5. Tratamento de Recorrência
            ...
            // 7. Cálculo Inicial de Saldo
            calculateAmountDue();

            // 8. Exibição Visual do Modal
        ...
        function calculateDifference() {
            ...
            if (Math.abs(difference) < 0.01) {
                diffMessageEl.innerHTML = '✅ **Caixa exato!** Os valores conferem com o sistema.';
                diffMessageEl.classList.add('bg-green-100', 'text-green-700');
            } else if (difference > 0) {
                diffMessageEl.innerHTML =
                    `⚠️ **Sobrou R$ ${Math.abs(difference).toFixed(2).replace('.', ',')}**. Dinheiro físico maior que o sistema.`;
                diffMessageEl.classList.add('bg-yellow-100', 'text-yellow-700');
            } else {
                diffMessageEl.innerHTML =
                    `🚨 **Faltou R$ ${Math.abs(difference).toFixed(2).replace('.', ',')}**. Dinheiro físico menor que o sistema.`;
