<x-app-layout>
    ...
    <script>
        ...
        // AJAX Genérico para outros formulários (ATUALIZADO PARA DÍVIDA)
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // Captura o ID dinamicamente - ADICIONADO debtReservaId
                const id = document.getElementById('modalReservaId')?.value ||
                    document.getElementById('noShowReservaId')?.value ||
                    document.getElementById('debtReservaId')?.value || // <--- Chave para o "Pagar Depois"
                    document.getElementById('reopenCashArenaId')?.value || '';

                // Debug preventivo: Se o ID estiver vazio, avisa no console
                if (!id && urlTemplate.includes('{id}')) {
                    console.error('Erro: ID não encontrado para o formulário ' + formId);
                    alert('Erro interno: ID da reserva não encontrado.');
                    return;
                }
                ...
                fetch(urlTemplate.replace('{id}', id), {
                    ...
                    .then(r => {
                        if (!r.ok) {
                            // Se o status for 404 ou 500, tenta ler o erro antes de dar throw
                    ...
                    .catch(err => {
                        if (error) {
                            error.innerText = err.message || 'Erro de conexão com o servidor';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 6. Pagar Depois (Dívida) - LINHA ADICIONADA
