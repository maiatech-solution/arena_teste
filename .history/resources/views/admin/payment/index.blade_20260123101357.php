    ...
    <script>
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // 1. Captura inteligente de ID
                // Prioriza IDs específicos de modais, depois nomes genéricos
                const idInput = form.querySelector('#modalReservaId') ||
                    form.querySelector('#noShowReservaId') ||
                    form.querySelector('#debtReservaId') ||
                    form.querySelector('#closeCashArenaId') ||
                    form.querySelector('#reopenCashArenaId') ||
                    form.querySelector('[name="reserva_id"]') ||
                    form.querySelector('[name="arena_id"]');

                let id = idInput ? idInput.value : '';

                // 2. Tratamento da URL
                // Se a rota for fixa (não tiver {id}), usamos o template puro.
                // Se tiver {id}, fazemos a substituição.
                let targetUrl = urlTemplate;
                if (targetUrl.includes('{id}')) {
                    if (!id || id === '1') {
                        alert('Erro: ID de operação inválido. Por favor, feche o modal e tente novamente.');
                        return;
                    }
                    targetUrl = targetUrl.replace('{id}', id);
                }

                console.log(`📡 Enviando ${formId} | URL: ${targetUrl} | ID: ${id}`);

                // 3. Interface: Bloqueio e Feedback
                ...
                fetch(targetUrl, {
                    ...
                    .catch(err => {
                        ...
                        // Libera interface em caso de erro
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 1. Inicialização de status e cálculos ao carregar a página
            if (typeof checkCashierStatus === "function") {
                checkCashierStatus();
            }

            // 2. Ouvinte para cálculo de diferença em tempo real no modal de fechamento
            document.getElementById('actualCashAmount')?.addEventListener('input', calculateDifference);

            // 3. Registro dos formulários AJAX
            try {
                // --- ROTAS SEM ID NA URL (O ID vai dentro do FormData) ---

                // Movimentação Avulsa (Sangria/Reforço)
