...
<div class="space-y-4">
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-3 lg:gap-4">
        ...
        <div class="bg-green-600 dark:bg-green-700 overflow-hidden shadow-md rounded-lg p-4 lg:p-3 xl:p-4 flex flex-col justify-center border-b-4 border-green-900">
            ...
            <div class="mt-1 text-2xl font-extrabold text-white truncate" title="Valor exato do banco: {{ $totalRecebidoDiaLiquido }}">
    ...
    if (Math.abs(difference) < 0.01) {
        ...
        diffMessageEl.innerHTML = `⚠️ **Sobrou R$ ${Math.abs(difference).toFixed(2).replace('.', ',')}**. Dinheiro físico maior que o sistema.`;
        diffMessageEl.classList.add('bg-yellow-100', 'text-yellow-700');
    } else {
        diffMessageEl.innerHTML = `🚨 **Faltou R$ ${Math.abs(difference).toFixed(2).replace('.', ',')}**. Dinheiro físico menor que o sistema.`;
