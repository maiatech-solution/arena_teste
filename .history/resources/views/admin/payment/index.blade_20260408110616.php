    ...
    <script>
        ...
        if (!window.__CAIXA_SCRIPT_LOADED) {
            ...
            function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
                ...
                if (!form) {
                    console.error(`%c [ERRO] Formulário #${formId} não existe no HTML!`, "color: red;");
                    return;
                }
                ...
                console.log(`%c [DEBUG] Monitorando formulário: ${formId}`, "color: green;");
                ...
                form.onsubmit = function(e) {
                    ...
                    console.log(`%c [EVENTO] Submit disparado no form: ${formId}`, "background: #222; color: #bada55");

                    if (window.caixaProcessandoGlobal[formId]) {
                        console.warn("[AVISO] Já existe um processamento em curso.");
                        return false;
                    }
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        console.log(`[NETWORK] Iniciando Fetch para: ${targetUrl}`);
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(res => {
                                console.log("[NETWORK] Status da Resposta:", res.status);
                                if (!res.ok) throw res; // Manda pro catch se der 403, 404, 500
                                return res.json();
                            })
                            .then(json => {
                                ...
                                if (json.success) {
                                    ...
                                    if (json.resumo) {
                                        ...
                                        // Função de formatação que NUNCA dá undefined
                                        const f = (val) => {
                                        ...
                                        console.log("[MODAL] Populando dados...");

                                        // Mapeamento Direto (Verifique se esses IDs existem no Blade!)
                                        const campos = {
                                            'resumoArena': r.arena || 'N/A',
                                            'resumoData': "FECHAMENTO: " + (r.data || '---'),
                                            'resumoDinheiro': f(r.gaveta || r.dinheiro),
                                            'resumoPix': f(r.banco || r.pix),
                                            'resumoCredito': f(r.credito),
                                            'resumoDebito': f(r.debito),
                                            'resumoSistema': f(r.sistema),
                                            'resumoInformado': f(r.informado),
                                            'resumoDiferenca': f(r.diferenca),
                                            'resumoOperador': r.operador || 'N/A'
                                        };

                                        Object.keys(campos).forEach(id => {
                                            const el = document.getElementById(id);
                                            if (el) el.innerText = campos[id];
                                            else console.warn(`[DOM] Elemento #${id} não encontrado.`);
                                        });
                                        ...
                                        if (modalResumo) {
                                            ...
                                            modalResumo.style.zIndex = '99999';
                                        } else {
                                            alert("Sucesso, mas #modalResumoFechamento não existe!");
                                        }
                            ...
                            .catch(async err => {
                                ...
                                if (err.json) {
                                    ...
                                    alert("Erro do Servidor: " + (errorJson.message ||
                                    "Erro desconhecido"));
                                }
                                resetarBotao();
                            });
                    ...
                    if (userRole === 'colaborador' && acoesRestritas.includes(formId)) {
                        if (typeof window.requisitarAutorizacao === 'function') {
                            ...
                            console.error("Função requisitarAutorizacao não definida!");
                            enviarParaOServidor();
                        }
                    } else {
                        enviarParaOServidor();
