<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- 4. FATURAMENTO SEGMENTADO POR ARENA (FILTRO RÁPIDO) --}}
            <div class="space-y-4 mb-6">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">

                    {{-- Lista de Arenas como Selectors --}}
                    <div class="flex-1">
                        <h4
                            class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 flex items-center">
                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path
                                    d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4">
                                </path>
                            </svg>
                            Selecione a Unidade para Operar
                        </h4>
                        <div class="flex flex-wrap gap-2">
                            {{-- Opção: Todas as Arenas (Visão Geral) --}}
                            <a href="{{ route('admin.payment.index', ['date' => $selectedDate]) }}"
                                class="px-4 py-2 rounded-lg border-2 transition-all text-sm font-bold {{ !request('arena_id') ? 'border-indigo-600 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300 dark:bg-gray-800 dark:border-gray-700' }}">
                                Geral
                            </a>

                            @foreach ($faturamentoPorArena as $arena)
                                <a href="{{ route('admin.payment.index', ['date' => $selectedDate, 'arena_id' => $arena->id]) }}"
                                    class="px-4 py-2 rounded-lg border-2 transition-all text-sm font-bold flex items-center gap-2 {{ request('arena_id') == $arena->id ? 'border-green-600 bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300 dark:bg-gray-800 dark:border-gray-700' }}">
                                    {{ $arena->name }}
                                    <span
                                        class="text-[10px] px-1.5 py-0.5 rounded-md {{ request('arena_id') == $arena->id ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-400' }}">
                                        R$ {{ number_format($arena->total, 0, ',', '.') }}
                                    </span>
                                </a>
                            @endforeach
                        </div>
                    ...
                    <div class="flex-shrink-0">
                        <button onclick="openTransactionModal()"
                            class="w-full md:w-auto inline-flex items-center justify-center px-4 py-[10px] bg-gray-800 dark:bg-gray-200 border border-transparent rounded-lg font-bold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 transition ease-in-out duration-150 {{ $isActionDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105 shadow-md' }}"
