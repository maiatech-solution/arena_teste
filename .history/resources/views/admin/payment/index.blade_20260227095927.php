<x-app-layout>
    ...
    <script>
        ...
        // 1. Variável global que sobrevive ao ciclo de vida da função
        window.caixaProcessando = window.caixaProcessando || false;
        ...
        function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
            ...
            form.onsubmit = function(e) {
                // 🛑 BLOQUEIO IMEDIATO: Se já clicou, não faz NADA no segundo clique
                if (window.caixaProcessando) {
                    e.preventDefault();
                    return false;
                }
                ...
                const enviarParaOServidor = (tokenRecebido = null) => {
                    ...
                    // Marca como processando ANTES do fetch
                    window.caixaProcessando = true;
                    ...
                    fetch(targetUrl, {
                        ...
                        .then(res => res.json())
                        .then(json => {
                            // 🛡️ FILTRO ANTI-VERGONHA: Identifica a mensagem de duplicidade
                            const isAlreadyPaid = json.message === "Esta reserva já foi baixada anteriormente.";

                            if (json.success) {
                                // ✅ TIRO 1: Sucesso real. Mostra o alerta de processado.
                                alert(json.message);
                                document.body.style.pointerEvents = 'none'; // Trava a tela
                                window.location.reload();
                            } else if (isAlreadyPaid) {
                                // 🤫 TIRO 2: A "mensagem fantasma".
                                // Não damos alert. Apenas garantimos que a página atualize
                                // caso o reload do primeiro tiro ainda não tenha ocorrido.
                                console.warn("🚫 Resposta duplicada silenciada pelo sistema.");
                                window.location.reload();
                            } else {
                                // ❌ ERRO REAL: (Ex: Valor excede o preço, erro de validação)
                                alert(json.message || 'Erro ao processar.');

                                // Libera a trava global para o usuário corrigir o erro e tentar de novo
                                window.caixaProcessando = false;
                        ...
                        .catch(err => {
                            ...
                            alert('Erro na comunicação. Verifique se o registro foi salvo.');
                            window.caixaProcessando = false;
                            if (btn) {
                                btn.disabled = false;
                                btn.style.pointerEvents = 'auto';
                ...
                if (acoesCriticas.includes(formId) && userRole === 'colaborador') {
                    window.requisitarAutorizacao(token => {
                        ...
                        else window.caixaProcessando = false;
                    });
                } else {
                    enviarParaOServidor();
                }
