                ...
                form.onsubmit = function(e) {
                    ...
                    const enviarParaOServidor = (tokenRecebido = null) => {
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                if (json.success) {
                                    // 1. LIMPEZA RADICAL: Esconde modais e remove o fundo escuro (backdrop)
                                    // Removemos backdrops do Bootstrap/Tailwind e fechamos modais abertos
                                    document.querySelectorAll(
                                            '.modal-backdrop, .modal, #modalSenha, [id*="Autorizacao"]')
                                        .forEach(el => {
                                            el.style.display = 'none';
                                            el.classList.add('hidden');
                                            if (el.classList.contains('modal-backdrop')) el
                                        .remove(); // Remove o fundo escuro de vez
                                        });
                                    document.body.classList.remove('modal-open');
                                    document.body.style.overflow =
                                    'auto'; // Garante que o scroll volte se travar

                                    // 2. SE FOR FECHAMENTO (TEM RESUMO NO JSON)
                                    if (json.resumo) {
                                        const r = json.resumo;

                                        // Preenche os dados no modal de resumo
                                        document.getElementById('resumoArena').innerText = r.arena;
                                        document.getElementById('resumoData').innerText = "FECHAMENTO: " + r
                                            .data;
                                        document.getElementById('resumoDinheiro').innerText = "R$ " + r.gaveta
                                            .toFixed(2).replace('.', ',');
                                        document.getElementById('resumoPix').innerText = "R$ " + r.banco
                                            .toFixed(2).replace('.', ',');
                                        document.getElementById('resumoCredito').innerText = "R$ " + (r
                                            .credito || 0).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoDebito').innerText = "R$ " + (r.debito ||
                                            0).toFixed(2).replace('.', ',');
                                        document.getElementById('resumoSistema').innerText = "R$ " + r.sistema
                                            .toFixed(2).replace('.', ',');
                                        document.getElementById('resumoInformado').innerText = "R$ " + r
                                            .informado.toFixed(2).replace('.', ',');
                                        document.getElementById('resumoDiferenca').innerText = "R$ " + r
                                            .diferenca.toFixed(2).replace('.', ',');
                                        document.getElementById('resumoOperador').innerText = r.operador;

                                        const difWrapper = document.getElementById('resumoDiferencaCor');
                                        if (difWrapper) {
                                            difWrapper.style.color = r.diferenca < 0 ? '#dc2626' : '#16a34a';
                                        }

                                        // EXIBE O MODAL SEM ALERT (O ALERT É O QUE TRAVA O FUNDO)
                                        const modal = document.getElementById('modalResumoFechamento');
                                        if (modal) {
                                            modal.classList.remove('hidden');
                                            modal.style.display = 'flex';
                                            modal.style.zIndex = '10001'; // Força ficar acima de tudo
                                        }

                                        // O sistema NÃO recarrega aqui. O usuário deve clicar em "CONCLUIR E SAIR" no modal.
                                    } else {
                                        // Para outros formulários (que não têm resumo), mantém o comportamento padrão
                                        if (json.message) alert(json.message);
                                        window.location.reload();
                                    }
                                } else {
                                    // Trata erros (ex: falta de token do supervisor)
                            ...
                            .catch(err => {
                                console.error("Erro no processamento AJAX:", err);
                                window.caixaProcessandoGlobal[formId] = false;
                                if (btn) {
                                    btn.disabled = false;
                                    btn.innerText = "ERRO NA REQUISIÇÃO";
                                }
                            });
                    };
                    ...
                    // Note: 'closeCashForm' foi removido das restritas aqui para evitar conflito de modais,
                    // a validação de diferença/supervisor já é feita no Controller PHP.
