<x-app-layout>
    ...
    `;
            ...
            // 🛠️ FUNÇÃO AJUSTADA PARA BATER COM O SEU WEB.PHP
            window.acionarManutencao = async function(reservaId) {
                if (!confirm(
                        "Deseja bloquear este horário para Manutenção Técnica? \n\nO cliente será preservado em nossos registros internos, mas o horário ficará BLOQUEADO (Rosa) no calendário."
                    )) {
                    return;
                }

                // ✅ AJUSTE AQUI: Adicionado /mover-manutencao para coincidir com seu web.php
                const url = `/admin/reservas/${reservaId}/mover-manutencao`;
                ...
                try {
                    const response = await fetch(url, {
                        ...
                        body: JSON.stringify({
                            _method: 'PATCH' // Mas o Laravel interpreta como PATCH por causa disso
                        })
                    });
                    ...
                    if (response.ok && result.success) {
                        ...
                        showDashboardMessage(result.message || "Erro ao ativar manutenção.", 'error');
                    }
                } catch (error) {
