            ...
            if (clientNameInput()) {
                clientNameInput().addEventListener('input', function() {
                    ...
                    debounceTimer = setTimeout(() => {
                        fetch(`/api/clientes/search?q=${encodeURIComponent(query)}&arena_id=${arenaId}`)
                                ...
                                if (data.length > 0) {
                                    data.forEach(client => {
                                        ...
                                        div.innerHTML = `
                                            ...
                                            <div class="text-xs text-gray-500">${client.contact}</div>
                                        `;
                                        div.onclick = () => {
                                            // Preenche os campos ao clicar
                                            clientNameInput().value = client.name;
                                            clientContactInput().value = client.contact.replace(
                                                /\D/g, '');

                                            // Dispara a validação e busca de reputação
                                            validateClientContact(clientContactInput().value);
