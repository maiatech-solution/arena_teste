    ...
    <div class="max-w-[1600px] mx-auto px-6 py-8" x-data="{
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div class="flex items-center gap-5">
                {{-- 🔙 BOTÃO VOLTAR PARA O PAINEL PDV --}}
                <a href="{{ route('bar.pdv.painel') }}"
            ...
            <div class="flex gap-4">
                <div
                    class="bg-gray-900 border border-gray-800 px-6 py-3 rounded-2xl border-l-4 border-l-green-500 shadow-xl">
                    ...
                    <span class="text-xl font-black text-white italic tracking-tighter font-mono">
                        R$
                        {{ number_format($vendas->whereIn('status', ['paid', 'pago'])->sum('total_value'), 2, ',', '.') }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[2rem] p-5 mb-8 shadow-xl">
            <form action="{{ route('bar.vendas.pdv.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
                <div class="w-32">
                    <label class="text-[9px] font-black text-gray-500 uppercase ml-2 mb-1 block tracking-widest">Ticket
                        ID</label>
                ...
                <div class="w-44">
                    <label
                        class="text-[9px] font-black text-gray-500 uppercase ml-2 mb-1 block tracking-widest">Data</label>
                ...
                <div class="flex gap-2">
                    <button type="submit"
                        class="bg-orange-600 hover:bg-orange-500 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase transition-all shadow-lg shadow-orange-600/20 active:scale-95">
                        Filtrar
                    </button>
                            ...
                            <tr class="hover:bg-white/[0.01] transition-colors group">
                                <td class="p-8">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="w-12 h-12 rounded-2xl bg-black border border-gray-800 flex flex-col items-center justify-center text-xs font-black {{ !$isPaga ? 'text-red-900' : 'text-orange-500' }} italic">
                                            #{{ $venda->id }}
                                        </div>
                                        <div>
                                            <span
                                            ...
                                            <span
                                                class="text-gray-600 text-[10px] font-bold uppercase tracking-widest">{{ $venda->updated_at->format('H:i') }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="p-8 text-right">
                                    <span
                                        class="text-2xl font-black {{ !$isPaga ? 'text-red-950 line-through' : 'text-white' }} italic tracking-tighter font-mono">
                                ...
                                <td class="p-8 text-center">
                                    @if ($isPaga)
                                        <span
                                            class="px-4 py-1.5 bg-green-500/10 text-green-500 text-[9px] font-black rounded-full border border-green-500/20 uppercase tracking-widest">Paga</span>
                                    @else
                                        <span
                                            class="px-4 py-1.5 bg-red-500/10 text-red-500 text-[9px] font-black rounded-full border border-red-500/20 uppercase tracking-widest">Anulada</span>
                                    @endif
                                </td>
                                <td class="p-8 text-center flex justify-center gap-2">
                                    ...
                                    @elseif(!$isPaga)
                                        <span class="text-[9px] text-gray-800 font-black uppercase italic">Sem
                                            Ações</span>
                                    @else
                                        <div title="Caixa Fechado"
                                            class="w-10 h-10 flex items-center justify-center bg-gray-800/30 text-gray-700 rounded-xl border border-gray-800/50 cursor-help opacity-40">
        ...
        {{-- 📋 MODAL DE DETALHES (ATUALIZADO COM DESCONTO) --}}
            ...
            <div @click.away="modalDetalhes = false"
                ...
                <div class="p-10 relative">
                    ...
                    <div x-show="!carregando" class="space-y-6">
                        ...
                        {{-- 💰 BLOCO DE TOTAIS COM DESCONTO DINÂMICO --}}
                        <div class="space-y-3 pt-4 border-t border-gray-800">

                            {{-- Mostra o detalhamento apenas se o desconto for maior que zero --}}
                            <template x-if="parseFloat(venda.desconto) > 0">
                                <div class="space-y-2 mb-4 bg-black/20 p-4 rounded-2xl">
                                    <div
                            ...
                            <div class="pt-4 border-t border-gray-800 flex justify-between items-end">
                                <span class="text-gray-500 font-black uppercase text-xs italic">Valor Final</span>
        ...
        <div id="modalCancelamento"
            ...
            <div
                ...
                <div
                    class="absolute -right-10 -top-10 text-9xl opacity-5 italic font-black text-white pointer-events-none">
                    VOID</div>
                <div class="text-center mb-8">
                    <div
                        class="w-20 h-20 bg-red-600/10 rounded-3xl flex items-center justify-center mx-auto mb-6 border border-red-600/20 shadow-xl">
                        <span class="text-4xl">🚫</span>
                    </div>
                    <h3 class="text-white text-2xl font-black uppercase italic tracking-tighter">Anular Venda <span
                            id="cancel_id_text" class="text-red-500"></span></h3>
                ...
                <form id="formCancelarVenda" method="POST">
                    ...
                    <div class="space-y-6">
                        <textarea name="reason" required placeholder="Justificativa..."
                            class="w-full bg-black border border-gray-800 rounded-3xl p-5 text-white text-xs h-28 focus:border-red-600 transition-all shadow-inner placeholder:text-gray-700"></textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-10">
                        ...
                        <button type="button" onclick="confirmarCancelamento()"
                            class="py-5 bg-red-600 hover:bg-red-500 text-white font-black rounded-3xl uppercase text-[10px] tracking-widest shadow-xl shadow-red-900/40 active:scale-95 transition-all">Confirmar</button>
                    </div>
                </form>
            </div>
        </div>
