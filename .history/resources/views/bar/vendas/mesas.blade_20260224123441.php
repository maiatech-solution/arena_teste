    ...
    <div class="max-w-[1600px] mx-auto px-6 py-8" x-data="{
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            ...
            <div class="flex gap-4">
                <div class="bg-gray-900 border border-gray-800 px-6 py-3 rounded-2xl border-l-4 border-l-orange-500 shadow-xl">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[2rem] p-5 mb-8 shadow-xl">
            <form action="{{ route('bar.vendas.mesas.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="bg-orange-600 hover:bg-orange-500 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase transition-all shadow-lg active:scale-95">
                        Filtrar
                    </button>
            ...
            <div class="overflow-x-auto no-scrollbar">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-800/40">
                            ...
                            <tr class="hover:bg-white/[0.01] transition-colors group">
                                <td class="p-8">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-orange-900/20 border border-orange-500/30 flex items-center justify-center text-xs font-black text-orange-400 italic">
                                            #{{ $venda->id }}
                                        </div>
                                        <div>
                                ...
                                <td class="p-8">
                                    <button @click="abrirDetalhes({{ $venda->id }})" class="flex items-center gap-2 px-4 py-2 bg-gray-950 border border-gray-800 rounded-xl text-[10px] font-black uppercase text-gray-400 hover:text-indigo-400 hover:border-indigo-500 transition-all">
                                        <span>🔍</span> Ver Itens
                                    </button>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="6" class="p-20 text-center text-gray-600 uppercase font-black italic">Vazio</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div x-show="modalDetalhes" x-transition.opacity class="fixed inset-0 z-[110] flex items-center justify-center bg-black/95 backdrop-blur-sm p-4">
            <div @click.away="modalDetalhes = false" class="bg-gray-900 border border-gray-800 w-full max-w-md rounded-[3rem] overflow-hidden shadow-2xl relative">
                <div class="absolute -right-10 -top-10 text-9xl opacity-5 italic font-black text-white pointer-events-none uppercase">MESA</div>

                <div class="p-10 relative">
                    <div class="flex justify-between items-start mb-8">
                        <div>
                            <h3 class="text-white text-3xl font-black uppercase italic tracking-tighter">Comanda <span class="text-indigo-500">#</span><span x-text="venda.id"></span></h3>
                    ...
                    <div x-show="carregando" class="py-20 text-center"><span class="animate-pulse text-indigo-500 font-black uppercase text-xs tracking-[0.3em]">Buscando Comanda...</span></div>

                    <div x-show="!carregando" class="space-y-6">
                        <div class="bg-black/40 rounded-[2rem] p-6 border border-gray-800/50">
                            <label class="text-[8px] font-black text-gray-600 uppercase tracking-widest block mb-4">Consumo Detalhado</label>
                            <div class="space-y-4 max-h-60 overflow-y-auto no-scrollbar">
                                <template x-for="item in venda.itens">
                                    <div class="flex justify-between items-center border-b border-gray-800 pb-3 last:border-0">
                                        <div>
                                            <span class="text-indigo-400 font-black text-xs" x-text="item.qtd + 'x '"></span>
        ...
        <div id="modalCancelamento" class="hidden fixed inset-0 z-[120] flex items-center justify-center bg-black/95 backdrop-blur-md p-4">
            <div class="bg-gray-900 border border-gray-800 w-full max-w-md rounded-[3rem] p-10 shadow-2xl relative overflow-hidden">
                <div class="absolute -right-10 -top-10 text-9xl opacity-5 italic font-black text-white pointer-events-none uppercase">VOID</div>
                <div class="text-center mb-8">
                    <div class="w-20 h-20 bg-indigo-600/10 rounded-3xl flex items-center justify-center mx-auto mb-6 border border-indigo-600/20 shadow-xl">
                        <span class="text-4xl">🚫</span>
                    </div>
                    <h3 class="text-white text-2xl font-black uppercase italic tracking-tighter">Anular Comanda <span id="cancel_id_text" class="text-indigo-500"></span></h3>
                ...
                <form id="formCancelarMesa" method="POST">
                    ...
                    <div class="space-y-6">
                        <textarea name="reason" required placeholder="Motivo do estorno..." class="w-full bg-black border border-gray-800 rounded-3xl p-5 text-white text-xs h-28 focus:border-indigo-500 transition-all shadow-inner placeholder:text-gray-700"></textarea>
                        <div class="p-6 bg-indigo-600/5 border border-indigo-600/20 rounded-[2.5rem]">
                            ...
                            @endif
                            <input type="password" id="pass_auth_cancel" placeholder="SENHA DO GESTOR" class="w-full bg-black border border-gray-800 rounded-2xl p-4 text-white text-center text-sm font-mono tracking-[0.3em] outline-none">
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4 mt-10">
                        ...
                        <button type="button" onclick="confirmarCancelamentoMesa()" class="py-5 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-3xl uppercase text-[10px] tracking-widest shadow-xl shadow-indigo-900/40 active:scale-95 transition-all">Confirmar</button>
                    </div>
                </form>
            </div>
        </div>
