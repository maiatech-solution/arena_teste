<x-bar-layout>
    <div class="max-w-[1600px] mx-auto px-6 py-8">
        ...
        {{-- 🔍 1. BARRA DE FILTROS (Adicionada para você localizar a Venda #12) --}}
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-800/40">
                        @forelse($vendas as $venda)
                            @php
                                // 2. Lógica Dinâmica do Cadeado
                                $caixaAberto = $venda->cashSession && $venda->cashSession->status === 'open';
                            @endphp
                            <tr class="hover:bg-white/[0.01] transition-colors group">
                                <td class="p-8">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-black border border-gray-800 flex flex-col items-center justify-center text-xs font-black {{ $venda->status == 'cancelled' ? 'text-red-900' : 'text-gray-400' }} italic shadow-inner">
                                            #{{ $venda->id }}
                                        </div>
                                        <div>
