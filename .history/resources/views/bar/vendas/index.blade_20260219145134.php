            ...
            <div class="flex items-center gap-5">
                ...
                <div>
                    ...
                    <p class="text-gray-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">
                        Exibindo histórico completo (Hoje e anteriores)
                    </p>
                </div>
            </div>

            {{-- Card de Resumo Dinâmico (Soma as vendas exibidas na página atual) --}}
            ...
            <form action="{{ route('bar.vendas.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->anyFilled(['id', 'date', 'status']))
                        <a href="{{ route('bar.vendas.index') }}" class="bg-gray-800 text-gray-400 px-6 py-3 rounded-xl font-black text-[10px] uppercase flex items-center hover:bg-gray-700 transition-all">
                            Limpar
                        </a>
                    @endif
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-800/40">
                        @forelse($vendas as $venda)
                            @php
                                // Trava de Segurança: Verifica se o caixa da venda está aberto
                                $caixaAberto = $venda->cashSession && $venda->cashSession->status === 'open';
                            @endphp
                            <tr class="hover:bg-white/[0.01] transition-colors group">
                                <td class="p-8">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-black border border-gray-800 flex flex-col items-center justify-center text-xs font-black {{ $venda->status == 'cancelled' ? 'text-red-900' : 'text-gray-400' }} italic">
                                            #{{ $venda->id }}
                                        </div>
                                        <div>
                                ...
                                <td class="p-8">
                                    <span class="text-gray-500 font-black text-[10px] uppercase italic">
                                ...
                                <td class="p-8 text-right">
                                    <span class="text-2xl font-black {{ $venda->status == 'cancelled' ? 'text-red-950 line-through' : 'text-white' }} italic tracking-tighter font-mono">
                                ...
                                <td class="p-8 text-center">
                                    @if($venda->status == 'paid')
                                        ...
                                        @else
                                            <div title="Caixa Fechado (Operação Bloqueada)" class="w-10 h-10 mx-auto flex items-center justify-center bg-gray-800/30 text-gray-700 rounded-xl border border-gray-800/50 cursor-help opacity-40">
