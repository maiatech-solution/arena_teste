                        ...
                        @forelse($vendas as $venda)
                            ...
                            <tr class="hover:bg-white/[0.01] transition-colors group">
                                <td class="p-8">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="w-14 h-14 rounded-2xl bg-orange-500/10 border border-orange-500/20 flex flex-col items-center justify-center">
                                            <span
                                                class="text-[8px] font-black text-orange-500/50 uppercase leading-none">Mesa</span>
                                            <span class="text-xl font-black text-orange-500 italic leading-none">
                                                {{ str_pad($venda->table->identifier ?? '00', 2, '0', STR_PAD_LEFT) }}
                                        ...
                                        <div>
                                            <span class="text-white font-black block text-sm tracking-tighter">Pedido
                                                #{{ $venda->id }}</span>
                                            <span
                                                class="text-gray-600 text-[10px] font-bold uppercase tracking-widest">{{ $venda->updated_at->format('d/m/Y H:i') }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-8 text-gray-400 font-black text-[11px] uppercase italic">
                                    {{ $venda->user->name ?? 'N/A' }}
                                </td>
                                <td class="p-8">
                                    <button @click="abrirDetalhes({{ $venda->id }})"
                                        class="px-4 py-2 bg-gray-950 border border-gray-800 rounded-xl text-[10px] font-black uppercase text-gray-400 hover:text-orange-400 hover:border-orange-500 transition-all">
                                        🔍 Ver Itens
                                    </button>
                                </td>
                                <td
                                ...
                                <td class="p-8 text-center">
                                    @if ($isPaga && $caixaAberto)
                                        <button
                                            onclick="abrirModalCancelamento({{ $venda->id }}, '{{ number_format($venda->total_value, 2, ',', '.') }}')"
                                            class="w-10 h-10 bg-gray-950 hover:bg-red-600 text-red-500 hover:text-white rounded-xl transition-all border border-gray-800">🚫</button>
                                    @else
                                        <span class="text-[10px] text-gray-800 font-black italic">BLOQUEADO</span>
                                    @endif
                                </td>
