<x-bar-layout>
    <div class="max-w-[1600px] mx-auto px-6 sm:px-8 lg:px-10 py-6">
        ...
        <div class="flex flex-col md:flex-row gap-8 overflow-hidden">
            ...
            <div class="flex-1 flex flex-col min-h-0 bg-gray-900 rounded-[2.5rem] border border-gray-800 shadow-2xl overflow-hidden">
                ...
                <div class="p-8 border-b border-gray-800 bg-gray-900/50 space-y-6">
                    <div class="flex flex-col lg:flex-row gap-6 items-center">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h2 class="text-xl font-black text-white uppercase tracking-tighter italic">Venda <span class="text-orange-500">Direta</span></h2>
                            </div>
                        </div>
                ...
                <div class="flex-1 overflow-y-auto p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 no-scrollbar" id="productsGrid">
                    @foreach ($products as $product)
                        <button
                            onclick="addToCart({{ $product->id }}, '{{ $product->name }}', {{ $product->sale_price }}, {{ $product->stock_quantity }}, {{ $product->manage_stock }})"
                            data-category="{{ $product->bar_category_id }}"
                            ...
                            <div class="absolute top-1.5 right-1.5">
                                <span class="text-[7px] font-black px-1.5 py-0.5 rounded bg-gray-950/60
                            ...
                            <div class="w-full">
                                <h3 class="text-[9px] font-black text-white uppercase leading-tight line-clamp-2 mb-1 group-hover:text-white px-1">
                                    {{ $product->name }}
                                </h3>
            ...
            <div class="w-full md:w-96 flex flex-col bg-gray-900 rounded-3xl border border-gray-800 shadow-2xl overflow-hidden">
                <div class="p-6 border-b border-gray-800 flex items-center justify-between">
                    <h3 class="text-lg font-black text-white uppercase italic">🧾 Carrinho</h3>
                    <button onclick="clearCart()" class="text-[10px] font-black text-red-500 hover:underline uppercase tracking-tighter">Limpar</button>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-3 no-scrollbar" id="cartItems">
                    <div id="emptyCart" class="text-center py-20">
                        ...
                        <p class="text-gray-500 text-[10px] font-black uppercase tracking-widest">Aguardando Produtos</p>
                    </div>
                </div>
    ...
    <div id="receiptModal" class="hidden fixed inset-0 bg-black/95 backdrop-blur-md z-[200] flex items-center justify-center p-4">
        <div class="bg-white p-8 rounded-lg w-full max-w-[320px] shadow-2xl text-black font-mono text-sm" id="receiptContent">
            <div class="text-center border-b border-dashed border-gray-400 pb-4 mb-4">
                <h2 class="text-xl font-black uppercase">BAR DO MAIA</h2>
                <p class="text-[10px]">CUPOM NÃO FISCAL</p>
            ...
            <div class="flex flex-col gap-2 no-print">
                <button onclick="window.print()" class="w-full py-3 bg-gray-900 text-white font-black rounded-xl uppercase text-[10px] tracking-widest active:scale-95 shadow-lg shadow-gray-900/20 transition-all">🖨️ Imprimir</button>
                <button onclick="shareWhatsApp()" class="w-full py-3 bg-green-600 text-white font-black rounded-xl uppercase text-[10px] tracking-widest active:scale-95 shadow-lg shadow-green-600/20 transition-all">📱 WhatsApp</button>
                <button onclick="location.reload()" class="w-full py-3 text-gray-400 font-black uppercase text-[10px] hover:text-black transition-colors">Nova Venda</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function addToCart(id, name, price, stock, manageStock) {
            ...
            if (existingItem) {
                ...
                cart.push({ id, name, price, quantity: 1, stock, manageStock });
            }
            renderCart();
        }
        ...
        function renderCart() {
            ...
            if (cart.length === 0) {
                container.innerHTML = '<div id="emptyCart" class="text-center py-20"><span class="text-5xl block mb-4 opacity-20">🛒</span><p class="text-gray-500 text-[10px] font-black uppercase tracking-widest">Aguardando Produtos</p></div>';
        ...
        function setPaymentMethod(method) {
            ...
            document.querySelectorAll('.pay-btn').forEach(btn => {
                btn.classList.remove('bg-orange-600', 'text-white', 'border-orange-500', 'shadow-lg', 'shadow-orange-600/20');
        ...
        function validateCheckout() {
            ...
            if (isValid) {
                ...
                finishBtn.classList.add('bg-green-600', 'text-white', 'cursor-pointer', 'shadow-lg', 'shadow-green-600/30', 'hover:bg-green-500', 'active:scale-95');
            } else {
                finishBtn.classList.remove('bg-green-600', 'text-white', 'cursor-pointer', 'shadow-lg', 'shadow-green-600/30');
        ...
        async function checkout() {
            ...
            try {
                const response = await fetch('{{ route("bar.pos.store") }}', {
                    ...
                    body: JSON.stringify({ items: cart, payment_method: paymentMethod, total_value: totalValue })
                });
        ...
        function showReceipt(total) {
            ...
            document.getElementById('receiptTotal').innerText = `R$ ${total.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`;
    ...
    <style>
        ...
        @media print {
            ...
            #receiptModal { position: fixed; inset: 0; background: white; display: flex; align-items: start; justify-content: center; }
