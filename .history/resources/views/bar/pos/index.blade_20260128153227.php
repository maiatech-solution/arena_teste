    ...
    <div class="h-[calc(100vh-120px)] flex flex-col md:flex-row gap-4 overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-h-0 bg-gray-900 rounded-3xl border border-gray-800 shadow-2xl overflow-hidden">
            <div class="p-6 border-b border-gray-800 flex justify-between items-center bg-gray-900/50">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="text-xl font-black text-white uppercase tracking-tighter">🛒 Venda <span class="text-orange-500">Rápida</span></h2>
                </div>
            ...
            <div class="px-6 py-3 flex gap-2 overflow-x-auto no-scrollbar border-b border-gray-800/50">
                <button onclick="filterCategory('all')" class="cat-btn active px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all bg-orange-600 text-white border border-orange-500">Todos</button>
            ...
            <div class="flex-1 overflow-y-auto p-6 grid grid-cols-2 lg:grid-cols-4 gap-4" id="productsGrid">
                @foreach($products as $product)
                    <button onclick="addToCart({{ $product->id }}, '{{ $product->name }}', {{ $product->sale_price }}, {{ $product->stock_quantity }})"
                        ...
                        <span class="text-xs font-bold text-white uppercase leading-tight h-8 overflow-hidden mb-2">{{ $product->name }}</span>
                        <span class="text-orange-500 font-black">R$ {{ number_format($product->sale_price, 2, ',', '.') }}</span>
                        <span class="mt-2 text-[9px] {{ $product->stock_quantity <= $product->min_stock ? 'text-red-500 animate-pulse' : 'text-gray-500' }} uppercase font-black">Estoque: {{ $product->stock_quantity }}</span>
                    </button>
                @endforeach
            </div>
        </div>
        ...
        <div class="w-full md:w-96 flex flex-col bg-gray-900 rounded-3xl border border-gray-800 shadow-2xl overflow-hidden">
            <div class="p-6 border-b border-gray-800">
                <h3 class="text-lg font-black text-white uppercase flex items-center gap-2 italic">🧾 Cupom <span class="text-orange-500 text-sm italic">Digital</span></h3>
            </div>
            ...
            <div class="p-6 bg-gray-950 border-t border-gray-800">
                ...
                <div class="grid grid-cols-3 gap-2 mb-6">
                    <button onclick="setPaymentMethod('dinheiro')" class="pay-btn flex flex-col items-center p-3 rounded-xl border border-gray-800 bg-gray-900 text-gray-500 transition-all" id="btn-dinheiro">
                        <span class="text-xl">💵</span>
                        <span class="text-[8px] font-black uppercase mt-1">Dinheiro</span>
                    </button>
                    <button onclick="setPaymentMethod('pix')" class="pay-btn flex flex-col items-center p-3 rounded-xl border border-gray-800 bg-gray-900 text-gray-500 transition-all" id="btn-pix">
                        <span class="text-xl">📱</span>
                        <span class="text-[8px] font-black uppercase mt-1">PIX</span>
                    </button>
                    <button onclick="setPaymentMethod('cartao')" class="pay-btn flex flex-col items-center p-3 rounded-xl border border-gray-800 bg-gray-900 text-gray-500 transition-all" id="btn-cartao">
                        <span class="text-xl">💳</span>
