<x-bar-layout>
    ...
    <script>
        ...
        // 🟢 ADICIONAR AO CARRINHO (COM LÓGICA DE GESTÃO)
        ...
        `).join('');
            ...
            if (paymentMethod) {
                calculatePayment(); // Recalcula troco se o carrinho mudar com método selecionado
            } else {
                validateCheckout(0);
            }
        }
        ...
        function setPaymentMethod(method) {
            ...
            inputLabel.innerText = `VALOR RECEBIDO (${method.toUpperCase()})`;

            // Sugere o valor total como padrão
            amountInput.value = currentCartTotal.toFixed(2);
            amountInput.focus();
            amountInput.select();
        ...
        function calculatePayment() {
            ...
            const diff = received - currentCartTotal;

            feedbackDiv.classList.remove('hidden', 'bg-green-900/20', 'border-green-500/30', 'text-green-500',
                'bg-orange-900/20', 'border-orange-500/30', 'text-orange-500');

            if (diff > 0) {
                // TROCO
                feedbackDiv.classList.add('bg-green-900/20', 'border-green-500/30', 'text-green-500');
                label.innerText = "Troco:";
                valueSpan.innerText = `R$ ${diff.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`;
            } else if (diff < 0) {
                // FALTA
                feedbackDiv.classList.add('bg-orange-900/20', 'border-orange-500/30', 'text-orange-500');
            ...
            validateCheckout(received);
        }

        function validateCheckout(received) {
            const finishBtn = document.getElementById('finishBtn');
            // Habilita se tiver itens, método e o valor for suficiente
            const isValid = cart.length > 0 && paymentMethod && received >= currentCartTotal;
            ...
            if (isValid) {
                ...
                finishBtn.classList.add('cursor-pointer', 'shadow-lg', 'shadow-green-600/30', 'active:scale-95');
            } else {
                finishBtn.classList.replace('bg-green-600', 'bg-gray-800');
                finishBtn.classList.replace('text-white', 'text-gray-500');
                finishBtn.classList.remove('cursor-pointer', 'shadow-lg', 'shadow-green-600/30', 'active:scale-95');
            }
        }
        ...
        function showReceipt() {
            ...
            // Alimenta novos campos de troco
        ...
        function shareWhatsApp() {
            let phone = prompt("Digite o número do cliente com DDD (ex: 91988887777):", "");
            ...
            text += `\n*RECEBIDO: ${document.getElementById('receiptReceived').innerText}*`;
            text += `\n*TROCO: ${document.getElementById('receiptChange').innerText}*`;
