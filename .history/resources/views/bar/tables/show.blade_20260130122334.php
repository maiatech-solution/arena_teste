<x-bar-layout>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 overflow-y-auto no-scrollbar pr-2" style="max-height: 600px;">
                @foreach($products as $product)
                    <button onclick="addItemToOrder({{ $product->id }})"
                        ...
                        <div class="w-full">
                            <h3 class="text-[10px] font-black text-white uppercase leading-tight line-clamp-2 mb-1 group-hover:text-white">{{ $product->name }}</h3>
                            <span class="text-orange-500 font-black text-xs group-hover:text-white">R$ {{ number_format($product->sale_price, 2, ',', '.') }}</span>
                        </div>
                    </button>
                @endforeach
            </div>
    ...
    <div id="modalCheckout" class="hidden fixed inset-0 bg-black/95 backdrop-blur-md z-[300] flex items-center justify-center p-4">
        <div class="bg-gray-900 border border-gray-800 rounded-[2.5rem] w-full max-w-5xl overflow-hidden shadow-2xl flex flex-col md:flex-row">
            ...
            <div class="p-8 bg-gray-800/30 border-r border-gray-800 w-full md:w-96">
                ...
                <div class="space-y-6">
                    <div class="bg-gray-950 p-6 rounded-3xl border border-gray-800">
                        <span class="text-gray-500 text-[9px] font-black uppercase block mb-1">Total da Mesa</span>
                        <span class="text-4xl font-black text-white">R$ {{ number_format($order->total_value, 2, ',', '.') }}</span>
                    </div>
                    <div class="bg-orange-600/10 p-6 rounded-3xl border border-orange-600/20">
                        ...
                        <span class="text-4xl font-black text-orange-500" id="textRestante">R$ {{ number_format($order->total_value, 2, ',', '.') }}</span>
                    </div>
            ...
            <div class="p-8 flex-1">
                <div class="flex justify-between mb-8">
                    ...
                    <button onclick="document.getElementById('modalCheckout').classList.add('hidden')" class="text-gray-500 hover:text-white transition-colors text-2xl">✕</button>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-8">
                    <div class="col-span-1">
                        ...
                        <input type="text" id="customer_name" placeholder="Ex: João Silva" class="w-full bg-gray-950 border-gray-800 rounded-2xl text-white p-4 font-bold outline-none focus:border-orange-600 transition-all">
                    </div>
                    <div class="col-span-1">
                        <label class="text-[9px] font-black text-gray-500 uppercase mb-2 block tracking-widest">WhatsApp</label>
                        <input type="text" id="customer_phone" placeholder="(91) 99999-9999" class="w-full bg-gray-950 border-gray-800 rounded-2xl text-white p-4 font-bold outline-none focus:border-orange-600 transition-all">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" id="inputValorPago" step="0.01" class="w-full bg-gray-950 border-gray-800 rounded-2xl text-white p-4 text-2xl font-black outline-none focus:border-orange-600">
                    </div>
                    <div>
                ...
                <button onclick="addPagamento()" class="w-full py-4 bg-gray-800 hover:bg-orange-600 text-white font-black rounded-2xl uppercase text-[9px] mb-6 transition-all border border-gray-700 hover:border-orange-500 tracking-[0.2em]">
                    + Adicionar na Lista
                </button>
                ...
                <form action="{{ route('bar.tables.close', $table->id) }}" method="POST" id="formFecharMesa">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <button type="button" onclick="submeterFechamento(true)" id="btnFinalizarPrint" disabled class="py-5 bg-gray-100 text-black opacity-30 cursor-not-allowed font-black rounded-2xl uppercase text-[9px] tracking-widest shadow-lg transition-all flex items-center justify-center gap-2">
                            🖨️ Pagar e Imprimir
                        </button>
                        <button type="button" onclick="submeterFechamento(false)" id="btnFinalizar" disabled class="py-5 bg-green-600 opacity-30 cursor-not-allowed text-white font-black rounded-2xl uppercase text-[9px] tracking-widest shadow-lg transition-all">
                            ✅ Apenas Finalizar
                        </button>
                    </div>
                </form>
    ...
    <script>
        ...
        function atualizarTelaPagamento() {
            ...
            lista.innerHTML = pagamentos.map((p, i) => `
                <div class="flex justify-between items-center bg-gray-950 p-4 rounded-2xl border border-gray-800 text-[10px] font-black uppercase">
            ...
            const btns = [document.getElementById('btnFinalizar'), document.getElementById('btnFinalizarPrint')];
            btns.forEach(btn => {
        ...
        function submeterFechamento(print) {
            ...
            document.getElementById('inputPrintCoupon').value = print ? "1" : "0";
