<x-bar-layout>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[2.5rem] w-full max-w-5xl overflow-hidden shadow-2xl flex flex-col md:flex-row">
            ...
            <div class="p-8 flex-1">
                ...
                <form action="{{ route('bar.tables.close', $table->id) }}" method="POST" id="formFecharMesa">
                    ...
                    <button type="button" onclick="abrirOpcoesFinais()" id="btnFinalizarGeral" disabled class="w-full py-6 bg-green-600 opacity-30 cursor-not-allowed text-white font-black rounded-[2rem] uppercase tracking-[0.3em] text-sm shadow-2xl transition-all">
                        🏁 Finalizar Venda
                    </button>
                </form>
            </div>
    ...
    <div id="modalPosFinalizar" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-[400] flex items-center justify-center p-4">
        <div class="bg-gray-900 border border-gray-800 p-10 rounded-[3rem] w-full max-w-md text-center shadow-2xl">
            ...
            <div class="flex flex-col gap-3">
                <button onclick="executarAcaoFinal('imprimir')" class="w-full py-4 bg-white text-black font-black rounded-2xl uppercase text-[10px] tracking-widest hover:bg-orange-500 hover:text-white transition-all">
                    🖨️ Imprimir Cupom
                </button>
                <button onclick="executarAcaoFinal('whatsapp')" class="w-full py-4 bg-green-600 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest hover:bg-green-500 transition-all">
                    📱 Enviar WhatsApp
                </button>
                <button onclick="executarAcaoFinal('sair')" class="w-full py-4 bg-gray-800 text-gray-400 font-black rounded-2xl uppercase text-[10px] tracking-widest hover:text-white transition-all">
                    ✖ Apenas Sair
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function abrirOpcoesFinais() {
            document.getElementById('modalPosFinalizar').classList.remove('hidden');
        }
