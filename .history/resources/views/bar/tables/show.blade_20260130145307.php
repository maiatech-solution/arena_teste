        ...
        <div class="flex-1 bg-gray-900 rounded-[2.5rem] border border-gray-800 p-8 flex flex-col shadow-2xl">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 overflow-y-auto no-scrollbar pr-2"
                ...
                @foreach ($products as $product)
                    @php
                        // Verifica se o produto está esgotado (se controla estoque e tem 0 ou menos)
                    ...
                    <button onclick="addItemToOrder({{ $product->id }})" data-name="{{ strtolower($product->name) }}"
                        {{-- Adiciona opacidade e remove o hover se estiver esgotado --}}
                ...
                @forelse($order->items as $item)
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            {{-- BOTÃO DE ESTORNO/REMOVER --}}
                            <form action="{{ route('bar.tables.remove_item', $item->id) }}" method="POST"
                                onsubmit="return confirm('Deseja estornar este item do estoque e remover da mesa?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-gray-700 hover:text-red-500 transition-colors p-1">
    ...
    <script>
        ...
        async function addItemToOrder(productId) {
            try {
                ...
                if (data.success) {
                    // Se deu certo, recarrega para atualizar a comanda na lateral
                    window.location.reload();
                } else {
                    // 🚨 Captura a mensagem de erro (ex: "Estoque insuficiente para: Cerveja")
                    alert("⚠️ Atenção: " + data.message);
                }
            } catch (error) {
