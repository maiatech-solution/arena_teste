<x-bar-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            @if (session('whatsapp_url'))
                // 1. Tenta abrir automaticamente
                let zapWindow = window.open("{{ session('whatsapp_url') }}", '_blank');

                // 2. Se o navegador bloqueou o pop-up, zapWindow será nulo
                if (!zapWindow || zapWindow.closed || typeof zapWindow.closed == 'undefined') {
                    // Criamos um alerta bonito ou redirecionamento manual se falhar
                    console.warn("Pop-up bloqueado pelo navegador.");

                    // Opção A: Redirecionar a própria aba atual após 2 segundos
                    setTimeout(() => {
