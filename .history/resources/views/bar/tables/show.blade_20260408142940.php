<x-bar-layout>
            ...
            <div class="p-8 flex-1">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select id="selectMetodo" onchange="verificarTravaVoucherMesa(this.value)"
                            ...
                            @foreach (\App\Models\Bar\BarCashMovement::getPaymentMethods() as $key => $label)
                                {{-- Se for voucher e não for gestor, adiciona o cadeado visual --}}
                                @php
                                    $needsLock =
                                        $key === 'voucher' && !in_array(auth()->user()->role, ['admin', 'gestor']);
                                @endphp
                                <option value="{{ $key }}">{{ $label }} {{ $needsLock ? '🔒' : '' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script>
        ...
        // 1. Verifica se precisa de senha ao selecionar Voucher nas mesas
        function verificarTravaVoucherMesa(metodo) {
            const isGestor = {{ in_array(auth()->user()->role, ['admin', 'gestor']) ? 'true' : 'false' }};
            const inputValor = document.getElementById('inputValorPago');

            if (metodo === 'voucher') {
                // Se for colaborador comum, pede senha
                if (!isGestor) {
                    // Se cancelar a senha, volta para 'dinheiro'
                    requisitarAutorizacao(
                        () => configurarVoucherMesa(),
                        () => {
                ...
                // Se mudar de voucher para outro método, libera o input
                inputValor.readOnly = false;
                inputValor.classList.remove('opacity-50');
                atualizarTelaPagamento(); // Volta a sugerir o valor restante
            }
        }

        // 2. Configura o Voucher para zerar o financeiro
        function configurarVoucherMesa() {
            const inputValor = document.getElementById('inputValorPago');
            // Voucher é cortesia, então o valor FINANCEIRO que entra é 0
        ...
        function atualizarTelaPagamento() {
            ...
            // 2. Calcula quanto já foi pago na lista de pagamentos
            pagoAcumulado = pagamentos.reduce((acc, p) => acc + p.value,
            0); // Ajustado para p.value se sua lista usar esse nome

            // 3. Verifica se o método que está SELECIONADO agora é Voucher
            const metodoAtual = document.getElementById('selectMetodo').value;
            const isVoucher = (metodoAtual === 'voucher');
    ...
    `).join('');
            ...
            // Se for VOUCHER, o "Restante" visualmente vira 0 para indicar que está coberto
            const displayFaltante = isVoucher ? 0 : (faltante > 0 ? faltante : 0);
            ...
            // 5. Validação do botão Finalizar (A MUDANÇA PRINCIPAL)
            const btn = document.getElementById('btnFinalizarGeral');

            // Libera se: (Valor pago >= Total) OU (O método selecionado for Voucher)
            if (isVoucher || pagoAcumulado >= (novoTotalMesa - 0.01)) {
                btn.disabled = false;
                btn.classList.remove('opacity-30', 'cursor-not-allowed');
                btn.classList.add('bg-green-600'); // Garante que o verde apareça
            } else {
                btn.disabled = true;
            ...
            // 7. Atualiza campo oculto do formulário para o PHP
