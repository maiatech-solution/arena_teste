<x-bar-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            @if (session('whatsapp_url'))
                console.log("Link do WhatsApp detectado na sessão!");

                const url = "{!! session('whatsapp_url') !!}";

                // Tenta abrir em nova aba
                let zapWindow = window.open(url, '_blank');

                // Se o navegador bloqueou o pop-up, ele abre na mesma aba após 1 segundo
                if (!zapWindow || zapWindow.closed || typeof zapWindow.closed == 'undefined') {
                    console.warn("Pop-up bloqueado! Redirecionando aba atual...");
                    setTimeout(() => {
