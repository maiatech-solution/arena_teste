            ...
            @foreach($tables as $table)
                <div class="relative group">
                    <div class="p-6 rounded-[2.5rem] border-2 transition-all duration-300 flex flex-col items-center justify-center min-h-[160px]
                        ...
                        <span class="text-3xl mb-2 {{ $table->status == 'occupied' ? 'text-orange-500' : 'text-gray-600' }}">🪑</span>
                        ...
                        <div class="mt-4 text-center">
                            @if($table->status == 'available')
                                ...
                                <form action="{{ route('bar.tables.open', $table->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="block mt-2 text-[10px] bg-white text-black px-4 py-1.5 rounded-full font-black hover:bg-orange-500 hover:text-white transition-colors uppercase">
                                        Abrir
                                    </button>
                                </form>
                            @elseif($table->status == 'occupied')
                                <span class="text-[9px] font-black text-orange-500 uppercase tracking-widest animate-pulse">Ocupada</span>
                                <a href="{{ route('bar.tables.show', $table->id) }}" class="block mt-2 text-[10px] bg-orange-600 text-white px-4 py-1.5 rounded-full font-black uppercase tracking-tighter shadow-lg shadow-orange-600/20">
                                    Comanda
                                </a>
                            @else
                                <span class="text-[9px] font-black text-gray-500 uppercase tracking-widest">Reservada</span>
                        ...
                        <form action="{{ route('bar.tables.toggle', $table->id) }}" method="POST" class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                            @csrf
                            <button type="submit" class="text-xs filter hover:brightness-150" title="Alternar Disponibilidade">
    ...
    <div id="modalSync" class="hidden fixed inset-0 bg-black/95 backdrop-blur-md z-[200] flex items-center justify-center p-4">
        <div class="bg-gray-900 border border-gray-800 p-8 rounded-[2.5rem] w-full max-w-sm text-center">
            ...
            <form action="{{ route('bar.tables.sync') }}" method="POST">
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="this.closest('#modalSync').classList.add('hidden')" class="flex-1 text-gray-400 font-black uppercase text-[10px] hover:text-white transition-colors">Voltar</button>
                    <button type="submit" class="flex-1 py-4 bg-orange-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg shadow-orange-600/20 active:scale-95 transition-all">Salvar Layout</button>
                </div>
            </form>
        </div>
    </div>
