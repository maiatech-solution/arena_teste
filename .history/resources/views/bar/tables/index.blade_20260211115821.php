    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            ...
            <div class="flex gap-3">
                {{-- 🔒 Somente Gestores e Admins podem ver e acessar a configuração do salão --}}
        ...
        <div class="bg-blue-600 text-white p-4 rounded-2xl mb-6 font-bold">
            O link gerado foi: {{ session('whatsapp_url') }}
        </div>
        @endif
            ...
            <div class="relative group">
                <div
                    ...
                    <h3 class="text-xl font-black text-white">Mesa
                        {{ str_pad($table->identifier, 2, '0', STR_PAD_LEFT) }}
                    </h3>
                    ...
                    <div class="mt-4 text-center">
                        ...
                        <a href="{{ route('bar.tables.show', $table->id) }}"
                            class="block mt-1 text-[10px] bg-orange-600 text-white px-4 py-1.5 rounded-full font-black uppercase tracking-tighter shadow-lg shadow-orange-600/20 hover:bg-orange-500 transition-all">
                            Comanda
                        </a>
                    ...
                    <form id="form-toggle-{{ $table->id }}" action="{{ route('bar.tables.toggle', $table->id) }}" method="POST"
                        class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                        @csrf
                        {{-- 🛡️ Trava de autorização para bloquear/desbloquear mesa --}}
                        <button type="button"
                            onclick="requisitarAutorizacao(() => document.getElementById('form-toggle-{{ $table->id }}').submit())"
                            class="text-xs filter hover:brightness-150"
