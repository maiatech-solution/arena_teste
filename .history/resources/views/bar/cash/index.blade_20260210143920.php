<x-bar-layout>
    <div class="max-w-[1600px] mx-auto px-6 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
            ...
            {{-- BOTÕES DE AÇÃO COM TRAVA DE SEGURANÇA (MESMO PADRÃO DO ESTOQUE) --}}
            @if ($openSession)
    ...
    <script>
        // 1. Variável para guardar o que o colaborador tentou abrir (Sangria ou Fechamento)
        let acaoPendente = null;

        /**
         * Esta função intercepta o clique do colaborador.
         * Ela limpa o campo de autorização antigo e pede o login do gestor.
         */
        function requisitarAutorizacao(callback) {
            const userRole = "{{ auth()->user()->role }}";

            // Se já for admin ou gestor, libera a ação na hora
            if (userRole === 'admin' || userRole === 'gestor') {
                callback();
                return;
            }

            // Se for colaborador, prepara o terreno
            acaoPendente = callback;

            // Limpa qualquer autorização anterior que tenha ficado no formulário
            const inputAuth = document.getElementById('authorized_by_user');
            if (inputAuth) inputAuth.value = '';

            // Abre o seu modal de login (ID padrão do seu sistema)
        ...
        /**
         * Esta função valida o seu e-mail e senha (Adriano).
         * Se estiver correto, ela injeta seu e-mail no formulário de sangria da Blenda.
         */
        async function validarLoginSupervisor() {
            ...
            try {
                const response = await fetch("{{ route('bar.auth.validate') }}", {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    // 🛡️ O PULO DO GATO: Injeta seu e-mail no campo oculto do formulário de sangria
                    const inputAuth = document.getElementById('authorized_by_user');
                    if (inputAuth) {
                        inputAuth.value = email;
                    ...
                    // Executa a ação (abrir o modal de Sangria ou Fechamento)
