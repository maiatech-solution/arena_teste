    ...
    <div class="bg-gray-900 border border-gray-800 w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden shadow-orange-900/20">
        ...
        <div class="p-10 border-b border-gray-800 text-center">
            ...
            <div class="grid grid-cols-2 gap-4 mt-6">
                ...
                <div class="bg-black/40 p-3 rounded-2xl border border-gray-800 text-center">
                    <span class="text-[8px] font-black text-gray-500 uppercase block mb-1 leading-tight">Seu Faturamento<br>({{ auth()->user()->name }})</span>
                    <span class="text-orange-500 font-black text-lg italic">R$ {{ number_format($dinheiroGeral, 2, ',', '.') }}</span>
                </div>
                {{-- O que tem no bar no total --}}
                <div class="bg-black/40 p-3 rounded-2xl border border-gray-800 text-center">
                    <span class="text-[8px] font-black text-gray-500 uppercase block mb-1 leading-tight">Total Geral<br>do Bar</span>
                    <span class="text-white font-black text-lg italic">R$ {{ number_format($openSession->expected_balance ?? 0, 2, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('bar.cash.close') }}" method="POST" class="p-10" id="formCloseCash">
            ...
            <div class="space-y-8">
                {{-- VALOR CONTADO --}}
                <div>
                    <label class="text-gray-500 uppercase text-[10px] font-black ml-4 mb-2 block tracking-widest text-center">Total Físico na Gaveta (Dinheiro Vivo)</label>
                    ...
                    <p class="text-[9px] text-gray-600 text-center mt-3 uppercase font-bold italic">Some o seu valor com o dos outros operadores que não fecharam.</p>
                </div>

                {{-- OBSERVAÇÕES --}}
                <div>
                    <label class="text-gray-500 uppercase text-[10px] font-black ml-4 mb-2 block tracking-widest">Notas / Observações</label>
            ...
            <div class="flex flex-col gap-4 mt-10">
                {{-- Botão com autorização --}}
                <button type="button"
                    onclick="requisitarAutorizacao(() => document.getElementById('formCloseCash').submit())"
