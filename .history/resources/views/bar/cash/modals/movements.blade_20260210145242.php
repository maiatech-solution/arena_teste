...
<div id="modalMovement"
    ...
    <div
        ...
        <form action="{{ route('bar.cash.movement') }}" method="POST" class="p-8" id="formMovement">
            ...
            <div class="space-y-6">
                <div>
                    <label class="text-gray-500 uppercase text-[10px] font-black ml-4 mb-2 block tracking-widest text-center">Valor do Lançamento (R$)</label>
...
<script>
    // 🧠 MEMÓRIA TEMPORÁRIA: Para não perder os dados quando o modal de login fecha
    let gestorEmailSalvo = '';
    let gestorSenhaSalvo = '';

    function openModalMovement(type) {
        // 💾 Captura os dados assim que o modal de sangria abre (antes que o layout limpe os campos)
        ...
        if (emailInput && passInput) {
            gestorEmailSalvo = emailInput.value;
            gestorSenhaSalvo = passInput.value;
        }
    ...
    function enviarComAutorizacao(idFormulario) {
        ...
        // Tentamos pegar do campo agora, ou usamos o que salvamos na memória
        const emailAtual = document.getElementById('authEmail')?.value || document.querySelector('input[name="email"]')?.value;
        const senhaAtual = document.getElementById('authPassword')?.value || document.querySelector('input[name="password"]')?.value;

        const emailFinal = emailAtual || gestorEmailSalvo;
        const senhaFinal = senhaAtual || gestorSenhaSalvo;

        if (form && emailFinal && senhaFinal) {
            ...
            if (mirrorEmail && mirrorPassword) {
                ...
                mirrorPassword.value = senhaFinal;

                console.log("🚀 Enviando com autorização de: " + emailFinal);
                form.submit();
            } else {
                console.error("Erro: Campos de espelho não encontrados.");
            }
        } else {
            alert("Erro: Não conseguimos capturar o login do supervisor. Por favor, tente reabrir a opção.");
        }
    }
