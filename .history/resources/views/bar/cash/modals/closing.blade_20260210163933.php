        ...
        <div class="p-8">
            ...
            <div class="space-y-4 mb-8">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="bg-black/40 p-5 rounded-3xl border border-gray-800">
                        <span class="text-[9px] font-black text-gray-500 uppercase block mb-1 tracking-widest leading-tight">Esperado em Dinheiro<br>(Gaveta)</span>
                        <span class="text-white font-black text-2xl italic">R$ {{ number_format($openSession->expected_balance ?? 0, 2, ',', '.') }}</span>
                    </div>
                    {{-- DIGITAL --}}
                    <div class="bg-black/40 p-5 rounded-3xl border border-gray-800">
                        <span class="text-[9px] font-black text-gray-500 uppercase block mb-1 tracking-widest leading-tight">Faturamento Digital<br>(PIX/Cartões)</span>
                        <span class="text-blue-400 font-black text-2xl italic">R$ {{ number_format($faturamentoDigital ?? 0, 2, ',', '.') }}</span>
                    </div>
                </div>

                {{-- BRUTO TOTAL (DESTAQUE CENTRAL) --}}
                <div class="bg-orange-600/10 p-5 rounded-3xl border border-orange-600/20 flex justify-between items-center px-8">
                    <div>
                        <span class="text-[10px] font-black text-orange-500 uppercase block tracking-[0.2em]">Faturamento Bruto Total</span>
                        <p class="text-[8px] text-gray-500 font-bold uppercase italic mt-1">(Total vendido sem o fundo de reserva)</p>
                    </div>
            ...
            <form action="{{ route('bar.cash.close') }}" method="POST" id="formCloseCash">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- VALOR CONTADO --}}
                    <div>
                        <label class="text-gray-500 uppercase text-[10px] font-black ml-2 mb-2 block tracking-widest">Contagem Real na Gaveta</label>
                    ...
                    {{-- OBSERVAÇÕES --}}
                    <div>
                        <label class="text-gray-500 uppercase text-[10px] font-black ml-2 mb-2 block tracking-widest">Observações do Turno</label>
