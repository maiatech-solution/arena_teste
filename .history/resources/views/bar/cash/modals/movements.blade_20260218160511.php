...
<div id="modalMovement" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
    <div class="bg-gray-900 border border-gray-800 w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form action="{{ route('bar.cash.movement') }}" method="POST" class="p-8" id="formMovement">
            ...
            <input type="hidden" name="supervisor_email" value="{{ auth()->user()->role === 'admin' || auth()->user()->role === 'gestor' ? auth()->user()->email : '' }}">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-gray-500 uppercase text-[10px] font-black ml-4 mb-2 block tracking-widest text-center">Método</label>
                        <select name="payment_method" required class="w-full bg-gray-800 border-none rounded-2xl p-4 text-white font-black text-xs uppercase outline-none focus:ring-2 focus:ring-orange-500">
                            <option value="dinheiro" selected>💵 Dinheiro</option>
                            <option value="pix">📱 PIX</option>
                        </select>
                    </div>
                    <div>
                        <label class="text-gray-500 uppercase text-[10px] font-black ml-4 mb-2 block tracking-widest text-center italic text-orange-500">Senha Gestor</label>
            ...
            <div class="flex gap-4 mt-10">
                ...
                <button type="button" onclick="dispararMovimento()" class="flex-1 py-4 bg-orange-600 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest shadow-lg active:scale-95">Confirmar</button>
            </div>
        </form>
    </div>
</div>
