        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
            <div>
                ...
                <div class="mt-4 flex items-center gap-3">
                    <form action="{{ route('bar.cash.index') }}" method="GET" id="filterForm" class="flex items-center gap-2">
                        <input type="date" name="date" value="{{ $date ?? date('Y-m-d') }}" onchange="document.getElementById('filterForm').submit()"
            ...
            @if($openSession)
                <div class="flex flex-wrap gap-3">
                    <button onclick="openModalMovement('sangria')" class="px-6 py-3 bg-red-600/10 border border-red-600/20 text-red-500 font-bold rounded-2xl uppercase text-xs hover:bg-red-600 hover:text-white transition-all shadow-lg">
                        🔻 Sangria
                    </button>
                    <button onclick="openModalMovement('reforco')" class="px-6 py-3 bg-blue-600/10 border border-blue-600/20 text-blue-500 font-bold rounded-2xl uppercase text-xs hover:bg-blue-600 hover:text-white transition-all shadow-lg">
                        🔺 Reforço
                    </button>
                    <button onclick="openModalClosing()" class="px-8 py-3 bg-white text-black font-black rounded-2xl uppercase text-[10px] tracking-widest hover:scale-105 transition-all shadow-xl border-b-4 border-gray-300">
                        🔒 Encerrar Turno
                    </button>
                </div>
            @endif
        ...
        @else
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <div class="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 relative overflow-hidden group shadow-2xl border-l-4 border-l-green-500">
                    ...
                    <p class="text-[9px] text-gray-600 mt-2 font-bold uppercase tracking-tighter italic">Soma em espécie do turno atual.</p>
                </div>
            ...
            <div class="bg-orange-600/5 border border-orange-600/20 p-8 rounded-[3rem] mb-10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl relative overflow-hidden">
                <div class="absolute top-0 left-0 w-1 h-full bg-orange-600"></div>
                <div>
                    <h4 class="text-orange-500 font-black uppercase text-xs italic tracking-[0.2em] mb-4">Turno Encerrado - Auditoria</h4>
                    <div class="flex flex-wrap gap-12">
                        <div>
                            <p class="text-gray-500 text-[9px] font-black uppercase mb-1">Esperado Sistema</p>
                            <p class="text-white font-black text-2xl italic">R$ {{ number_format($currentSession->expected_balance, 2, ',', '.') }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-[9px] font-black uppercase mb-1">Contado Físico</p>
                            <p class="text-white font-black text-2xl italic">R$ {{ number_format($currentSession->closing_balance, 2, ',', '.') }}</p>
                        </div>
                        <div>
                            ...
                            <p class="font-black text-2xl italic {{ $diff < 0 ? 'text-red-500' : ($diff > 0 ? 'text-green-500' : 'text-blue-500') }}">
                ...
                <div class="flex flex-col items-end gap-3">
                    <div class="bg-gray-950 p-6 rounded-3xl border border-gray-800 min-w-[300px]">
                        <p class="text-gray-400 text-xs italic font-medium leading-relaxed">"{{ $currentSession->notes ?? 'Sem observações.' }}"</p>
                    </div>
                    {{-- BOTÃO REABRIR (Admin) --}}
                    @if(auth()->user()->hasRole('admin') && !$openSession)
                        <form action="{{ route('bar.cash.reopen', $currentSession->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="text-[9px] font-black text-orange-500 uppercase underline tracking-widest hover:text-orange-400 transition-colors">
                ...
                <div class="p-8 border-b border-gray-800 flex justify-between items-center bg-gray-800/20">
                    ...
                    <div class="flex items-center gap-4">
                         ...
                         @if($currentSession->status == 'open')
                            <span class="text-green-500 text-[10px] font-black uppercase tracking-widest animate-pulse flex items-center gap-2">
                                <span class="w-2 h-2 bg-green-500 rounded-full"></span> Aberto
                            </span>
                         @else
                            <span class="text-red-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 font-black">
                        ...
                        <tbody class="divide-y divide-gray-800/50">
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="p-6">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[8px] uppercase font-black px-2 py-0.5 rounded {{ $mov->type == 'sangria' ? 'bg-red-500/10 text-red-500' : ($mov->type == 'reforco' ? 'bg-blue-500/10 text-blue-500' : 'bg-green-500/10 text-green-500') }}">
                                            {{ $mov->type }}
                                        </span>
                                        <span class="text-[8px] text-gray-600 font-bold uppercase italic">{{ $mov->payment_method }}</span>
                                    </div>
                                </td>
                                <td class="p-6 text-gray-400 text-[10px] font-bold uppercase tracking-tighter italic tracking-widest">{{ $mov->user->name }}</td>
                                <td class="p-6 text-right font-black italic text-xl {{ $mov->type == 'sangria' ? 'text-red-500 font-black' : 'text-white font-black' }}">
                            ...
                            <tr>
                                <td colspan="4" class="p-24 text-center opacity-20">
                                    <p class="text-gray-600 font-black uppercase tracking-widest italic text-3xl">Sem movimentações</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
