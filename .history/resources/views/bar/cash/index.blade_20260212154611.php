<x-bar-layout>
    ...
    <script>
        ...
        function enviarComAutorizacao(idFormulario) {
            ...
            // 🛡️ PLANO C (Caso Adriano Gestor): Se ainda estiver vazio, tenta pegar o e-mail do próprio form ou via prompt
            if (!emailFinal || !passFinal) {
                // Se você já está logado como gestor, o PHP pode ter injetado o e-mail no hidden
                const hiddenEmail = form.querySelector('input[name="supervisor_email"]')?.value;
                if (hiddenEmail) {
                    emailFinal = hiddenEmail;
                    passFinal = prompt("⚠️ Confirme sua senha de Gestor para encerrar o turno:");
                }
            }
            ...
            if (form && emailFinal && passFinal) {
                ...
                alert(
                    "⚠️ Autorização necessária: As credenciais do supervisor não foram detectadas. Digite o e-mail e senha na janela de autorização.");
            }
        }
