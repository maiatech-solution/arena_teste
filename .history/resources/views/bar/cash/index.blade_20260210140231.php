<x-bar-layout>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
            ...
            @if ($openSession)
                <div class="flex flex-wrap gap-3">
                    {{-- Botão Sangria: Pede PIN antes de abrir --}}
                    ...
                    {{-- Botão Reforço: Pede PIN antes de abrir --}}
                    ...
                    {{-- Botão Encerrar: Pede PIN antes de abrir --}}
    ...
    {{-- MODAL DE AUTORIZAÇÃO DE SUPERVISOR (REUTILIZÁVEL) --}}
<div id="modalAuthSupervisor" class="hidden fixed inset-0 bg-black/95 backdrop-blur-md z-[500] flex items-center justify-center p-4">
    <div class="bg-gray-900 border border-gray-800 p-8 rounded-[2.5rem] w-full max-w-sm text-center shadow-2xl">
        <div class="text-4xl mb-4">🔑</div>
        <h3 class="text-xl font-black text-white uppercase italic mb-2">Autorização</h3>
        <p class="text-gray-500 text-[10px] uppercase font-bold tracking-widest mb-6">Ação restrita a Gestores ou Admins</p>

        <div class="space-y-4">
            <input type="password" id="authSupervisorPin" placeholder="Digite seu PIN"
                class="w-full bg-gray-950 border-gray-800 rounded-2xl text-white p-4 text-center text-2xl font-black outline-none focus:border-orange-600 transition-all">

            <div class="flex gap-3">
                <button onclick="document.getElementById('modalAuthSupervisor').classList.add('hidden')"
                    class="flex-1 text-gray-500 font-black uppercase text-[10px] hover:text-white transition-colors">Cancelar</button>
                <button onclick="validarPinSupervisor()"
                    class="flex-1 py-4 bg-orange-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg shadow-orange-600/20 active:scale-95 transition-all">
                    Validar
                </button>
            </div>
        </div>
...
<script>
    ...
    // Função que decide se pede senha ou libera direto
    function requisitarAutorizacao(callback) {
        const userRole = "{{ auth()->user()->role }}";

        // Se for admin ou gestor, já executa a função (callback) direto
        if (userRole === 'admin' || userRole === 'gestor') {
            callback();
            return;
        }

        // Se for colaborador, guarda a ação e pede o PIN
        acaoPendente = callback;
        document.getElementById('modalAuthSupervisor').classList.remove('hidden');
        document.getElementById('authSupervisorPin').value = '';
        document.getElementById('authSupervisorPin').focus();
    }

    // Função que valida o PIN digitado
    function validarPinSupervisor() {
        const pin = document.getElementById('authSupervisorPin').value;

        // Aqui você usa o PIN que definiu (ex: 1234)
        if (pin === "1234") {
            document.getElementById('modalAuthSupervisor').classList.add('hidden');

            // Executa a ação que estava esperando (abrir modal de sangria, fechar, etc)
