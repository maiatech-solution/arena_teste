<x-bar-layout>
     ...
     * Captura os dados no exato momento da digitação para não perder nada
     * mesmo que o modal de login feche e limpe os campos.
     */
     ...
     * Gerencia se abre o modal de senha ou libera direto para Admin
     */
    function requisitarAutorizacao(callback) {
        const userRole = "{{ auth()->user()->role }}";

        // Se já for admin, libera a ação sem pedir senha
        if (userRole === 'admin' || userRole === 'gestor') {
            callback();
            return;
        }

        // Se for colaborador, guarda a ação e abre o modal de senha
        acaoPendente = callback;
        const modalAuth = document.getElementById('modalAuthSupervisor');
        if (modalAuth) {
            modalAuth.classList.remove('hidden');
            // Foca no e-mail e garante que os campos de memória iniciem limpos
            document.getElementById('authEmail').focus();
        }
    }

    /**
     * 3. ENVIAR COM AUTORIZAÇÃO (CHAMADA PELOS MODAIS):
     * Esta função é o motor que junta os dados do supervisor com o formulário
     */
    function enviarComAutorizacao(idFormulario) {
        const form = document.getElementById(idFormulario);

        // Prioridade: 1. Valor no input (se existir) | 2. Valor na memória global
        const emailFinal = document.getElementById('authEmail')?.value || window.supervisorMemoriaEmail;
        const passFinal = document.getElementById('authPassword')?.value || window.supervisorMemoriaPass;

        console.log("🚀 Debug Caixa - Enviando Form:", idFormulario, " | Autorizador:", emailFinal);

        if (form && emailFinal && passFinal) {
            // Localiza os campos 'mirror' que devem existir nos seus modais
