<x-bar-layout>
    ...
    <script>
        // 🧠 MEMÓRIA GLOBAL REFORÇADA
        window.supervisorMemoriaEmail = "";
        window.supervisorMemoriaPass = "";

        /**
         * 1. MONITOR DE INPUT (AGRESSIVO)
         * Captura os dados em tempo real de qualquer campo de e-mail ou senha na página.
         */
        document.addEventListener('input', function(e) {
            const t = e.target;

            // Captura E-mail
            if (t.type === 'email' || t.name === 'supervisor_email' || t.name === 'email' || t.id === 'authEmail') {
                window.supervisorMemoriaEmail = t.value;
            }

            // Captura Senha
            if (t.type === 'password' || t.name === 'supervisor_password' || t.name === 'password' || t.id ===
                'authPassword') {
        ...
        function tentarEncerrarTurno() {
            ...
            // Chama a janelinha de autorização do layout
        ...
        function openModalMovement(type) {
            ...
            if (modal) {
                ...
                document.getElementById('modalTitle').innerText = type === 'sangria' ? '🔻 Sangria de Caixa' :
                    '🔺 Reforço (Aporte)';
                document.getElementById('btnSubmit').className = type === 'sangria' ?
                    "flex-1 py-4 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest bg-red-600" :
                    "flex-1 py-4 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest bg-blue-600";
        ...
        /**
         * 4. 🚀 ENVIAR COM AUTORIZAÇÃO (Versão Totalmente Blindada)
         */
        function enviarComAutorizacao(idFormulario) {
            const form = document.getElementById(idFormulario);

            // Passo A: Tenta pegar da memória global
            let email = window.supervisorMemoriaEmail;
            let pass = window.supervisorMemoriaPass;

            // Passo B: Se estiver vazio, varre a página por IDs conhecidos
            if (!email || !pass) {
                const eField = document.getElementById('authEmail') || document.getElementById('supervisor_email');
                const pField = document.getElementById('authPassword') || document.getElementById('supervisor_password');
                if (eField && pField) {
                    email = eField.value;
                    pass = pField.value;
                }
            ...
            // Passo C: Último recurso - Varre por seletores de tipo (Plano de Emergência)
            if (!email || !pass) {
                const anyEmail = document.querySelector('input[type="email"]');
                const anyPass = document.querySelector('input[type="password"]');
                if (anyEmail && anyPass) {
            ...
            if (form && email && pass) {
                ...
                if (mEmail && mPass) {
                    mEmail.value = email;
                    mPass.value = pass;
                    console.log("Autorização vinculada com sucesso. Enviando...");
                    form.submit();
                } else {
        ...
        // Exportação global para garantir acessibilidade
