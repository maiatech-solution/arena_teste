<x-bar-layout>
    <div class="max-w-7xl mx-auto px-4 py-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-gray-500 font-bold text-xs uppercase tracking-widest mt-1">Gestão de perdas, quebras e estornos autorizados</p>
                </div>
            </div>
            ...
            <form action="{{ route('bar.reports.cancelations') }}" method="GET" class="flex items-center gap-2 bg-gray-900 p-2 rounded-2xl border border-gray-800">
                ...
                <button type="submit" class="bg-orange-600 hover:bg-orange-50 text-white px-5 py-2 rounded-xl font-black text-[10px] uppercase transition-all shadow-lg active:scale-95">
                    Filtrar
                </button>
            </form>
        </div>
        ...
        <div class="bg-gray-900 rounded-[2.5rem] border border-red-500/30 shadow-2xl overflow-hidden mb-12 bg-gradient-to-b from-red-500/5 to-transparent">
            <div class="p-8 border-b border-gray-800 flex justify-between items-center">
                ...
                <span class="bg-red-600 text-white text-[9px] font-black px-3 py-1 rounded-full animate-pulse tracking-widest">DESCARTE / VENCIMENTO</span>
            </div>

            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="text-gray-500 text-[10px] font-black uppercase tracking-widest border-b border-gray-800 bg-red-900/10">
                    ...
                    <tbody class="divide-y divide-gray-800/50">
                        @forelse($perdasReais as $perda)
                            <tr class="hover:bg-red-500/5 transition-colors">
                                <td class="p-6 text-xs text-gray-500 font-bold">{{ $perda->created_at->format('d/m H:i') }}</td>
                                <td class="p-6 text-white font-black text-xs uppercase tracking-tight">{{ $perda->product->name }}</td>
                                <td class="p-6 text-center">
                                    <span class="text-red-500 font-black text-xl">-{{ $perda->quantity }}</span>
                                </td>
                                <td class="p-6 text-gray-400 text-xs italic">{{ $perda->description }}</td>
                                <td class="p-6 text-gray-500 text-xs font-bold uppercase">{{ $perda->user->name }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="p-16 text-center text-gray-700 font-black uppercase italic">Nenhuma perda física registrada neste período.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="bg-gray-900 rounded-[2.5rem] border border-gray-800 shadow-xl overflow-hidden opacity-60 hover:opacity-100 transition-opacity duration-500">
            <div class="p-8 border-b border-gray-800 bg-black/20">
                ...
                <p class="text-[9px] text-gray-600 font-bold uppercase mt-1">Produtos que retornaram à prateleira após cancelamento de mesa ou PDV</p>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-800/50">
                        @foreach($retornosEstoque as $retorno)
                            <tr class="hover:bg-white/[0.01]">
                                <td class="p-5 text-[11px] text-gray-600 font-bold">{{ $retorno->created_at->format('d/m H:i') }}</td>
                                <td class="p-5 text-[11px] text-gray-400 font-black uppercase italic">{{ $retorno->product->name }}</td>
                                <td class="p-5 text-center">
                                    <span class="text-green-500 font-black text-xs">+{{ $retorno->quantity }}</span>
                                </td>
                                <td class="p-5 text-[10px] text-gray-600 italic uppercase">{{ $retorno->description }}</td>
