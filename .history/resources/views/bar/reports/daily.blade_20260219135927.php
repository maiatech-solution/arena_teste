<x-bar-layout>
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {{-- CARD DE RESUMO RÁPIDO NO TOPO --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-blue-500">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Recorde Diário (Mês)</p>
                <h3 class="text-2xl font-black text-white italic">
                    R$
                    {{ number_format(collect($datas)->max(fn($d) => ($d['mesas'] ?? 0) + ($d['pdv'] ?? 0)), 2, ',', '.') }}
                </h3>
            </div>
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-orange-500">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Média por Dia Operado</p>
                @php
                    $diasComVenda = collect($datas)->filter(fn($d) => $d['mesas'] + $d['pdv'] > 0)->count();
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[3rem] p-8 mb-10 shadow-2xl relative overflow-hidden">
            <div class="absolute right-0 top-0 p-10 opacity-5 text-8xl italic font-black text-white uppercase">Gráfico
            </div>
            <h2
                class="text-xs font-black text-gray-400 uppercase tracking-widest mb-10 italic text-center md:text-left">
                Desempenho de Vendas no Mês Selecionado</h2>

            <div
                class="flex items-end justify-between gap-1 h-64 px-2 border-b border-gray-800/50 pb-2 overflow-x-auto no-scrollbar">
                @php
                ...
                @foreach ($datas as $dia => $valores)
                    ...
                    @if ($carbonDia->format('m') == $mesAtualFiltro)
                        <div class="flex-1 min-w-[30px] flex flex-col items-center group relative">
                            ...
                            <div
                                class="absolute -top-10 bg-orange-600 text-white text-[9px] font-black px-2 py-1 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-all z-10 whitespace-nowrap">
                            ...
                            {{-- Legenda Inferior --}}
                            <span
                                class="mt-3 text-[8px] font-black {{ $isHoje ? 'text-orange-500' : 'text-gray-600' }}">
                                {{ $carbonDia->format('d') }}
                            </span>
                            <span
                                class="text-[6px] font-bold uppercase text-gray-700">{{ $diasSemana[$carbonDia->format('l')] }}</span>
                        </div>
                    @endif
                @endforeach
            </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[3rem] shadow-2xl overflow-hidden">
            <div class="p-8 border-b border-gray-800 bg-black/20 flex justify-between items-center">
                <h2 class="text-[10px] font-black text-white uppercase tracking-widest italic font-mono uppercase">
                    Detalhamento por Dia</h2>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-black/40">
                            <th class="p-8 text-[10px] font-black text-gray-500 uppercase tracking-widest uppercase">
                                Data / Dia da Semana</th>
                                ...
                                <tr
                                    ...
                                    <td class="p-8">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-12 h-12 rounded-2xl bg-black border border-gray-800 flex flex-col items-center justify-center text-sm font-black {{ $carbon->isToday() ? 'text-orange-500' : 'text-gray-500' }} uppercase italic shadow-inner">
                                                <span>{{ $carbon->format('d') }}</span>
                                                <span class="text-[7px]">{{ $diasSemana[$carbon->format('l')] }}</span>
                                            </div>
                                            <div>
                                                <span
                                                    class="text-white font-black block uppercase text-sm tracking-tighter group-hover:text-orange-500 transition-colors">
                                                    {{ $diasSemanaFull[$carbon->format('l')] }}
                                                </span>
                                                <span
                                                    class="text-[10px] text-gray-600 font-bold uppercase tracking-widest">{{ $carbon->format('d/m/Y') }}</span>
                                            </div>
                                        </div>
                                    </td>
                    ...
                    <tfoot>
                        <tr class="bg-black/60 border-t-2 border-orange-600/50">
                            <td class="p-10 text-white font-black uppercase italic tracking-[0.2em] text-sm">Faturamento
                                Acumulado (Mês)</td>
                            <td colspan="3"
                                class="p-10 text-right text-4xl font-black text-orange-500 italic tracking-tighter font-mono">
