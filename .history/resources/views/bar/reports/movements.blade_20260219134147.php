    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h1 class="text-4xl font-black text-white uppercase tracking-tighter italic leading-none">
                        Fluxo de <span class="text-orange-600">Estoque</span>
                    </h1>
                    <p class="text-gray-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">
                        Auditoria e Rastreabilidade de Insumos
                    </p>
                </div>
            </div>
            ...
            <form action="{{ route('bar.reports.movements') }}" method="GET"
                class="flex flex-wrap items-center gap-3 bg-gray-900/50 p-2 rounded-[2rem] border border-gray-800/50 shadow-2xl">

                {{-- Busca por Nome --}}
                <div class="relative group">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="BUSCAR PRODUTO..."
                        class="bg-black border-none rounded-2xl text-white text-[10px] font-black uppercase pl-6 pr-6 py-3 outline-none focus:ring-1 focus:ring-orange-500 transition-all min-w-[220px] placeholder:text-gray-700">
                </div>
                ...
                @if (request('type') || request('date') || request('search'))
                    <a href="{{ route('bar.reports.movements') }}"
                        class="px-4 py-2 text-red-500 hover:text-white transition-colors text-[9px] font-black uppercase tracking-tighter italic">
                        Limpar ✕
                    </a>
                @endif
            </form>
        ...
        {{-- 📋 TABELA DE HISTÓRICO --}}
        <div class="bg-gray-900 border border-gray-800 rounded-[3rem] shadow-2xl overflow-hidden relative mb-12">
            <div class="p-6 bg-black/20 border-b border-gray-800 flex justify-between items-center">
                <h2 class="text-[10px] font-black text-white uppercase tracking-widest">Logs de Movimentação</h2>
                <div class="flex gap-2">
                    <span class="w-2 h-2 rounded-full bg-orange-600 animate-pulse"></span>
                    <span class="text-[8px] text-gray-500 font-black uppercase">Sincronizado</span>
                </div>
            </div>
        ...
        {{-- 📊 POSIÇÃO DE ESTOQUE (INVENTÁRIO ATUAL) --}}
        <div class="mb-10 bg-gray-900 border border-gray-800 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden">
                ...
                @foreach ($inventorySummary as $product)
                    <div class="bg-black/60 border {{ $product->stock_quantity <= 10 ? 'border-red-600/50 shadow-[0_0_20px_rgba(220,38,38,0.1)]' : 'border-gray-800' }} p-5 rounded-[2rem] flex flex-col items-center justify-center text-center group hover:scale-105 transition-all">
                        ...
                        <div class="w-full bg-gray-900 h-1.5 rounded-full overflow-hidden border border-gray-800">
                            <div class="h-full {{ $product->stock_quantity <= 10 ? 'bg-red-600 shadow-[0_0_10px_rgba(220,38,38,0.5)]' : 'bg-orange-600 shadow-[0_0_10px_rgba(234,88,12,0.5)]' }}"
