<x-bar-layout>
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-10 bg-gray-900 border border-gray-800 rounded-[2.5rem] p-8 shadow-2xl">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h2 class="text-lg font-black text-white uppercase italic tracking-tighter">Posição Atual de <span class="text-orange-600">Estoque</span></h2>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
                @foreach($inventorySummary as $product)
                    <div class="bg-black/40 border {{ $product->stock_quantity <= 10 ? 'border-red-900/50 animate-pulse' : 'border-gray-800' }} p-4 rounded-2xl flex flex-col items-center justify-center text-center group hover:border-orange-500/50 transition-all">
                        <span class="text-gray-500 text-[9px] font-black uppercase mb-1 tracking-widest line-clamp-1">{{ $product->name }}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-green-600">
                ...
                <h3 class="text-2xl font-black text-white italic">
                    + {{ $movimentacoes->where('type', 'entrada')->sum('quantity') }} <span class="text-xs text-gray-500 font-medium">unidades</span>
                </h3>
            </div>
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-red-600">
                ...
                <h3 class="text-2xl font-black text-white italic">
                    - {{ abs($movimentacoes->where('type', 'saida')->sum('quantity')) }} <span class="text-xs text-gray-500 font-medium">unidades</span>
                </h3>
            </div>
                        ...
                        @forelse($movimentacoes as $mov)
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="p-6">
                                    ...
                                    <span class="text-gray-500 text-[10px] font-medium leading-tight block max-w-xs">{{ $mov->description }}</span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-24 text-center">
                                    <p class="text-gray-500 text-xs font-black uppercase tracking-widest">Nenhum registro encontrado.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <style>
        ...
        input[type="date"]:focus { border: 1px solid #ea580c !important; box-shadow: 0 0 0 1px #ea580c; }
    </style>
</x-bar-layout>
