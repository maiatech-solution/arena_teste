<x-bar-layout>
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[3rem] p-8 mb-10 shadow-2xl relative overflow-hidden">
            ...
            <div class="flex items-end justify-between gap-2 h-64 px-4 border-b border-gray-800/50 pb-2">
                @php
                    $maxValor = collect($datas)->max(fn($d) => ($d['mesas'] ?? 0) + ($d['pdv'] ?? 0)) ?: 1;
                    // Array de tradução manual para garantir o PT-BR sem depender do servidor
                    $diasSemana = [
                ...
                @foreach($datas as $dia => $valores)
                    ...
                    <div class="flex-1 flex flex-col items-center group relative">
                        ...
                        {{-- Barra --}}
                        <div class="w-full max-w-[35px] rounded-t-lg transition-all duration-500 relative
                        ...
                        <div class="mt-4 flex flex-col items-center">
                            <span class="text-[9px] font-black {{ $isHoje ? 'text-orange-500' : 'text-gray-600' }}">
                                {{ $diaNum }}
                            </span>
                            <span class="text-[7px] font-bold uppercase text-gray-700">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[3rem] shadow-2xl overflow-hidden">
            <div class="p-8 border-b border-gray-800 bg-black/20 flex justify-between items-center">
                <h2 class="text-[10px] font-black text-white uppercase tracking-widest italic">Detalhamento Financeiro Diário</h2>
                <div class="flex gap-6">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[9px] text-gray-500 font-black uppercase">PDV / Balcão</span>
                    </div>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-black/40">
                            ...
                            <th class="p-8 text-[10px] font-black text-white uppercase tracking-widest text-right italic">Total do Dia</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group {{ $dataCarbon->isToday() ? 'bg-orange-600/[0.03]' : '' }}">
                                <td class="p-8">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-black border border-gray-800 flex items-center justify-center text-sm font-black text-orange-500 uppercase italic shadow-inner">
                                            {{ $dataCarbon->format('d') }}
                                        </div>
                                        <div>
                                            <span class="text-white font-black block uppercase text-sm tracking-tighter group-hover:text-orange-500 transition-colors">
                                                {{ $diasSemanaFull[$dataCarbon->format('l')] }}
                                            </span>
                                ...
                                <td class="p-8 text-right font-bold text-blue-400/80 text-xs font-mono">
                                    R$ {{ number_format($valores['mesas'] ?? 0, 2, ',', '.') }}
                                </td>
                                <td class="p-8 text-right font-bold text-orange-400/80 text-xs font-mono">
                    ...
                    <tfoot>
                        <tr class="bg-black/60 border-t-2 border-orange-600/50 shadow-[0_-10px_30px_rgba(234,88,12,0.1)]">
                            <td class="p-10 text-white font-black uppercase italic tracking-[0.2em] text-sm">Faturamento Acumulado</td>
                            <td colspan="3" class="p-10 text-right text-4xl font-black text-orange-500 italic tracking-tighter">
