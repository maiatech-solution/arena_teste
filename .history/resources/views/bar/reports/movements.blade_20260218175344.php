<x-bar-layout>
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
            ...
            <form action="{{ route('bar.reports.movements') }}" method="GET"
                ...
                @if (request('type') || request('date') || request('search'))
                    <button type="submit" class="hidden"></button> {{-- Trigger para o Enter no campo search --}}
        ...
        {{-- GRID DE RESUMO DE MOVIMENTAÇÃO (FILTRADO) --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-green-600">
                ...
                <h3 class="text-2xl font-black text-white italic">
                    + {{ $movimentacoes->where('type', 'entrada')->sum('quantity') }} <span
                        class="text-xs text-gray-500 font-medium">unidades</span>
                </h3>
            </div>
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-red-600">
                ...
                <h3 class="text-2xl font-black text-white italic">
                    - {{ abs($movimentacoes->where('type', 'saida')->sum('quantity')) }} <span
                        class="text-xs text-gray-500 font-medium">unidades</span>
                </h3>
            </div>
            <div class="bg-orange-600 p-6 rounded-[2rem] shadow-lg shadow-orange-600/20">
                ...
                <h3 class="text-2xl font-black text-white italic">CONFERIDO ✅</h3>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="p-6">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 bg-black rounded-xl flex flex-col items-center justify-center border border-gray-800 group-hover:border-orange-500 transition-colors relative">
                                            <span
                                                class="text-[8px] text-gray-600 font-black uppercase leading-none italic">ID</span>
                                            <span
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if ($mov->product->stock_quantity <= 10)
                                                    <span
                                                        class="text-[8px] font-black text-red-500 uppercase px-1.5 py-0.5 bg-red-500/10 border border-red-500/20 rounded-md">
                                ...
                                <td class="p-6 text-center">
                                    <span
                                        class="px-3 py-1 {{ $mov->type == 'entrada' ? 'bg-green-950 text-green-500 border-green-500/30' : 'bg-red-950 text-red-500 border-red-500/30' }} text-[9px] font-black uppercase rounded-full border">
                                        ● {{ $mov->type }}
                                    </span>
                                </td>
                                ...
                                <td class="p-6">
                                    ...
                                    <span
                                        class="text-gray-500 text-[10px] font-medium leading-tight block max-w-xs">{{ $mov->description }}</span>
                                </td>

                                <td class="p-6 text-right">
                                    <button
                                        class="p-2 text-gray-600 hover:text-white opacity-0 group-hover:opacity-100 transition-all transform hover:scale-125">
                                        🔍
                                    </button>
                                </td>
                            </tr>
        ...
        {{-- RESUMO DE INVENTÁRIO ATUAL (POSIÇÃO REAL) --}}
        <div class="mb-10 bg-gray-900 border border-gray-800 rounded-[2.5rem] p-8 shadow-2xl">
            <div class="flex items-center gap-3 mb-6">
                <span class="text-xl">📊</span>
                <h2 class="text-lg font-black text-white uppercase italic tracking-tighter">Posição Atual de <span
                        class="text-orange-600">Estoque</span></h2>
            </div>

            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
                @foreach ($inventorySummary as $product)
                    <div
                        class="bg-black/40 border {{ $product->stock_quantity <= 10 ? 'border-red-900/50 animate-pulse' : 'border-gray-800' }} p-4 rounded-2xl flex flex-col items-center justify-center text-center group hover:border-orange-500/50 transition-all">
                        <span
                            class="text-gray-500 text-[9px] font-black uppercase mb-1 tracking-widest line-clamp-1">{{ $product->name }}</span>

                        <div class="flex items-baseline gap-1">
                            <span
                                class="text-xl font-black {{ $product->stock_quantity <= 10 ? 'text-red-500' : 'text-white' }}">
                                {{ $product->stock_quantity }}
                            </span>
                            <span class="text-[8px] text-gray-600 font-bold uppercase">un</span>
                        </div>
