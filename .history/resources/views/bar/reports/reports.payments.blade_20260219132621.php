    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h1 class="text-4xl font-black text-white uppercase tracking-tighter italic">Meios de <span class="text-orange-600">Pagamento</span></h1>
                    <p class="text-gray-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Consolidado de Mesas e PDV</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div class="bg-orange-600 p-8 rounded-[2.5rem] shadow-xl shadow-orange-600/20 col-span-1 md:col-span-2">
                <p class="text-[10px] font-black text-orange-200 uppercase tracking-widest mb-1">Faturamento Bruto Total</p>
            ...
            <div class="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 flex flex-col justify-center">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Total de Vendas</p>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-black/40">
                        <th class="p-8 text-[10px] font-black text-gray-500 uppercase tracking-widest">Método</th>
                        <th class="p-8 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Transações</th>
                            ...
                            <td class="p-8">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 bg-black rounded-2xl flex items-center justify-center border border-gray-800 group-hover:border-orange-500 transition-all text-xl">
                                        {{ str_contains(strtolower($p->payment_method), 'pix') ? '💎' : (str_contains(strtolower($p->payment_method), 'dinheiro') ? '💵' : '💳') }}
                                    </div>
                                    <span class="text-white font-black text-sm uppercase">{{ $p->payment_method ?: 'Não Informado' }}</span>
                                </div>
                            </td>
                            <td class="p-8 text-center text-gray-400 font-black font-mono text-lg">{{ $p->qtd }}</td>
                            <td class="p-8 text-right">
                                <span class="text-2xl font-black text-orange-500 italic">R$ {{ number_format($p->total, 2, ',', '.') }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="3" class="p-32 text-center text-gray-600 font-black uppercase tracking-widest">Sem movimentação</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
