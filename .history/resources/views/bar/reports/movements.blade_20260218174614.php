<x-bar-layout>
    ...
    @forelse($movimentacoes as $mov)
        <tr class="hover:bg-white/[0.02] transition-colors group">
            ...
            {{-- PRODUTO COM ALERTA DE ESTOQUE --}}
            <td class="p-6">
                <div class="flex items-center gap-3">
                   <div class="w-10 h-10 bg-black rounded-xl flex items-center justify-center border border-gray-800 group-hover:border-orange-500 transition-colors relative text-xl">
    {{-- Lógica Inteligente de Ícones por Categoria --}}
    @php
        $categoria = strtolower($mov->product->category->name ?? '');
        $icon = '📦'; // Ícone padrão caso não encontre correspondência

        if (str_contains($categoria, 'cerveja') || str_contains($categoria, 'chopp')) {
            $icon = '🍺';
        } elseif (str_contains($categoria, 'refri') || str_contains($categoria, 'suco') || str_contains($categoria, 'refrigerante')) {
            $icon = '🥤';
        } elseif (str_contains($categoria, 'porção') || str_contains($categoria, 'comida') || str_contains($categoria, 'petisco')) {
            $icon = '🍽️';
        } elseif (str_contains($categoria, 'dose') || str_contains($categoria, 'destilado') || str_contains($categoria, 'whisky')) {
            $icon = '🥃';
        } elseif (str_contains($categoria, 'vinho')) {
            $icon = '🍷';
        } elseif (str_contains($categoria, 'água')) {
            $icon = '💧';
        } elseif (str_contains($categoria, 'doce') || str_contains($categoria, 'sobremesa')) {
            $icon = '🍬';
        }
    @endphp
    ...
    {{-- 🔥 Mantém o seu Alerta Visual de Estoque Crítico (Pisca-Pisca) --}}
    ...
    @endif
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-gray-600 text-[9px] block uppercase font-bold">
                                {{ $mov->product->category->name ?? 'Sem Categoria' }}
                            </span>

                            {{-- Badge de Alerta no texto --}}
                            @if($mov->product->stock_quantity <= 10)
                                <span class="text-[8px] font-black text-red-500 uppercase px-1.5 py-0.5 bg-red-500/10 border border-red-500/20 rounded-md animate-pulse">
                                    Estoque Crítico: {{ $mov->product->stock_quantity }}
                                </span>
                            @endif
                        </div>
                    </div>
            ...
            <td class="p-6 text-center">
                @if ($mov->type == 'entrada')
                    <span class="px-3 py-1 bg-green-950 text-green-500 text-[9px] font-black uppercase rounded-full border border-green-500/30">
                        ● Entrada
                    </span>
                @else
                    <span class="px-3 py-1 bg-red-950 text-red-500 text-[9px] font-black uppercase rounded-full border border-red-500/30">
                        ● Saída
                    </span>
                @endif
            </td>
            ...
            <td class="p-6">
                <span class="text-gray-300 font-black text-[10px] uppercase block tracking-tighter">
                    {{ $mov->user->name ?? 'Sistema' }}
                </span>
                <span class="text-gray-500 text-[10px] font-medium leading-tight block max-w-xs">
                    {{ $mov->description }}
                </span>
            </td>
            ...
            <td class="p-6 text-right">
                <button class="p-2 text-gray-600 hover:text-white opacity-0 group-hover:opacity-100 transition-all transform hover:scale-125" title="Ver detalhes">
                    🔍
                </button>
            </td>
        </tr>
        ...
        <tr>
            <td colspan="6" class="p-24 text-center">
                <div class="inline-block p-8 bg-gray-950 rounded-full mb-4">
                    <span class="text-6xl grayscale opacity-20 italic">🔎</span>
                </div>
