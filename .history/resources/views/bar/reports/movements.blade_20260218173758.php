    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    ...
                    <p class="text-gray-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Auditoria detalhada de entradas e saídas</p>
                </div>
            </div>

            {{-- FILTROS RÁPIDOS --}}
            <form action="{{ route('bar.reports.movements') }}" method="GET" class="flex flex-wrap items-center gap-3 bg-gray-900/50 p-2 rounded-[2rem] border border-gray-800/50">
                <div class="relative">
                ...
                <input type="date" name="date" value="{{ request('date') }}" onchange="this.form.submit()"
                       class="bg-black border-none rounded-2xl text-white text-[10px] font-black uppercase px-6 py-3 outline-none focus:ring-1 focus:ring-orange-500">

                <a href="{{ route('bar.reports.movements') }}" class="p-3 text-gray-500 hover:text-white transition-colors" title="Limpar Filtros">
                    ✕
                </a>
            </form>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-green-600">
                ...
                <h3 class="text-2xl font-black text-white italic">+ {{ $movimentacoes->where('type', 'entrada')->sum('quantity') }} <span class="text-xs text-gray-500 font-medium">unidades</span></h3>
            </div>
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-red-600">
                ...
                <h3 class="text-2xl font-black text-white italic">- {{ $movimentacoes->where('type', 'saida')->sum('quantity') }} <span class="text-xs text-gray-500 font-medium">unidades</span></h3>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-800/50">
                        @forelse($movimentacoes as $mov)
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="p-6">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-gray-600 text-[9px] block uppercase font-bold">{{ $mov->product->category->name ?? 'Sem Categoria' }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="p-6">
                                    <span class="text-gray-300 font-black text-[10px] uppercase block tracking-tighter">{{ $mov->user->name ?? 'Sistema' }}</span>
                                    <span class="text-gray-500 text-[10px] font-medium">{{ $mov->description }}</span>
                                </td>
                                <td class="p-6 text-right">
                                    <button class="p-2 text-gray-600 hover:text-white opacity-0 group-hover:opacity-100 transition-all">
                                        🔍
                                    </button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="p-24 text-center">
                                    ...
                                    <p class="text-gray-500 text-xs font-black uppercase tracking-widest">Nenhum registro encontrado para este período.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
