<x-bar-layout>
    <div class="py-12 bg-black min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 px-4 sm:px-0">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 font-bold uppercase text-xs tracking-widest mt-1 italic">Gestão de Colaboradores e Acessos Administrativos</p>
                    </div>
                </div>
            ...
            <div class="bg-gray-900 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] sm:rounded-[2.5rem] p-8 border border-gray-800">
                ...
                @if (session('success'))
                <div class="bg-green-500/10 border-l-4 border-green-500 text-green-500 p-4 mb-6 rounded-xl shadow-lg animate-bounce-short" role="alert">
                ...
                @if (session('error'))
                <div class="bg-red-500/10 border-l-4 border-red-500 text-red-500 p-4 mb-6 rounded-xl shadow-lg" role="alert">
                    <div class="flex items-center gap-3">
                ...
                <div class="mb-8">
                    <form method="GET" action="{{ route('bar.users.index') }}" class="flex flex-col md:flex-row items-center gap-3">
                        <div class="relative flex-grow w-full">
                            <input type="text" name="search" placeholder="Buscar colaborador por nome, email ou contato..."
                                value="{{ $search ?? '' }}"
                        ...
                        <div class="flex gap-2 w-full md:w-auto">
                            <button type="submit" class="flex-grow md:flex-none px-8 py-4 bg-orange-600 text-white font-black rounded-2xl hover:bg-orange-500 transition-all uppercase text-xs tracking-widest shadow-lg shadow-orange-600/20">
                                Buscar
                            </button>
                    ...
                    <table class="min-w-full divide-y divide-gray-800">
                        <thead class="bg-gray-800/50">
                            <tr>
                                ...
                                <th class="px-8 py-5 text-left text-[10px] font-black text-orange-500 uppercase tracking-widest">Acesso / Unidade</th>
                        ...
                        <tbody class="divide-y divide-gray-800">
                            ...
                            <tr class="hover:bg-orange-600/[0.03] transition-colors group">
                                <td class="px-8 py-6 whitespace-nowrap">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center text-white font-black border border-gray-600 group-hover:border-orange-500/50 transition-all duration-300">
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                @if ($user->role === 'admin')
                                                    <span class="px-2 py-0.5 rounded text-[8px] font-black bg-red-500/10 text-red-500 uppercase border border-red-500/20">Admin Full</span>
                                                @else
                                                    <span class="px-2 py-0.5 rounded text-[8px] font-black bg-indigo-500/10 text-indigo-500 uppercase border border-indigo-500/20">Gestor Staff</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td class="px-8 py-6 whitespace-nowrap text-xs text-gray-400 font-mono tracking-tighter">
                                    ...
                                    <div class="flex justify-center items-center gap-3">
                                        ...
                                        <form action="{{ route('bar.users.destroy', $user) }}" method="POST" onsubmit="return false;" class="inline">
                                            ...
                                            <button type="button" onclick="showCustomConfirmation(this)"
                                                ...
                                                <svg class="w-4 h-4 group-hover/del:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr>
                                <td colspan="5" class="px-8 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 font-black uppercase tracking-widest text-xs italic">Nenhum integrante da equipe encontrado.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
    ...
    <div id="confirmation-modal" class="fixed inset-0 bg-black/95 backdrop-blur-md z-[100] hidden transition-all duration-300 opacity-0 flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div id="modal-content" class="bg-gray-900 border border-gray-800 rounded-[2.5rem] shadow-2xl p-10 max-w-sm w-full transform transition-all duration-300 scale-95 opacity-0 text-center">
            <div class="h-20 w-20 rounded-full bg-red-500/10 flex items-center justify-center mx-auto mb-6 border border-red-500/20">
                <svg class="h-10 w-10 text-red-500 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <p class="text-gray-400 font-bold text-sm mb-8 px-4 leading-relaxed uppercase tracking-widest">
                Você está prestes a remover <br><span id="username-placeholder" class="text-orange-500"></span> <br>da equipe do bar.
            </p>

            <div class="flex flex-col gap-3">
                <button type="button" id="confirm-delete-btn" class="w-full bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-2xl transition-all uppercase text-xs tracking-widest shadow-lg shadow-red-600/20 active:scale-95">Sim, Excluir Agora</button>
                <button type="button" onclick="closeCustomConfirmation()" class="w-full bg-gray-800 hover:bg-gray-700 text-gray-300 font-black py-4 rounded-2xl transition-all uppercase text-xs tracking-widest border border-gray-700 active:scale-95">Manter Colaborador</button>
            </div>
        </div>
    </div>
