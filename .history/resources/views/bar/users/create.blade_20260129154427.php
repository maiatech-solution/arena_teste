                ...
                <form method="POST" action="{{ route('bar.users.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            {{-- Normalização do Role para garantir a comparação --}}
                            @php $userRole = strtolower(trim(Auth::user()->role)); @endphp

                            @if($userRole === 'admin')
                                {{-- Apenas para quem é EXATAMENTE admin no banco --}}
                            ...
                            @else
                                ...
                                <div class="w-full bg-gray-800/50 border border-gray-700 rounded-2xl p-4 text-gray-400 font-bold flex justify-between items-center select-none shadow-inner">
                                    <span class="uppercase text-xs tracking-widest text-gray-500">Gestor de Bar (Staff)</span>
                                    <span class="text-[9px] bg-orange-600/20 text-orange-500 border border-orange-500/20 px-2 py-1 rounded-lg font-black tracking-widest">PADRÃO PARA EQUIPE</span>
                                </div>
                    ...
                    <div class="flex gap-4 pt-6">
                        <button type="submit"
                            class="flex-grow bg-orange-600 hover:bg-orange-700 text-white font-black py-4 rounded-2xl transition-all uppercase text-xs tracking-widest shadow-lg shadow-orange-600/20 active:scale-95">
                            Finalizar Cadastro
                        </button>
                        <a href="{{ route('bar.users.index') }}"
                            class="px-8 bg-gray-800 hover:bg-gray-700 text-gray-400 font-black py-4 rounded-2xl transition-all uppercase text-[10px] tracking-widest border border-gray-700 text-center flex items-center active:scale-95">
                            Cancelar
                        </a>
                    </div>
                </form>
