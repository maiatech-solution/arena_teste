        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-gray-900 border border-gray-800 p-6 rounded-3xl shadow-xl">
                ...
                <h3 class="text-2xl font-black text-white">
                    R$ {{ number_format($products->sum(fn($p) => $p->stock_quantity * $p->purchase_price), 2, ',', '.') }}
                </h3>
        ...
        <div class="bg-gray-900 p-4 rounded-2xl mb-6 border border-gray-800 shadow-lg">
            <form action="{{ route('bar.products.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                <input type="text" name="search" value="{{ request('search') }}"
                       ...
                       autofocus>
                <button type="submit" class="px-8 py-3 bg-orange-600 hover:bg-orange-500 text-white font-black rounded-xl transition uppercase text-xs">
                    FILTRAR
                </button>
            </form>
        </div>
                        ...
                        <td class="p-4 text-right">
                            <div class="flex justify-end gap-2">
                                ...
                                <form action="{{ route('bar.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Deseja realmente desativar este produto?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 bg-gray-800 hover:bg-red-600 text-white rounded-lg transition-all active:scale-90" title="Desativar">
                                        🗑️
                                    </button>
                                </form>
                            </div>
