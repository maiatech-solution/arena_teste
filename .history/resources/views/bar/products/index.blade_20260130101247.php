<x-bar-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {{-- Status Crítico - Agora já filtrado pelo Controller para ignorar quem não tem controle --}}
            <div class="bg-gray-900 border {{ $lowStockProducts->count() > 0 ? 'border-red-500/50 ring-1 ring-red-500/20 shadow-[0_0_20px_rgba(239,68,68,0.1)]' : 'border-gray-800' }} p-6 rounded-3xl transition-all duration-500">
                ...
                <p class="text-gray-500 text-xs font-bold uppercase mt-1 tracking-tighter">Itens que exigem compra</p>
            </div>

            <div class="bg-gray-900 border border-gray-800 p-6 rounded-3xl shadow-xl hover:border-green-500/30 transition-all">
                <div class="flex items-center justify-between mb-4">
                ...
                <p class="text-gray-500 text-xs font-bold uppercase mt-1 tracking-tighter">Custo total em mercadoria</p>
            </div>

            <div class="bg-gray-900 border border-gray-800 p-6 rounded-3xl shadow-xl hover:border-blue-500/30 transition-all">
                <div class="flex items-center justify-between mb-4">
                    <span class="p-3 rounded-2xl bg-blue-500/20 text-blue-500">📦</span>
                    <span class="text-[10px] font-black text-gray-500 uppercase tracking-widest">Mix de Produtos</span>
                </div>
                <h3 class="text-3xl font-black text-white">{{ $products->total() }}</h3>
                <p class="text-gray-500 text-xs font-bold uppercase mt-1 tracking-tighter">Itens ativos e inativos</p>
            </div>
        </div>
            ...
            <form action="{{ route('bar.products.index') }}" method="GET" class="space-y-6">
                <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <select name="bar_category_id" onchange="this.form.submit()"
                        ...
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}"
                                {{ request('bar_category_id') == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-gray-800">
                    ...
                    <a href="{{ route('bar.products.index', array_merge($queryParams, ['filter_status' => 'all'])) }}"
                        class="{{ $btnBase }} {{ $currentStatus == 'all' ? 'bg-orange-600 text-white border-orange-500 shadow-lg shadow-orange-600/20' : 'bg-gray-800 text-gray-500 border-gray-700 hover:bg-gray-700' }}">
                        Todos
                    </a>

                    <a href="{{ route('bar.products.index', array_merge($queryParams, ['filter_status' => 'active'])) }}"
                        class="{{ $btnBase }} {{ $currentStatus == 'active' ? 'bg-green-600 text-white border-green-500 shadow-lg shadow-green-600/20' : 'bg-gray-800 text-gray-500 border-gray-700 hover:bg-gray-700' }}">
                        Ativos
                    </a>

                    <a href="{{ route('bar.products.index', array_merge($queryParams, ['filter_status' => 'inactive'])) }}"
                        class="{{ $btnBase }} {{ $currentStatus == 'inactive' ? 'bg-red-600 text-white border-red-500 shadow-lg shadow-red-600/20' : 'bg-gray-800 text-gray-500 border-gray-700 hover:bg-gray-700' }}">
                        Inativos
                    </a>

                    <a href="{{ route('bar.products.index', array_merge($queryParams, ['filter_status' => 'low_stock'])) }}"
                        class="{{ $btnBase }} {{ $currentStatus == 'low_stock' ? 'bg-yellow-600 text-white border-yellow-500 animate-pulse' : 'bg-gray-800 text-gray-500 border-gray-700 hover:bg-gray-700' }}">
                        ⚠️ Estoque Baixo
                    </a>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-800/50 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">
                        ...
                        <th class="p-6 text-center">Gestão</th> {{-- NOVA COLUNA --}}
                ...
                <tbody class="divide-y divide-gray-800/50">
                    @forelse($products as $product)
                        <tr class="hover:bg-gray-800/30 transition-all duration-300 {{ !$product->is_active ? 'opacity-40 grayscale' : '' }}">
                            ...
                            {{-- COLUNA GESTÃO: Avisa se permite negativo --}}
                            <td class="p-6 text-center">
                            ...
                            <td class="p-6 text-center">
                                @if($product->manage_stock)
                                    <span class="px-4 py-1.5 rounded-xl text-[11px] font-black uppercase border
                                        {{ $product->stock_quantity <= $product->min_stock ? 'bg-red-900/40 text-red-500 border-red-500/30 animate-pulse' : 'bg-gray-950 text-green-500 border-green-500/20' }}">
                                        {{ $product->stock_quantity }} UNID.
                                    </span>
                                @else
                                    <span class="px-4 py-1.5 rounded-xl text-[11px] font-black uppercase bg-gray-950 text-gray-400 border border-gray-800">
                                        {{ $product->stock_quantity }} UNID.
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="p-6 text-right">
                                <div class="flex justify-end gap-3">
                                    {{-- Botão Perda só aparece se o controle estiver ligado --}}
                                    @if($product->manage_stock)
                                        <button type="button"
                                            onclick="openLossModal({{ $product->id }}, '{{ $product->name }}')"
                                            class="w-10 h-10 flex items-center justify-center bg-gray-800 hover:bg-red-950 text-red-500 rounded-xl transition-all border border-red-500/10 {{ !$product->is_active ? 'opacity-20 cursor-not-allowed' : '' }}"
                                    ...
                                    <form id="form-status-{{ $product->id }}"
                                        ...
                                        <button type="button"
                                            ...
                                            title="{{ $product->is_active ? 'Inativar' : 'Reativar' }}">
                                            {!! $product->is_active ? '🗑️' : '✅' !!}
                                        </button>
                                    </form>
                                </div>
