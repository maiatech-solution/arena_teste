<x-bar-layout>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-800">
                    ...
                    @empty
                        <tr>
                            <td colspan="6"
                                class="p-12 text-center text-gray-500 font-medium italic uppercase text-xs">Nenhum
                                produto cadastrado ou encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="lossModal"
        ...
        <div class="bg-gray-900 border border-gray-800 p-8 rounded-3xl w-full max-w-md shadow-2xl">
            <div class="mb-6">
                <h3
                    class="text-white font-black uppercase text-lg leading-tight tracking-tighter flex items-center gap-2">
            ...
            <form action="{{ route('bar.products.record_loss') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-[10px] font-black text-gray-500 uppercase mb-2 tracking-widest">Qtd.
                            Perdida</label>
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-gray-500 uppercase mb-2 tracking-widest">Motivo
                            / Descrição</label>
                        <input type="text" name="description" required
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white p-3 focus:border-red-500 outline-none transition-colors"
                            placeholder="Ex: Garrafa quebrou ao repor a prateleira">
                        <p class="text-[9px] text-gray-600 mt-1 italic uppercase font-bold tracking-tighter">Escreva
                            detalhadamente o que houve.</p>
                    </div>
                </div>
                ...
                <div class="flex gap-3 mt-8">
                    ...
                    <button type="submit"
                        class="flex-1 py-4 bg-red-600 text-white rounded-2xl font-black text-xs uppercase shadow-lg shadow-red-600/20 active:scale-95 transition-all">Confirmar
                        Baixa</button>
                </div>
            </form>
        </div>
    ...
    <script>
        /**
         * Lógica para Mudança de Status (Ativar/Desativar) com Motivo
         */
        function confirmStatusChange(id, isActive) {
            const action = isActive ? 'DESATIVAR' : 'REATIVAR';

            // Abre o prompt nativo do navegador para capturar o motivo
            const reason = prompt(
                `Deseja realmente ${action} este produto?\n\nInforme o motivo obrigatório para o histórico de auditoria:`
                );

            // Verifica se o usuário preencheu algo (não nulo e não apenas espaços)
            if (reason !== null && reason.trim() !== "") {
                ...
                alert("Ação cancelada. É obrigatório informar um motivo para registrar no histórico de movimentação.");
            }
        }

        /**
         * Lógica para o Modal de Registro de Perda (⚠️)
         */
        function openLossModal(id, name) {
            ...
            // Foca automaticamente no campo de quantidade após 100ms
            setTimeout(() => {
                const qtyInput = document.querySelector('#lossModal input[name="quantity"]');
        ...
        function closeLossModal() {
            ...
            document.querySelector('#lossModal input[name="quantity"]').value = '';
            document.querySelector('#lossModal input[name="description"]').value = '';
        }
        ...
        document.addEventListener('keydown', function(event) {
            // Fecha o modal de perda ao apertar ESC
            if (event.key === "Escape") {
                closeLossModal();
            }
        });
