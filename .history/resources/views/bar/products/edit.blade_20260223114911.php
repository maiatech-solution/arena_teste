<x-bar-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
         x-data="{
            ...
            comboItems: {{ $product->compositions->map(fn($c) => ['id' => $c->id, 'child_id' => $c->child_id, 'name' => $c->childProduct->name, 'quantity' => $c->quantity])->toJson() }}
         }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2 bg-orange-600/5 border border-orange-600/20 p-6 rounded-2xl mb-4" x-show="isCombo" x-transition>
                        <div class="flex items-center justify-between mb-4 border-b border-orange-600/10 pb-4">
                            <div>
                                <h3 class="text-orange-500 font-black text-xs uppercase italic tracking-widest">📋 Itens da Composição</h3>
                                <p class="text-[9px] text-gray-500 uppercase font-bold">Gerencie os itens que saem do estoque neste combo</p>
                            </div>
                            <button type="button" @click="comboItems.push({id: Date.now(), child_id: '', name: '', quantity: 1})"
                            ...
                            <template x-for="(item, index) in comboItems" :key="item.id">
                                <div class="flex gap-3 items-center animate-in fade-in slide-in-from-top-1 bg-black/20 p-3 rounded-xl border border-gray-800">
                                    ...
                                    <div class="w-24">
                                        ...
                                        <input type="number" :name="`combo_items[${index}][quantity]`" x-model="item.quantity" min="1" required
                                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white text-xs p-2.5 text-center focus:border-orange-500">
                                    </div>
                                    <button type="button" @click="comboItems = comboItems.filter(i => i.id !== item.id)"
                    ...
                    <div x-show="!isCombo">
                        ...
                        <input type="number" name="content_quantity"
                            value="{{ old('content_quantity', $product->content_quantity ?? 1) }}" min="1" :required="!isCombo"
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                    </div>
