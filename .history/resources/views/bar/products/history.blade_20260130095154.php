<x-bar-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {{-- BARRA DE FILTROS --}}
        <div class="bg-gray-900/50 border border-gray-800 p-4 rounded-3xl mb-6 shadow-lg">
            <form action="{{ route('bar.products.history') }}" method="GET" class="flex flex-col md:flex-row gap-4">

                <div class="flex-1">
                    <label class="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-2 mb-1 block">Filtrar por Tipo</label>
                    <select name="type" onchange="this.form.submit()"
                        class="w-full bg-gray-950 border-gray-800 rounded-xl text-white text-xs font-bold focus:ring-orange-500 focus:border-orange-500 transition-all">
                        <option value="">🔄 Todas as movimentações</option>
                        <option value="entrada" {{ request('type') == 'entrada' ? 'selected' : '' }}>🟢 Apenas Entradas</option>
                        <option value="saida" {{ request('type') == 'saida' ? 'selected' : '' }}>🔴 Apenas Saídas</option>
                    </select>
                </div>

                <div class="flex-1">
                    <label class="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-2 mb-1 block">Filtrar por Mês</label>
                ...
                <div class="flex items-end">
                    <a href="{{ route('bar.products.history') }}"
                       class="bg-gray-800 hover:bg-red-900/40 text-gray-400 hover:text-red-500 p-2.5 rounded-xl transition-all border border-gray-700"
                    ...
                    <tr class="hover:bg-gray-950/50 transition">
                        ...
                        <td class="p-4 text-right">
                            <div class="flex flex-col items-end">
                                ...
                                <span class="text-[9px] text-gray-500 uppercase tracking-tighter italic">Responsável</span>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="p-20 text-center">
                            <div class="flex flex-col items-center text-gray-600">
                                ...
                                <p class="italic font-bold uppercase text-xs tracking-widest">Nenhuma movimentação encontrada para os filtros aplicados.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <style>
        ...
        nav[role="navigation"] svg { width: 1.25rem; display: inline; }
        ...
        .relative.z-0.inline-flex a, .relative.z-0.inline-flex span {
            color: #9ca3af;
            border-color: #374151;
        }
