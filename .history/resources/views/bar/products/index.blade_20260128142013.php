<x-bar-layout>
                            ...
                            <td class="p-4 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('bar.products.destroy', $product->id) }}" method="POST"
                                        onsubmit="return confirm('Deseja realmente {{ $product->is_active ? 'DESATIVAR' : 'REATIVAR' }} este produto?')">
                                        @csrf
                                        @method('DELETE')

                                        @if ($product->is_active)
                                            <button type="submit"
                                                class="p-2 bg-gray-800 hover:bg-red-600 text-red-500 hover:text-white rounded-lg transition-all active:scale-90 border border-red-500/20"
                                                title="Desativar">
                                                🗑️
                                            </button>
                                        @else
                                            <button type="submit"
                                                class="p-2 bg-green-600 hover:bg-green-500 text-white rounded-lg transition-all active:scale-90 shadow-lg shadow-green-600/20"
                                                title="Reativar">
                                                ✅
                                            </button>
                                        @endif
    ...
    <script>
        function openLossModal(id, name) {
            ...
            setTimeout(() => {
                document.querySelector('#lossModal input[name="quantity"]').focus();
            }, 100);
        }
