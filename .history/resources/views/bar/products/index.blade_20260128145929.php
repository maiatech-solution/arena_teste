<x-bar-layout>
            ...
            <form action="{{ route('bar.products.index') }}" method="GET" class="space-y-6">
                ...
                <div class="flex flex-wrap items-center gap-2 pt-4 border-t border-gray-800">
                    ...
                    @php
                        ...
                        // Criamos uma base de parâmetros para os links não perderem a busca e a categoria atual
                    ...
                    @if (request()->anyFilled(['search', 'bar_category_id', 'filter_status']))
                        <a href="{{ route('bar.products.index') }}"
                            class="ml-auto text-[10px] font-black text-orange-500 hover:underline uppercase tracking-tighter">
                            Limpar Filtros
                        </a>
                    @endif
                </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-800">
                    ...
                    @empty
                        <tr>
                            <td colspan="6"
                                class="p-12 text-center text-gray-500 font-medium italic uppercase text-xs">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="number" name="quantity" required min="1"
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white p-3 focus:border-red-500 outline-none">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-gray-500 uppercase mb-2">Motivo /
                            Descrição</label>
                        <input type="text" name="description" required
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white p-3 focus:border-red-500 outline-none"
                            placeholder="Ex: Garrafa quebrou">
                    </div>
                </div>
                <div class="flex gap-3 mt-8">
                    <button type="button" onclick="closeLossModal()"
                        class="flex-1 py-3 text-gray-500 font-bold text-xs uppercase">Cancelar</button>
                    <button type="submit"
                        class="flex-1 py-4 bg-red-600 text-white rounded-2xl font-black text-xs uppercase shadow-lg shadow-red-600/20">Confirmar
                        Baixa</button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        document.addEventListener('keydown', (e) => {
            if (event.key === "Escape") closeLossModal();
        });
    </script>
</x-bar-layout>
