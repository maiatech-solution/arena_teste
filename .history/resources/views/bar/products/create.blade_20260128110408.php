        ...
        <div class="bg-gray-900 rounded-3xl border border-gray-800 p-8 shadow-2xl">
            <form action="{{ route('bar.products.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Categoria</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Estoque Inicial</label>
                        <input type="number" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" required
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Estoque Mínimo (Alerta)</label>
                        <input type="number" name="min_stock" value="{{ old('min_stock', 5) }}" required
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                        <p class="text-[10px] text-gray-500 mt-1 italic">O sistema avisará quando o estoque baixar deste valor.</p>
                    </div>
                </div>
