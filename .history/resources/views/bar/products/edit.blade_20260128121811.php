        ...
        <div class="bg-gray-900 rounded-3xl border border-gray-800 p-8 shadow-2xl">
            ...
            <form action="{{ route('bar.products.update', ['product' => $product->id]) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-1">
                        ...
                        <select name="bar_category_id" id="category_select" required
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('bar_category_id', $product->bar_category_id) == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Código de Barras</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Estoque Atual</label>
                        <input type="number" name="stock_quantity" value="{{ old('stock_quantity', $product->stock_quantity) }}" required
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Estoque Mínimo</label>
                        <input type="number" name="min_stock" value="{{ old('min_stock', $product->min_stock) }}" required
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Status do Produto</label>
                        <select name="is_active"
                            class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                            <option value="1" {{ old('is_active', $product->is_active) == 1 ? 'selected' : '' }}>🟢 ATIVO (Disponível)</option>
                            <option value="0" {{ old('is_active', $product->is_active) == 0 ? 'selected' : '' }}>🔴 INATIVO (Ocultar)</option>
                        </select>
                    </div>
                </div>
    ...
    <div id="catModal" class="hidden fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-gray-900 border border-gray-800 p-6 rounded-3xl w-full max-w-sm shadow-2xl">
            ...
            <div class="flex gap-3">
                <button type="button" onclick="closeCategoryModal()" class="flex-1 py-2 text-gray-500 font-bold text-xs uppercase transition-colors hover:text-white">Cancelar</button>
                <button type="button" onclick="saveCategory()" class="flex-1 py-3 bg-orange-600 text-white rounded-xl font-black text-xs uppercase shadow-lg shadow-orange-600/20 active:scale-95 transition-transform">Salvar</button>
            </div>
        </div>
    </div>
