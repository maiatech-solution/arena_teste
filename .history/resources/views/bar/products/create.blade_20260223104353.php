            ...
            <form action="{{ route('bar.products.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-1 flex items-end pb-1">
                        <div class="w-full bg-gray-950 border border-gray-800 p-2.5 rounded-xl flex items-center justify-between">
                            <div class="flex flex-col">
                                <span class="text-[10px] font-black text-orange-500 uppercase tracking-widest ml-2">Produto tipo Combo?</span>
                                <span class="text-[8px] text-gray-500 ml-2 uppercase font-bold italic">Vende um grupo de itens</span>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_combo" value="1" class="sr-only peer" x-model="isCombo">
                    ...
                    <div class="md:col-span-2 bg-orange-600/5 border border-orange-600/20 p-6 rounded-2xl mb-4" x-show="isCombo" x-transition>
                        ...
                        <div class="space-y-3">
                            ...
                            <div x-show="comboItems.length === 0" class="text-center py-6 border-2 border-dashed border-gray-800/50 rounded-xl text-gray-600 text-[10px] font-bold uppercase tracking-widest">
                                ...
                                ⚠️ Obrigatório ao menos 1 item para salvar como combo
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- GESTÃO DE INVENTÁRIO (SÓ EXIBE SE NÃO FOR COMBO) --}}
                    <div class="md:col-span-2 bg-gray-950/40 p-5 rounded-2xl border border-gray-800/50 my-2" x-show="!isCombo">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div class="max-w-md">
                                ...
                                <p class="text-[10px] text-gray-500 font-bold uppercase leading-tight">
                                    Deseja que o sistema controle o saldo deste produto e emita alertas de estoque baixo?
                                </p>
                            </div>
                            <div class="flex bg-gray-900 p-1 rounded-xl border border-gray-800">
                                <label class="flex items-center cursor-pointer">
                                    <input type="radio" name="manage_stock" value="1" class="hidden peer" checked>
                    ...
                    {{-- UNIDADES E CONTEÚDO (SÓ EXIBE SE NÃO FOR COMBO) --}}
                    ...
                    {{-- ESTOQUE (SÓ EXIBE SE NÃO FOR COMBO) --}}
                    <div x-show="!isCombo" class="grid grid-cols-1 md:grid-cols-2 gap-6 md:col-span-2">
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Estoque Inicial (UN)</label>
                            <input type="number" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" :required="!isCombo"
                                class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-2 tracking-widest">Estoque Mínimo (UN)</label>
                            <input type="number" name="min_stock" value="{{ old('min_stock', 5) }}" :required="!isCombo"
                                class="w-full bg-gray-950 border-gray-800 rounded-xl text-white focus:border-orange-500 focus:ring-orange-500 p-3">
                        </div>
                    </div>
                </div>
