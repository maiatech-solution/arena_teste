...
<div class="bg-gray-900/80 backdrop-blur-xl border border-gray-800 p-6 rounded-[2rem] mb-8 shadow-2xl">
    <form action="{{ route('bar.products.history') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
        ...
        <div class="flex gap-2">
            <button type="submit" class="flex-1 bg-orange-600 hover:bg-orange-500 text-white font-black p-4 rounded-2xl transition-all shadow-lg uppercase text-[10px] tracking-widest">
                Filtrar
            </button>
            <a href="{{ route('bar.products.history') }}" class="bg-gray-800 hover:bg-red-900/40 text-gray-400 p-4 rounded-2xl border border-gray-700 transition-all" title="Limpar">
                🧹
            </a>
        </div>
    </form>
