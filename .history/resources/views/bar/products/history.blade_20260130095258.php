<x-bar-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-gray-900/80 backdrop-blur-xl border border-gray-800 p-6 rounded-[2rem] mb-8 shadow-2xl">
            <form action="{{ route('bar.products.history') }}" method="GET"
                ...
                <div class="flex-1 w-full space-y-2">
                    <label
                        class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                        <span class="text-orange-500">📑</span> Tipo de Movimentação
                    </label>
                    <div class="relative group">
                        <select name="type" onchange="this.form.submit()"
                            class="w-full bg-gray-950 border-gray-800 rounded-2xl text-white text-xs font-bold p-4 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all appearance-none cursor-pointer group-hover:border-gray-700">
                            <option value="">🔄 Todas as movimentações</option>
                            <option value="entrada" {{ request('type') == 'entrada' ? 'selected' : '' }}>🟢 Apenas
                                Entradas</option>
                            <option value="saida" {{ request('type') == 'saida' ? 'selected' : '' }}>🔴 Apenas Saídas
                            </option>
                        </select>
                        <div
                            class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-600 text-[10px]">
                            ▼
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 w-full space-y-2">
                    <label
                        class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                        <span class="text-orange-500">📅</span> Período Mensal
                    </label>
                    <div class="relative group">
                        <input type="month" name="month" value="{{ request('month') }}"
                            onchange="this.form.submit()"
                            class="w-full bg-gray-950 border-gray-800 rounded-2xl text-white text-xs font-bold p-4 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all cursor-pointer group-hover:border-gray-700 color-scheme-dark">

                        {{-- O ícone do calendário nativo será invertido via CSS abaixo --}}
                    </div>
                </div>

                <div class="flex items-center gap-2">
                    <a href="{{ route('bar.products.history') }}"
                        class="bg-gray-800 hover:bg-orange-600 text-white p-4 rounded-2xl transition-all duration-300 border border-gray-700 hover:border-orange-500 shadow-lg flex items-center justify-center group"
        ...
        <style>
            /* Força o seletor de data (pop-up) a seguir o tema escuro do sistema */
            .color-scheme-dark {
                color-scheme: dark;
            }

            /* Estiliza o ícone do calendário nativo para não ficar sumido no fundo preto */
            input[type="month"]::-webkit-calendar-picker-indicator {
                background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="15" viewBox="0 0 24 24" fill="none" stroke="%23f97316" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>');
                filter: saturate(2);
                cursor: pointer;
                padding: 5px;
            }
