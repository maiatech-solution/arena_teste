<x-app-layout>
    ...
    <script>
        ...
        // 🎯 ATUALIZADO: Adiciona signalValue como argumento
        function openCancellationModal(reservaId, method, urlBase, message, buttonText, signalValue = 0) {
            ...
            // Verifica se há sinal > 0 para gerenciar
            const isRefundable = parseFloat(signalValue) > 0;
            const signalFormatted = parseFloat(signalValue).toFixed(2).replace('.', ',');
            ...
            if (isRefundable) {
                ...
                // Reseta para "Manter" como padrão mais seguro
                document.getElementById('refund-choice-no').checked = true;
            } else {
                refundArea.classList.add('hidden');
                // Garante que o valor do sinal no display escondido seja 0,00 para a requisição
        ...
        // 🎯 ATUALIZADO: Coleta a decisão de estorno
        async function sendCancellationRequest(reservaId, method, urlBase, reason) {
            ...
            const signalValueToCheck = cleanAndConvertForApi(signalDisplay.textContent); // Usa a função de limpeza
            ...
            if (signalValueToCheck > 0 && !refundArea.classList.contains('hidden')) {
                ...
                // Se o radio for 'refund', deve estornar
                shouldRefund = refundChoice && refundChoice.value === 'refund';

                // Validação de segurança, embora o padrão seja 'keep'
                if (!refundChoice) {
            ...
            const bodyData = {
                ...
                should_refund: shouldRefund, // <--- NOVO CAMPO NO PAYLOAD
                _token: csrfToken,
                _method: method,
            };
