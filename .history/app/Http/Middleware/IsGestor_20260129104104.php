...
class IsGestor
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // --- NOVA ORDEM DE PRIORIDADE (FLUXO NEUTRO) ---

        // PRIORIDADE 1: Se a empresa não tem nome, obriga a preencher o SETUP NEUTRO
        if (!$company || empty($company->nome_fantasia)) {
            if (in_array($user->email, $masterAdmins)) {
                // Se não estiver na rota de setup, redireciona para lá
                if (!$request->is('setup-unidade*')) {
                    return redirect()->route('onboarding.setup');
                }
            } else {
                abort(403, 'O sistema está aguardando o cadastro inicial da unidade.');
            }
        }

        // PRIORIDADE 2: Se já tem nome, mas modules_active for 0, manda para os CARDS
        elseif ($company->modules_active === 0) {
            if (in_array($user->email, $masterAdmins)) {
                ...
                abort(403, 'O sistema está aguardando a ativação do módulo.');
            }
        }

        // 3. Controle de Acesso aos Módulos (Após configuração concluída)
        if ($company && $company->modules_active > 0) {
            ...
            if ($company->modules_active == 1 && $request->is('bar*')) {
                abort(403, 'O Módulo PDV System não está ativo.');
            }
            ...
            if ($company->modules_active == 2 && ($request->is('dashboard') || $request->is('admin*') || $request->is('financeiro*'))) {
                // Permite apenas a edição de dados da empresa dentro do admin
