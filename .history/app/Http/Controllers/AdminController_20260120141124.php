    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                $valorParaMover = (float) $reserva->total_paid;
                $nomeOriginal = $reserva->client_name;
                $userIdOriginal = $reserva->user_id;

                if ($valorParaMover > 0 && $action === 'transfer') {
                    ...
                    if ($proximaReserva) {
                        // 🎯 ESCREVE DIRETO NO BANCO (SEM INTERFACE DA MODEL)
                        DB::table('reservas')->where('id', $proximaReserva->id)->update([
                            'total_paid' => DB::raw("COALESCE(total_paid, 0) + $valorParaMover"),
                            'signal_value' => DB::raw("COALESCE(signal_value, 0) + $valorParaMover"),
                            'payment_status' => 'partial',
                            'user_id' => $userIdOriginal // Crucial para aparecer na lista do dia 28
                        ]);

                        // 🎯 MOVE AS TRANSAÇÕES (Mantém a data original de hoje)
                ...
                // 3. Atualiza a reserva atual para Manutenção e limpa os campos
                // Usamos Query Builder aqui também para não ter erro de visualização
                DB::table('reservas')->where('id', $id)->update([
                    ...
                    'signal_value' => 0,
                    'is_fixed' => 1,
                    'notes' => "[TRANSFERIDO R$ {$valorParaMover}] " . ($reserva->notes ?? '')
                ]);
