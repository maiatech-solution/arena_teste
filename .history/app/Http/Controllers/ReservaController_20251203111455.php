...
class ReservaController extends Controller
{
     ...
     *
     * @param Reserva $reserva 御reserva de cliente que está sendo liberada.
     * @return void
     */
    ...
    public function finalizarPagamento(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            if ($request->boolean('apply_to_series') && $reserva->is_recurrent) {
                ...
                $reservaStartTime = $reserva->start_time;

                // 🛑 DEBUG: Loga os parâmetros de busca
                Log::debug("[FinalizarPagamento Debug] Reserva ID Atual: {$reserva->id}, Master ID: {$masterId}, Data de Corte: {$reservaDate}, Start Time: {$reservaStartTime}, Preço: {$newPriceForSeries}");

                // ✅ QUERY CRÍTICA: Busca todas as reservas da série que ocorrem *após* a reserva atual (incluindo tempo no mesmo dia).
                $updatedCount = Reserva::where(function ($query) use ($masterId) {
                   ...
                   // 🛑 CRÍTICO: Filtra por data e hora, garantindo que seja estritamente no futuro.
                   ->where(function ($query) use ($reservaDate, $reservaStartTime) {
                        // Condição 1: Slots em dias futuros (data > data atual)
                        $query->whereDate('date', '>', $reservaDate)
                              // Condição 2: Slots no mesmo dia, mas em horário posterior (start_time > start_time atual)
                              // Nota: Como o seu agendamento é recorrente semanal, a Condição 1 é a principal,
                              // mas incluímos a 2 para robustez em cenários com slots múltiplos no mesmo dia.
                              ->orWhere(function ($q) use ($reservaDate, $reservaStartTime) {
                                  $q->whereDate('date', $reservaDate)
                                    ->where('start_time', '>', $reservaStartTime);
                              });
                   })
                   // ✅ MANUTENÇÃO: Exclui explicitamente o ID da reserva que acabou de ser paga para evitar redundância.
                   // A sua observação está correta: o slot pago já foi atualizado no Passo 2 e esta query é para os futuros.
                   ->where('id', '!=', $reserva->id)

                   // Filtros de tempo e status (garante que estamos no mesmo slot recorrente)
                   ->where('start_time', $reservaStartTime)
                   ->where('end_time', $reserva->end_time)
                ...
                // 🛑 DEBUG: Loga o número de atualizações
                Log::debug("[FinalizarPagamento Debug] Slots Atualizados (final): {$updatedCount}.");
