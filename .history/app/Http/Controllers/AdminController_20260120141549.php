...
class AdminController extends Controller
{
    ...
    /**
     * 🛠️ Move para MANUTENÇÃO transferindo crédito para a próxima reserva.
     */
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                $valorPagoOriginal = (float) $reserva->total_paid;
                $nomeOriginal = $reserva->client_name;
                $userIdOriginal = $reserva->user_id;

                // 1. GERAÇÃO DE BACKUP ROBUSTO
                $backupData = [
                    'name' => $nomeOriginal,
                    'status' => $reserva->status,
                    'is_fixed' => $reserva->is_fixed,
                    'user_id' => $userIdOriginal,
                    'total_paid_orig' => $valorPagoOriginal
                ];
                $backupString = "###BACKUP###" . json_encode($backupData) . "###END###";

                // 2. LÓGICA DE TRANSFERÊNCIA DE VALORES
                if ($valorPagoOriginal > 0 && $action === 'transfer') {
                    ...
                    if ($proximaReserva) {
                        // Atualiza a reserva destino via SQL Puro para garantir persistência
                        DB::table('reservas')->where('id', $proximaReserva->id)->update([
                            'total_paid' => DB::raw("total_paid + $valorPagoOriginal"),
                            'signal_value' => DB::raw("signal_value + $valorPagoOriginal"),
                            'payment_status' => 'partial',
                            'user_id' => $userIdOriginal // Garante que apareça no caixa do dia 28
                        ]);

                        // Vincula as transações financeiras à nova reserva (mantendo data original de hoje)
                        DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->update(['reserva_id' => $proximaReserva->id]);
                    }
                }

                // 3. LÓGICA DE ESTORNO (REFUND)
                elseif ($valorPagoOriginal > 0 && $action === 'refund') {
                ...
                // 4. CONVERTE A RESERVA ATUAL EM MANUTENÇÃO
                $reserva->update([
                    'status' => 'maintenance',
    ...
    {
        try {
            $reservaOriginal = Reserva::findOrFail($id);
            $valorOriginal = (float)$reservaOriginal->total_paid;
            $seriesId = $reservaOriginal->recurrent_series_id;

            \Log::info("--- INICIANDO TENTATIVA DE TRANSFERÊNCIA #{$id} ---");

            $proxima = Reserva::where('recurrent_series_id', $seriesId)
                ->where('date', '>', $reservaOriginal->date)
                ->where('status', '!=', 'cancelled')
                ->orderBy('date', 'asc')
                ->first();

            if ($proxima && $valorOriginal > 0) {
                \Log::info("Alvo encontrado: ID {$proxima->id}. Tentando atualizar...");

                // Tenta atualizar a próxima reserva
                $updateReserva = DB::table('reservas')->where('id', $proxima->id)->update([
                    'total_paid' => $valorOriginal,
                    'signal_value' => $valorOriginal,
                    'payment_status' => 'partial',
                    'user_id' => $reservaOriginal->user_id
                ]);

                // Tenta atualizar a transação financeira
                $updateTransacao = DB::table('financial_transactions')
                    ->where('reserva_id', $reservaOriginal->id)
                    ->update(['reserva_id' => $proxima->id]);

                \Log::info("Resultado Update Reserva: " . ($updateReserva ? 'SUCESSO' : 'FALHA (0 linhas afetadas)'));
                \Log::info("Resultado Update Transação: " . ($updateTransacao ? 'SUCESSO' : 'FALHA (0 linhas afetadas)'));
            }

            // Sobrescreve os dados da reserva atual para manutenção
            $reservaOriginal->update([
                'status' => 'maintenance',
                'client_name' => '🛠️ MANUTENÇÃO',
                'total_paid' => 0, // <-- Isso deveria zerar o "Já Recebido" que você viu
                'is_fixed' => true,
            ]);

            \Log::info("Reserva #{$id} movida para manutenção. Total Paid final: " . $reservaOriginal->total_paid);
