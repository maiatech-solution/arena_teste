    ...
    public function moverManutencao(Request $request, $id)
{
    return DB::transaction(function () use ($request, $id) {
        try {
            ...
            // 1. BACKUP dos dados para reativação futura
            $backupData = [
            ...
            if ($valorOriginal > 0 && $action === 'transfer') {
                // Localiza a próxima reserva recorrente do mesmo cliente/série
                ...
                // B) Determina o novo status de pagamento
                $novoPrecoFinal = (float)($proximaReserva->final_price ?? $proximaReserva->price);
                if ($proximaReserva->total_paid >= $novoPrecoFinal) {
                    $proximaReserva->payment_status = 'paid';
                } elseif ($proximaReserva->total_paid > 0) {
                    $proximaReserva->payment_status = 'partial';
                }

                $proximaReserva->save();

                // C) MOVE AS TRANSAÇÕES NO BANCO (Ajustando ID e DATA para o dia 28)
                // Usamos Carbon para setar o horário às 12:00 para evitar conflitos de fuso no Caixa
                $novaDataPagamento = \Carbon\Carbon::parse($proximaReserva->date)->setHour(12)->setMinute(0);

                \App\Models\FinancialTransaction::where('reserva_id', $reserva->id)
                    ->update([
                        'reserva_id' => $proximaReserva->id,
                        'paid_at'    => $novaDataPagamento,
                        'description' => DB::raw("CONCAT(description, ' [REPASSADO DA MANUTENÇÃO #{$reserva->id}]')")
                    ]);

                // D) Limpa os valores da reserva que sai de cena
            ...
            // 4. Conclui a entrada em manutenção
            $reserva->update([
                'status'      => 'maintenance',
            ...
            return response()->json([
                'success' => true,
                'message' => 'Manutenção aplicada! O saldo foi transferido para o dia ' . \Carbon\Carbon::parse($proximaReserva->date ?? now())->format('d/m')
            ]);
