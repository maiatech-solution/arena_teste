...
class AdminController extends Controller
{
    ...
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            // Se o usuário escolheu "Liberar Horário (Vago)"
            if ($decisao === 'release_slot') {
                $reserva->delete();
                return redirect()->route('admin.reservas.index')
                    ->with('success', 'Manutenção removida. O horário agora está livre.');
            }

            // Se o usuário escolheu "Restaurar Cliente Original"
            if ($decisao === 'restore_client') {
                // Tenta recuperar os dados do backup nas notas
            ...
            // Caso de segurança: se não houver backup ou der erro na decisão, libera o slot para não "travar" a agenda
            $reserva->delete();
            return redirect()->route('admin.reservas.index')
                ->with('success', 'Horário liberado (não havia dados de backup).');
        } catch (\Exception $e) {
