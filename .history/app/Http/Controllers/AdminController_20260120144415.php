    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                $telefoneOriginal = $reserva->client_contact; // Backup do WhatsApp
                $userIdOriginal = $reserva->user_id;

                // 1. BACKUP (Mantemos o telefone no backup para garantir a reativação)
                $backupData = [
                    'name' => $nomeOriginal,
                    'contact' => $telefoneOriginal,
                ...
                // 2. LÓGICA FINANCEIRA (Busca Inteligente de Próxima Reserva)
                $transferenciaSucesso = false;

                if ($valorPagoOriginal > 0 && $action === 'transfer') {
                    // Identifica o ID da série (seja a reserva atual a mãe ou uma filha)
                    $idDaSerie = $reserva->recurrent_series_id ?? $reserva->id;

                    $proximaReserva = Reserva::where(function ($query) use ($idDaSerie) {
                        $query->where('recurrent_series_id', $idDaSerie)
                    ...
                    if ($proximaReserva) {
                        // Transfere o valor para a próxima
                        ...
                        // Move o histórico de transações para a nova reserva
                ...
                // Se for PONTUAL ou se a transferência falhou/não foi escolhida, faz o ESTORNO
                if ($valorPagoOriginal > 0 && !$transferenciaSucesso) {
                ...
                // 3. CONVERTE PARA MANUTENÇÃO (Mantemos o client_contact intacto para o WhatsApp funcionar)
                DB::table('reservas')->where('id', $id)->update([
                    ...
                    'is_fixed' => 1,
                    'notes' => $backupString . "\n" . ($reserva->notes ?? '')
                    // Note que NÃO limpamos o client_contact aqui para o botão de zap não sumir
                ]);

                return response()->json([
                    'success' => true,
                    'message' => $transferenciaSucesso ? 'Manutenção aplicada e valor transferido!' : 'Manutenção aplicada!'
                ]);
            } catch (\Exception $e) {
    ...
    public function reativarManutencao(\App\Http\Requests\UpdateReservaStatusRequest $request, $id)
    {
        try {
            ...
            if ($decisao === 'release_slot' || empty($decisao)) {
                ...
                return redirect()->back()->with('success', '✅ Agenda liberada!');
            }
            ...
            if ($decisao === 'restore_client') {
                if (preg_match('/###BACKUP###(.*?)###END###/s', $reserva->notes, $matches)) {
                    ...
                    return redirect()->route('admin.reservas.show', $reserva->id)->with('success', '👤 Cliente restaurado!');
                }
