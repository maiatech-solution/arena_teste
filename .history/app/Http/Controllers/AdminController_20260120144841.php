    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 1. BACKUP (Marcação robusta para reativação)
                $backupData = [
                    'name' => $nomeOriginal,
                ...
                if ($valorPagoOriginal > 0 && $action === 'transfer') {
                    // Identifica o ID da série (Mãe ou Filha)
                    $idDaSerie = $reserva->recurrent_series_id ?? $reserva->id;

                    // Busca a próxima reserva da série (Busca Inteligente)
                    $proximaReserva = Reserva::where(function ($query) use ($idDaSerie) {
                        $query->where('recurrent_series_id', $idDaSerie)
                            ->orWhere('id', $idDaSerie);
                    })
                        ->where('id', '!=', $reserva->id) // Não pode ser ela mesma
                        ->where('date', '>', $reserva->date) // Tem que ser no futuro
                        ->where('status', '!=', 'cancelled')
                        ->orderBy('date', 'asc')
                        ->first();

                    if ($proximaReserva) {
                        // Atualiza a reserva destino (Dia 28) via SQL Puro
                        ...
                        // Move as transações financeiras para a nova reserva
                ...
                // 3. SE NÃO TRANSFERIU (Seja por ser pontual ou por não achar a próxima), FAZ ESTORNO
                ...
                // 4. CONVERTE PARA MANUTENÇÃO (Preservando contato para o WhatsApp)
                DB::table('reservas')->where('id', $id)->update([
                    ...
                    'is_fixed' => 1,
                    'notes' => $backupString . "\n" . ($reserva->notes ?? '')
                    // NÃO limpamos o client_contact para manter o botão de WhatsApp ativo
                ]);

                return response()->json([
                    'success' => true,
                    'message' => $transferenciaSucesso ? 'Transferido para a próxima reserva!' : 'Manutenção aplicada com estorno.'
                ]);
            } catch (\Exception $e) {
