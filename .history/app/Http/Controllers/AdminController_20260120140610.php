    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                $nomeOriginal = $reserva->client_name; // Guardamos o nome
                ...
                if ($valorOriginal > 0 && $action === 'transfer') {
                    ...
                    if ($proximaReserva) {
                        // 1. Garante a transferência do crédito para a reserva destino
                        ...
                        // 2. Vincula as transações financeiras à nova reserva
                ...
                // 3. Atualiza a reserva atual para Manutenção
                // Mantemos o nome do cliente para você saber de quem era o horário
                $reserva->update([
                    'status' => 'maintenance',
                    ...
                    'signal_value' => 0,
                    'is_fixed' => true,
                    'notes' => "[CREDITO TRANSFERIDO R$ {$valorOriginal}] " . ($reserva->notes ?? '')
                ]);
