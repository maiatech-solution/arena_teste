...
class AdminController extends Controller
{
     ...
     * Ajustado para gerar link de WhatsApp pedindo CHAVE PIX e forçar exibição do Toast.
     */
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 2. FINANCEIRO: Processa estorno se solicitado (saída do caixa)
                if ($reserva->total_paid > 0 && $action === 'refund') {
                    $totalEstornado = (float) $reserva->total_paid;
                ...
                // 3. BLOQUEIO: Altera status para manutenção e aplica backup nas notas
                $reserva->update([
                    'status'      => 'maintenance',
                ...
                // 4. GERAÇÃO DO LINK WHATSAPP (Card Verde no topo)
                if ($reserva->client_contact) {
                    ...
                    // Recupera o nome original através do backup para a mensagem
                    $dadosBackup = json_decode(explode('BACKUP_DATA:', $backupInfo)[1] ?? '{}', true);
                    $clienteOriginal = $dadosBackup['name'] ?? 'Cliente';

                    $mensagem = "Olá *{$clienteOriginal}*! 👋\n\nInformamos que o seu horário do dia *{$dataBR}* às *{$horaBR}* precisou ser interrompido para *MANUTENÇÃO DE EMERGÊNCIA* na quadra ({$motivo}).";

                    // Pedido da chave PIX se houve retirada de dinheiro do caixa
                    if ($totalEstornado > 0) {
                        $valorFormatado = number_format($totalEstornado, 2, ',', '.');
                        ...
                        $mensagem .= "\n\nO horário foi liberado no sistema para reparos técnicos.";
                    }
                    ...
                    /** * 🛑 CRÍTICO: session()->save() força o Laravel a persistir os dados da sessão
                     * imediatamente. Sem isso, requisições AJAX (Fetch) podem não salvar o 'flash'
                     * a tempo do window.location.reload() ler a informação.
                     */
                    session()->save();
                }

                return response()->json(['success' => true, 'message' => 'Horário bloqueado e estorno registrado!']);
            } catch (\Exception $e) {
     ...
     * Finaliza o bloqueio e decide se restaura o cliente anterior ou libera o slot (vago).
     */
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            // --- CASO 1: LIBERAR HORÁRIO (DEIXAR VAGO PARA O MERCADO) ---
            if ($decisao === 'release_slot') {
                DB::beginTransaction();
                try {
                    // Log de entrada
                    \Log::info("Iniciando liberação de slot. ID Manutenção: " . $reserva->id);

                    $dadosTemporarios = clone $reserva;

                    // 1. Deletar a manutenção
                    $deleted = $reserva->delete();
                    \Log::info("Registro de manutenção deletado? " . ($deleted ? 'SIM' : 'NÃO'));

                    // 2. Tentar recriar o slot
                    \Log::info("Chamando recreateFixedSlot para Arena: {$dadosTemporarios->arena_id}, Data: {$dadosTemporarios->date}");
                    $this->reservaController->recreateFixedSlot($dadosTemporarios);

                    // 3. Verificar se o slot foi mesmo criado no banco logo após o comando
                    $verificar = \App\Models\Reserva::where('is_fixed', true)
                        ->where('arena_id', $dadosTemporarios->arena_id)
                        ->where('date', $dadosTemporarios->date)
                        ->where('start_time', $dadosTemporarios->start_time)
                        ->where('status', 'free')
                        ->first();

                    if ($verificar) {
                        \Log::info("SUCESSO: Novo slot livre encontrado no banco. ID: " . $verificar->id);
                    } else {
                        \Log::error("FALHA: O slot deveria ter sido criado, mas não foi encontrado no banco.");
                    }

                    DB::commit();
                    return redirect()->route('admin.reservas.index', $routeParams)
                        ->with('success', 'Processado. Verifique os logs se sumiu.');
                } catch (\Exception $e) {
                    DB::rollBack();
                    \Log::error("Erro Crítico no Debug: " . $e->getMessage());
                    return redirect()->back()->with('error', 'Erro: ' . $e->getMessage());
                }
            }

            // --- CASO 2: RESTAURAR CLIENTE ORIGINAL ---
            if ($decisao === 'restore_client') {
                if (preg_match('/\[BACKUP_DATA:(.*?)\]/', $reserva->notes, $matches)) {
                    ...
                    // Geração da Notificação de WhatsApp (Aviso de Valor Integral)
                    $waLink = null;
                    if ($reserva->client_contact) {
                        $phone = preg_replace('/\D/', '', $reserva->client_contact);
                        $dataBR = \Carbon\Carbon::parse($reserva->date)->format('d/m');
            ...
            // --- SEGURANÇA: Se cair aqui sem decisão válida, libera o slot usando a mesma lógica ---
            DB::beginTransaction();
            $dadosSeguranca = clone $reserva;
            $reserva->delete();
            $this->reservaController->recreateFixedSlot($dadosSeguranca);
            DB::commit();
