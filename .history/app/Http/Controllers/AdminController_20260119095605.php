...
class AdminController extends Controller
{
    ...
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            // OPÇÃO A: Liberar horário (Vago)
            if ($decisao === 'release_slot') {
                $reserva->delete();
                return redirect()->route('admin.reservas.index')->with('success', 'Horário liberado e vago!');
            }

            // OPÇÃO B: Restaurar Cliente
            // Procuramos o marcador [BACKUP_DATA:...] nas notas
            if (preg_match('/\[BACKUP_DATA:(.*?)\]/', $reserva->notes, $matches)) {
                ...
                $reserva->update([
                    ...
                    // Remove o marcador de backup das notas para limpar o campo
                    'notes'       => trim(preg_replace('/\[BACKUP_DATA:.*?\]/', '', $reserva->notes))
                ]);

                return redirect()->back()->with('success', 'Reserva restaurada com sucesso!');
            }

            // Caso não encontre backup (reserva criada manualmente como manutenção)
            $reserva->delete();
