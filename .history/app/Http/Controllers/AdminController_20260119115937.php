    ...
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            if ($decisao === 'release_slot') {
                DB::beginTransaction();
                try {
                    // 1. Clonamos os dados da reserva de manutenção antes de deletar
                    $dadosTemporarios = clone $reserva;

                    // 2. Removemos o bloqueio de manutenção PRIMEIRO
                    // Isso limpa o horário no banco para que a função de recriação funcione
                    $reserva->delete();

                    // 3. Agora recriamos o slot fixo vago ('free') usando os dados clonados
                    $this->reservaController->recreateFixedSlot($dadosTemporarios);

                    DB::commit();
                    return redirect()->route('admin.reservas.index', $routeParams)
                        ->with('success', '✅ Manutenção finalizada. O horário agora está disponível para novos agendamentos!');
                } catch (\Exception $e) {
                    if (DB::transactionLevel() > 0) DB::rollBack();
                    throw $e;
                }
