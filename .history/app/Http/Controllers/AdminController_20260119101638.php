...
class AdminController extends Controller
{
    ...
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            // Isso garante que voltaremos para a data e arena corretas no calendário
            $routeParams = [
                'arena_id' => $reserva->arena_id,
                'date'     => \Carbon\Carbon::parse($reserva->date)->format('Y-m-d')
            ];

            // --- CASO 1: LIBERAR HORÁRIO (VAGO) ---
            if ($decisao === 'release_slot') {
                $reserva->delete();

                // Redireciona para o dashboard com os filtros preservados
            ...
            if ($decisao === 'restore_client') {
                if (preg_match('/\[BACKUP_DATA:(.*?)\]/', $reserva->notes, $matches)) {
                    ...
                    // Mantemos o redirecionamento para o SHOW para conferência rápida,
                    // mas o botão "Voltar" daquela página agora usará os mesmos parâmetros.
                    return redirect()->route('admin.reservas.show', $reserva->id)
                        ->with('success', '👤 Reserva de ' . $reserva->client_name . ' restaurada com sucesso!');
                }
            }

            // --- CASO 3: SEGURANÇA (BACKUP NÃO ENCONTRADO) ---
            $reserva->delete();
            return redirect()->route('admin.reservas.index', $routeParams)
                ->with('success', '✅ Horário liberado (sem dados de backup).');
        } catch (\Exception $e) {
