...
class ReservaController extends Controller
{
    ...
    public function finalizarPagamento(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            // --- 2. Atualiza a Reserva Atual (Dia 04 no seu exemplo) ---
            $reserva->update([
            ...
            if ($request->boolean('apply_to_series') && !empty($reserva->recurrent_series_id)) {
                ...
                $masterId = $reserva->recurrent_series_id;

                // ✅ CORREÇÃO CRÍTICA: Simplificamos a query para garantir que todos os membros da série (cópias e mestra)
                // que são estritamente futuros sejam atualizados pelo ID da série.
                $updatedCount = Reserva::where('recurrent_series_id', $masterId)
                       ->where('date', '>', $reservaDate) // Garante que a atualização seja só para o futuro
                       ->where('is_fixed', false)
                       ->whereIn('status', [Reserva::STATUS_CONFIRMADA, Reserva::STATUS_PENDENTE]) // Apenas reservas ativas
                       ->update([
    ...
    public function updatePriceAndSeries(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            if ($isRecurrent && $applyToSeries) {
                ...
                $slotsToUpdate = Reserva::where('recurrent_series_id', $masterId)
                    ->orWhere(function($query) use ($masterId) {
                         // Inclui o Master ID se ele for o próprio ID da série
                         $query->where('id', $masterId)->whereNull('recurrent_series_id');
                    })
                    ->where('is_fixed', false)
