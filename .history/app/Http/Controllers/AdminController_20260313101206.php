...
class AdminController extends Controller
{
    ...
    public function cancelarReservaRecorrente(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            // Se houver estorno, a venda "deixa de existir", então final_price = 0.
            // Se NÃO houver estorno (retenção), o faturamento real é o que o cliente já pagou (o sinal).
            if ($validated['should_refund']) {
                $reserva->final_price = 0;
            } else {
                // Importante: se retemos o dinheiro, o preço final da venda vira o valor retido
                // para que o relatório de ocupação/vendas não mostre prejuízo indevido.
                $reserva->final_price = $pagoAteAgora;
            }

            $reserva->cancellation_reason = '[Gestor - Pontual Recorrência] ' . $validated['cancellation_reason'];

            // Salvamos antes de enviar para o finalizeStatus
            $reserva->save();

            // 🚀 O SEGREDO: Aqui ela chama a função que já corrigimos.
            // O finalizeStatus agora vai olhar o banco, ver se o sinal foi PIX/Cartão
            // e fazer a saída pelo balde correto.
            $result = $this->reservaController->finalizeStatus(
                $reserva,
                Reserva::STATUS_CANCELADA,
            ...
            $message = "Reserva recorrente cancelada com sucesso!" . ($result['message_finance'] ?? '');
            return response()->json(['success' => true, 'message' => $message], 200);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function moverManutencao(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 🔍 IDENTIFICAÇÃO DO MÉTODO ORIGINAL (Para evitar divergência)
                ...
                // --- LÓGICA DE TRANSFERÊNCIA DE CRÉDITO ---
                        ...
                        foreach ($transacoes as $t) {
                            DB::table('financial_transactions')
                                ...
                                ->update([
                                    ...
                                    // Mantém o payment_method original na transferência!
                                ]);
                        }
                ...
                // --- LÓGICA DE ESTORNO (SAÍDA REAL DO CAIXA) ---
                if ($valorRestante > 0 && ($action === 'refund' || !$transferenciaSucesso)) {
                    ...
                    if (!$jaEstornado) {
                        \App\Models\FinancialTransaction::create([
                            ...
                            'type'           => 'refund',
                            'payment_method' => $metodoOriginal, // ✨ ANTES ERA FIXO 'cash_out'
                ...
                // --- BACKUP E ATUALIZAÇÃO DA RESERVA ---
                $backup = [
                    'name' => $nomeLimpo,
                    'contact' => $contato,
                    'user_id' => $userId,
                    ...
                    'original_method' => $metodoOriginal // Guarda o método original para conferência
                ];
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Manutenção aplicada e financeiro espelhado!',
