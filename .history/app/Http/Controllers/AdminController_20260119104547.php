...
class AdminController extends Controller
{
    ...
    /**
     * 🔄 Reativação Inteligente (Processa a escolha do Modal)
     */
    /**
     * 🔄 Reativação Inteligente de Horário em Manutenção
     * Finaliza o bloqueio e decide se restaura o cliente anterior ou libera o slot.
     */
    public function reativarManutencao(Request $request, $id)
    {
        try {
            // Buscamos a reserva com os relacionamentos necessários
            $reserva = Reserva::findOrFail($id);
            $decisao = $request->input('action');

            // 🧠 UX: Capturamos os parâmetros da reserva antes de qualquer alteração
            // Isso garante que voltaremos para a data e arena corretas no calendário/dashboard
            $routeParams = [
                'arena_id' => $reserva->arena_id,
                'date'     => \Carbon\Carbon::parse($reserva->date)->format('Y-m-d')
            ];

            // --- CASO 1: LIBERAR HORÁRIO (DEIXAR VAGO) ---
            if ($decisao === 'release_slot') {
                $reserva->delete();

                return redirect()->route('admin.reservas.index', $routeParams)
                    ->with('success', '✅ Manutenção finalizada. O horário agora está vago no calendário!');
            }

            // --- CASO 2: RESTAURAR CLIENTE ORIGINAL ---
            if ($decisao === 'restore_client') {
                // Tenta recuperar os dados do backup injetados nas notas durante o bloqueio
                if (preg_match('/\[BACKUP_DATA:(.*?)\]/', $reserva->notes, $matches)) {
                    ...
                    $reserva->update([
                        ...
                        'status'      => 'confirmed',
                        // Converte string/boolean do JSON para o formato correto
                        'is_fixed'    => ($dados['is_fixed'] === true || $dados['is_fixed'] === 'true'),
                        // Limpa o marcador de backup das notas para manter o histórico limpo
                        'notes'       => trim(preg_replace('/\[BACKUP_DATA:.*?\]/', '', $reserva->notes))
                    ]);

                    // 📱 PREPARAÇÃO DO LINK WHATSAPP (Aviso de Reativação)
                    $waLink = null;
                    if ($reserva->client_contact) {
                        ...
                        $mensagem = "Olá *{$reserva->client_name}*! 👋\n\nInformamos que a manutenção técnica na quadra foi concluída e seu horário para o dia *{$dataBR}* às *{$horaBR}* está *CONFIRMADO*! 🏟️\n\nTe esperamos na arena!";

                        $waLink = "https://wa.me/55{$phone}?text=" . urlencode($mensagem);
                    }

                    // Redireciona para o detalhe da reserva restaurada para conferência
                    return redirect()->route('admin.reservas.show', $reserva->id)
                        ->with('success', '👤 Reserva de ' . $reserva->client_name . ' restaurada com sucesso!')
                        ->with('whatsapp_link', $waLink);
                }
            }

            // --- CASO 3: SEGURANÇA (DADOS DE BACKUP NÃO ENCONTRADOS) ---
            // Se por algum motivo o JSON nas notas sumiu, deletamos para não travar a agenda
            $reserva->delete();

            return redirect()->route('admin.reservas.index', $routeParams)
                ->with('success', '✅ Horário liberado (não foram encontrados dados para restauração).');
        } catch (\Exception $e) {
            // Log do erro para depuração técnica se necessário
            \Log::error("Erro ao reativar manutenção: " . $e->getMessage());

            return redirect()->back()->with('error', '❌ Erro crítico ao processar reativação: ' . $e->getMessage());
        }
    }
}
