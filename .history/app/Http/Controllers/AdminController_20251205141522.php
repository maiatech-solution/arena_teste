...
class AdminController extends Controller
{
    ...
    public function indexFinancialDashboard(Request $request) // 🛑 RENOMEADO: De indexPagamentos para indexFinancialDashboard
    {
        ...
        // ✅ NOVO KPI: Total de Sinais Recebidos HOJE (Apenas type='signal')
        $totalSignalsToday = $financialTransactions->where('type', 'signal')->sum('amount');
        ...
        Log::info("DEBUG FINANCEIRO: KPIs FINAIS para View - Data: {$date} | Saldo Líquido HOJE (\$totalReceived): R$ " . number_format($totalReceived, 2, ',', '.') . " | Sinais HOJE (\$totalSignalsToday): R$ " . number_format($totalSignalsToday, 2, ',', '.') . " | Saldo GLOBAL (\$totalGlobalBalance): R$ " . number_format($totalBalance, 2, ',', '.'));
        ...
        return view('admin.payment.index', [ // 🛑 CORREÇÃO DA VIEW: Usando admin.payment.index
            ...
            'search' => $search,
            'totalGlobalBalance' => $totalBalance, // Saldo acumulado total
        ]);
    }
}
