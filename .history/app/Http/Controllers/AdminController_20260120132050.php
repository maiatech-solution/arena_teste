    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                $action = $request->input('finance_action'); // 'refund' ou 'transfer'
                $motivo = $request->input('reason', 'Manutenção');

                // 1. FAZ BACKUP (JSON nas notas) para recuperação posterior
                $backupData = [
                    'name' => $reserva->client_name,
                    'status' => $reserva->status,
                    'is_fixed' => $reserva->is_fixed,
                    'total_paid_at_time' => $reserva->total_paid // Salva quanto ele tinha pago
                ];
                $backupInfo = " [BACKUP_DATA:" . json_encode($backupData) . "]";

                $valorParaTransferir = (float) $reserva->total_paid;
                $proximaReserva = null;
                $messageExtra = "";

                // 2. LÓGICA DE TRANSFERÊNCIA DE CRÉDITO (O que estava faltando!)
                if ($valorParaTransferir > 0 && $action === 'transfer' && $reserva->recurrent_series_id) {
                    // Localiza a próxima reserva da série que não esteja cancelada
                    $proximaReserva = Reserva::where('recurrent_series_id', $reserva->recurrent_series_id)
                        ->where('date', '>', $reserva->date)
                        ->whereNotIn('status', [Reserva::STATUS_CANCELADA, 'rejected'])
                        ->orderBy('date', 'asc')
                        ->first();

                    if ($proximaReserva) {
                        // Incrementa os valores na reserva destino
                        $proximaReserva->total_paid += $valorParaTransferir;
                        $proximaReserva->signal_value += $valorParaTransferir;

                        // Atualiza status de pagamento da próxima (se quitou ou se é parcial)
                        $proximaReserva->payment_status = ($proximaReserva->total_paid >= $proximaReserva->final_price) ? 'paid' : 'partial';
                        $proximaReserva->save();

                        // 🚩 VÍNCULO FINANCEIRO: Move as transações para a nova reserva
                        // Isso faz com que o extrato e o caixa reconheçam o dinheiro no novo dia
                        FinancialTransaction::where('reserva_id', $reserva->id)
                            ->update([
                                'reserva_id' => $proximaReserva->id,
                                'description' => DB::raw("CONCAT(description, ' (Transferido da reserva #{$reserva->id} via Manutenção)')")
                            ]);

                        $messageExtra = " Crédito de R$ " . number_format($valorParaTransferir, 2, ',', '.') . " transferido para a reserva do dia " . Carbon::parse($proximaReserva->date)->format('d/m') . ".";

                        // Zera o pago da reserva atual para ela entrar limpa em manutenção
                ...
                // 3. LÓGICA DE ESTORNO (Já existia, mantida e refinada)
                elseif ($valorParaTransferir > 0 && $action === 'refund') {
                    FinancialTransaction::create([
                        'reserva_id' => $reserva->id,
                        'arena_id'   => $reserva->arena_id,
                        'amount'     => -$valorParaTransferir,
                        'type'       => 'refund',
                    ...
                    $reserva->total_paid = 0;
                    $reserva->signal_value = 0;
                    $messageExtra = " Estorno de R$ " . number_format($valorParaTransferir, 2, ',', '.') . " registrado no caixa.";
                }

                // 4. BLOQUEIO: Altera status para manutenção
                $reserva->update([
                    'status'      => 'maintenance',
                ...
                // 5. GERAÇÃO DO LINK WHATSAPP
                $waLink = null;
                if ($reserva->client_contact) {
                    $phone = preg_replace('/\D/', '', $reserva->client_contact);
                    $dataBR = Carbon::parse($reserva->date)->format('d/m');
                    $horaBR = Carbon::parse($reserva->start_time)->format('H:i');
                    $clienteOriginal = $backupData['name'] ?? 'Cliente';

                    $mensagem = "Olá *{$clienteOriginal}*! 👋\n\nInformamos que o seu horário do dia *{$dataBR}* às *{$horaBR}* precisou ser interrompido para *MANUTENÇÃO* ({$motivo}).";

                    if ($action === 'transfer' && $proximaReserva) {
                        $proxData = Carbon::parse($proximaReserva->date)->format('d/m');
                        $mensagem .= "\n\n✅ *CRÉDITO TRANSFERIDO:* O seu pagamento de *R$ " . number_format($valorParaTransferir, 2, ',', '.') . "* foi movido para o seu próximo horário no dia *{$proxData}*.";
                    } elseif ($action === 'refund') {
                        $mensagem .= "\n\n💰 *ESTORNO:* O valor de *R$ " . number_format($valorParaTransferir, 2, ',', '.') . "* foi estornado. Por favor, envie sua chave PIX.";
                    }

                    $waLink = "https://wa.me/55{$phone}?text=" . urlencode($mensagem);
                    session()->flash('whatsapp_link', $waLink);
                }

                return response()->json([
                    'success' => true,
                    'message' => 'Horário em manutenção!' . $messageExtra,
