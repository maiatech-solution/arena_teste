    ...
    public function getConfirmedReservas(Request $request)
    {
        try {
            ...
            return response()->json($reservas->map(function ($reserva) {
                ...
                // Se for um slot fixo de base (Cinza)
                if ($reserva->is_fixed) {
                    return [
                        'id' => $reserva->id,
                        'title' => 'Livre (Fixo)',
                ...
                $color = '#4f46e5'; // Azul (Padrão)
                $title = ($reserva->user->name ?? $reserva->client_name ?? 'Cliente');

                if ($reserva->status === Reserva::STATUS_PENDENTE) $color = '#f59e0b'; // Laranja
                if ($reserva->status === Reserva::STATUS_CONCLUIDA) $color = '#10b981'; // Verde

                // 🛠️ NOVO: Tratar Manutenção
                if ($reserva->status === 'maintenance') {
                    $color = '#DB2777'; // Rosa Choque (Pink 600)
                    $title = '🛠️ MANUTENÇÃO';
                }
                ...
                return [
                    ...
                    'extendedProps' => [
                        ...
                        'price' => (float)$reserva->price,
                        'final_price' => (float)($reserva->final_price > 0 ? $reserva->final_price : $reserva->price),
