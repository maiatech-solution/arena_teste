...
class ConfigurationController extends Controller
{
    ...
    public function toggleFixedReservaStatus(Request $request, $id)
    {
        ...
        try {
            // ✅ CORREÇÃO CRÍTICA: Validação agora usa as CONSTANTES do Model Reserva.
            // Os valores 'confirmed' (Reserva::STATUS_CONFIRMADA) e 'cancelled' (Reserva::STATUS_CANCELADA)
            // estão sendo enviados pelo JS e devem corresponder às constantes.
            ...
            // 🛑 CRÍTICO: Se o novo status for CONFIRMED, o status no DB deve ser STATUS_FREE,
            // pois slots fixos não são "confirmados" (pelo cliente), eles são "livres" ou "cancelados" (manutenção).
            // No Blade: 'confirmed' => 'Disponível'
            // No DB: 'free' => 'Livre (Slot)'

            if ($newStatus === Reserva::STATUS_CONFIRMADA) {
                 $finalStatus = Reserva::STATUS_FREE;
            } else {
                 $finalStatus = Reserva::STATUS_CANCELADA; // 'cancelled'
            }
