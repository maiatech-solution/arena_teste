...
class ReservaController extends Controller
{
     ...
     * 🛑 CORRIGIDO: Lógica robusta de sobreposição de intervalos.
    ...
    public function checkOverlap(string $date, string $startTime, string $endTime, bool $isFixed, ?int $ignoreReservaId = null): bool
    {
        ...
        try {
            ...
            // Caso raro onde o formato é inválido, assume 1h de duração para o check
            $startTimeNormalized = Carbon::parse($startTime)->format('H:i:s');
            $endTimeNormalized = Carbon::parse($startTime)->addHour()->format('H:i:s');
        }
        ...
        // Isso checa conflito com QUALQUER coisa que esteja ocupando o horário.
    ...
    protected function findOrCreateClient(array $data): User
    {
        ...
        $name = $data['name'];
        // Garante que email seja null se vazio
        ...
        if ($user) {
            // 2. Cliente encontrado: Atualiza o nome e email (se fornecido)
            $updateData = [
                'name' => $name,
            ];
            // Somente atualiza o e-mail se um novo for fornecido
            ...
            $newUser = User::create([
                ...
                'role' => 'cliente', // Certifique-se de que a role é 'cliente'
                'is_admin' => false,
                'data_nascimento' => $data['data_nascimento'] ?? null,
            ]);
            Log::info("Novo cliente criado (ID: {$newUser->id}).");
            return $newUser;
        }
    }
    ...
    public function storePublic(Request $request)
    {
        $rules = [
            ...
            // 🛑 NOTA: Regex estrito, requer exatamente 10 ou 11 dígitos, sem formatação.
