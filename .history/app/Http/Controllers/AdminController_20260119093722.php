...
class AdminController extends Controller
{
    ...
    /**
     * 🛠️ Move uma reserva (Confirmada ou Concluída) para o status de MANUTENÇÃO.
     * Gerencia estornos reais no caixa ou transferência de crédito para mensalistas.
     */
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                // Busca a reserva com transações para garantir precisão financeira
                $reserva = Reserva::with('transactions', 'arena')->findOrFail($id);

                $action = $request->input('finance_action'); // 'refund', 'credit' ou 'none'
                $motivo = $request->input('reason', 'Manutenção programada');
                $valorPago = (float)$reserva->total_paid;
                $msgAdicional = "";
                ...
                if ($valorPago > 0) {
                    if ($action === 'refund') {
                        ...
                        FinancialTransaction::create([
                            ...
                            'manager_id'     => Auth::id(),
                            'amount'         => -$valorPago, // VALOR NEGATIVO
                        ...
                        $msgAdicional = " Valor de R$ " . number_format($valorPago, 2, ',', '.') . " estornado do caixa.";
                    } elseif ($action === 'credit' && $reserva->is_recurrent) {
                        // LÓGICA DE CRÉDITO PARA MENSALISTA
                        // Busca a próxima reserva CONFIRMADA da série recorrente
                        $proxima = Reserva::where('recurrent_series_id', $reserva->recurrent_series_id)
                            ->where('id', '!=', $reserva->id)
                            ->where('date', '>', $reserva->date)
                        ...
                        if ($proxima) {
                            $proxima->increment('total_paid', $valorPago);
                            $proxima->update([
                                'notes' => $proxima->notes . "\n[CRÉDITO: R$ {$valorPago} vindo da manutenção de " . $reserva->date->format('d/m') . "]"
                            ]);
                            $reserva->total_paid = 0;
                            $msgAdicional = " Valor transferido como crédito para o jogo de " . $proxima->date->format('d/m') . ".";
                        } else {
                            throw new \Exception('Não foi encontrada uma reserva futura nesta série para aplicar o crédito.');
                        }
                    }
                }

                // --- 2. TRANSFORMAÇÃO EM MANUTENÇÃO ---
                // Guardamos os dados do cliente original no histórico das notas
                $historicoNota = "\n⚠️ [MANUTENÇÃO] Slot bloqueado em " . now()->format('d/m H:i');
                $historicoNota .= "\nCliente removido: " . $reserva->client_name;
                $historicoNota .= "\nMotivo: " . $motivo;

                $reserva->update([
                    'status' => Reserva::STATUS_MAINTENANCE,
                    'client_name' => '🛠️ MANUTENÇÃO',
                    'notes' => $reserva->notes . $historicoNota,
                    'is_fixed' => true, // Volta a se comportar como slot fixo (Rosa no Calendário)
                ]);

                return response()->json([
                    'success' => true,
                    'message' => 'Horário bloqueado para manutenção!' . $msgAdicional
                ]);
            } catch (\Exception $e) {
                Log::error("Erro ao mover para manutenção: " . $e->getMessage());
    ...
    /**
     * 🔄 Reativa o horário que estava em manutenção.
     */
    public function reativarManutencao($id)
    {
        try {
            $reserva = Reserva::findOrFail($id);

            // Armazenamos a data para poder avisar no log ou mensagem, se quiser
            $dataReserva = \Carbon\Carbon::parse($reserva->date)->format('d/m/Y');

            // Deletamos o slot de manutenção (bloqueio)
            $reserva->delete();

            // 🚨 CORREÇÃO: Redirecionar para a listagem de confirmadas (ou outra view estável)
            // Não use ->back() aqui pois a página atual (show) não existe mais para este ID.
            return redirect()->route('admin.reservas.confirmadas')
                ->with('success', "Manutenção de {$dataReserva} finalizada. O horário foi liberado!");
        } catch (\Exception $e) {
            // Se falhar antes de deletar, o back() ainda funciona
