...
class AdminController extends Controller
{
    ...
    public function moverManutencao(Request $request, $id)
    {
        ...
        // Identifica como o banco está guardando esses IDs
        $idDaSerieRaw = $reserva->recurrent_series_id;
        $idDaReserva = $reserva->id;

        // Tenta a busca que deveria encontrar o Ademar/Alan da semana que vem
        ...
        // 🕵️ O QUE VAMOS DESCOBRIR AQUI:
        dd([
            'RESERVA_CLICADA' => [
                'id' => $idDaReserva,
                'recurrent_series_id_no_banco' => $idDaSerieRaw,
                'tipo_do_id_no_banco' => gettype($idDaSerieRaw),
            ],
            'BUSCA_RESULTADO' => [
                'encontrou_proxima' => $proxima ? 'SIM' : 'NÃO',
                'id_da_proxima' => $proxima->id ?? 'N/A',
                'data_da_proxima' => $proxima->date ?? 'N/A',
                'series_id_da_proxima' => $proxima->recurrent_series_id ?? 'N/A',
            ],
            'ESTADO_DO_REQUEST' => $request->all()
        ]);
    }
