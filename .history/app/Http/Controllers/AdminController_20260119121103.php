...
class AdminController extends Controller
{
     ...
     * Corrigido: Ordem de execução para evitar sumiço e chamada correta do Financeiro.
     */
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            // 🎯 VALIDAÇÃO DE CAIXA (Chamada estática correta conforme seu FinanceiroController)
            ...
            if ($decisao === 'release_slot') {
                DB::beginTransaction();
                try {
                    // 1. Clonamos os dados da reserva de manutenção antes de deletar para não perder a referência
                    $dadosTemporarios = clone $reserva;

                    // 2. Removemos o bloqueio de manutenção PRIMEIRO
                    // Isso limpa o "espaço" no banco de dados
                    $reserva->delete();

                    // 3. Agora recriamos o slot fixo vago ('free') usando os dados que clonamos
                    // Como o banco está limpo, a função recreateFixedSlot funcionará 100%
                    $this->reservaController->recreateFixedSlot($dadosTemporarios);

                    DB::commit();
                    return redirect()->route('admin.reservas.index', $routeParams)
                        ->with('success', '✅ Manutenção finalizada. O horário voltou a ficar disponível!');
                } catch (\Exception $e) {
                    if (DB::transactionLevel() > 0) DB::rollBack();
                    throw $e;
                }
            ...
            // --- SEGURANÇA: Se cair aqui sem decisão válida ---
            DB::beginTransaction();
            ...
            return redirect()->route('admin.reservas.index', $routeParams)->with('success', '✅ Horário liberado.');
        } catch (\Exception $e) {
            if (DB::transactionLevel() > 0) DB::rollBack();
            \Log::error("ERRO NA REATIVAÇÃO: " . $e->getMessage());
