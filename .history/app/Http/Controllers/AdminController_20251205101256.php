...
class AdminController extends Controller
{
        ...
        try {
            ...
            if ($amountPaid > 0) {
                if ($shouldRefund) {
                    ...
                    // 🛑 SOLUÇÃO PARA CASCADE DELETE: Criar a transação de retenção
                    // com `reserva_id = null` para que ela sobreviva ao CASCADE DELETE
                    // que ocorre quando o status da reserva muda (se houver um evento no Model
                    // que deleta o sinal original) ou se a FK é configurada para deletar.

                    FinancialTransaction::create([
                        'reserva_id' => null, // 🛑 CRÍTICO: Remove o vínculo FK para sobreviver à exclusão
                        ...
                        'type' => 'RETEN_CANC_FREE', // Tipo diferente para rastrear a compensação
                        'payment_method' => 'retained_funds',
                        'description' => "Retenção do valor pago (R$ " . number_format($amountPaid, 2, ',', '.') . ") após cancelamento (Compensação de sinal perdido).",
                        'paid_at' => Carbon::now(),
                    ]);
                    $messageFinance = " O valor de R$ " . number_format($amountPaid, 2, ',', '.') . " foi RETIDO no caixa (Compensação).";
                    Log::info("DEBUG FINANCEIRO: Transação de RETENÇÃO (RETEN_CANC_FREE) criada com reserva_id=null para ID {$reserva->id}.");
                }
            }

            // 2. Agora que a transação de compensação está segura (reserva_id=null),
            // salvamos a reserva. Isso dispara o CASCADE DELETE (removerá a transação 'signal' original).
            ...
            if ($amountPaid > 0) {
                if ($shouldRefund) {
                    ...
                    FinancialTransaction::create([
                        ...
                        'manager_id' => Auth::id(),
                        'amount' => $amountPaid, // Valor positivo para retenção (fica no caixa)
                        'type' => 'RETEN_CANC_P_FREE', // Tipo diferente para rastrear a compensação
                        'payment_method' => 'retained_funds',
                        'description' => "Retenção do valor pago (R$ " . number_format($amountPaid, 2, ',', '.') . ") após cancelamento pontual recorrente (Compensação).",
                        'paid_at' => Carbon::now(),
                    ]);
                    $messageFinance = " O valor de R$ " . number_format($amountPaid, 2, ',', '.') . " foi RETIDO no caixa (Compensação).";
                    Log::info("DEBUG FINANCEIRO: Transação de RETENÇÃO (RETEN_CANC_P_FREE) criada com reserva_id=null para ID {$reserva->id}.");
                }
            }


            // 2. Agora atualiza e salva a reserva (dispara o CASCADE DELETE do sinal original)
            $reserva->status = Reserva::STATUS_CANCELADA;
            $reserva->manager_id = Auth::id();
    ...
    public function cancelarSerieRecorrente(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            // 1. Salva a transação de compensação (se for retenção) ANTES do loop de cancelamento.
            // 2. O loop cancela os slots (disparando o CASCADE DELETE em cada um, removendo os sinais).
            ...
            if ($amountPaidForRefund > 0) {
                if ($shouldRefund) {
                    ...
                    FinancialTransaction::create([
                        ...
                        'type' => 'RETEN_CANC_S_FREE', // Tipo diferente para rastrear a compensação
                        'payment_method' => 'retained_funds',
                    ...
                    Log::info("DEBUG FINANCEIRO: Transação de RETENÇÃO (RETEN_CANC_S_FREE) criada com reserva_id=null para série ID {$masterId}.");
                }
            }
