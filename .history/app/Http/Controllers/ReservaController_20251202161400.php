    ...
    public function finalizarPagamento(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            if ($request->boolean('apply_to_series') && !empty($reserva->recurrent_series_id)) {
                ...
                // ✅ CORREÇÃO: Usando a condição OR para garantir que todas as reservas futuras pertencentes à série
                // (seja a mestra ou uma cópia) sejam atualizadas. A restrição de data (> $reservaDate) garante que
                // apenas os slots futuros sejam afetados.
