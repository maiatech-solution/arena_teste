...
class AdminController extends Controller
{
    ...
    public function moverManutencao(Request $request, $id)
    {
        try {
            ...
            // 🔍 CAPTURA DE DADOS PARA O LOG
            $idDaSerieRaw = $reserva->recurrent_series_id;
            $idDaReserva = $reserva->id;

            // Tenta a busca exatamente como o sistema faz
            $proxima = Reserva::where(function ($query) use ($idDaSerieRaw, $idDaReserva) {
                $query->where('recurrent_series_id', $idDaSerieRaw)
                    ->orWhere('id', $idDaSerieRaw);
            })
                ->where('id', '!=', $idDaReserva)
                ->where('date', '>', $reserva->date)
                ->where('status', '!=', 'cancelled')
                ->orderBy('date', 'asc')
                ->first();

            // Escreve no arquivo storage/logs/laravel.log
            \Log::info("--- DEBUG MANUTENÇÃO INTELECTUAL ---");
            \Log::info("RESERVA ATUAL: ID #{$idDaReserva}, Série ID: " . ($idDaSerieRaw ?? 'NULL') . " (Tipo: " . gettype($idDaSerieRaw) . ")");
            \Log::info("BUSCA PRÓXIMA: " . ($proxima ? "ENCONTROU ID #{$proxima->id} para data {$proxima->date}" : "NÃO ENCONTROU NADA"));
            \Log::info("ACTION RECEBIDA: " . $request->input('finance_action'));
            \Log::info("-------------------------------------");

            return response()->json([
                'success' => false,
                'message' => 'DEBUG ATIVO: Verifique o final do arquivo storage/logs/laravel.log'
            ]);
        } catch (\Exception $e) {
