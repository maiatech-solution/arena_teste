    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 1. Guardamos os dados originais nas notas para recuperação posterior
                $dadosOriginais = [
                    'client_name' => $reserva->client_name,
                    'status_anterior' => $reserva->status,
                    'is_fixed_anterior' => $reserva->is_fixed
                ];

                // 2. Tratamento Financeiro (Estorno)
                ...
                $reserva->update([
                    ...
                    'notes' => $reserva->notes . "\n[ORIGINAL_DATA]:" . json_encode($dadosOriginais) . "\nMotivo: " . $motivo,
                    'is_fixed' => true,
                ]);
    ...
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            if ($decisao === 'release_slot') {
                $reserva->delete();
                return redirect()->route('admin.reservas.index')->with('success', 'Horário liberado (vago).');
            }

            // Tenta recuperar dados originais das notas
            preg_match('/\[ORIGINAL_DATA\]:(.*)/', $reserva->notes, $matches);
            $dados = isset($matches[1]) ? json_decode($matches[1], true) : null;

            $reserva->update([
                'status' => $dados['status_anterior'] ?? 'confirmed',
                'client_name' => $dados['client_name'] ?? 'Cliente Recuperado',
                'is_fixed' => $dados['is_fixed_anterior'] ?? false,
                'notes' => str_replace($matches[0] ?? '', '', $reserva->notes) // Limpa a nota técnica
            ]);

            return redirect()->back()->with('success', 'Reserva restaurada para o cliente!');
        } catch (\Exception $e) {
