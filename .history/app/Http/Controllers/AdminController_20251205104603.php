...
class AdminController extends Controller
{
    ...
    public function registerNoShow(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            // 🛑 NOVO: Excluir o sinal original explicitamente, se existir.
            // Isso garante que o valor seja removido do caixa antes de decidirmos o que fazer.
    ...
    public function cancelarReserva(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            if ($amountPaid > 0) {
                if ($shouldRefund) {
                     // 2.1 Estorno: Cria uma transação negativa (saída do caixa). O sinal já saiu (pelo delete acima),
                     // então esta é a prova de que o estorno foi processado.
                    FinancialTransaction::create([
                        'reserva_id' => $reserva->id,
    ...
    public function cancelarReservaRecorrente(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            // 2. 🛑 FLUXO: Mudar status, disparar CASCADE DELETE.
            $reserva->status = Reserva::STATUS_CANCELADA;
            $reserva->manager_id = Auth::id();
    ...
    public function indexFinancialDashboard(Request $request) // 🛑 RENOMEADO: De indexPagamentos para indexFinancialDashboard
    {
        ...
        $totalReceived = FinancialTransaction::whereDate('paid_at', $date)
            // Tipos POSITIVOS (Entrada no caixa). Adicionado o novo tipo de NO-SHOW de retenção
            ->whereIn('type', ['signal', 'full_payment', 'partial_payment', 'RETEN_CANC_COMP', 'RETEN_CANC_P_COMP', 'RETEN_CANC_S_COMP', 'RETEN_NOSHOW_COMP'])
            ->sum('amount');
