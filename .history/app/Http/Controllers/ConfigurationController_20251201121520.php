...
class ConfigurationController extends Controller
{
    ...
    public function index()
    {
        ...
        $fixedReservas = Reserva::where('is_fixed', true)
            ...
            // 🛑 CORRIGIDO: Status de Slot Livre e Slot Cancelado (Manutenção)
            ->whereIn('status', [Reserva::STATUS_FREE, Reserva::STATUS_CANCELADA])
            ->orderBy('date')
            ->orderBy('start_time')
            ->limit(50)
            ->get();
    ...
    public function toggleFixedReservaStatus(Request $request, $id)
    {
        ...
        try {
            // ✅ CORREÇÃO CRÍTICA: Validação agora usa as STRINGS LITERAIS que o JS envia.
            // Os valores são 'confirmed' e 'cancelled' (status que o JS usa no toggle).
            ...
            $newStatus = $request->status; // Será 'confirmed' ou 'cancelled'

            if ($reserva->client_name === 'Slot Fixo de 1h') {
                $reserva->client_name = 'Slot Editado (Gestor: ' . Auth::user()->name . ')';
            }

            // 🛑 CRÍTICO: Mapeamento de Status Front-end para Back-end/DB
            // JS envia 'confirmed' para DISPONIBILIZAR, mas o DB usa 'free' para isso.
            // JS envia 'cancelled' para INDISPONIBILIZAR, e o DB usa 'cancelled' para isso.

            if ($newStatus === 'confirmed') {
                 $finalStatus = Reserva::STATUS_FREE; // Valor real para "Disponível" no banco ('free')
            } else {
                 $finalStatus = Reserva::STATUS_CANCELADA; // Valor real para "Indisponível" no banco ('cancelled')
            }

            $reserva->manager_id = Auth::id();
            $reserva->status = $finalStatus; // Atualiza com a constante correta do DB
            $reserva->save();
