...
class AdminController extends Controller
{
        ...
        try {
            ...
            if ($signalValue > 0) {
                FinancialTransaction::create([
                    ...
                    // 🛑 CORREÇÃO DE FUSO HORÁRIO: Força o uso do fuso horário brasileiro (UTC-3) ao gravar.
                    'paid_at' => Carbon::now('America/Sao_Paulo'),
                ]);
            }
    ...
    public function indexFinancialDashboard(Request $request) // 🛑 RENOMEADO: De indexPagamentos para indexFinancialDashboard
    {
        ...
        // --- 🛑 CRÍTICO: Cálculo correto dos limites de data para WHEREBETWEEN ---
        // O Carbon::parse com o timezone do Model deve garantir que $startDate e $endDate
        // estejam no fuso da aplicação, mas apontando para o dia completo no DB (em UTC).
        // A CRIAÇÃO da data no DB usa America/Sao_Paulo. A consulta deve ser robusta.
        $startDate = Carbon::parse($selectedDate)->startOfDay();
        $endDate = Carbon::parse($selectedDate)->endOfDay();
        // ----------------------------------------------------------------------

        // 🛑 NOVO LOG DE DEBUG PARA CONFIRMAR LIMITES DA CONSULTA
        Log::info("DEBUG FINANCEIRO: Filtro de Data Aplicado: {$selectedDate}. Start: {$startDate->toDateTimeString()}, End: {$endDate->toDateTimeString()}");
        // 🛑 FIM NOVO LOG DE DEBUG
        ...
        // 3. Busca todas as transações financeiras do dia para auditoria na view
        // ✅ CORREÇÃO: Usando whereBetween no paid_at
        // 🛑 CRÍTICO: Garantir que a consulta de paid_at respeite o fuso horário.
        $financialTransactions = FinancialTransaction::whereBetween('paid_at', [$startDate, $endDate])
            ->orderBy('paid_at', 'asc') // Ordena por data/hora para ver a ordem dos eventos
            ->get();

        // 🛑 NOVO LOG PARA DIAGNÓSTICO DO PROBLEMA DA TABELA VAZIA
        Log::info("DEBUG FINANCEIRO: Transações carregadas para o dia (Contagem): {$financialTransactions->count()}.");

        // 3.1 Total Recebido HOJE (Cash in Hand - Saldo Líquido Total do dia)
        // ✅ CÁLCULO COERENTE: Calcula a soma a partir da coleção JÁ FILTRADA.
        $totalReceived = $financialTransactions->sum('amount');

        // 🛑 CORREÇÃO CRÍTICA: Total de Sinais Recebidos HOJE (KPI de Entradas Brutas)
        // Soma todas as transações de entrada (amount > 0) para o dia.
        // Isso garante que o KPI reflita R$ 250,00, o total de todas as entradas positivas.
        $totalSignalsToday = FinancialTransaction::whereBetween('paid_at', [$startDate, $endDate])
            ->where('amount', '>', 0)
            ->sum('amount');
        // 🛑 FIM DA CORREÇÃO
        ...
        Log::info("DEBUG FINANCEIRO: Consulta Movimentação Detalhada. Data: {$selectedDate}. Transações encontradas: {$financialTransactions->count()}");
        $financialTransactions->each(function ($transaction) {
            // Usando toDateTimeString() para garantir que a hora exibida esteja no fuso do Laravel/Model (America/Sao_Paulo)
            $paidAtString = $transaction->paid_at ? $transaction->paid_at->toDateTimeString() : 'N/A';
            Log::info("DEBUG DETALHE: ID: {$transaction->id}, Reserva: {$transaction->reserva_id}, Tipo: {$transaction->type}, Valor: {$transaction->amount}, Paid_at: {$paidAtString}");
        });
        // 🛑 FIM NOVO LOG DE DEBUG
        ...
        foreach ($activeReservas as $reserva) {
            $totalExpected += $reserva->final_price ?? $reserva->price; // ✅ CORRIGIDO: Usa final_price se existir
            $totalPaidBySignals += $reserva->total_paid;
        }

        // 🛑 CORREÇÃO CRÍTICA: Ajuste no Cálculo Pendente
        // O valor pago ($totalPaidBySignals) já inclui o pagamento final (R$ 100),
        // o sinal (R$ 50), e todos os outros pagamentos.
        ...
        // ✅ NOVO KPI: Contagem de Reservas Pagas (RESERVAS)
        $paidReservaCount = Reserva::whereDate('date', $date)
            ->where('is_fixed', false)
            ->whereIn('payment_status', ['paid', 'completed']) // Status de pagamento (totalmente pago)
            ->whereIn('status', [Reserva::STATUS_CONFIRMADA, Reserva::STATUS_COMPLETED]) // Apenas reservas ativas ou concluídas
            ->count();

        // 4. Saldo Total (Global) - Agora correto e separado para o KPI global.
        $totalGlobalBalance = $this->calculateTotalBalance(); // 🛑 RENOMEADO no método para refletir a variável de saída.

        // 🛑 LOG FINAL: Exibe exatamente os valores passados para a view
        Log::info("DEBUG FINANCEIRO: KPIs FINAIS para View - Data: {$date} | Saldo Líquido HOJE (\$totalReceived): R$ " . number_format($totalReceived, 2, ',', '.') . " | Sinais HOJE (\$totalSignalsToday): R$ " . number_format($totalSignalsToday, 2, ',', '.') . " | Saldo GLOBAL (\$totalGlobalBalance): R$ " . number_format($totalGlobalBalance, 2, ',', '.'));


        return view('admin.payment.index', [ // 🛑 CORREÇÃO DA VIEW: Usando admin.payment.index
            'reservas' => $reservasQuery, // Tabela de agendamentos (inclui canceladas e no_show)
            'financialTransactions' => $financialTransactions, // 🛑 Transações para auditoria
            'selectedDate' => $selectedDate,
            'highlightReservaId' => $reservaId, // Para destacar linha se vier do calendário

            // KPIs para a view
            'totalReceived' => $totalReceived, // Saldo Líquido do Dia (Entradas - Saídas)
            // ✅ CORREÇÃO APLICADA: Usa o totalSignalsToday CALCULADO
            'totalSignalsToday' => $totalSignalsToday, // Sinais/Entradas Reais (Valor POSITIVO do dia)
            ...
            'paidReservaCount' => $paidReservaCount, // ✅ NOVO KPI: Reservas Pagas

            // Variáveis globais/de filtro
            'pageTitle' => 'Gerenciamento de Caixa & Pagamentos',
            'search' => $search,
            'totalGlobalBalance' => $totalGlobalBalance, // Saldo acumulado total
        ]);
    }
}
