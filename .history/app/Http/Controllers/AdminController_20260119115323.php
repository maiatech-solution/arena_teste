...
class AdminController extends Controller
{
     ...
     * Ajustado para recriar o inventário (slot vago) ao liberar horário.
     */
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            // --- CASO 1: LIBERAR HORÁRIO (DEIXAR VAGO PARA OUTRO CLIENTE) ---
            if ($decisao === 'release_slot') {
                DB::beginTransaction();

                // 1. Recria o slot fixo vago para que o horário volte a aparecer como "Disponível" no calendário
                // Isso usa o helper do ReservaController que você já tem injetado
                $this->reservaController->recreateFixedSlot($reserva);

                // 2. Remove o bloqueio de manutenção
                $reserva->delete();

                DB::commit();

                return redirect()->route('admin.reservas.index', $routeParams)
                    ->with('success', '✅ Manutenção finalizada. O horário agora está disponível para reserva!');
            }
            ...
            // Segurança: Se cair aqui sem decisão válida, libera o slot e apaga
            $this->reservaController->recreateFixedSlot($reserva);
            $reserva->delete();
