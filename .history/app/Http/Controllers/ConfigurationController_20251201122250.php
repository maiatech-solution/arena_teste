...
use Illuminate\Support\Facades\Validator; // Adicionado para garantia
...
class ConfigurationController extends Controller
{
    ...
    public function index()
    {
        ...
        // ✅ CORREÇÃO: Removemos o filtro is_fixed e o filtro de status para trazer o calendário completo.
    ...
    public function toggleFixedReservaStatus(Request $request, $id)
    {
        ...
        try {
            ...
            // Se for reserva de cliente, aceitamos a ação, mas apenas mudamos o status.
            // Para reservas fixas, garantimos que is_fixed seja true
            if (!$reserva->is_fixed && $reserva->status !== Reserva::STATUS_PENDENTE && $reserva->status !== Reserva::STATUS_CONFIRMADA) {
            ...
            $newStatus = $request->status; // Será 'confirmed' (Disponibilizar) ou 'cancelled' (Manutenção)

            // 🛑 CRÍTICO: Mapeamento de Status
            if ($reserva->is_fixed) {
                // Para slots fixos: 'confirmed' (JS) => 'free' (DB); 'cancelled' (JS) => 'cancelled' (DB)
                $finalStatus = $newStatus === 'confirmed' ? Reserva::STATUS_FREE : Reserva::STATUS_CANCELADA;

                if ($reserva->client_name === 'Slot Fixo de 1h') {
                    $reserva->client_name = 'Slot Editado (Gestor: ' . Auth::user()->name . ')';
                }
            } else {
                // Para reservas de clientes (is_fixed=false):
                // Ação de "cancelar" (JS: 'cancelled') significa que o GESTOR está cancelando a reserva do cliente.
                if ($newStatus === 'cancelled') {
                    $finalStatus = Reserva::STATUS_CANCELADA;
                    // Opcional: registrar motivo no cancellation_reason
                    $reserva->cancellation_reason = 'Cancelamento forçado pelo gestor via tela de Configuração/Manutenção.';
                } else {
                     // Não se pode "disponibilizar" (JS: 'confirmed') uma reserva de cliente já existente.
                     // Deixamos o status original (confirmed/pending).
            ...
            $action = $newStatus === 'confirmed' ? 'disponibilizado' : 'marcado como indisponível (manutenção)';
            ...
            Log::error("Erro fatal ao alternar status da reserva fixa #{$id}: " . $e->getMessage());
            return response()->json(['success' => false, 'error' => 'Erro interno do servidor.'], 500);
        }
    }
