...
class ReservaController extends Controller
{
    ...
    public function getConfirmedReservations(Request $request)
    {
        ...
        // CORREÇÃO CRÍTICA: Inclui APENAS os status de reserva ativos e concluídos.
        // O filtro estava incorretamente incluindo status de pagamento ('paid', 'partial', 'unpaid')
        // e excluindo reservas não recorrentes.
        $visibleStatuses = [
        ...
        $reservations = Reserva::whereIn('status', $visibleStatuses)
            ...
            ->where('is_cancelled', false) // Garante que canceladas não apareçam
    ...
    protected function mapToFullCalendarEvents($reservations)
    {
        ...
        foreach ($reservations as $reserva) {
            ...
            // Lógica de is_paid
            // A reserva está paga/concluída SE o status for CONCLUIDA OU o payment_status for paid
            ...
            // Estilo Pendente (Sobrescreve cor e nome, não é considerado pago)
            ...
            // Se o status for CONCLUIDA (Pago), mantemos a cor base (recorrente/avulsa)
            // e dependemos exclusivamente da flag is_paid para o efeito de desbotamento no frontend.
