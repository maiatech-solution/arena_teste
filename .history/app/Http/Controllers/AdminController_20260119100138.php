...
class AdminController extends Controller
{
    ...
    public function reativarManutencao(Request $request, $id)
    {
        try {
            ...
            // OPÇÃO A: Liberar horário (Deixa o slot livre/vago)
            if ($decisao === 'release_slot') {
                $reserva->delete();
                return redirect()->route('admin.reservas.index')
                    ->with('success', 'Manutenção removida. Horário vago no calendário!');
            }

            // OPÇÃO B: Restaurar Cliente
            // Procuramos o backup nas notas
            if (preg_match('/\[BACKUP_DATA:(.*?)\]/', $reserva->notes, $matches)) {
                $dados = json_decode($matches[1], true);

                $reserva->update([
                    'client_name' => $dados['name'] ?? 'Cliente Recuperado',
                    'status'      => 'confirmed', // Garante que volta como confirmado
                    'is_fixed'    => false,       // 🚨 IMPORTANTE: False para ser reserva ativa, não bloqueio
                    'notes'       => trim(preg_replace('/\[BACKUP_DATA:.*?\]/', '', $reserva->notes))
                ]);

                return redirect()->route('admin.reservas.show', $reserva->id)
                    ->with('success', 'Reserva restaurada e visível no calendário!');
            }

            // Se não houver backup (bloqueio manual), apenas deleta para liberar o slot
            $reserva->delete();
            return redirect()->route('admin.reservas.index')
