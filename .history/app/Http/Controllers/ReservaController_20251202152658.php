...
class ReservaController extends Controller
{
    ...
    /**
     * ✅ NOVO MÉTODO: Atualiza o preço de UMA RESERVA DE SÉRIE recorrente e as futuras.
     * Este método é chamado pelo AdminController se a reserva for recorrente.
     *
     * @param \Illuminate\Http\Request $request
    ...
    {
        // 1. Validação dos dados (reutiliza a validação do AdminController)
        $validated = $request->validate([
        ...
        DB::beginTransaction();
        try {
            // 2. Atualiza o preço para a reserva atual e TODAS as reservas futuras na série.
            // A reserva atual pode estar no passado ou futuro, mas aplicamos a mudança
            // para todas as reservas ativas da série a partir de HOJE.
            ...
            if ($slotsToUpdate->isEmpty()) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Nenhuma reserva futura ativa encontrada para esta série. O preço não foi alterado.',
                ], 404);
            }
            ...
            foreach ($slotsToUpdate as $slot) {
                // Apenas atualiza se o preço for diferente
                if ((float)$slot->price !== (float)$newPrice) {
                    $slot->price = $newPrice;
                    $slot->save();
                    $updatedCount++;
                }
            ...
            if ($updatedCount === 0) {
                 ...
                 return response()->json([
                    'success' => false,
                    'message' => 'O novo preço é o mesmo que o preço atual em todos os slots futuros da série. Nenhuma alteração feita.',
                 ], 400);
            }

            // 3. Registrar a auditoria da mudança de preço
            Log::info("Preço da Série Recorrente MASTER ID {$masterId} alterado de R$ {$oldPrice} para R$ {$newPrice} (em {$updatedCount} slots futuros) por " . auth()->user()->name . ". Justificativa: {$justification}");
            ...
            DB::rollBack();
            Log::error("Erro ao processar a alteração de preço para série recorrente MASTER ID {$masterId}: " . $e->getMessage(), ['exception' => $e]);
            return response()->json([
                'success' => false,
                'message' => 'Erro ao processar a alteração de preço na série recorrente: ' . $e->getMessage(),
            ], 500);
        }
    }
