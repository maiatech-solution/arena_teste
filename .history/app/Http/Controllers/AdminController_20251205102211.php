...
class AdminController extends Controller
{
    ...
    public function cancelarReserva(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            $reserva->save(); // <--- O CASCADE DELETE da transação 'signal' original acontece AQUI, removendo a entrada de R$ 50,00.

            // 1. Gera Transação Financeira: Estorno OU Retenção (APÓS o CASCADE DELETE)
            if ($amountPaid > 0) {
                if ($shouldRefund) {
                     // 1.1 Estorno: Cria uma transação negativa (saída do caixa). O sinal já saiu,
                    ...
                    // 1.2 Retenção: Cria a transação POSITIVA para COMPENSAR o R$ 50,00 que foi removido
                    // pelo CASCADE DELETE.

                    FinancialTransaction::create([
                        'reserva_id' => $reserva->id, // 🛑 CRÍTICO: Voltamos a usar o ID, mas só DEPOIS do CASCADE DELETE
