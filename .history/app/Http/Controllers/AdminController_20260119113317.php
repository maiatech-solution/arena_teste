...
class AdminController extends Controller
{
     ...
     * Ajustado para gerar link de WhatsApp na sessão (Card Verde).
     */
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                if ($reserva->total_paid > 0 && $action === 'refund') {
                    $totalEstornado = $reserva->total_paid;
                ...
                // 📱 4. GERAÇÃO DO LINK WHATSAPP (Para exibir o card verde no topo)
                $waLink = null;
                if ($reserva->client_contact) {
                    ...
                    $clienteOriginal = json_decode(explode('BACKUP_DATA:', $backupInfo)[1] ?? '{}', true)['name'] ?? 'Cliente';
                    ...
                    if ($totalEstornado > 0) {
                        ...
                        $mensagem .= "\n\nRealizamos o estorno de *R$ {$valorFormatado}* no nosso sistema. Por favor, envie sua *CHAVE PIX* para completarmos a devolução.";
                    }

                    $waLink = "https://wa.me/55{$phone}?text=" . urlencode($mensagem);

                    // IMPORTANTE: Coloca na sessão para o Blade ler
