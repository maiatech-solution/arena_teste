        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                if ($valorOriginal > 0 && $action === 'transfer') {
                    ...
                    if ($proximaReserva) {
                        // Atualiza via Query Builder para garantir gravação física
                        ...
                        // Vincula a transação financeira
                ...
                // Move a reserva atual para manutenção
                $reserva->update([
                    'status' => 'maintenance',
                    ...
                    'signal_value' => 0,
                    'is_fixed' => true,
                    'notes' => "[TRANSFERIDO R$ {$valorOriginal}] " . ($reserva->notes ?? '')
                ]);
