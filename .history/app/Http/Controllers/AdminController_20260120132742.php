    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 1. BACKUP dos dados (importante para o sistema de reativação que você já tem)
                $backupData = [
                ...
                if ($valorOriginal > 0 && $action === 'transfer') {
                    ...
                    // A) Atualiza o campo total_paid na reserva de destino
                    $proximaReserva->total_paid += $valorOriginal;
                    // Atualiza o status financeiro da próxima reserva
                    $proximaReserva->payment_status = ($proximaReserva->total_paid >= $proximaReserva->final_price) ? 'paid' : 'partial';
                    $proximaReserva->save();

                    // B) MOVE AS TRANSAÇÕES NO BANCO (Isso resolve o seu Caixa do dia 28)
                    // Alteramos o reserva_id e a data (paid_at) para que o faturamento caia no dia 28
                    FinancialTransaction::where('reserva_id', $reserva->id)
                        ->update([
                            'reserva_id' => $proximaReserva->id,
                            'paid_at'    => $proximaReserva->date, // Faz o dinheiro "aparecer" no dia 28
                            'description' => DB::raw("CONCAT(description, ' (Transferido via Manutenção da Reserva #{$reserva->id})')")
                        ]);

                    // C) Zera os valores da reserva que vai para manutenção
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Manutenção aplicada e financeiro atualizado!'
                ]);
            } catch (\Exception $e) {
