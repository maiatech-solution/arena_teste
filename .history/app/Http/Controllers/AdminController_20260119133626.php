        ...
        try {
            ...
            if ($decisao === 'release_slot' || empty($decisao)) {
                ...
                try {
                    ...
                    // 3. Recriamos o slot vago usando a instância global do ReservaController
                    // Isso garante que o método recreateFixedSlot() seja executado sem erros de dependência
                    app(\App\Http\Controllers\ReservaController::class)->recreateFixedSlot($backupData);

                    DB::commit();

                    \Log::info("MANUTENÇÃO: Horário {$backupData->start_time} na Arena {$backupData->arena_id} liberado com sucesso.");
