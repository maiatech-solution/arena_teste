...
class ReservaController extends Controller
{
    ...
    public function confirmar(Request $request, Reserva $reserva)
    {
        ...
        // 🛑 CORREÇÃO CRÍTICA: Checar explicitamente se o valor é a string '1' (do checkbox)
        $isRecurrent = ($validated['is_recurrent'] ?? '0') === '1';
        $signalValue = (float)($validated['signal_value'] ?? 0.00);

        // 🚨 NOVO LOG DE DIAGNÓSTICO CRÍTICO
        Log::critical("DIAGNÓSTICO RECORRÊNCIA: isRecurrent (bool): " . ($isRecurrent ? 'TRUE' : 'FALSE') . ", Raw Validated: " . ($validated['is_recurrent'] ?? 'null'));
