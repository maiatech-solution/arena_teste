...
class AdminController extends Controller
{
    ...
    public function indexPagamentos(Request $request)
    {
        ...
        // Query de transações (aplica filtros)
        $transactions = FinancialTransaction::query()
            ...
            ->when($search, function ($query, $search) {
                // Procura na descrição ou em campos relevantes
                return $query->where(function ($q) use ($search) {
            ...
            // A ordenação é crucial para relatórios
        ...
        // 🛑 Loga os tipos de transação que estão sendo incluídos para debug
        $transactionTypes = FinancialTransaction::select('type')->distinct()->pluck('type');
        Log::info("DEBUG FINANCEIRO: Tipos de Transação Existentes: " . $transactionTypes->implode(', '));

        return view('admin.financial.index', [ // Assume que a view é admin.financial.index
            'transactions' => $transactions,
            'totalBalance' => $totalBalance,
            'pageTitle' => 'Relatório de Caixa e Pagamentos',
            'search' => $search,
            'startDate' => $startDate,
            'endDate' => $endDate,
