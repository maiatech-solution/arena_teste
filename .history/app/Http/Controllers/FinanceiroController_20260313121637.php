...
class FinanceiroController extends Controller
{
    ...
    public function relatorioCancelamentos(Request $request)
    {
        ...
        // 1. Busca as Reservas com status de perda
        ...
        // 🎯 SEPARAÇÃO DE MÉTRICAS PARA OS CARDS (Bater com o Dashboard)
        $faltasApenas = $cancelamentos->where('status', Reserva::STATUS_NO_SHOW);
        $countFaltas = $faltasApenas->count(); // Aqui dará "2" (André e Vitor)

        $rejeitadasApenas = $cancelamentos->where('status', Reserva::STATUS_REJEITADA);
        $countRejeitadas = $rejeitadasApenas->count(); // Aqui dará "1" (Everton)

        // 2. BUSCA AS MULTAS NO FINANCEIRO
        ...
        // 💰 CÁLCULO FINANCEIRO DO PREJUÍZO DE FALTAS
        // (Preço dos horários que ficaram vazios - o que foi retido no caixa)
        $prejuizoFaltasReal = $faltasApenas->sum('price') + $valorMultasFinanceiro;
        ...
        return view('admin.financeiro.cancelamentos', compact(
            ...
            'multasAvulsas',
            'countFaltas',      // Enviando a contagem limpa
            'countRejeitadas',  // Enviando a contagem limpa
            'prejuizoFaltasReal'
        ));
    }
