...
class ReservaController extends Controller
{
    ...
    public function storeQuickReservaApi(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'payment_method' => 'required|string', // 🛡️ Campo Vital
        ...
        // 🔥 O PULO DO GATO: Garantir que o método de pagamento não seja genérico
        // Se vier 'outro' ou vazio, forçamos 'dinheiro' ou algo mapeado para evitar divergência
        if (in_array(strtolower($validated['payment_method']), ['outro', 'outros', 'ajuste', ''])) {
            $validated['payment_method'] = 'dinheiro'; // Padronização interna
        }
        ...
        try {
            // 🚀 CRIAÇÃO DA RESERVA
            // Passamos o $validated['payment_method'] para que a transação de sinal use o método correto
            $newReserva = $this->createConfirmedReserva($validated, $clientUser, null);

            // Força a Arena correta
