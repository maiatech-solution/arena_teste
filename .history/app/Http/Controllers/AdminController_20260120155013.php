...
class AdminController extends Controller
{
     ...
     * Ajustado para respeitar a decisão de ESTORNO ou CRÉDITO.
     */
        ...
        try {
            ...
            if ($decisao === 'restore_client') {
                if (preg_match('/###BACKUP###(.*?)###END###/s', $reserva->notes, $matches)) {
                    ...
                    // 🧠 O SEGREDO: Recuperamos o que foi feito no momento do bloqueio
                    $valorOriginal = (float) ($dados['total_paid_orig'] ?? 0);
                    $acaoRealizada = $dados['finance_action'] ?? 'refund'; // Padrão é estorno se não achar
                    ...
                    if ($valorOriginal > 0) {
                        // 🎯 SÓ USA A FRASE DE MENSALISTA SE VOCÊ REALMENTE TRANSFERIU O CRÉDITO
                        ...
                        // 💰 SE VOCÊ ESTORNOU (MESMO SENDO RECORRENTE), USA ESTA:
                        else {
