...
class AdminController extends Controller
{
    ...
    public function indexFinancialDashboard(Request $request) // 🛑 RENOMEADO: De indexPagamentos para indexFinancialDashboard
    {
        ...
        // 🛑 CORREÇÃO CRÍTICA: Total de Sinais Recebidos HOJE (Calculado diretamente do DB para robustez)
        // Usa a mesma lógica de whereBetween para garantir consistência.
        // Soma apenas as transações de ENTRADA (type: 'signal' ou 'payment')
        $totalSignalsToday = FinancialTransaction::whereBetween('paid_at', [$startDate, $endDate])
            ->whereIn('type', ['signal', 'payment'])
            ->where('amount', '>', 0) // Garante que seja apenas entrada (exceto compensações de retenção)
            ->sum('amount');
        // 🛑 FIM DA CORREÇÃO
