...
class AdminController extends Controller
{
    ...
    {
        // 1. Validação dos dados (Adicionamos o campo 'scope')
        $validated = $request->validate([
            'new_price'     => 'required|numeric|min:0',
            'justification' => 'required|string|min:5',
            'scope'         => 'nullable|string|in:single,series', // ✅ Novo campo
        ], [
            'new_price.required'     => 'O novo preço é obrigatório.',
            'justification.required' => 'A justificativa é obrigatória.',
        ]);

        // 🛡️ TRAVA DE SEGURANÇA PARA O CAIXA
        // Se a reserva já tiver qualquer pagamento (total ou parcial), bloqueia a alteração.
        if (in_array($reserva->payment_status, ['paid', 'partial'])) {
            return response()->json([
                'success' => false,
                'message' => '🛑 Bloqueio de Segurança: Esta reserva já possui pagamentos registrados. Para mudar o valor, você precisaria estornar o pagamento no caixa primeiro.',
            ], 403);
        }
        ...
        try {
            ...
            $scope = $request->input('scope', 'single'); // Padrão é 'single'

            if ($oldPrice === $newPrice) {
                return response()->json([
                    'success' => false,
                    'message' => 'O preço novo é igual ao valor atual.',
                ], 400);
            }

            $adminName = auth()->user()->name;

            // 🔄 LÓGICA DE ATUALIZAÇÃO (PONTUAL OU SÉRIE)
            if ($scope === 'series' && $reserva->recurrent_token) {
                // Atualiza todas as reservas FUTURAS da série que ainda estão pendentes de pagamento
                $affectedCount = \App\Models\Reserva::where('recurrent_token', $reserva->recurrent_token)
                    ->where('date', '>=', $reserva->date)
                    ->where('payment_status', 'pending') // Garante que não mexe em nada já pago na série
                    ->update(['price' => $newPrice]);

                Log::info("Preço em SÉRIE (Token: {$reserva->recurrent_token}) alterado para R$ {$newPrice} por {$adminName}. Justificativa: {$justification}");

                $msg = "Preço de toda a série ({$affectedCount} reservas) atualizado com sucesso!";
            } else {
                // Atualiza apenas esta reserva específica
                $reserva->price = $newPrice;
                $reserva->save();

                Log::info("Preço da Reserva ID {$reserva->id} alterado de R$ {$oldPrice} para R$ {$newPrice} por {$adminName}. Justificativa: {$justification}");

                $msg = "Preço da reserva atualizado com sucesso!";
            }
