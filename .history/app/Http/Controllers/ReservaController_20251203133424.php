    ...
    public function finalizarPagamento(Request $request, Reserva $reserva)
    {
        ...
        try {
            ...
            if ($request->boolean('apply_to_series') && $reserva->is_recurrent) {
                ...
                // Adiciona um log para checar o valor original da série
                $oldPrice = Reserva::where(function ($query) use ($masterId) {
                       $query->where('recurrent_series_id', $masterId)
                             ->orWhere('id', $masterId);
                   })
                   ->whereDate('date', '>', $reservaDate)
                   ->value('price');
                Log::debug("Preço original das futuras reservas (amostra): R$ {$oldPrice}");
                Log::debug("Novo preço a aplicar: R$ {$newPriceForSeries}");


                // CRÍTICO: Atualiza todas as reservas futuras (data estritamente MAIOR)
                // Adicionamos a condição para garantir que o preço realmente mudou antes de atualizar.
                if ($newPriceForSeries != $oldPrice) {
                    ...
                    Log::info("Propagação ignorada: O novo preço é o mesmo que o preço original nas reservas futuras.");
                    $updatedCount = 0;
                }
