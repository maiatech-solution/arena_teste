...
class AdminController extends Controller
{
     ...
     * Ajustado para gerar link de WhatsApp pedindo CHAVE PIX para estorno.
     */
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                if ($reserva->client_contact) {
                    ...
                    // Recupera o nome do backup para a mensagem
                    ...
                    // 🚨 ADICIONA O PEDIDO DA CHAVE PIX SE HOUVE ESTORNO
                    if ($totalEstornado > 0) {
                        ...
                        $mensagem .= "\n\nO horário foi liberado no sistema e não identificamos pagamentos pendentes de estorno.";
                    }

                    $waLink = "https://wa.me/55{$phone}?text=" . urlencode($mensagem);

                    // Coloca na sessão para o card verde aparecer no topo da View
                    session()->flash('whatsapp_link', $waLink);
                }

                return response()->json(['success' => true, 'message' => 'Horário bloqueado e estorno registrado no caixa!']);
            } catch (\Exception $e) {
