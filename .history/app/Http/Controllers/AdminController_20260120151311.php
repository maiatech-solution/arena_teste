    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 1. BACKUP (Marcação para reativação)
                $backupData = [
                    'name' => $nomeOriginal,
                ...
                // 3. SE NÃO TRANSFERIU, FAZ ESTORNO
                ...
                // --- 🚀 NOVO: GERAÇÃO DO LINK WHATSAPP ---
                $msg = "Olá *{$nomeOriginal}*! Informamos que seu horário no dia " . date('d/m', strtotime($reserva->date)) . " às " . date('H:i', strtotime($reserva->start_time)) . "h precisou ser bloqueado por: {$motivo}.";

                if ($transferenciaSucesso) {
                    $msg .= " Seu crédito de R$ " . number_format($valorOriginal, 2, ',', '.') . " foi transferido para o seu próximo jogo.";
                } else if ($valorOriginal > 0) {
                    $msg .= " O valor pago de R$ " . number_format($valorOriginal, 2, ',', '.') . " foi estornado no sistema.";
                }

                $waLink = "https://wa.me/55" . preg_replace('/\D/', '', $contatoOriginal) . "?text=" . urlencode($msg);

                // Retornamos o link para o JavaScript capturar antes do reload
                return response()->json([
                    'success' => true,
        ...
        try {
            ...
            if ($decisao === 'restore_client') {
                if (preg_match('/###BACKUP###(.*?)###END###/s', $reserva->notes, $matches)) {
                    ...
                    // Atualiza a reserva com os dados recuperados
                    $reserva->update([
                    ...
                    // 🚀 GERAR LINK PARA REATIVAÇÃO (WhatsApp)
                    $dataFormatada = date('d/m', strtotime($reserva->date));
                    $horaFormatada = date('H:i', strtotime($reserva->start_time));
                    $nomeCliente   = $dados['name'] ?? 'Cliente';

                    $msgReativar = "Boas notícias *{$nomeCliente}*! A manutenção da quadra foi concluída e seu horário no dia {$dataFormatada} às {$horaFormatada}h está *REATIVADO*! Te aguardamos.";

                    // Limpa o contato para o link wa.me
                    $telefoneLimpo = preg_replace('/\D/', '', $dados['contact'] ?? '');
                    $waLink = "https://wa.me/55{$telefoneLimpo}?text=" . urlencode($msgReativar);

                    // Retorna para a tela de detalhes com o link na sessão
