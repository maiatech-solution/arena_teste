    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 💡 AJUSTE: Aceita 'credit' (visto no log) ou 'transfer'
                ...
                // 🎯 Se houver valor e a ação for 'credit' (transferir crédito)
                if ($valorOriginal > 0 && ($action === 'transfer' || $action === 'credit')) {
                    $idDaSerie = $reserva->recurrent_series_id ?? $reserva->id;

                    // Busca a próxima (conforme o log confirmou que encontra #573)
                    ...
                    if ($proxima) {
                        // Atualiza a próxima reserva (SQL Puro para garantir persistência)
                        ...
                        // Move a transação financeira (Mantém o dinheiro no caixa de hoje)
                ...
                // 3. SE NÃO TRANSFERIU, FAZ ESTORNO (Garante que o caixa não fique "sujo")
                ...
                // 4. ATUALIZA PARA MANUTENÇÃO (Sem apagar o contato para o WhatsApp)
                ...
                return response()->json(['success' => true, 'message' => $transferenciaSucesso ? 'Crédito transferido!' : 'Manutenção aplicada!']);
            } catch (\Exception $e) {
