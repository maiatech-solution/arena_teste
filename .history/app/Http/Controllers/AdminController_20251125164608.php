...
use App\Models\FinancialTransaction; // << GARANTINDO A IMPORTAÇÃO
...
class AdminController extends Controller
{
    ...
    // ------------------------------------------------------------------------
    // FUNÇÃO HELPER: TRATAR VALORES MONETÁRIOS (CRÍTICA)
    // ------------------------------------------------------------------------
    private function cleanAndConvertMoney($value)
    {
        if (is_null($value) || $value === '') return 0.00;
        // 1. Remove separadores de milhar (pontos)
        $value = str_replace('.', '', $value);
        // 2. Troca vírgula por ponto decimal
        $value = str_replace(',', '.', $value);
        // 3. Converte para float e garante que é um número (fallback para 0)
    ...
    {
        // Alterado price e signal_value para 'string' na validação para aceitar vírgulas
        $validated = $request->validate([
            ...
            'signal_value' => 'nullable|string|max:255',
        ]);
        ...
        // Aplica a limpeza e conversão: CRÍTICO
        $price = $this->cleanAndConvertMoney($validated['price']);
        $sinal = $this->cleanAndConvertMoney($validated['signal_value'] ?? '0.00');
        $totalPaid = $sinal;

        // LOG DE ALTA PRIORIDADE PARA DEBUG
        Log::info("STORE_MANUAL_CONVERSION: Valores convertidos. Preço: {$price}, Sinal: {$sinal}, Total Pago: {$totalPaid}");
        ...
        try {
            ...
            $reserva = Reserva::create([
                ...
                'is_recurrent' => false,
                'manager_id' => Auth::id(),
                'signal_value' => $sinal,        // Salva o sinal
                'total_paid' => $totalPaid,      // Salva o total pago inicial
                'payment_status' => $paymentStatus, // Salva o status de pagamento
            ]);

            // CRÍTICO: Geração da Transação Financeira (Entrada no Caixa) se houver sinal
            if ($sinal > 0) {
            ...
            DB::rollBack();
            Log::error("STORE_MANUAL: Erro ao criar reserva manual ou transação.", ['exception' => $e, 'data' => $validated]);
