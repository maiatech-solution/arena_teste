    ...
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 1. BACKUP para reativação futura
                $backupData = [
                ...
                // 2. TRANSFERÊNCIA (O Log confirmou que a próxima é a ID + 1 da série)
                if ($valorOriginal > 0 && $action === 'transfer') {
                    ...
                    if ($proximaReserva) {
                        // A) Atualizamos a reserva do dia 28 (Destino)
                        // Usamos Query Builder para garantir que o banco escreva o valor AGORA
                        DB::table('reservas')->where('id', $proximaReserva->id)->update([
                            'total_paid' => DB::raw("total_paid + $valorOriginal"),
                            'signal_value' => DB::raw("signal_value + $valorOriginal"),
                            'payment_status' => 'partial'
                        ]);

                        // B) Movemos a transação financeira (Mantendo a data do dia 20 para o seu caixa não mudar)
                        // O Log mostrou que a transação ID 62 existe e está ligada à reserva antiga
                        DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->update([
                                'reserva_id' => $proximaReserva->id,
                                'description' => DB::raw("CONCAT(description, ' [Repassado da Reserva #{$reserva->id}]')")
                            ]);

                        // C) Limpamos a reserva atual
                ...
                return response()->json(['success' => true, 'message' => 'Manutenção aplicada e crédito transferido!']);
            } catch (\Exception $e) {
                \Log::error("Erro na Manutenção: " . $e->getMessage());
