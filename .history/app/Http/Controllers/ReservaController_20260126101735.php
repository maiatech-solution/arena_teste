...
class ReservaController extends Controller
{
    ...
    public function storeRecurrentReservaApi(Request $request)
    {
        ...
        // 🛡️ TRAVA DE CONFLITO DE MENSALISTA FUTURO (CORRIGIDA COM DAY_OF_WEEK)
        $futureMensalista = Reserva::where('arena_id', $arenaId)
            ->where('day_of_week', $dayOfWeek) // 🎯 Filtra pelo dia da semana (Ex: só Terças)
        ...
        if ($futureMensalista) {
            return response()->json([
                'success' => false,
                'message' => "⚠️ BLOQUEIO DE RECORRÊNCIA: Já existe um mensalista ('{$futureMensalista->client_name}') para este horário nas próximas semanas neste mesmo dia da semana."
            ], 422);
        }
