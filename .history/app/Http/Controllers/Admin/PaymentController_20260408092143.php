        ...
        try {
            ...
            return response()->json([
                ...
                'resumo'  => [
                    ...
                    'id'      => $reserva->id,
                    'data'    => \Carbon\Carbon::parse($reserva->date)->format('d/m/Y'),
                    'periodo' => substr($reserva->start_time, 0, 5) . ' - ' . substr($reserva->end_time, 0, 5),
                    'total'   => number_format($reserva->final_price ?? $reserva->price, 2, ',', '.'),
                    'pago'    => number_format($newTotalPaid, 2, ',', '.'),
                    'metodo'  => ucfirst($metodoFinal)
                ]
            ]);
        } catch (\Exception $e) {
