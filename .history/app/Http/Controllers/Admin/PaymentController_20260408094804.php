...

{
    ...
    public function closeCash(Request $request)
    {
        ...
        try {
            ...
            // --- 🛡️ VALIDAÇÃO DE PENDÊNCIAS (Mantido igual) ---
            $agora = now();
            ...
            if ($pendenciasCriticas->count() > 0) {
                return response()->json([
                    'success' => false,
                    'message' => "🚨 Bloqueio: Existem {$pendenciasCriticas->count()} jogo(s) finalizados sem definição financeira."
                ], 403);
            }

            // --- 📊 CÁLCULOS PARA O RESUMO ---
            $allTransactions = FinancialTransaction::whereDate('paid_at', $date)
                ->where('arena_id', $arenaId)
                ->get();

            $vCash    = $allTransactions->where('payment_method', 'dinheiro')->sum('amount');
            $vDigital = $allTransactions->whereIn('payment_method', ['pix', 'cartao', 'credito', 'debito'])->sum('amount');

            $calculated = round((float)$allTransactions->sum('amount'), 2);
            $actual     = round((float)$validated['actual_amount'], 2);
            $difference = round($actual - $calculated, 2);

            // 🚀 REGRA DE SUPERVISOR (Mantido)
            if ($difference != 0 && Auth::user()->role === 'colaborador') {
                if (!$request->filled('supervisor_token')) {
                    return response()->json(['success' => false, 'message' => 'Divergência requer autorização.'], 403);
                }
            }
            ...
            // 🔥 O SEGREDO: GRAVAR NA SESSION PARA O BLADE MOSTRAR O RECIBO APÓS O RELOAD
            $company = DB::table('company_infos')->first();

            session()->flash('success', 'Turno encerrado');
            session()->flash('arena_nome_print', $company->nome_fantasia ?? 'MAIATECH ARENA');
            session()->flash('opening_balance_print', '0,00');
            session()->flash('vendas_dinheiro_print', number_format($vCash, 2, ',', '.'));
            session()->flash('vendas_digital_print', number_format($vDigital, 2, ',', '.'));
            session()->flash('reforcos_print', '0,00');
            session()->flash('sangrias_print', '0,00');
            session()->flash('total_esperado_print', number_format($calculated, 2, ',', '.'));
            session()->flash('valor_informado_print', number_format($actual, 2, ',', '.'));
            session()->flash('diferenca_print', number_format($difference, 2, ',', '.'));
    ...
    public function processPayment(Request $request, $reservaId)
    {
        ...
        try {
            ...
            // No método de fechar caixa (PHP)
            return response()->json([
                'success' => true,
                'message' => 'Caixa fechado com sucesso!',
                'resumo' => [
                    'titulo'   => 'FECHAMENTO DE UNIDADE',
                    'data'     => now()->format('d/m/Y H:i'),
                    'total'    => number_format($valorTotalContado, 2, ',', '.'),
                    'detalhes' => "DINHEIRO: R$ " . number_format($gaveta, 2, ',', '.') . "<br>" .
                        "DIGITAL: R$ " . number_format($banco, 2, ',', '.') . "<br>" .
    ...
    /**
     * Gera a visualização do cupom térmico para a bobina.
     */
    public function imprimirCupom(\App\Models\Reserva $reserva)
    {
        $reserva->load(['arena', 'transactions.manager']);

        return view('admin.payment.print_receipt', [
            'reserva' => $reserva,
            'arena'   => $reserva->arena,
            'transactions' => $reserva->transactions()->orderBy('paid_at', 'asc')->get()
        ]);
    }
}
