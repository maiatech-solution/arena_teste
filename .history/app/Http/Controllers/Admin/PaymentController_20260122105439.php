...

{
    ...
    /**
     * 💸 REGISTRAR SANGRIA: Saída avulsa de dinheiro para despesas imediatas.
     */
    public function registerSangria(Request $request)
    {
        $validated = $request->validate([
        ...
        try {
            ...
            if (\App\Http\Controllers\FinanceiroController::isCashClosed($validated['date'])) {
                return response()->json(['success' => false, 'message' => 'Caixa encerrado para esta data.'], 403);
            }

            FinancialTransaction::create([
                'arena_id'       => $validated['arena_id'],
                'manager_id'     => Auth::id(),
                'amount'         => -$validated['amount'], // Valor negativo para saída
                'type'           => 'sangria',
                'payment_method' => 'money',
                'description'    => "SANGRIA: " . $validated['description'],
                'paid_at'        => $validated['date'],
            ]);

            return response()->json(['success' => true, 'message' => 'Saída de caixa registrada!']);
        } catch (\Exception $e) {
