...

{
    ...
    /**
     * 🎯 FECHAR CAIXA: Grava a auditoria e retorna URL para impressão térmica.
     */
        ...
        try {
            ...
            if ($pendenciasCriticas->count() > 0) {
                return response()->json([
                    'success' => false,
                    'message' => "🚨 Bloqueio: Existem {$pendenciasCriticas->count()} jogo(s) finalizados sem definição financeira. Resolva-os para fechar."
                ], 403);
            }
            ...
            // --- PERSISTÊNCIA ---
            $cashier = DB::transaction(function () use ($date, $arenaId, $calculated, $actual, $difference, $request) {
                return Cashier::updateOrCreate(
            ...
            // 🚀 URL PARA O COMPROVANTE TÉRMICO (Ajustado para o novo método)
            $printUrl = route('admin.payment.comprovante_fechamento', $cashier->id);
    ...
    /**
     * 📄 GERA O CONTEÚDO DO COMPROVANTE
     */
    public function comprovanteFechamento($id)
    {
        $caixa = Cashier::with(['user', 'arena'])->findOrFail($id);

        // Detalhamento por método para o cupom
        $detalhesPagamento = FinancialTransaction::whereDate('paid_at', $caixa->date)
            ->where('arena_id', $caixa->arena_id)
            ->select('payment_method', DB::raw('SUM(amount) as total'))
            ->groupBy('payment_method')
            ->get();

        return view('admin.payment.comprovante_fechamento', compact('caixa', 'detalhesPagamento'));
    }
