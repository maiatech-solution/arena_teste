        ...
        try {
            ...
            if ($pendenciasCriticas->count() > 0) {
                return response()->json([
                    'success' => false,
                    'message' => "🚨 Bloqueio: Existem {$pendenciasCriticas->count()} jogo(s) pendentes."
                ], 403);
            }
            ...
            $totalOutros = (float)($totalCalculated - ($totalGaveta + $totalBanco));
            ...
            if ($difference != 0 && Auth::user()->role === 'colaborador' && !$request->filled('supervisor_token')) {
                return response()->json(['success' => false, 'message' => 'Divergência requer supervisor.'], 403);
            }
            ...
            // 🚀 RESPOSTA: Chaves 'gaveta' e 'banco' AGORA CASAM com o JS
            return response()->json([
                'success' => true,
                'resumo' => [
                    'arena'     => \App\Models\Arena::find($arenaId)->name,
                    'data'      => \Carbon\Carbon::parse($date)->format('d/m/Y'),
                    'sistema'   => $totalCalculated,
                    'gaveta'    => $totalGaveta,  // Antes era 'dinheiro' (ERRO)
                    'banco'     => $totalBanco,   // Antes era 'pix' (ERRO)
                    'credito'   => 0,
                    'debito'    => $totalOutros,
                    'informado' => $actual,
