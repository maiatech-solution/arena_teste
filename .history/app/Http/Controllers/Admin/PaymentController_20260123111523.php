...

{
    ...
    public function index(Request $request)
    {
        ...
        // 9. KPIs Dinâmicos (Refinados para separar Fluxo de Caixa de Inadimplência Autorizada)

        // Total Esperado: Apenas jogos ativos (Ignora dívidas já autorizadas para não inflar o esperado do dia)
        $totalExpected = $reservas->whereNotIn('status', ['canceled', 'rejected', 'completed', 'no_show'])
            ->sum(fn($r) => $r->final_price ?? $r->price);

        // PENDÊNCIA REAL: Apenas o que falta receber de jogos confirmados/pendentes.
        // Quando você marcar "Pagar Depois", o status vira 'completed' e sai desta soma, limpando o alerta no topo.
        ...
        // DÍVIDA AUTORIZADA: Soma apenas o que você marcou como "Pagar Depois" hoje.
        ...
        return view('admin.payment.index', [
            ...
            'totalPending'               => $totalPending,           // Agora limpo (R$ 0,00 se tudo for dívida)
            'totalAuthorizedDebt'        => $totalAuthorizedDebt,   // Para o Card 4
