    ...
    public function processPayment(Request $request, $reservaId)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($validated, $reserva, &$paymentStatus, $dataOperacional) {
                // 🔒 LOCK PARA EVITAR CONCORRÊNCIA (Impede múltiplos processos na mesma reserva)
                ...
                // 🛡️ TRAVA 1: ANTI-CLIQUE DUPLO (Aumentada para 10 segundos para segurança)
                ...
                // Soma estornos/saídas orfãs (caso existam no seu fluxo)
                ...
                // ✅ VERIFICAÇÃO DE DUPLICIDADE LÓGICA (O coração da solução)
                // Se já pagou o total e está tentando pagar mais, bloqueia.
                if ($saldoDevedor <= 0 && $amountReceivedNow > 0) {
                    throw new \Exception('ALREADY_PAID');
                }

                // Segurança contra pagar mais que o necessário (Trava o valor extra)
                ...
                if ($amountReceivedNow > 0) {
                    // 🔍 LÓGICA DE INTELIGÊNCIA DE MÉTODO
                    $metodoFinal = $validated['payment_method'];

                    // Se o método vier como 'outro' ou vazio, tentamos recuperar o método do sinal original
                    ...
                    \App\Models\FinancialTransaction::create([
                        ...
                        'payment_method' => $metodoFinal, // ✨ Usa o método inteligente
