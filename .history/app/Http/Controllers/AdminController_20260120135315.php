...
class AdminController extends Controller
{
    ...
    {
        // 1. Captura os dados da reserva que você clicou
        $reservaOriginal = Reserva::find($id);

        // 2. Tenta localizar a próxima reserva para onde o dinheiro deveria ir
        $proximaReserva = Reserva::where('recurrent_series_id', $reservaOriginal->recurrent_series_id)
            ->where('date', '>', $reservaOriginal->date)
            ->where('status', '!=', 'cancelled')
            ->orderBy('date', 'asc')
            ->first();

        // 3. Captura as transações financeiras ligadas a essa reserva
        $transacoes = \App\Models\FinancialTransaction::where('reserva_id', $id)->get();

        // 4. PARA TUDO E MOSTRA OS DADOS (O famoso dump and die)
        dd([
            'INFO_RESERVA_CLICADA' => [
                'id' => $reservaOriginal->id,
                'cliente' => $reservaOriginal->client_name,
                'valor_pago_no_campo' => $reservaOriginal->total_paid,
                'recurrent_series_id' => $reservaOriginal->recurrent_series_id,
            ],
            'PROXIMA_RESERVA_ENCONTRADA' => $proximaReserva ? [
                'id' => $proximaReserva->id,
                'data' => $proximaReserva->date,
                'status_atual' => $proximaReserva->status,
                'recurrent_series_id' => $proximaReserva->recurrent_series_id,
            ] : 'NENHUMA RESERVA FUTURA ENCONTRADA COM ESSE RECURRENT_SERIES_ID',
            'TRANSACOES_NO_BANCO' => $transacoes->map(fn($t) => [
                'id' => $t->id,
                'valor' => $t->amount,
                'reserva_id' => $t->reserva_id,
                'data_pago' => $t->paid_at
            ]),
            'REQUEST_RECEBIDO' => $request->all()
        ]);
    }
