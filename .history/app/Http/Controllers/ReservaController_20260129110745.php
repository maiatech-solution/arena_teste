    ...
    public function cancelSeries(int $masterId, string $reason, bool $shouldRefund, float $amountPaidRef)
    {
        ...
        $seriesReservas = Reserva::where(function ($query) use ($masterId) {
            ...
            ->whereIn('status', [
                ...
                'completed',
                'concluida',
                Reserva::STATUS_CANCELADA // Incluímos para a trava de segurança abaixo
            ])
            ->get();
        ...
        // --- 💰 PASSO FINANCEIRO ANTECIPADO ---
        // Registramos o financeiro APENAS se houver valor e se o Master não estiver cancelado
        if ($amountPaidRef > 0 && $anchorReserva->status !== Reserva::STATUS_CANCELADA) {
            if ($shouldRefund) {
                FinancialTransaction::create([
                    'reserva_id'     => $masterId,
        ...
        // --- 🗓️ LOOP DE EXCLUSÃO COM TRAVA DE INTEGRIDADE ---
        foreach ($seriesReservas as $slot) {

            // 🛡️ TRAVA CRÍTICA: Se a reserva já foi cancelada individualmente,
            // saltamos para a próxima para não estornar/deletar de novo.
            if ($slot->status === Reserva::STATUS_CANCELADA || $slot->status === 'cancelled') {
                continue;
            }
            ...
            // Atualiza status e preço final antes de deletar
            ...
            // Devolve o slot "verde" para a agenda
            $this->recreateFixedSlot($slot);

            // Remove rastros financeiros individuais para não duplicar no caixa
