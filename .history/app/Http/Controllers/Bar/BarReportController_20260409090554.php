...
class BarReportController extends Controller
{
    ...
    public function payments(Request $request)
    {
        ...
        // 🔥 TRAVA DE SEGURANÇA: Se não for admin, vê apenas os SEUS pagamentos
        if (!$isAdmin) {
        ...
        // 🚀 BLOCO DE DEBUG (Ative digitando "debug" na busca do relatório)
        if ($request->search === 'debug') {
            dd([
                'periodo' => "De $startDate ate $endDate",
                'filtros' => $request->all(),
                'total_vouchers_individual' => DB::table('bar_cash_movements')
                    ->where('type', 'venda')
                    ->where('payment_method', 'like', '%voucher%')
                    ->whereBetween('created_at', [$startDate, $endDate])
                    ->when(!$isAdmin, fn($q) => $q->where('user_id', $user->id))
                    ->get() // Aqui você verá a lista de cada um dos 11 lançamentos
            ]);
        }
        ...
        // 🔥 CÁLCULO DE AUDITORIA (Excluindo Vouchers do Total Geral)
        $totalFinanceiroReal = $pagamentos->filter(function ($p) {
            return !str_contains(strtolower($p->payment_method), 'voucher');
        })->sum('total');

        return view('bar.reports.payments', compact('pagamentos', 'mesReferencia', 'totalFinanceiroReal'));
    }
