...
class BarReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // --- 1. FATURAMENTO REAL (Dinheiro no Bolso) ---
        ...
        // --- 2. CÁLCULO DE VOUCHERS (Valor de Face das Cortesias) ---
        $orderVoucherIds = $queryVouchersOrders->pluck('id');
        $saleVoucherIds = $queryVouchersSales->pluck('id');

        // Soma o valor que os itens TERIAM se fossem pagos
        $valorVoucherMesas = BarOrderItem::whereIn('bar_order_id', $orderVoucherIds)->sum('subtotal');

        // 🚀 AJUSTADO: Usando price_at_sale (ou o nome correto da sua coluna de preço)
        ...
        // Otimizado: soma direta no banco (Quantidade * Preço Unitário)
        $valorVoucherBalcao = BarSaleItem::whereIn('bar_sale_id', $saleVoucherIds)
        ...
        // --- 3. VOLUME DE ITENS (Eficiência na busca de IDs) ---
        // Reaproveitamos a lógica para não dar 4 plucks no banco
