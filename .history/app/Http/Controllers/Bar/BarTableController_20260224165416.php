...
class BarTableController extends Controller
{
    ...
    public function closeOrder(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            ...
            if (!$session) {
                return redirect()->route('bar.tables.index')->with('error', '⚠️ Caixa fechado.');
            }

            $order = $table->orders()->where('status', 'open')->latest()->first();
            if (!$order) return redirect()->route('bar.tables.index')->with('error', '⚠️ Comanda não encontrada.');

            $discountValue = $request->discount_value ?? 0;
            $finalValue = $order->total_value - $discountValue;

            // --- 💳 EXTRAÇÃO DOS MÉTODOS DE PAGAMENTO ---
            $nomesMetodos = [];
            // O log confirmou que o nome é 'pagamentos'
            $pagamentosArray = json_decode($request->pagamentos, true);

            if (is_array($pagamentosArray)) {
                foreach ($pagamentosArray as $pag) {
                    // Verificamos 'metodo' conforme seu log: {"metodo":"dinheiro","valor":15}
                    if (isset($pag['metodo']) && isset($pag['valor']) && $pag['valor'] > 0) {
                        $nomesMetodos[] = strtoupper($pag['metodo']);
                    }
                }
            }

            // Se houver métodos, junta em string (ex: "DINHEIRO, PIX"). Se não, usa "PAGO".
            $metodosString = !empty($nomesMetodos) ? implode(', ', array_unique($nomesMetodos)) : 'PAGO';

            // 3. ATUALIZA A COMANDA
            $order->update([
                'status'              => 'paid',
                'customer_name'       => $request->customer_name,
                'customer_phone'      => $request->customer_phone,
                'payment_method'      => $metodosString, // ✅ Agora vai salvar "DINHEIRO"
            ...
            if ($request->print_coupon == "1") {
                return redirect()->route('bar.tables.receipt', $order->id)->with('show_success_modal', true);
            }

            return redirect()->route('bar.tables.index')->with('success', 'Venda finalizada!');
        });
    }
