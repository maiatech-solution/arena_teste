...
class BarOrderController extends Controller
{
    ...
    public function indexVendas(Request $request)
    {
        ...
        // Filtro por Status (Paid ou Cancelled)
        if ($request->filled('status')) {
            $query->where('status', $request->status);
        }

        // Filtro por Data (Se vazio, mostra tudo ordenado pela mais recente)
        if ($request->filled('date')) {
            $query->whereDate('updated_at', $request->date);
        }

        // Ordenação: Sempre a mais recente no topo (Hoje aparecerá primeiro)
        $vendas = $query->orderBy('updated_at', 'desc')
            ->paginate(15)
