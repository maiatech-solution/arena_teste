...
class BarTableController extends Controller
{
    ...
    public function closeOrder(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            ...
            $jsonPagamentos = $request->pagamentos;
            $pagamentosArray = json_decode($jsonPagamentos, true);
            $nomesMetodos = [];
            ...
            if (is_array($pagamentosArray)) {
                foreach ($pagamentosArray as $pag) {
                    ...
                    if ($valorItem > 0) {
                        // Converte "dinheiro" para "DINHEIRO"
            ...
            // Gera a string final: "DINHEIRO" ou "DINHEIRO, PIX"
            $metodosString = !empty($nomesMetodos) ? implode(', ', array_unique($nomesMetodos)) : 'PAGO';

            // 3. ATUALIZAÇÃO DA COMANDA (Forçando o Save)
            $order->status = 'paid';
            $order->customer_name = $request->customer_name;
            $order->customer_phone = $request->customer_phone;
            $order->payment_method = $metodosString; // Aqui entra o "DINHEIRO"
            $order->discount_value = $discountValue;
            $order->total_value = $finalValue;
            $order->closed_at = now();
            $order->bar_cash_session_id = $session->id;
            $order->save(); // Usando save() direto para garantir a persistência
