    ...
    public function getDetails($tipo, $id)
    {
        try {
            ...
            if ($tipoLower === 'mesa' || $tipoLower === 'mesas') {
                ...
                /** * 🔍 LÓGICA DE RECUPERAÇÃO DO PAGAMENTO PARA MESAS
                 * Como bar_orders não tem a coluna payment_method, buscamos na bar_sales
                 * o registro que finalizou esta comanda.
                 */
                $metodoPagamento = BarSale::where('bar_order_id', $venda->id)
                    ->where('status', 'paid')
            ...
            // 💰 Lógica do Desconto: Se o banco tiver a coluna usa ela, se não, faz a conta matemática
            ...
            return response()->json([
                ...
                // 💳 Retorna o método encontrado ou um padrão caso ainda esteja vazio
                'pagamento' => strtoupper($metodoPagamento ?? 'NÃO INFORMADO'),

                'total'     => number_format($valorPagoFinal, 2, ',', '.'),

                // Campos cruciais para a lógica de cálculos no Modal Alpine.js
