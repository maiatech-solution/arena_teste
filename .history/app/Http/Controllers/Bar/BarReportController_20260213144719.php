...
class BarReportController extends Controller
{
    ...
    public function cashier(Request $request)
    {
        ...
        // Carregamos a sessão com o usuário que é dono dela (quem abriu)
        ...
        foreach ($sessoes as $sessao) {
            ...
            // 🟢 BUSCA TOTAL: Usamos um intervalo de 1 minuto a mais para garantir que vendas no segundo exato do fechamento entrem
            $vendasMesas = \App\Models\Bar\BarOrder::where('status', 'paid')
                ->whereBetween('updated_at', [$inicio->copy()->subMinute(), $fim->copy()->addMinute()])
                ->sum('total_value');

            $vendasPDV = \App\Models\Bar\BarSale::where('status', 'paid')
                ->whereBetween('created_at', [$inicio->copy()->subMinute(), $fim->copy()->addMinute()])
                ->sum('total_value');
