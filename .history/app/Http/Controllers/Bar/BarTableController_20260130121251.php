    ...
    public function open($id)
    {
        ...
        try {
            DB::transaction(function () use ($table) {
                // 1. Tenta criar a comanda
                \App\Models\Bar\BarOrder::create([
                    'bar_table_id' => $table->id,
                    'user_id' => auth()->id(), // Certifique-se que você está logado!
                    'status' => 'aberto',
                    'total_value' => 0
                ]);

                // 2. Tenta mudar o status da mesa
            ...
            // Se der erro no banco, ele vai PARAR aqui e te mostrar o porquê
            dd("ERRO NO BANCO: " . $e->getMessage());
        }
    }
