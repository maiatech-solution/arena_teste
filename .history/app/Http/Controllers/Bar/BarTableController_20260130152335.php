...
class BarTableController extends Controller
{
    ...
    public function sync(Request $request)
    {
        ...
        if ($totalDesejado > $atual) {
            ...
            // 1. Reativa quem está dentro do limite
            // Usamos o CAST para garantir que o banco compare como número e não como texto
            BarTable::whereRaw('CAST(identifier AS UNSIGNED) <= ?', [$totalDesejado])
                ->where('status', 'reserved')
                ->update(['status' => 'available']);

            // 2. Desativa (esconde) quem está acima do limite
            BarTable::whereRaw('CAST(identifier AS UNSIGNED) > ?', [$totalDesejado])
