...
class BarReportController extends Controller
{
    ...
    public function cancelations(Request $request)
    {
        ...
        // 1. Estornos Financeiros (Dinheiro que saiu do caixa)
        ...
        $movimentacoesEstoque = \App\Models\Bar\BarStockMovement::with(['product', 'user'])
            ...
            ->where(function ($query) {
                // 🛡️ Ajustado para incluir 'perda' (seu controller) e 'input' (cancelamentos de mesa/pdv)
                $query->whereIn('type', ['perda', 'loss', 'output'])
                    ->orWhere(function ($q) {
                        $q->where('type', 'input')
                            ->where('description', 'like', '%CANCELAMENTO%')
