...
class BarTableController extends Controller
{
    ...
    {
        // O erro estava no ponto (.) antes do get(). O correto é a seta (->)
    ...
    public function sync(Request $request)
    {
        ...
        // 1. Quantas mesas reais existem no banco hoje (independente do status)
        $atualNoBanco = BarTable::count();

        // 2. Se o usuário quer mais do que já cadastramos um dia, criamos as novas
        ...
        // 3. A MÁGICA: Atualizamos o status de TODO MUNDO de uma vez só
        // Quem está dentro do limite vira 'available' (se não estiver ocupada)
        BarTable::whereRaw('CAST(identifier AS UNSIGNED) <= ?', [$totalDesejado])
            ->where('status', '!=', 'occupied')
            ->update(['status' => 'available']);

        // Quem está acima do limite vira 'reserved' (esconde)
        BarTable::whereRaw('CAST(identifier AS UNSIGNED) > ?', [$totalDesejado])
