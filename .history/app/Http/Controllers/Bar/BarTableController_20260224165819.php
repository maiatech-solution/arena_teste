...
class BarTableController extends Controller
{
     ...
     */
    /**
     * 🏁 FINALIZAR MESA (Com extração correta de Meio de Pagamento)
     */
    public function closeOrder(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            ...
            // --- 💳 TRATAMENTO DOS NOMES DE PAGAMENTO PARA O HISTÓRICO ---
            $jsonPagamentos = $request->input('pagamentos');
            $nomesMetodos = [];

            if ($jsonPagamentos) {
                // Decodifica o JSON que confirmamos no log: [{"metodo":"dinheiro","valor":15}]
                $pagamentosArray = is_array($jsonPagamentos) ? $jsonPagamentos : json_decode($jsonPagamentos, true);

                if (is_array($pagamentosArray)) {
                    foreach ($pagamentosArray as $pag) {
                        $metodoNome = isset($pag['metodo']) ? trim($pag['metodo']) : null;
                        $valorMetodo = isset($pag['valor']) ? (float)$pag['valor'] : 0;

                        if ($metodoNome && $valorMetodo > 0) {
                            $nomesMetodos[] = mb_strtoupper($metodoNome, 'UTF-8');
                        }
                    }
                }
            }

            // Se houver métodos, junta em string (ex: "DINHEIRO, PIX"). Caso contrário, padrão 'PAGO'.
            ...
            $order->update([
                ...
                'payment_method'      => $metodosString, // Salva o nome real do método
            ...
            // 💰 4. INTEGRAÇÃO COM O CAIXA (MOVIMENTAÇÕES INDIVIDUAIS)
            if (isset($pagamentosArray) && is_array($pagamentosArray)) {
                foreach ($pagamentosArray as $pag) {
                    if ($pag['valor'] > 0) {
