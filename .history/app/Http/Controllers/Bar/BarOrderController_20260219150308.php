...
class BarOrderController extends Controller
{
    ...
    {
        // Adicionamos as relações necessárias
        $query = BarOrder::with(['items.product', 'user', 'cashSession'])
            ->whereIn('status', ['paid', 'cancelled']); // Garante que só pegamos concluídas ou anuladas
        ...
        // 🔥 A MUDANÇA ESTÁ AQUI: Ordenar por ID garante que o #12 suba pro topo agora
        $vendas = $query->orderBy('id', 'desc')
            ->paginate(15)
            ->withQueryString();
