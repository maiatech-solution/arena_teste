...
class BarReportController extends Controller
{
    ...
    public function payments(Request $request)
    {
        ...
        // Pegamos os pagamentos das Mesas (Orders)
        $orders = DB::table('bar_orders')
            ->select('payment_method', DB::raw('SUM(total_value) as total'), DB::raw('COUNT(*) as qtd'))
            ->where('status', 'paid')
            ->whereBetween('updated_at', [$startDate, $endDate])
            ->groupBy('payment_method');

        // Unimos com os pagamentos do PDV (Sales) e agrupamos o resultado final
        $pagamentos = DB::table('bar_sales')
            ->select('payment_method', DB::raw('SUM(total_value) as total'), DB::raw('COUNT(*) as qtd'))
            ->where('status', 'paid')
            ->whereBetween('created_at', [$startDate, $endDate])
            ->groupBy('payment_method')
        ...
        // Como o UnionAll pode trazer métodos repetidos (ex: 'Pix' em orders e 'Pix' em sales),
        // vamos agrupar no PHP para garantir o total único por método.
        $pagamentosAgrupados = $pagamentos->groupBy('payment_method')->map(function ($group) {
            return (object) [
        ...
        return view('bar.reports.payments', [
            'pagamentos' => $pagamentosAgrupados,
