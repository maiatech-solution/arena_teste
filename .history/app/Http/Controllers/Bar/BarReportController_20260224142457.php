    ...
    public function getDetails($tipo, $id)
    {
        try {
            if ($tipo === 'mesa') {
                ...
                $metodoPgto = $venda->payment_method;
                // Busca o desconto da mesa
                $desconto = $venda->discount_value ?? 0;
            } else {
                $venda = BarSale::with(['items.product', 'user'])->findOrFail($id);
                $metodoPgto = $venda->payment_method;
                // Busca o desconto do PDV (Venda Direta)
                $desconto = $venda->discount_value ?? 0;
            }
            ...
            return response()->json([
                ...
                'pagamento' => strtoupper($metodoPgto ?? 'Não informado'),
                'total'     => number_format($venda->total_value, 2, ',', '.'),
                'total_raw' => $venda->total_value, // Importante para o cálculo no JS
                'desconto'  => $desconto,           // Valor do desconto puro para o Alpine.js
                'itens'     => $venda->items->map(function ($item) {
                    return [
                        'nome'     => $item->product->name ?? 'Produto Removido',
                        'qtd'      => $item->quantity,
                        'preco'    => number_format($item->price_at_sale ?? $item->unit_price, 2, ',', '.'),
                        'subtotal' => number_format($item->subtotal ?? ($item->quantity * ($item->price_at_sale ?? $item->unit_price)), 2, ',', '.')
                    ];
                })
            ]);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Venda não encontrada ou erro interno.'], 404);
        }
    }
}
