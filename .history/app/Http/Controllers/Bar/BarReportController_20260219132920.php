...
class BarReportController extends Controller
{
    ...
    public function payments(Request $request)
    {
        ...
        // 2. Buscamos os dados da bar_cash_movements (onde centraliza tudo)
        // Usamos 'payment_method' que é a coluna que você tem nessa tabela
        $pagamentos = DB::table('bar_cash_movements')
            ->select(
                'payment_method',
                DB::raw('SUM(amount) as total'),
                DB::raw('COUNT(*) as qtd')
            )
            ->where('type', 'venda') // Garante que não pegue sangrias ou reforços
            ->whereBetween('created_at', [$startDate, $endDate])
