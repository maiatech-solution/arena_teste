...
class BarTableController extends Controller
{
    ...
    public function closeOrder(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            ...
            // Tentamos pegar o JSON de 'pagamentos' ou 'pagamento' para evitar erros de nome de campo
            $jsonPagamento = $request->pagamentos ?? $request->pagamento;
            $pagamentosArray = json_decode($jsonPagamento, true);
            $nomesMetodos = [];

            if (is_array($pagamentosArray)) {
                foreach ($pagamentosArray as $pag) {
                    if (isset($pag['valor']) && $pag['valor'] > 0) {
                        $nomesMetodos[] = strtoupper($pag['metodo']);
                    }
                }
            }

            // Transforma o array em string separada por vírgula (Ex: "PIX, DINHEIRO")
            $metodosString = implode(', ', array_unique($nomesMetodos));
            ...
            $order->update([
                ...
                'payment_method'      => $metodosString ?: 'PAGO', // Agora deve salvar o nome real
