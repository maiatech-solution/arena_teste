    ...
    public function cancelations(Request $request)
{
    ...
    // 1. Estornos Financeiros (O que saiu do dinheiro do caixa)
    // Buscamos na tabela de movimentos de caixa para pegar Motivo e Supervisor
    ...
    // 2. Perdas e Estornos de Estoque (Movimentação física de produtos)
    // Buscamos 'input' (estornos que voltaram ao estoque) e 'loss' (perdas manuais/quebras)
    $movimentacoesEstoque = \App\Models\Bar\BarStockMovement::with(['product', 'user'])
        ->whereIn('type', ['input', 'loss', 'output'])
        ->where(function($query) {
            $query->where('description', 'like', '%CANCELAMENTO%')
                  ->orWhere('description', 'like', '%ESTORNO%')
                  ->orWhere('description', 'like', '%PERDA%');
        })
        ->whereBetween('created_at', [$startDate, $endDate])
        ->orderBy('created_at', 'desc')
        ->get();
