...
class BarReportController extends Controller
{
    ...
    public function cashier(Request $request)
    {
        ...
        foreach ($sessoes as $sessao) {
            $inicio = $sessao->opened_at;
            // Se o caixa fechou, usamos a data de fechamento, senão usamos o 'agora'
            $fim = $sessao->closed_at ?? now();

            // 🟢 ESTRATÉGIA DE SEGURANÇA: Janela de 5 minutos para capturar vendas no limite do fechamento
            $vendasMesas = \App\Models\Bar\BarOrder::where('status', 'paid')
                ->whereBetween('updated_at', [$inicio->copy()->subMinutes(5), $fim->copy()->addMinutes(5)])
                ->sum('total_value');

            $vendasPDV = \App\Models\Bar\BarSale::where('status', 'paid')
                ->whereBetween('created_at', [$inicio->copy()->subMinutes(5), $fim->copy()->addMinutes(5)])
                ->sum('total_value');
            ...
            // Total que o sistema deveria ter
