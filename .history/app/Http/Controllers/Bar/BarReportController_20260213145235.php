...
class BarReportController extends Controller
{
    ...
    public function cashier(Request $request)
    {
        ...
        foreach ($sessoes as $sessao) {
            // Criamos uma margem de 10 minutos para capturar vendas no limite da abertura/fechamento
            $inicioTurno = $sessao->opened_at->copy()->subMinutes(10);
            $fimTurno = ($sessao->closed_at ?? now())->copy()->addMinutes(10);

            // 1. Soma Mesas no intervalo do turno
            $vendasMesas = \App\Models\Bar\BarOrder::where('status', 'paid')
                ->whereBetween('updated_at', [$inicioTurno, $fimTurno])
                ->sum('total_value');

            // 2. Soma PDV no intervalo do turno
            $vendasPDV = \App\Models\Bar\BarSale::where('status', 'paid')
                ->whereBetween('created_at', [$inicioTurno, $fimTurno])
                ->sum('total_value');

            // 3. Movimentações (Sangrias/Reforços)
