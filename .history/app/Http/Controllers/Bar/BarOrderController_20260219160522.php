        ...
        try {
            DB::transaction(function () use ($order, $supervisor, $request, $caixaAberto) {
                ...
                foreach ($order->items as $item) {
                    // 🔍 AJUSTE AQUI: Verificamos qual coluna de ID existe para não enviar NULL
                    ...
                    if ($productId && $item->product) {
                        ...
                        BarStockMovement::create([
                            'bar_product_id' => $productId, // Garante que o ID do produto seja enviado
                            'user_id' => auth()->id(),
                            'type' => 'input',
                            'quantity' => $item->quantity,
                ...
                // 5. Atualizar status
                // 🔥 AJUSTE: Usando 'cancelado' para evitar erro de tamanho de coluna (Data Truncated)
                $order->update(['status' => 'cancelado']);
            });

            return back()->with('success', "✅ Comanda #{$order->id} anulada!");
        } catch (\Exception $e) {
            // Log para você debugar se algo mais der errado
            Log::error("Erro ao cancelar mesa: " . $e->getMessage());
            return back()->with('error', 'Erro ao cancelar: ' . $e->getMessage());
        }
    }
}
