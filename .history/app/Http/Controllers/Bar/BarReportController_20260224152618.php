...
class BarReportController extends Controller
{
    ...
    public function getDetails($tipo, $id)
    {
        ...
        $venda = ($tipoLower === 'mesa' || $tipoLower === 'mesas')
            ? \App\Models\Bar\BarOrder::with(['items.product'])->findOrFail($id)
            : \App\Models\Bar\BarSale::with(['items.product'])->findOrFail($id);

        // 🕵️ DEBUG PARA ACHAR O DESCONTO
        dd([
            'ID_DA_VENDA' => $venda->id,
            'VALOR_PAGO_NO_BANCO' => $venda->total_value,
            'SOMA_REAL_DOS_ITENS' => $venda->items->sum('subtotal'),
            'PAGAMENTOS_JSON' => json_decode($venda->payment_method, true),
            'ATRIBUTOS_BRUTOS' => $venda->getAttributes()
        ]);
    }
}
