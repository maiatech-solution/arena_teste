...
class BarOrderController extends Controller
{
    ...
    {
        $query = BarOrder::with(['items.product', 'user']);
        ...
        if ($request->filled('date')) {
            ...
            // Se não houver filtro de data, opcionalmente você pode limitar aos últimos 30 dias
            // $query->where('updated_at', '>=', now()->subDays(30));
        }

        $vendas = $query->orderBy('updated_at', 'desc')->paginate(15)->withQueryString();
