...
class BarTableController extends Controller
{
    ...
    {
        // Buscamos apenas as mesas que não foram desativadas pelo sync
        // E ordenamos corretamente por número
        $tables = BarTable::where('status', '!=', 'reserved')
    ...
    public function sync(Request $request)
    {
        ...
        $totalDesejado = $request->total_tables;
        $atual = BarTable::count();

        if ($totalDesejado > $atual) {
            // Criar novas mesas se o número aumentou
            ...
            // 🛑 EM VEZ DE DELETAR, VAMOS APENAS "ESCONDER" AS MESAS EXCEDENTES
            // Isso evita o erro de integridade e mantém o histórico de vendas.

            // 1. Reativamos as mesas que estão dentro do novo limite (caso estivessem desativadas)
            BarTable::where('identifier', '<=', $totalDesejado)
                ->where('status', 'reserved') // ou o status que você usa para 'desativado'
                ->update(['status' => 'available']);

            // 2. Desativamos as mesas que ficaram fora do novo limite
            // Usamos 'reserved' ou você pode criar um status 'inactive' no banco
            BarTable::where('identifier', '>', $totalDesejado)
                ->update(['status' => 'reserved']);
        }

        return back()->with('success', 'Layout do salão atualizado com sucesso!');
    }
