...
class BarReportController extends Controller
{
    ...
    public function cancelations(Request $request)
    {
        ...
        // 1. Financeiro (Estornos de Caixa) - AGORA EM ORDEM DECRESCENTE
        $cancelamentosFinanceiros = \App\Models\Bar\BarCashMovement::with(['user'])
            ->where('type', 'estorno')
            ->whereBetween('created_at', [$startDate, $endDate])
            ->orderBy('created_at', 'desc') // 👈 Adicionado para mostrar o mais novo primeiro
            ->get();

        // 2. Prejuízo Real (Perdas/Vencidos) - TIPO 'perda'
        ...
        // 3. Apenas Retorno (Itens que voltaram para o estoque) - TIPO 'input'
