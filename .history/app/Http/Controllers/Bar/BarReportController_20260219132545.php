...
class BarReportController extends Controller
{
    ...
    public function payments(Request $request)
    {
        ...
        // 1. Buscamos do PDV (Aqui existe a coluna payment_method)
        $pagamentosPDV = DB::table('bar_sales')
            ->select('payment_method', DB::raw('SUM(total_value) as total'), DB::raw('COUNT(*) as qtd'))
            ->where('status', 'pago') // Note que aqui o status é 'pago' (português)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->groupBy('payment_method')
            ->get();

        // 2. Buscamos das Mesas
        // Como bar_orders não tem a coluna de método, vamos somar o total geral de mesas pagas
        // e categorizar como "Mesa / Comanda" ou buscar do movimento de caixa se preferir.
        $totalMesas = DB::table('bar_orders')
            ->where('status', 'paid') // Aqui o status é 'paid' (inglês)
            ->whereBetween('updated_at', [$startDate, $endDate])
            ->sum('total_value');

        $qtdMesas = DB::table('bar_orders')
            ->where('status', 'paid')
            ->whereBetween('updated_at', [$startDate, $endDate])
            ->count();

        // 3. Unificamos os dados para a View
        $pagamentos = $pagamentosPDV->map(function ($item) {
            return (object) [
