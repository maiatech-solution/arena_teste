...
class ReservaController extends Controller
{
    ...
    {
        // Removidos os logs de debug que não estavam aparecendo no ambiente do usuário.
        ...
        // 🛑 NOVA LÓGICA: Checagem robusta contra string ou array
        $isRecurrent = false;
        $recurrentInput = $request->input('is_recurrent');

        if (is_array($recurrentInput)) {
            // Se for um array (que é o caso do hidden + checkbox), checa se '1' está presente
            $isRecurrent = in_array('1', $recurrentInput, true);
        } elseif (is_string($recurrentInput) || is_numeric($recurrentInput)) {
            // Se for string/número, checa se é '1'
            $isRecurrent = $recurrentInput === '1';
        }
        ...
        try {
            ...
            Log::info("Reserva ID: {$reserva->id} confirmada por Gestor ID: " . Auth::id() . ". Sinal: R$ " . number_format($signalValue, 2, ',', '.') . ", Canceladas: 0 ");
    ...
    public function confirmar(Request $request, Reserva $reserva)
    {
        ...
        $validated = $request->validate([
            ...
            // Remove 'in:0,1' para aceitar o formato array ['0', '1'] que o navegador está enviando.
        ...
        if (is_array($recurrentInput)) {
            // Caso seja um array (Hidden field '0' + Checkbox '1'), checa se o valor '1' está na lista.
            // Ex: $recurrentInput = ['0', '1'] -> $isRecurrent = TRUE
            $isRecurrent = in_array('1', $recurrentInput, true);
        } elseif (is_string($recurrentInput) || is_numeric($recurrentInput)) {
            // Caso seja uma string ou número (apenas o Hidden field '0' se desmarcado, ou '1' se não usasse hidden)
            // Ex: $recurrentInput = '1' -> $isRecurrent = TRUE
            // Ex: $recurrentInput = '0' -> $isRecurrent = FALSE (Isso é impossível com o HTML atual, mas é uma segurança)
            $isRecurrent = $recurrentInput === '1';
        }
