...
class AdminController extends Controller
{
    ...
    /**
     * 🛠️ Move para MANUTENÇÃO preservando dados do cliente.
     */
    public function moverManutencao(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 1. BACKUP DOS DADOS: Salvamos o nome original e o status antes de mudar
                $backup = [
                ...
                // 2. FINANCEIRO: Se houver valor pago e pedir estorno
                if ($valorPago > 0 && $action === 'refund') {
                ...
                // 3. BLOQUEIO: Transforma em manutenção mas guarda o backup nas notas
                $reserva->update([
                    'status'      => 'maintenance',
                    'client_name' => '🛠️ MANUTENÇÃO',
                    'is_fixed'    => true,
                    'notes'       => $reserva->notes . "\n[ORIGINAL_DATA]:" . json_encode($backup) . "\nMotivo: " . $motivo
                ]);

                return response()->json(['success' => true, 'message' => 'Manutenção ativada!']);
            } catch (\Exception $e) {
    ...
    public function reativarManutencao(Request $request, $id)
    {
        try {
            $reserva = Reserva::findOrFail($id);
            $decisao = $request->input('action'); // 'restore_client' ou 'release_slot'

            if ($decisao === 'release_slot') {
                $reserva->delete();
                return redirect()->route('admin.reservas.index')->with('success', 'Horário liberado!');
            }

            // Recupera os dados do backup salvos nas notas
            preg_match('/\[ORIGINAL_DATA\]:(.*)/', $reserva->notes, $matches);
            $dados = isset($matches[1]) ? json_decode($matches[1], true) : null;

            $reserva->update([
                'status'      => $dados['status'] ?? 'confirmed',
                'client_name' => $dados['client_name'] ?? 'Cliente Restaurado',
                'is_fixed'    => $dados['is_fixed'] ?? false,
                // Limpa o marcador técnico das notas
                'notes'       => trim(preg_replace('/\[ORIGINAL_DATA\]:(.*)/', '', $reserva->notes))
            ]);

            return redirect()->back()->with('success', 'Reserva devolvida ao cliente original!');
        } catch (\Exception $e) {
