                        ...
                        <form method="GET" action="{{ route('admin.reservas.todas') }}" class="flex flex-col md:flex-row items-end md:items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
                            ...
                            <div class="flex space-x-2 w-full md:w-auto items-end flex-grow md:flex-grow-0">
                                ...
                                <div class="flex items-end space-x-1 h-[42px]">
                                    <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white h-full p-2 rounded-lg shadow-md transition duration-150 flex-shrink-0 flex items-center justify-center" title="Buscar">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider min-w-[100px]">Arena</th>
                            ...
                            <tr class="odd:bg-white even:bg-gray-50 hover:bg-indigo-50 transition duration-150">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap min-w-[120px]">
                                    ...
                                    <div class="flex flex-wrap gap-1 mt-1">
                                        ...
                                        <span class="text-[9px] font-bold text-indigo-700 bg-indigo-100 px-1 rounded uppercase">Recorrente</span>
                                        @else
                                        <span class="text-[9px] font-bold text-blue-700 bg-blue-100 px-1 rounded uppercase">Pontual</span>
                                        @endif
                                        @if ($reserva->is_fixed)
                                        <span class="text-[9px] font-bold text-gray-700 bg-gray-200 px-1 rounded uppercase">Fixo</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-left">
                                    @if ($reserva->is_fixed)
                                    <div class="text-sm font-bold text-gray-700 uppercase tracking-tight">Slot Inventário</div>
                                    ...
                                    <div class="text-[10px] text-gray-500 font-medium">{{ $reserva->client_contact ?? 'Sem contato' }}</div>
                                    @endif
                                </td>

                                {{-- 3. ARENA --}}
                                <td class="px-4 py-3 text-left">
                                    <span class="inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-black bg-indigo-50 text-indigo-700 border border-indigo-100 uppercase whitespace-nowrap">
                                ...
                                <td class="px-4 py-3 text-center whitespace-nowrap">
                                    ...
                                    try {
                                    $baseDate = is_string($reserva->date) ? explode(' ', $reserva->date)[0] : $reserva->date->format('Y-m-d');
                                    $reservaEndTime = \Carbon\Carbon::parse($baseDate . ' ' . $reserva->end_time);
                                ...
                                <td class="px-4 py-3 text-left min-w-[120px]">
                                    @if ($reserva->manager)
                                    <span class="font-medium text-purple-700 bg-purple-50 px-2 py-0.5 text-[10px] rounded-full uppercase border border-purple-100">
...
<td class="px-4 py-3 text-sm font-medium min-w-[100px]">
    <div class="flex flex-col space-y-1">
        ...
        @endif
                            ...
                            @empty
                            <tr>
                                <td colspan="8" class="px-6 py-10 whitespace-nowrap text-center text-sm text-gray-500 italic font-medium bg-gray-50">
    ...
    <div id="noshow-modal" class="fixed inset-0 bg-gray-600 bg-opacity-75 hidden items-center justify-center z-50 transition-opacity duration-300">
        <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8 m-4 transform transition-transform duration-300 scale-95 opacity-0" id="noshow-modal-content" onclick="event.stopPropagation()">
            ...
            <div class="flex items-center gap-3 mb-6 border-b pb-4">
                ...
                <h3 id="noshow-modal-title" class="text-xl font-black text-gray-900 uppercase tracking-tighter">Registrar No-Show</h3>
            </div>
            ...
            <div class="bg-gray-50 rounded-2xl p-5 mb-6 border border-gray-100">
                ...
                <p class="text-[11px] text-gray-500 mt-2 italic">Esta ação liberará o horário no sistema e aplicará as regras de falta ao cliente.</p>
            </div>
    ...
    <div id="reactivation-modal" class="fixed inset-0 bg-gray-600 bg-opacity-75 hidden items-center justify-center z-50 transition-opacity duration-300">
        <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8 m-4 transform transition-transform duration-300 scale-95 opacity-0" id="reactivation-modal-content" onclick="event.stopPropagation()">
            ...
            <div class="flex items-center gap-3 mb-6 border-b pb-4">
                ...
                <h3 id="reactivation-modal-title" class="text-xl font-black text-gray-900 uppercase tracking-tighter">Reativar Reserva</h3>
            </div>

            <div class="bg-gray-50 rounded-2xl p-5 mb-6 border border-gray-100">
                <p id="reactivation-modal-message" class="text-sm text-gray-700 font-bold leading-tight"></p>
                <p class="text-[11px] text-gray-500 mt-2 italic">A reserva voltará a ocupar o slot na agenda e aparecerá como confirmada.</p>
            </div>
    ...
    {{-- 🆕 NOVO MODAL DE ALTERAÇÃO DE PREÇO REFINADO --}}
    <div id="price-update-modal" class="fixed inset-0 bg-gray-600 bg-opacity-75 hidden items-center justify-center z-50 transition-opacity duration-300">
        <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8 m-4 transform transition-transform duration-300 scale-95 opacity-0" id="price-update-modal-content" onclick="event.stopPropagation()">
            ...
            <div class="bg-gray-50 rounded-2xl p-5 mb-6 border border-gray-100">
                <label class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1">Alvo da Alteração</label>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                    <label class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1">Preço Atual</label>
                    <span id="current-price-display" class="text-lg font-black text-gray-700 font-mono">R$ 0,00</span>
                </div>
                <div class="bg-blue-50 p-4 rounded-2xl border border-blue-100">
                    <label for="new-price-input" class="text-[9px] font-black text-blue-500 uppercase tracking-widest block mb-1">Novo Preço (R$)</label>
            ...
            <div class="mb-8">
                <label for="price-justification-input" class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">
                ...
                <p id="price-justification-error" class="text-[10px] font-bold text-red-500 mt-2 hidden uppercase tracking-tight"></p>
            </div>
    ...
    <script>
        ...
        async function handleFixedSlotToggle(id, targetAction) {
            ...
            sendAjaxRequest(id, 'POST', UPDATE_SLOT_STATUS_URL, null, {
                status: targetAction
            });
        }
