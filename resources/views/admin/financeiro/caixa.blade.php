        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    🏦 Caixa: {{ request('arena_id') ? \App\Models\Arena::find(request('arena_id'))->name : 'Todas as Unidades' }}
                </h2>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 shadow-sm rounded-xl border border-gray-100 dark:border-gray-700 print:hidden">
                <form id="caixaFilterForm" method="GET" action="{{ route('admin.financeiro.relatorio_caixa') }}"
                    ...
                    <div class="md:col-span-1">
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1 tracking-wider italic">🏟️ Unidade</label>
                        <select name="arena_id" onchange="this.form.submit()" class="w-full rounded-lg border-gray-300 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:ring-2 focus:ring-indigo-500 font-bold text-sm">
                    ...
                    <div class="md:col-span-1">
                        <label for="dataInput" class="block text-xs font-bold text-gray-500 uppercase mb-1 tracking-wider italic">📅 Data do Caixa</label>
                        <input type="date" name="data" id="dataInput" value="{{ $data }}" onchange="this.form.submit()" class="w-full rounded-lg border-gray-300 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:ring-2 focus:ring-indigo-500 cursor-pointer">
                    </div>
                    ...
                    <div class="md:col-span-2 flex gap-2">
                        ...
                        <button type="button" onclick="window.print()" class="flex-1 bg-emerald-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-emerald-700 transition flex items-center justify-center gap-2 text-sm shadow-sm">
            ...
            <div id="reportContent" class="bg-white dark:bg-gray-800 p-8 shadow-lg rounded-xl print:shadow-none print:p-0 border border-gray-100 dark:border-gray-700">
                ...
                <div class="flex justify-between items-start border-b-2 border-gray-100 dark:border-gray-700 pb-6 mb-8">
                    <div>
                        <h1 class="text-3xl font-black text-gray-800 dark:text-white uppercase tracking-tighter">Relatório de Caixa</h1>
                    ...
                    <div class="text-right">
                        ...
                        <span class="bg-green-100 text-green-700 px-4 py-1 rounded-full text-[10px] font-black uppercase border border-green-200 italic">✅ Caixa Conferido e Fechado</span>
                        @else
                        <span class="bg-amber-100 text-amber-700 px-4 py-1 rounded-full text-[10px] font-black uppercase border border-amber-200 italic">🔓 Movimentação em Aberto</span>
                        @endif
                        <p class="text-[10px] text-gray-400 mt-2 italic font-mono uppercase tracking-tighter">Gerado em: {{ now()->format('d/m/Y H:i') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    <div class="space-y-3">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900/40 p-4 rounded-xl space-y-2 border border-gray-100 dark:border-gray-700">
                            ...
                            <div class="flex justify-between items-center text-sm border-b border-gray-100 dark:border-gray-800 last:border-0 pb-1">
                                <span class="text-gray-500 uppercase font-bold italic">{{ $traducaoMetodos[$metodo] ?? $metodo }}</span>
                    ...
                    <div class="bg-indigo-600 dark:bg-indigo-900 p-8 rounded-2xl flex flex-col justify-center items-center shadow-inner text-white relative overflow-hidden">
                        ...
                        <span class="text-xs font-bold uppercase opacity-80 tracking-widest text-center">Saldo Líquido Esperado em Caixa</span>
                ...
                <div class="mb-10">
                    <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <span class="w-2 h-2 bg-fuchsia-500 rounded-full"></span> Auditoria de Fechamento (Sobra/Falta de Dinheiro)
                    </h3>
                    <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm">
                        <table class="w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                                    <td class="py-3 px-4 font-bold text-gray-700 dark:text-gray-300 text-xs italic">{{ $hist->arena->name ?? 'Geral' }}</td>
                                    <td class="py-3 px-2 text-gray-500 text-xs font-bold">{{ $hist->user->name ?? 'Sistema' }}</td>
                                    <td class="py-3 px-2 text-right text-gray-500 font-mono italic">R$ {{ number_format($hist->calculated_amount, 2, ',', '.') }}</td>
                                    <td class="py-3 px-2 text-right text-gray-800 dark:text-gray-200 font-bold font-mono">R$ {{ number_format($hist->actual_amount, 2, ',', '.') }}</td>
                                    <td class="py-3 px-4 text-right font-black">
                                        ...
                                        <span class="text-amber-500 text-[11px]">+ R$ {{ number_format($hist->difference, 2, ',', '.') }} (SOBRA)</span>
                                        @else
                                        <span class="text-red-500 text-[11px]">R$ {{ number_format($hist->difference, 2, ',', '.') }} (FALTA) 🚨</span>
                                        @endif
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="5" class="py-8 text-center text-gray-400 italic text-xs">Nenhum registro de conferência encontrado.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition duration-150">
                                <td class="py-3 px-2 text-gray-400 font-mono text-xs text-center italic">{{ $m->paid_at->format('H:i') }}</td>
                                <td class="py-3 px-2 text-[10px] font-bold text-gray-500 uppercase italic">{{ $m->arena->name ?? '---' }}</td>
                                <td class="py-3 px-2 pl-4">
                                    <div class="font-bold dark:text-gray-200">#{{ $m->reserva_id ?? 'AVULSO' }}</div>
                                    <div class="text-[10px] text-gray-500 truncate max-w-[150px] italic font-bold uppercase">{{ $m->reserva->client_name ?? 'Lançamento Manual' }}</div>
                                </td>
                                <td class="py-3 px-2 text-center text-[9px] font-black uppercase">
                                    ...
                                    $corTipo = 'bg-gray-100 text-gray-600';

                                    if($tipoRaw == 'signal') $corTipo = 'bg-blue-100 text-blue-700 border border-blue-200';
                                    if(in_array($tipoRaw, ['full_payment', 'payment', 'payment_settlement'])) $corTipo = 'bg-emerald-100 text-emerald-700 border border-emerald-200';
                                    if($tipoRaw == 'refund' || $m->amount < 0) $corTipo='bg-red-100 text-red-700 border border-red-200' ;
                                        if(str_contains($tipoRaw, 'reten' )) $corTipo='bg-amber-100 text-amber-700 border border-amber-200' ;
                                        @endphp
                                        <span class="px-2 py-1 rounded {{ $corTipo }}">
                                        {{ $textoTipo }}
                                        </span>
                                </td>
                                <td class="py-3 px-2 text-center text-[10px] text-gray-600 dark:text-gray-400 font-bold italic uppercase">
                                ...
                                <td class="py-3 px-2 text-right font-mono font-bold {{ $m->amount < 0 ? 'text-red-500' : 'text-gray-800 dark:text-white' }}">
                                    {{ $m->amount < 0 ? '-' : '' }} R$ {{ number_format(abs($m->amount), 2, ',', '.') }}
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                ...
                <div class="mt-20 grid grid-cols-2 gap-16 text-center print:mt-10">
                    <div class="space-y-2">
                        ...
                        <p class="text-[10px] uppercase font-black text-gray-400 italic">Responsável pelo Caixa (Operacional)</p>
                        @if ($caixaStatus && $caixaStatus->user)<p class="text-[9px] text-indigo-500 font-bold italic uppercase">{{ $caixaStatus->user->name }}</p>@endif
                    </div>
                    <div class="space-y-2">
                        <div class="border-b border-gray-400 dark:border-gray-600 w-full h-8"></div>
                        <p class="text-[10px] uppercase font-black text-gray-400 italic">Conferência Gerencial (Assinatura)</p>
                    </div>
                </div>
            </div>
        </div>
